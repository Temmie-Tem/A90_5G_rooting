...
> **✅ POST-PROMOTION OBSERVABILITY RECORD (V2822–V2827, 2026-06-19): Tier-C audio status/selftest/screen UI observability is device-validated on `0.10.5`.** Artifact `workspace/private/inputs/boot_images/boot_linux_v2826_audio_menu_screens.img` (SHA256 `0662749d76651d50db837685b7778aedbbe83cc7071d16c703ea060e9e9d47a6`, tag `v2826-audio-menu-screens`) keeps the promoted audio core, keeps the V2820 read-only `selftest verbose` audio row (`PASS audio ... boost=blocked sp=unverified`), keeps the V2822 `screenapp audio-status` summary, keeps display-only `screenapp audio-map` / `screenapp speaker-map`, and adds the APPS/AUDIO menu page with `AUDIO STATUS` and `ROUTE MAP` actions. V2827 validated `audio status` (16/16), `selftest verbose` audio markers (8/8), and `screenapp audio-map` markers (6/6) on-device, then rolled back to `v2321` with `selftest fail=0` and no recovery fallback. This is the latest observability candidate, not a rollback-net replacement.
