...
> **✅ POST-PROMOTION OBSERVABILITY RECORD (V2820/V2821, 2026-06-19): Tier-C audio status/selftest observability is device-validated on `0.10.2`.** Artifact `workspace/private/inputs/boot_images/boot_linux_v2820_audio_selftest_policy.img` (SHA256 `73094ccf1288fa142ae8520d3b47ac6bb31a5c4be90e920e1e27714acbbbea41`, tag `v2820-audio-selftest-policy`) keeps the promoted audio core, exposes the `audio status` core/profile/safety fields, and fixes the read-only `selftest verbose` audio row so it prints `PASS audio ... boost=blocked sp=unverified`. V2821 validated both marker sets on-device and rolled back to `v2321` with `selftest fail=0` (recovery fallback used for rollback). This is an observability candidate, not a rollback-net replacement.
