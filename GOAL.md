# Goal: host-only regression test harness for the analysis tooling

Delegated to a Codex `/goal` run. This file is the **source of truth + progress
ledger**: the objective, the runner contract, and a coverage checklist that the
agent checks off one target at a time. See `AGENTS.md` (repo root) for the hard
