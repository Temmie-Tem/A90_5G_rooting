...
runner = load_script("workspace/public/src/scripts/server-distro/run_wsta163_seccomp_helper_apply_gate_chroot_proof.py")


class ServerDistroWsta163SeccompHelperApplyGateChrootProofTests(unittest.TestCase):
    ...
    def write_filter_artifact(self, root: Path) -> tuple[Path, Path]:
        ...
        object_path.write_bytes(b"fake-wsta163-object")
    ...
    def test_apply_gate_and_helper_block_are_proven(self) -> None:
        with self.private_tmp() as tmp:
            ...
            with (
                ...
                mock.patch.object(
                    runner,
                    "run_chroot_launcher_apply",
                    side_effect=[
                        {"returncode": 65, "stdout": missing_gate_stdout, "stderr": ""},
                        {"returncode": 65, "stdout": helper_block_stdout, "stderr": ""},
                    ],
                ),
            ):
                result = runner.run(runner.build_arg_parser().parse_args([
                    ...
                    str(helper_binary),
                    "--execute-helper-apply-gate-chroot-proof",
                ]))

        self.assertEqual(result["decision"], runner.PASS_DECISION)
        self.assertTrue(result["checks"]["helper_schema_is_wsta161"])
        self.assertTrue(result["checks"]["helper_apply_code_compiled"])
        self.assertTrue(result["proof_checks"]["missing_gate_blocks_before_helper"])
        self.assertTrue(result["proof_checks"]["helper_blocks_load_gate"])
        self.assertTrue(result["proof_checks"]["helper_no_load_attempt"])
    ...
    def test_gate_blocks_without_explicit_flag_or_private_helper(self) -> None:
        ...
        self.assertEqual(result["decision"], "wsta163-blocked-explicit-gate-required")
        ...
        self.assertEqual(result["decision"], "wsta163-blocked-helper-manifest-nonprivate")
