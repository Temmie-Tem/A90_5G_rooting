...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3230_gpu_h3_vpc_lm_siv_probe.py"
)


class NativeGpuH3VpcLmSivSourceV3230Tests(unittest.TestCase):
    def test_v3230_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3230")
        self.assertEqual(runner.INIT_VERSION, "0.11.42")
        self.assertEqual(runner.INIT_BUILD, "v3230-gpu-h3-vpc-lm-siv-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.42", required)
        self.assertIn(b"v3230-gpu-h3-vpc-lm-siv-probe", required)
        self.assertIn(
    ...
    def test_dispatch_programs_mesa_vpc_lm_siv_registers(self) -> None:
    ...
    def test_builder_manifest_records_vpc_lm_siv_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"source_baseline": "v3228-gpu-h3-fragment-input-probe"', source)
        ...
        self.assertIn('"state_reg_writes_expected": 81', source)
        ...
        self.assertIn("preserve-v3228-ramdisk-overlay-v3230-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3230"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3230 boot image too large for boot partition", source)
