...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m33_p28_wdt_prefix_park_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m33_p28_wdt_prefix_park_live_gate", SCRIPT)
...
class S22PlusM33P28WdtPrefixParkLiveGateTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_policy_markers_include_hashes_tokens_and_prefix_semantics(self):
        ...
        self.assertIn("prefix_targets=28", markers)
        self.assertIn("module_load_only=1", markers)
        self.assertIn("DWC3-without-ACM prefix", markers)
        self.assertIn("no ACM function", markers)
    ...
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m33_p28_manifest_hashes=", text)
            self.assertIn("m33_manifest_safety=", text)
            self.assertIn("m33_p28_manifest_closure=", text)
            self.assertEqual(len(self.module.EXPECTED_MODULES), 44)
            self.assertIn("arm_smmu.ko", self.module.EXPECTED_MODULES)
            self.assertIn("phy-msm-snps-hs.ko", self.module.EXPECTED_MODULES)
            self.assertIn("phy-msm-snps-eusb2.ko", self.module.EXPECTED_MODULES)
            self.assertIn("usb_f_ss_mon_gadget.ko", self.module.EXPECTED_MODULES)
