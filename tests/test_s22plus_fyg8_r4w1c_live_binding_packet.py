...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("r4w1c_live_binding_packet_tested", SCRIPT)
...
class S22PlusFyg8R4W1CLiveBindingPacketTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_parser_exposes_only_host_modes(self):
        ...
        self.assertIn("preconnected_check", options)
        self.assertIn("emit_after_connected", options)
    ...
    def test_connected_binding_reopens_pass_and_result_identities(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(binding["result_size"], 6)
    ...
    def test_emit_packet_creates_exact_clause_without_editing_agents(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            clause = (output / "AGENTS_R4W1C_LIVE_CLAUSE.md").read_text().strip()
    ...
    def test_main_preconnected_has_zero_device_actions(self):
        module = self.module
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            with mock.patch.object(module, "repo_root", return_value=root), mock.patch.object(
                ...
                rc = module.main(["--preconnected-check"])
