...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m34_s4_role_lever_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m34_s4_role_lever_live_gate", SCRIPT)
...
class S22PlusM34S4RoleLeverLiveGateTest(unittest.TestCase):
    def setUp(self):
        self.module = load_module()

    def test_policy_markers_include_hashes_tokens_and_role_lever_semantics(self):
    ...
    def test_agents_exception_draft_satisfies_same_policy_markers(self):
        ...
        self.assertIn("does not authorize S1/S2/S3 repeat", " ".join(draft.split()))
    ...
    def test_verify_agents_exception_rejects_draft_only_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("draft-only M34 S4", str(caught.exception))
            self.assertIn("agents_exception_draft_only_present=1", log_path.read_text(encoding="utf-8"))

    @unittest.skipUnless(MANIFEST.exists(), "private M34 v0.3 manifest missing")
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            log_path = Path(tmp) / "m34_s4_manifest_check.log"
            self.module.verify_m34_manifest(MANIFEST, log_path)
            text = log_path.read_text(encoding="utf-8")
            self.assertIn("m34_s4_manifest_hashes=", text)
            self.assertIn("m34_manifest_safety=", text)
            self.assertIn("m34_s4_manifest_runtime_steps=", text)
            self.assertIn("m34_s4_manifest_closure=", text)
    ...
    def test_acm_match_requires_stock_samsung_vid_pid(self):
        self.assertTrue(self.module.is_m34_s4_acm({"vendor": "04E8", "product": "6860"}))
        self.assertFalse(self.module.is_m34_s4_acm({"vendor": "04e8", "product": "685d"}))
        self.assertFalse(self.module.is_m34_s4_acm({"vendor": "18d1", "product": "4ee7"}))
