...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3234_gpu_h3_shader_output_probe.py"
)


class NativeGpuH3ShaderOutputSourceV3234Tests(unittest.TestCase):
    def test_v3234_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3234")
        self.assertEqual(runner.INIT_VERSION, "0.11.44")
        self.assertEqual(runner.INIT_BUILD, "v3234-gpu-h3-shader-output-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.44", required)
        self.assertIn(b"v3234-gpu-h3-shader-output-probe", required)
        self.assertIn(
            b"gpu.h3.draw.scope=first-triangle-h3-shader-output-r1-mov-f32-shader",
            required,
        )
        self.assertIn(
    ...
    def test_dispatch_programs_mesa_shader_output_registers(self) -> None:
    ...
    def test_builder_manifest_records_shader_output_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"source_baseline": "v3232-gpu-h3-static-context-probe"', source)
        self.assertIn('"scope": "first-triangle-h3-shader-output-r1-mov-f32-shader"', source)
        ...
        self.assertIn("preserve-v3232-ramdisk-overlay-v3234-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3234"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3234 boot image too large for boot partition", source)
