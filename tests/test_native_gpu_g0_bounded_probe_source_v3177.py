...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3177_gpu_g0_bounded_probe.py"
)


class NativeGpuG0BoundedProbeSourceV3177Tests(unittest.TestCase):
    def test_v3177_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3177")
        self.assertEqual(runner.INIT_VERSION, "0.11.19")
        self.assertEqual(runner.INIT_BUILD, "v3177-gpu-g0-bounded-probe")

        required = b"\n".join(runner.REQUIRED_STRINGS)
        self.assertIn(b"0.11.19", required)
        self.assertIn(b"v3177-gpu-g0-bounded-probe", required)
