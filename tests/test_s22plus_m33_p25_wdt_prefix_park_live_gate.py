...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m33_p25_wdt_prefix_park_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m33_p25_wdt_prefix_park_live_gate", SCRIPT)
...
class S22PlusM33P25WdtPrefixParkLiveGateTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_policy_markers_include_hashes_tokens_and_prefix_semantics(self):
        ...
        self.assertIn("prefix_targets=25", markers)
        self.assertIn("module_load_only=1", markers)
        self.assertIn("SMMU-only prefix", markers)
        self.assertIn("no HS/eUSB2 PHY", markers)
        self.assertIn("no DWC3 or ACM function", markers)
    ...
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("m33_p25_manifest_hashes=", text)
            self.assertIn("m33_manifest_safety=", text)
            self.assertIn("m33_p25_manifest_closure=", text)
            self.assertEqual(len(self.module.EXPECTED_MODULES), 29)
            self.assertIn("arm_smmu.ko", self.module.EXPECTED_MODULES)
            self.assertNotIn("phy-msm-snps-hs.ko", self.module.EXPECTED_MODULES)
            self.assertNotIn("phy-msm-snps-eusb2.ko", self.module.EXPECTED_MODULES)
            self.assertNotIn("usb_f_ss_mon_gadget.ko", self.module.EXPECTED_MODULES)
            self.assertNotIn("dwc3-msm.ko", self.module.EXPECTED_MODULES)
            self.assertNotIn("usb_f_ss_acm.ko", self.module.EXPECTED_MODULES)
