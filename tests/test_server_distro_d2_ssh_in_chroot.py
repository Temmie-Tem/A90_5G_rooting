...
class ServerDistroD2SshInChrootTests(unittest.TestCase):
    def test_setup_script_starts_key_only_dropbear_without_userdata_or_flash(self) -> None:
        ...
        self.assertIn('chown 0:0 "$MNT/root" "$MNT/root/.ssh"', script)
        self.assertIn('chown 0:0 "$MNT/root/.ssh/authorized_keys"', script)
        self.assertIn("-E -F", script)
        self.assertIn("-s -j -k", script)
        self.assertIn("dropbear_foreground=1", script)
        self.assertIn("dropbear_alive=0", script)
        self.assertIn("a90_d2_dropbear.log", script)
        self.assertIn('printf \'%s\\n\' "$PID" > "$MNT/tmp/a90_d2_dropbear.pid"', script)
