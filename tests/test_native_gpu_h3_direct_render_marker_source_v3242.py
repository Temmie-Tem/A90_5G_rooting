...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3242_gpu_h3_direct_render_marker_probe.py"
)


class NativeGpuH3DirectRenderMarkerSourceV3242Tests(unittest.TestCase):
    def test_v3242_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3242")
        self.assertEqual(runner.INIT_VERSION, "0.11.48")
        self.assertEqual(runner.INIT_BUILD, "v3242-gpu-h3-direct-render-marker-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.48", required)
        self.assertIn(b"v3242-gpu-h3-direct-render-marker-probe", required)
        self.assertIn(
        ...
        self.assertIn(b"gpu.h3.draw.gras_sc_msaa_sample_pos_cntl=0x%x", required)
        self.assertIn(b"gpu.h3.draw.rb_msaa_sample_pos_cntl=0x%x", required)
        self.assertIn(b"gpu.h3.draw.tpl1_msaa_sample_pos_cntl=0x%x", required)
        self.assertIn(
        ...
        self.assertIn(b"gpu.h3.draw.rb_ccu_cntl=0x%x", required)
        self.assertIn(b"gpu.h3.draw.rb_ccu_color_offset=0x%x", required)
        self.assertIn(b"gpu.h3.draw.rb_ccu_depth_offset=0x%x", required)
    ...
    def test_dispatch_programs_mesa_direct_render_marker_before_state(self) -> None:
        ...
        self.assertIn("#define GPU_H3_REG_GRAS_SC_MSAA_SAMPLE_POS_CNTL 0x80a4U", source)
        self.assertIn("#define GPU_H3_REG_RB_MSAA_SAMPLE_POS_CNTL 0x8804U", source)
        self.assertIn("#define GPU_H3_REG_TPL1_MSAA_SAMPLE_POS_CNTL 0xb304U", source)
        self.assertIn("#define GPU_H3_REG_RB_CCU_CNTL 0x8e07U", source)
        ...
        self.assertIn("#define GPU_H3_GRAS_SC_MSAA_SAMPLE_POS_CNTL 0x00000000U", source)
        self.assertIn("#define GPU_H3_RB_MSAA_SAMPLE_POS_CNTL 0x00000000U", source)
        self.assertIn("#define GPU_H3_TPL1_MSAA_SAMPLE_POS_CNTL 0x00000000U", source)
        self.assertIn("#define GPU_H3_RB_CCU_CNTL 0x10000000U", source)
        self.assertIn("#define GPU_H3_RB_CCU_COLOR_OFFSET 0x00020000U", source)
        self.assertIn("#define GPU_H3_RB_CCU_DEPTH_OFFSET 0x00000000U", source)
        ...
        self.assertIn(
            "GPU_H3_REG_GRAS_SC_MSAA_SAMPLE_POS_CNTL,\n                              GPU_H3_GRAS_SC_MSAA_SAMPLE_POS_CNTL",
            source,
        )
        self.assertIn(
            "GPU_H3_REG_RB_MSAA_SAMPLE_POS_CNTL,\n                              GPU_H3_RB_MSAA_SAMPLE_POS_CNTL",
            source,
        )
        self.assertIn(
            "GPU_H3_REG_TPL1_MSAA_SAMPLE_POS_CNTL,\n                              GPU_H3_TPL1_MSAA_SAMPLE_POS_CNTL",
            source,
        )
        self.assertIn(
        ...
        self.assertNotIn("GPU_H3_IR3_MOV_F32F32_R0X_R0X_LO, GPU_H3_IR3_MOV_F32F32_R0X_R0X_HI", source)
        self.assertNotIn("GPU_H3_IR3_MOV_F32F32_R0Y_R0Y_LO, GPU_H3_IR3_MOV_F32F32_R0Y_R0Y_HI", source)
        ...
        self.assertIn(
            '"gpu.h3.draw.rb_ccu_source=mesa-freedreno-a6xx-fd6-emit-gmem-cache-cntl-sysmem-adreno640v2',
            source,
        )
        self.assertIn(
        ...
        self.assertIn('"gpu.h3.draw.cp_set_marker=0x%x', source)
        self.assertIn('"gpu.h3.draw.gras_sc_msaa_sample_pos_cntl=0x%x', source)
        self.assertIn('"gpu.h3.draw.rb_msaa_sample_pos_cntl=0x%x', source)
        self.assertIn('"gpu.h3.draw.tpl1_msaa_sample_pos_cntl=0x%x', source)
        self.assertIn('"gpu.h3.draw.rb_ccu_cntl=0x%x', source)
        self.assertIn('"gpu.h3.draw.rb_ccu_color_offset=0x%x', source)
        self.assertIn('"gpu.h3.draw.rb_ccu_depth_offset=0x%x', source)
    ...
    def test_builder_manifest_records_direct_render_marker_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn(
            '"source_baseline": "v3240-sample-location-disable-plus-rb-ccu-sysmem-prep-on-v3232-ramdisk-base"',
            source,
        )
        self.assertIn('"scope": "first-triangle-h3-direct-render-marker-r1-footprint2-mov-f32-shader"', source)
        self.assertIn("verify-fresh-boot-fwclass-path-before-h3", source)
        self.assertIn("gpu-h3-direct-render-marker-r1-footprint2-mov-f32-shader-timeout-guard", source)
        ...
        self.assertIn('"render_marker_source": "Mesa A6xx fd6_set_render_mode CP_SET_MARKER RM6_DIRECT_RENDER"', source)
        self.assertIn('"cp_set_marker_opcode": "0x65"', source)
        self.assertIn('"cp_set_marker_payload": "0x00000001"', source)
        self.assertIn('"cp_set_marker_mode": "RM6_DIRECT_RENDER"', source)
        self.assertIn('"cp_set_marker_uses_gmem": False', source)
        self.assertIn('"rb_ccu_source": "Mesa A6xx fd6_emit_gmem_cache_cntl sysmem cache control"', source)
        self.assertIn('"rb_ccu_device_basis": "Adreno640v2 sysfs gpu_model plus Mesa GPUId(640) num_ccu=2"', source)
        self.assertIn('"rb_ccu_cntl": "0x10000000"', source)
        self.assertIn('"rb_ccu_color_offset": "0x00020000"', source)
        self.assertIn('"rb_ccu_depth_offset": "0x00000000"', source)
        self.assertIn('"gras_sc_msaa_sample_pos_cntl": "0x00000000"', source)
        self.assertIn('"rb_msaa_sample_pos_cntl": "0x00000000"', source)
        self.assertIn('"tpl1_msaa_sample_pos_cntl": "0x00000000"', source)
        ...
        self.assertIn("preserve-v3232-ramdisk-overlay-v3242-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3242"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3242 boot image too large for boot partition", source)
