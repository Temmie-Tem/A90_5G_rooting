...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("r4w1c2_measured_live_gate_tested", SCRIPT)
...
class S22PlusFyg8R4W1C2LiveGateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def measured_snapshot(self, module, path="/dev/bus/usb/001/002"):
        match = module.odin_core.ODIN_DEVICE_RE.fullmatch(path)
        self.assertIsNotNone(match)
        bus, device = (int(value) for value in path.rsplit("/", 2)[-2:])
        minor = (bus - 1) * 128 + device - 1
        return module.usbfs_identity.UsbfsNodeSnapshot(
            path=path,
            st_dev=11,
            st_ino=22,
            st_rdev=os.makedev(189, minor),
            st_nlink=1,
            st_file_type=stat.S_IFCHR,
            st_mode=0o660,
            st_uid=0,
            st_gid=46,
            birth_time_ns=1_721_234_567_000_000_000,
            device_major=189,
            device_minor=minor,
            st_atime_ns=101,
            st_ctime_ns=102,
    ...
    def test_exact_frozen_connected_and_measured_core_pins(self):
        module = self.module
        self.assertEqual(
        ...
        self.assertNotEqual(
            module.ODIN_CORE_SHA256, module.connected.EXPECTED_ODIN_CORE_SHA256
        )
        self.assertEqual(
            module.ODIN_CORE_SHA256,
            "c9abb179158bb45039574465e743f1f5bee18f993cbddd2f0b40e9048d1ca6b3",
        )
        self.assertEqual(
            module.USBFS_IDENTITY_SHA256,
        ...
        for observation in incomplete_values:
            with self.subTest(observation=observation):
                self.assertEqual(
                    ...
                    ("FAIL_R4W1C2_CANDIDATE_OBSERVATION_REQUIRED", 25),
                )
    ...
    def test_finish_never_allows_pass_after_stock_cleanup_intent(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["verdict"], "FAIL_R4W1C2_STOCK_CLEANUP_TAINTED")
    ...
    def test_endpoint_usb_identity_restats_after_sysfs_reads(self):
        ...
        measured = self.measured_snapshot(module)
        measured_identity = module.usbfs_identity.immutable_identity(measured)
        ...
        with mock.patch.object(
            ...
            module.usbfs_identity, "snapshot_node", return_value=measured
        ), mock.patch.object(
        ...
        self.assertEqual(identity["device_identity"], measured_identity)
        self.assertEqual(snapshot.call_count, 2)

        replaced = dataclasses.replace(measured, st_ino=measured.st_ino + 1)
        with mock.patch.object(
            module,
            "endpoint_node_snapshot",
            side_effect=[
            ...
            "snapshot_node",
            side_effect=[measured, replaced],
        ), mock.patch.object(
            Path, "resolve", return_value=sysfs_device
        ), mock.patch.object(Path, "read_text", side_effect=fields * 2):
            with self.assertRaisesRegex(module.GateError, "immutable identity changed"):
    ...
    def test_endpoint_usb_identity_allows_only_measured_timestamp_change(self):
        module = self.module
        metadata = SimpleNamespace(st_rdev=os.makedev(189, 1))
        before = self.measured_snapshot(module)
        after = dataclasses.replace(
            before,
            st_atime_ns=before.st_atime_ns + 1,
            st_ctime_ns=before.st_ctime_ns + 2,
            st_mtime_ns=before.st_mtime_ns + 3,
        )
        base = {
            "vendor": "04e8",
            "product": module.DOWNLOAD_USB_PRODUCT,
        ...
        with mock.patch.object(
            module,
            "endpoint_node_snapshot",
            side_effect=[(metadata, "legacy-before"), (metadata, "legacy-after")],
        ), mock.patch.object(
            module.usbfs_identity, "snapshot_node", side_effect=[before, after]
        ), mock.patch.object(
            Path,
            "resolve",
            return_value=Path("/sys/devices/platform/usb1/1-2"),
        ), mock.patch.object(
            module, "read_download_sysfs_identity", return_value=base
        ):
            identity = module.endpoint_usb_identity("/dev/bus/usb/001/002")
        self.assertEqual(
            identity["device_identity"],
            module.usbfs_identity.immutable_identity(before),
        )
    ...
    def test_bound_download_sample_requires_exact_identity_and_absent_serial(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            measured = self.measured_snapshot(module, "/dev/bus/usb/002/014")
            measured_identity = module.usbfs_identity.immutable_identity(measured)
            with mock.patch.object(
                module,
                "endpoint_node_snapshot",
    ...
    def test_wait_for_endpoint_shares_deadline_and_binds_stable_node(self):
        ...
        ticket = module.odin_core.EndpointTicket(
            device="/dev/bus/usb/002/014",
            device_identity=node["immutable_identity"],
            generation=1,
            snapshot_sequence=0,
            snapshot_receipt="receipt",
            snapshot_receipt_sha256="a" * 64,
        ...
        self.assertIs(
            wait.call_args.kwargs["endpoint_observer_factory"],
            module.odin_core.measured_usbfs_observer,
        )
    ...
    def test_wait_for_endpoint_rejects_same_path_replaced_identity(self):
        ...
        ticket = module.odin_core.EndpointTicket(
            device="/dev/bus/usb/002/014",
            device_identity="usbfs-immutable-v1:" + "b" * 64,
            generation=1,
            snapshot_sequence=0,
            snapshot_receipt="receipt",
            snapshot_receipt_sha256="a" * 64,
    ...
    def test_revalidate_ticket_uses_measured_usbfs_observer(self):
        module = self.module
        ticket = module.odin_core.EndpointTicket(
            device="/dev/bus/usb/002/014",
            device_identity="usbfs-immutable-v1:" + "a" * 64,
            generation=1,
            snapshot_sequence=0,
            snapshot_receipt="receipt",
        ...
        with mock.patch.object(
            ...
            return_value={"phase": "revalidated"},
        ) as revalidate:
            device, sequence, record = module.revalidate_ticket(
                Path("odin4"), Path("run"), ticket, sequence=4, lease="lease"
            )
        self.assertEqual((device, sequence), (ticket.device, 5))
        self.assertEqual(record, {"phase": "revalidated"})
        self.assertIs(
            revalidate.call_args.kwargs["endpoint_observer_factory"],
            module.odin_core.measured_usbfs_observer,
        )
    ...
    def test_pass_is_downgraded_when_timeline_is_incomplete(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                result["verdict"], "FAIL_R4W1C2_PASS_TIMELINE_INCOMPLETE"
            )
    ...
    def test_live_run_orders_topology_consumption_and_sealed_candidate(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(module, "policy_active", return_value=True), mock.patch.object(
                ...
                module, "verify_artifacts", return_value=artifacts
            ), mock.patch.object(
            ...
            self.assertIs(
                wait_absent.call_args.kwargs["endpoint_observer_factory"],
                module.odin_core.measured_usbfs_observer,
            )
            self.assertEqual(
