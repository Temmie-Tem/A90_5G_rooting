...
runner = load_script("workspace/public/src/scripts/revalidation/build_native_init_boot_v3147_doomgeneric_sfx_stream_sigpipe_safe.py")


class NativeDoomgenericSfxStreamSigpipeSafeSourceV3147Tests(unittest.TestCase):
    def test_loop_only_enables_sfx_sound(self) -> None:
        source = runner.v3147_adapter_source()
        ...
        self.assertIn("a90.doomgeneric.v3147.audio=real-sfx-pcm-stream-sigpipe-safe-music-disabled", source)
    ...
    def test_generated_sfx_backend_uses_fifo_not_alsa(self) -> None:
        ...
        self.assertIn("#include <signal.h>", backend)
        self.assertIn("signal(SIGPIPE, SIG_IGN)", backend)
        self.assertIn("static int open_stream_once", backend)
        self.assertIn("open(A90_SFX_STREAM_PATH, O_WRONLY | O_NONBLOCK | O_CLOEXEC)", backend)
        self.assertIn("stream_fd = open_stream_once();", backend)
