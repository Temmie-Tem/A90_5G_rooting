"""Tests for the V2796 audio ION devnode fix."""
...
BUILDER = REPO / "workspace/public/src/scripts/revalidation/build_native_init_boot_v2796_audio_ion_devnode.py"
LIVE = REPO / "workspace/public/src/scripts/revalidation/native_audio_ion_devnode_live_handoff_v2796.py"


def load_live_module():
    sys.path.insert(0, str(LIVE.parent))
    spec = importlib.util.spec_from_file_location("v2796_live", LIVE)
...
class NativeAudioIonDevnodeV2796(unittest.TestCase):
    @classmethod
    ...
    def test_builder_uses_v2796_patch_artifact_identity(self) -> None:
        text = BUILDER.read_text(encoding="utf-8")

        self.assertIn('CYCLE = "V2796"', text)
        self.assertIn('INIT_VERSION = "0.9.309"', text)
        self.assertIn('INIT_BUILD = "v2796-audio-ion-devnode"', text)
        self.assertIn('boot_linux_v2796_audio_ion_devnode.img', text)
        self.assertIn('NATIVE_INIT_V2796_AUDIO_ION_DEVNODE_SOURCE_BUILD_2026-06-19.md', text)
    ...
    def test_live_runner_targets_v2796_and_allowed_manifest_path(self) -> None:
        module = self.live
        args = module.parse_args(["--dry-run"])
        state = module.preflight_state()
        payload = module.dry_run_payload(args, state)

        self.assertEqual(module.CANDIDATE_VERSION, "0.9.309")
        self.assertIn("boot_linux_v2796_audio_ion_devnode.img", str(module.CANDIDATE_IMAGE))
        self.assertEqual(
