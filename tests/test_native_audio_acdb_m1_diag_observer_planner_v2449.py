...
class AcdbM1DiagnosticObserverPlannerV2449(unittest.TestCase):
    def test_helper_source_adds_diagnostic_counters_without_native_ioctls(self) -> None:
        ...
        self.assertTrue(state["contains_aarch64_anonymous_mmap_fd_filter"])
        self.assertTrue(state["contains_early_dmabuf_fd_duplication"])
        self.assertTrue(state["contains_remote_mmap_fallback"])
        self.assertTrue(state["contains_source_buffer_capture"])
        self.assertTrue(state["contains_acdb_source_target_filter"])
        ...
        self.assertIn("raw == 0xffffffffUL", text)
        self.assertIn("raw == (unsigned long)-1L", text)
        self.assertIn("dup_mmap_fd_if_dmabuf", text)
        self.assertIn("ok-early-dup", text)
        self.assertIn("ok-remote-mmap", text)
        self.assertIn("source_buffer_capture", text)
        self.assertIn("ok-source-read", text)
        self.assertIn("ok-source-mmap", text)
        self.assertIn("A90_COMPAT_ARM_NR_PREAD64", text)
        self.assertIn("is_acdb_source_target", text)
        self.assertIn("/vendor/etc/acdbdata", text)
        self.assertIn("/vendor/lib/", text)
    ...
    def test_dry_run_defines_v2449_boundary_and_collection_contract(self) -> None:
        ...
        self.assertTrue(payload["diagnostic_contract"]["adds_early_dmabuf_fd_duplication"])
    ...
    def test_command_safety_requires_diagnostics_and_rejects_replay_paths(self) -> None:
        ...
        self.assertIn("ok-early-dup", full_payload)
