...
runner = load_script("workspace/public/src/scripts/revalidation/build_native_init_boot_v3110_doomgeneric_hw_plane_diagnostics.py")


class NativeDoomgenericHwPlaneDiagnosticsSourceV3110Tests(unittest.TestCase):
    def test_builder_contract_pins_v3110_candidate(self) -> None:
        self.assertEqual(runner.CYCLE, "V3110")
        self.assertEqual(runner.INIT_VERSION, "0.10.110")
        self.assertEqual(runner.INIT_BUILD, "v3110-doomgeneric-hw-plane-diagnostics")
        self.assertEqual(runner.ENGINE_RAMDISK_PATH, "bin/a90_doomgeneric_private_engine_v3110")
        self.assertEqual(runner.ENGINE_NAME, "doomgeneric-private-link-v3110-hw-plane-diagnostics")
        self.assertEqual(runner.FRAME_PATH, "/tmp/a90-doomgeneric-v3110-raw-fallback-frame.xbgr8888")
        self.assertEqual(runner.SHARED_FRAME_PATH, "/tmp/a90-doomgeneric-v3110-shared-frame.bin")
        self.assertEqual(runner.INPUT_STATE_PATH, "/tmp/a90-doomgeneric-v3110-input.state")
        self.assertEqual(runner.INPUT_SOCKET_PATH, "/tmp/a90-doomgeneric-v3110-input.sock")
        self.assertEqual(runner.PACE_SOCKET_PATH, "/tmp/a90-doomgeneric-v3110-pace.sock")
        self.assertEqual(runner.TICK_TELEMETRY_PATH, "/tmp/a90-doomgeneric-v3110-tick-telemetry.txt")
        self.assertEqual(runner.FRAME_SCALE, "3:2-hw-plane-diagnostics")
        self.assertEqual(runner.SCALE_PATH, "drm-plane-srcdst-diagnostic")
    ...
    def test_adapter_source_uses_v3110_paths_and_diagnostic_marker(self) -> None:
        source = runner.v3110_adapter_source()

        self.assertIn(runner.SCALE_MARKER, source)
        self.assertIn(runner.TICK_TELEMETRY_MARKER, source)
        self.assertIn("scale_path=drm-plane-srcdst-diagnostic", source)
    ...
    def test_apply_globals_enables_v3110_identity_and_diagnostics(self) -> None:
        ...
        try:
            ...
            self.assertIs(runner.v3108.V3059.v3059_adapter_source, runner.v3110_adapter_source)
        finally:
    ...
    def test_report_template_records_next_live_gate(self) -> None:
        manifest = {
            ...
            "boot_image": "workspace/private/inputs/boot_images/boot_linux_v3110.img",
        ...
        self.assertIn("Native Init V3110 DOOMGENERIC Hardware Plane Diagnostics Source Build", report)
        self.assertIn("V3109 proved", report)
        self.assertIn("attempted=0", report)
        self.assertIn("protocol-end-missing", report)
        self.assertIn("DRM_CLIENT_CAP_ATOMIC", report)
        self.assertIn("stale ioctl `errno`", report)
        self.assertIn("Run ID: `V3111`", report)
