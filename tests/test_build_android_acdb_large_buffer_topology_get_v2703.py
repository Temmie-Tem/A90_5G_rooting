"""Tests for the V2703 ACDB large-buffer custom-topology GET build gate."""
...
v2703 = load_revalidation("build_android_acdb_large_buffer_topology_get_v2703")


def args(**overrides: object) -> argparse.Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2703-test-"))
...
class BuildAndroidAcdbLargeBufferTopologyGetV2703(unittest.TestCase):
    def test_source_state_requires_large_buffer_get_and_no_preinit_set(self) -> None:
        state = v2703.source_state()
        ...
        self.assertEqual(state["v2703_delta"]["word0"], "force lower custom topology GET input word0 to 65536")
    ...
    def test_dry_run_contract_is_host_only_and_targets_v2702_buffer_gap(self) -> None:
        payload = v2703.make_payload(args())
    ...
    def test_patched_constants_restore_v2692(self) -> None:
        original_preinit = v2703.v2692.PREINIT_SOURCE_REL
        original_tap = v2703.v2692.ACDBTAP_SOURCE_REL
        original_preload = v2703.v2692.PRELOAD_ARTIFACT_NAME

        with v2703.patched_v2692_constants():
            self.assertEqual(v2703.v2692.PREINIT_SOURCE_REL, v2703.PREINIT_SOURCE_REL)
            self.assertEqual(v2703.v2692.ACDBTAP_SOURCE_REL, v2703.ACDBTAP_SOURCE_REL)
            self.assertEqual(v2703.v2692.PRELOAD_ARTIFACT_NAME, v2703.PRELOAD_ARTIFACT_NAME)

        self.assertEqual(v2703.v2692.PREINIT_SOURCE_REL, original_preinit)
        self.assertEqual(v2703.v2692.ACDBTAP_SOURCE_REL, original_tap)
        self.assertEqual(v2703.v2692.PRELOAD_ARTIFACT_NAME, original_preload)

    @unittest.skipUnless(
        (v2703.v2692.v2674.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/clang").exists()
        and (v2703.v2692.v2674.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/ld.lld").exists(),
        "private Android clang/lld unavailable",
    )
    def test_build_outputs_arm32_large_buffer_get_helper_and_preload(self) -> None:
        payload = v2703.make_payload(args(build=True))
        ...
        self.assertTrue(preload["checks"]["soname_v2703"])
    ...
    def test_cli_writes_manifest_and_report(self) -> None:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "workspace/public/src/scripts/revalidation/build_android_acdb_large_buffer_topology_get_v2703.py",
                "--build-root",
        ...
        self.assertIn("large-buffer", local_args.report.read_text(encoding="utf-8"))
