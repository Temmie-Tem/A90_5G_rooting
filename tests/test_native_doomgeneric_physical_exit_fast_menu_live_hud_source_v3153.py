...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3153_doomgeneric_physical_exit_fast_menu_live_hud.py"
)


class NativeDoomgenericPhysicalExitFastMenuLiveHudSourceV3153Tests(unittest.TestCase):
    def test_v3153_identity_and_contract(self) -> None:
        self.assertEqual(runner.CYCLE, "V3153")
        self.assertEqual(runner.INIT_VERSION, "0.10.135")
        self.assertEqual(
            runner.INIT_BUILD,
            "v3153-doomgeneric-physical-exit-fast-menu-live-hud",
        )
        ...
        self.assertIn(b"v3153-doomgeneric-physical-exit-fast-menu-live-hud", required)
    ...
    def test_restore_uses_fast_menu_live_hud_with_input_reader_restart(self) -> None:
        hud = Path("workspace/public/src/native-init/v319/30_status_hud.inc.c").read_text(
            encoding="utf-8"
        )
    ...
    def test_adapter_source_rewrites_to_v3153(self) -> None:
        source = runner.v3153_adapter_source()

        self.assertIn(
            "a90.doomgeneric.v3153.audio=real-sfx-pcm-stream-long-window-physical-exit-fast-menu-live-hud",
            source,
        )
        self.assertIn("a90.doomgeneric.v3153.demo_hud=large-grouped-hw-doom-input", source)
