...
class NativeAudioStopPlanApiV2762(unittest.TestCase):
    def test_stop_subcommand_is_exposed_as_first_class_audio_api(self) -> None:
        ...
        self.assertIn("audio.stop.execute_supported=0", text)
    ...
    def test_stop_execute_remains_blocked(self) -> None:
        text = source_text()

        self.assertIn("audio.stop.refused=execute-not-implemented-native-cleanup", text)
        self.assertRegex(
            text,
            re.compile(r'if \(execute_mode\).*?execute-not-implemented-native-cleanup.*?return -EPERM;', re.DOTALL),
        )
