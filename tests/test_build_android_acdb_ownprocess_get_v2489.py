...
class BuildAndroidAcdbOwnprocessGetV2489(unittest.TestCase):
    def test_source_state_matches_pure_read_boundary(self) -> None:
        ...
        self.assertTrue(state["required"]["uses_loader_namespace_fallback"])
        self.assertTrue(state["required"]["uses_loader_dlopen_ext_fallback"])
        self.assertTrue(state["required"]["uses_default_scope_symbol_probe"])
        self.assertTrue(state["required"]["records_symbol_probe_events"])
