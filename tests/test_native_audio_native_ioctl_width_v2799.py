"""Tests for the V2799 audio native ioctl width fix."""
...
BUILDER = REPO / "workspace/public/src/scripts/revalidation/build_native_init_boot_v2799_audio_native_ioctl_width.py"
LIVE = REPO / "workspace/public/src/scripts/revalidation/native_audio_native_ioctl_width_live_handoff_v2799.py"


def load_live_module():
    sys.path.insert(0, str(LIVE.parent))
    spec = importlib.util.spec_from_file_location("v2799_live", LIVE)
...
class NativeAudioNativeIoctlWidthV2799(unittest.TestCase):
    @classmethod
    ...
    def test_builder_uses_v2799_patch_artifact_identity(self) -> None:
        text = BUILDER.read_text(encoding="utf-8")

        self.assertIn('CYCLE = "V2799"', text)
        self.assertIn('INIT_VERSION = "0.9.312"', text)
        self.assertIn('INIT_BUILD = "v2799-audio-native-ioctl-width"', text)
        self.assertIn('boot_linux_v2799_audio_native_ioctl_width.img', text)
        self.assertIn('NATIVE_INIT_V2799_AUDIO_NATIVE_IOCTL_WIDTH_SOURCE_BUILD_2026-06-19.md', text)
        ...
        self.assertIn('"native_ioctl_width_compiled": True', text)
        ...
        self.assertIn('"v2798_blocker": "audio.setcal.execute.ioctl.0.rc=-1 errno=14"', text)
        self.assertIn('"/sys/class/misc/ion/dev"', text)
        self.assertIn('"/dev/ion"', text)

    def test_live_runner_targets_v2799_and_allowed_manifest_path(self) -> None:
        module = self.live
    ...
    def test_worker_output_classifies_v2799_efault_frontier(self) -> None:
        module = self.live
        summary = module.classify_play_output(
            "\n".join([
                "audio.play.worker.started=1",
