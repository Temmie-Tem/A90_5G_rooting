...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3312_gpu_2d_d3_video_texture_present_probe.py"
)


class NativeGpu2dD3VideoTexturePresentSourceV3312Tests(unittest.TestCase):
    def test_v3312_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3312")
        self.assertEqual(runner.INIT_VERSION, "0.11.84")
        self.assertEqual(runner.INIT_BUILD, "v3312-gpu-2d-d3-video-texture-present-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.84", required)
        self.assertIn(b"v3312-gpu-2d-d3-video-texture-present-probe", required)
