...
MANIFEST = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_15/manifest.json")
S10B_LABELS = ["S10B0", "S10B1", "S10B2", "S10B3", "S10B4", "S10B5", "S10B6"]
S10C_LABELS = ["S10C0"]
S11_LABELS = ["S11P0", "S11P1"]
STAGE_LABELS = ["S1", "S2", "S3", "S4", "S5", "S6", "S7A", "S7A2", "S8B1", "S8B1A", "S9", "S10A", *S10B_LABELS, *S10C_LABELS, *S11_LABELS]
...
class S22PlusM34RuntimeGadgetSplitBuildTest(unittest.TestCase):
    ...
    def test_stage_matrix_is_incremental_runtime_gadget_split(self):
        ...
        self.assertEqual([stage.number for stage in stages], list(range(1, 23)))
        ...
        self.assertFalse(by_label["S11P0"].configfs_gadget)
        self.assertFalse(by_label["S11P0"].udc_none)
        self.assertFalse(by_label["S11P0"].max_speed_high_speed)
        self.assertFalse(by_label["S11P0"].usb_role_force)
        self.assertFalse(by_label["S11P0"].ssusb_speed_high_speed)
        self.assertFalse(by_label["S11P0"].ssusb_mode_peripheral)
        self.assertFalse(by_label["S11P0"].udc_bind)
        self.assertFalse(by_label["S11P0"].soft_connect)
        self.assertTrue(by_label["S11P0"].stock_softdep_parity)
        self.assertTrue(by_label["S11P0"].qmp_module_included)
        self.assertTrue(by_label["S11P0"].eud_module_included)
        self.assertTrue(by_label["S11P0"].ucsi_glink_included)
        self.assertTrue(by_label["S11P0"].session_producer_parity)
        self.assertTrue(by_label["S11P0"].max77705_session_modules_included)
        self.assertFalse(by_label["S11P0"].typec_readback_markers)
        self.assertTrue(by_label["S11P0"].geni_i2c_transport_parity)
        self.assertFalse(by_label["S11P0"].typec_role_write_discriminator)
        self.assertIsNone(by_label["S11P0"].beacon_probe)
        self.assertEqual(by_label["S11P0"].module_load_probe, self.module.M34_S11P0_MODULE_LOAD_PROBE)
        self.assertTrue(by_label["S11P0"].devlink_supplier_closure)
        self.assertFalse(by_label["S11P1"].configfs_gadget)
        self.assertFalse(by_label["S11P1"].udc_none)
        self.assertFalse(by_label["S11P1"].max_speed_high_speed)
        self.assertFalse(by_label["S11P1"].usb_role_force)
        self.assertFalse(by_label["S11P1"].ssusb_speed_high_speed)
        self.assertFalse(by_label["S11P1"].ssusb_mode_peripheral)
        self.assertFalse(by_label["S11P1"].udc_bind)
        self.assertFalse(by_label["S11P1"].soft_connect)
        self.assertTrue(by_label["S11P1"].stock_softdep_parity)
        self.assertTrue(by_label["S11P1"].qmp_module_included)
        self.assertTrue(by_label["S11P1"].eud_module_included)
        self.assertTrue(by_label["S11P1"].ucsi_glink_included)
        self.assertTrue(by_label["S11P1"].session_producer_parity)
        self.assertTrue(by_label["S11P1"].max77705_session_modules_included)
        self.assertFalse(by_label["S11P1"].typec_readback_markers)
        self.assertTrue(by_label["S11P1"].geni_i2c_transport_parity)
        self.assertFalse(by_label["S11P1"].typec_role_write_discriminator)
        self.assertIsNone(by_label["S11P1"].beacon_probe)
        self.assertEqual(by_label["S11P1"].module_load_probe, self.module.M34_S11P1_MODULE_LOAD_PROBE)
        self.assertTrue(by_label["S11P1"].devlink_supplier_closure)
    ...
    def test_source_has_stage_guards_and_no_unintended_reboot_syscall(self):
        ...
        self.assertIn("#elif M34_STAGE == 21", text)
        self.assertIn("#elif M34_STAGE == 22", text)
        ...
        self.assertIn("phase=s11_proc_modules_positive_control_probe", text)
        self.assertIn("phase=s11p1_timed_loader_result_probe", text)
        ...
        self.assertIn("finit_cmd_db_accepted_and_watchdog_proc_visible", text)
        self.assertIn("timed_first_failure_or_proc_modules_result", text)
        self.assertIn("download_delay_sec=", text)
        self.assertIn("qcom_wdt_core", text)
        self.assertIn("gh_virt_wdt", text)
    ...
    def test_current_manifest_is_host_only_boot_only_stage_matrix(self):
        ...
        self.assertEqual(manifest["matrix"]["next_host_only_candidate"], "S11P1")
        ...
        self.assertEqual(manifest["matrix"]["s11p0_module_load_probe"], self.module.M34_S11P0_MODULE_LOAD_PROBE)
        self.assertEqual(manifest["matrix"]["s11p0_probe_module"], "cmd-db.ko")
        self.assertEqual(manifest["matrix"]["s11p0_probe_proc_name"], "cmd_db")
        self.assertEqual(manifest["matrix"]["s11p0_positive_control_proc_names"], self.module.M34_S11P0_POSITIVE_CONTROL_PROC_NAMES)
        self.assertEqual(manifest["matrix"]["s11p0_positive_control_modules"], self.module.M34_S11P0_POSITIVE_CONTROL_MODULES)
        self.assertEqual(manifest["matrix"]["s11p0_true_action"], "reboot(download)")
        self.assertEqual(manifest["matrix"]["s11p0_false_action"], "park")
        self.assertTrue(manifest["matrix"]["s11p0_starts_from_s10c0_module_recipe"])
        self.assertTrue(manifest["matrix"]["s11p0_uses_direct_finit_module_rc"])
        self.assertTrue(manifest["matrix"]["s11p0_beacon_hit_means_proc_modules_can_see_watchdog"])
        self.assertTrue(manifest["matrix"]["s11p0_skips_downstream_configfs_and_udc_to_isolate_module_load"])
        self.assertEqual(manifest["matrix"]["s11p1_module_load_probe"], self.module.M34_S11P1_MODULE_LOAD_PROBE)
        self.assertEqual(manifest["matrix"]["s11p1_probe_module"], "cmd-db.ko")
        self.assertEqual(manifest["matrix"]["s11p1_probe_proc_name"], "cmd_db")
        self.assertEqual(manifest["matrix"]["s11p1_positive_control_proc_names"], self.module.M34_S11P0_POSITIVE_CONTROL_PROC_NAMES)
        self.assertEqual(manifest["matrix"]["s11p1_positive_control_modules"], self.module.M34_S11P0_POSITIVE_CONTROL_MODULES)
        self.assertEqual(manifest["matrix"]["s11p1_true_action"], "timed_reboot(download)")
        self.assertEqual(manifest["matrix"]["s11p1_false_action"], "timed_reboot(download)")
        self.assertTrue(manifest["matrix"]["s11p1_always_reboots_download"])
        self.assertTrue(manifest["matrix"]["s11p1_timed_download_beacon"])
        self.assertEqual(manifest["matrix"]["s11p1_delay_model"], self.module.M34_S11P1_DELAY_MODEL)
        self.assertTrue(manifest["matrix"]["s11p1_maps_first_fail_index_to_module_list"])
        self.assertTrue(manifest["matrix"]["s11p1_starts_from_s10c0_module_recipe"])
        self.assertTrue(manifest["matrix"]["s11p1_uses_direct_finit_module_rc"])
        self.assertTrue(manifest["matrix"]["s11p1_skips_downstream_configfs_and_udc_to_isolate_module_load"])
        ...
        self.assertEqual(manifest["safety"]["auto_reboot"], "download-if-probe-true-or-s11p1-timed-always")
        self.assertTrue(manifest["safety"]["intended_reboot_syscall"])
        self.assertEqual(manifest["safety"]["reboot_request"], "download-if-probe-true-or-s11p1-timed-always")
        ...
        self.assertTrue(manifest["safety"]["stage_s11p0_starts_from_s10c0_module_recipe"])
        self.assertEqual(manifest["safety"]["stage_s11p0_module_load_probe"], self.module.M34_S11P0_MODULE_LOAD_PROBE)
        self.assertEqual(manifest["safety"]["stage_s11p0_positive_control_proc_names"], self.module.M34_S11P0_POSITIVE_CONTROL_PROC_NAMES)
        self.assertTrue(manifest["safety"]["stage_s11p0_true_reboot_download_false_park"])
        self.assertTrue(manifest["safety"]["stage_s11p0_no_configfs_udc_or_role_write"])
        self.assertTrue(manifest["safety"]["stage_s11p0_driver_load_only_no_manual_power_write"])
        self.assertTrue(manifest["safety"]["stage_s11p1_starts_from_s10c0_module_recipe"])
        self.assertEqual(manifest["safety"]["stage_s11p1_module_load_probe"], self.module.M34_S11P1_MODULE_LOAD_PROBE)
        self.assertEqual(manifest["safety"]["stage_s11p1_positive_control_proc_names"], self.module.M34_S11P0_POSITIVE_CONTROL_PROC_NAMES)
        self.assertTrue(manifest["safety"]["stage_s11p1_true_timed_reboot_download_false_timed_reboot_download"])
        self.assertTrue(manifest["safety"]["stage_s11p1_always_reboots_download_after_bounded_delay"])
        self.assertEqual(manifest["safety"]["stage_s11p1_delay_model"], self.module.M34_S11P1_DELAY_MODEL)
        self.assertTrue(manifest["safety"]["stage_s11p1_no_configfs_udc_or_role_write"])
        self.assertTrue(manifest["safety"]["stage_s11p1_driver_load_only_no_manual_power_write"])

        by_label = {stage["label"]: stage for stage in manifest["stages"]}
        if built_labels != STAGE_LABELS:
            self.assertIn(built_labels, (["S10C0"], ["S11P0"], ["S11P1"]))
            stage = by_label[built_labels[0]]
            self.assertEqual(stage["hashes"]["base_boot"], "2e541703951dc725bad35850faf7028c2d910dd5f21166449b63f1248c29967e")
            self.assertEqual(stage["hashes"]["m34_modules"], "c07425f4c738b53822e9f6783a142a2b5eafd72a15bd34c06fb3b49357c8fe26")
            self.assertEqual(stage["tar_members"], ["boot.img.lz4"])
            self.assertEqual(stage["ramdisk"]["module_files_injected_into_boot_ramdisk"], 0)
            self.assertEqual(stage["ramdisk"]["module_list_files_injected_into_boot_ramdisk"], 1)
            self.assertEqual(stage["closure"]["module_count"], 89)
            self.assertIn("cmd-db.ko", stage["closure"]["modules"])
            self.assertIn("sec_debug_region.ko", stage["closure"]["modules"])
            required = set(stage["init"]["required_strings"])
            if built_labels == ["S10C0"]:
                self.assertEqual(stage["stage_number"], 20)
                self.assertIn("version=0.11", required)
                self.assertIn("module_load_probe=finit_cmd_db_accepted", required)
                self.assertIn("proc_modules=0", required)
                self.assertIn("direct_finit_rc=1", required)
                self.assertIn("probe_module=cmd-db.ko", required)
                self.assertIn("phase=s10c_module_loader_audit_probe", required)
                self.assertIn("predicate=cmd_db_finit_accepted", required)
                self.assertIn("cmd_db_rc=", required)
                self.assertIn("true_action=reboot_download", required)
                self.assertIn("false_action=park", required)
                self.assertNotIn("/proc/modules", required)
            elif built_labels == ["S11P0"]:
                self.assertEqual(stage["stage_number"], 21)
                self.assertIn("version=0.12", required)
                self.assertIn(f"module_load_probe={self.module.M34_S11P0_MODULE_LOAD_PROBE}", required)
                self.assertIn("s11_proc_modules_positive_control=1", required)
                self.assertIn("proc_modules=1", required)
                self.assertIn("direct_finit_rc=1", required)
                self.assertIn("probe_module=cmd-db.ko", required)
                self.assertIn("positive_control=watchdog_proc_visible", required)
                self.assertIn("positive_control_proc_names=qcom_wdt_core,gh_virt_wdt", required)
                self.assertIn("positive_control_modules=qcom_wdt_core.ko,gh_virt_wdt.ko", required)
                self.assertIn("phase=s11_proc_modules_positive_control_probe", required)
                self.assertIn("predicate=cmd_db_finit_accepted_and_watchdog_proc_visible", required)
                self.assertIn("cmd_db_rc=", required)
                self.assertIn("cmd_db_proc_seen=", required)
                self.assertIn("qcom_wdt_core_proc_seen=", required)
                self.assertIn("gh_virt_wdt_proc_seen=", required)
                self.assertIn("watchdog_proc_seen=", required)
                self.assertIn("/proc/modules", required)
            else:
                self.assertEqual(stage["stage_number"], 22)
                self.assertIn("version=0.13", required)
                self.assertIn(f"module_load_probe={self.module.M34_S11P1_MODULE_LOAD_PROBE}", required)
                self.assertIn("s11p1_timed_loader_result=1", required)
                self.assertIn("proc_modules=1", required)
                self.assertIn("timed_download_beacon=1", required)
                self.assertIn("always_reboot_download=1", required)
                self.assertIn("download_delay_model=first_fail_index_or_proc_result", required)
                self.assertIn("phase=s11p1_timed_loader_result_probe", required)
                self.assertIn("predicate=timed_first_failure_or_proc_modules_result", required)
                self.assertIn("present=1", required)
                self.assertIn("result_code=", required)
                self.assertIn("result=", required)
                self.assertIn("download_delay_sec=", required)
                self.assertIn("first_fail_index=", required)
                self.assertIn("first_fail_rc=", required)
                self.assertIn("first_fail_name=", required)
                self.assertIn("cmd_db_seen=", required)
                self.assertIn("cmd_db_rc=", required)
                self.assertIn("direct_cmd_db=", required)
                self.assertIn("cmd_db_proc_seen=", required)
                self.assertIn("qcom_wdt_core_proc_seen=", required)
                self.assertIn("gh_virt_wdt_proc_seen=", required)
                self.assertIn("watchdog_proc_seen=", required)
                self.assertIn("modules_open_or_read_fail", required)
                self.assertIn("cmd_db_not_attempted", required)
                self.assertIn("cmd_db_rc_fail", required)
                self.assertIn("first_module_failure", required)
                self.assertIn("proc_watchdog_missing", required)
                self.assertIn("watchdog_visible_cmd_db_proc_missing", required)
                self.assertIn("proc_watchdog_and_cmd_db_visible", required)
                self.assertIn("true_action=timed_reboot_download", required)
                self.assertIn("false_action=timed_reboot_download", required)
                self.assertIn("phase=s11p1_timed_loader_result_reboot_returned", required)
                self.assertIn("/proc/modules", required)
            return
        self.assertEqual(
