...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3240_gpu_h3_sample_location_probe.py"
)


class NativeGpuH3SampleLocationSourceV3240Tests(unittest.TestCase):
    def test_v3240_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3240")
        self.assertEqual(runner.INIT_VERSION, "0.11.47")
        self.assertEqual(runner.INIT_BUILD, "v3240-gpu-h3-sample-location-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.47", required)
        self.assertIn(b"v3240-gpu-h3-sample-location-probe", required)
        self.assertIn(
    ...
    def test_dispatch_programs_mesa_sample_location_registers(self) -> None:
        ...
        self.assertIn("#define GPU_H3_VS_OUTPUT_REGID 4U", source)
        self.assertIn("#define GPU_H3_PS_OUTPUT_REGID 4U", source)
        ...
        self.assertIn("GPU_H3_IR3_MOV_F32F32_R1X_R0X_LO, GPU_H3_IR3_MOV_F32F32_R1X_R0X_HI", source)
        self.assertIn("GPU_H3_IR3_MOV_F32F32_R1Y_R0Y_LO, GPU_H3_IR3_MOV_F32F32_R1Y_R0Y_HI", source)
        self.assertIn("GPU_H3_IR3_F32_0_LO, GPU_H3_IR3_MOV_F32F32_R1Z_HI", source)
        self.assertIn("GPU_H3_IR3_F32_1_LO, GPU_H3_IR3_MOV_F32F32_R1W_HI", source)
        self.assertIn("static const uint32_t fs_shader[GPU_H3_FS_SHADER_DWORDS]", source)
        self.assertIn("GPU_H3_IR3_F32_1_LO, GPU_H3_IR3_MOV_F32F32_R1X_HI", source)
    ...
    def test_builder_manifest_records_sample_location_boundary(self) -> None:
        ...
        self.assertIn('"scope": "first-triangle-h3-sample-location-disable-r1-footprint2-mov-f32-shader"', source)
        self.assertIn("verify-fresh-boot-fwclass-path-before-h3", source)
        self.assertIn("gpu-h3-sample-location-disable-r1-footprint2-mov-f32-shader-timeout-guard", source)
        ...
        self.assertIn('"sample_location_source": "Mesa A6xx fd6_context sample_locations_disable_stateobj"', source)
        ...
        self.assertIn('"state_reg_writes_expected": 91', source)
        self.assertIn('"pm4_dwords_expected": 229', source)
        ...
        self.assertIn("preserve-v3232-ramdisk-overlay-v3240-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3240"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3240 boot image too large for boot partition", source)
