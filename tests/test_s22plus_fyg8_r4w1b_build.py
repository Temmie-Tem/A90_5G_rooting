...
SCRIPT = ROOT / (
    "workspace/public/src/scripts/revalidation/s22plus_fyg8_r4w1b_build.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("s22plus_fyg8_r4w1b_build_tested", SCRIPT)
...
class S22PlusFyg8R4W1BBuildTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def make_outputs(
        ...
        historical = self.module.HISTORICAL_R4W1_MARKER_FAMILY
        payload = marker * marker_count + historical * historical_count
        (dist / "Image").write_bytes(payload + bytes(max(0, image_size - len(payload))))
        (dist / "vmlinux").write_bytes(
            marker * marker_count + historical * historical_count
        )
        config.write_text(
    ...
    def test_witness_gate_accepts_exact_marker_and_config(self):
        ...
        with mock.patch.object(
            self.module, "FIXED_KERNEL_SLOT_CAPACITY", 4096
        ), mock.patch.object(self.module, "STOCK_IMAGE_SIZE", 1024):
    ...
    def test_witness_gate_rejects_missing_fips(self):
        temporary, root = self.make_outputs(fips_count=0)
        self.addCleanup(temporary.cleanup)
        with mock.patch.object(
            self.module, "FIXED_KERNEL_SLOT_CAPACITY", 4096
        ), mock.patch.object(self.module, "STOCK_IMAGE_SIZE", 1024):
    ...
    def test_witness_gate_rejects_historical_family(self):
        temporary, root = self.make_outputs(historical_count=1)
        self.addCleanup(temporary.cleanup)
        with mock.patch.object(
            self.module, "FIXED_KERNEL_SLOT_CAPACITY", 4096
        ), mock.patch.object(self.module, "STOCK_IMAGE_SIZE", 1024):
        ...
        self.assertEqual(result["image_historical_family_count"], 1)
    ...
    def test_witness_gate_rejects_layout_overflow(self):
        ...
        with mock.patch.object(
            self.module, "FIXED_KERNEL_SLOT_CAPACITY", 4096
        ), mock.patch.object(self.module, "STOCK_IMAGE_SIZE", 4097):
            result = self.module.witness_output_gate(root)
        self.assertFalse(result["verified"])
    ...
    def test_witness_gate_rejects_smaller_alignment_bucket(self):
        ...
        self.addCleanup(temporary.cleanup)
        with mock.patch.object(
            self.module, "FIXED_KERNEL_SLOT_CAPACITY", 8192
        ), mock.patch.object(self.module, "STOCK_IMAGE_SIZE", 4096):
            result = self.module.witness_output_gate(root)
        self.assertFalse(result["verified"])
