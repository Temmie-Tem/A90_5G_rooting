...
class NativeDoomgenericVisibleLoopSourceV3033Tests(unittest.TestCase):
    def test_builder_contract_pins_v3033_visible_loop_candidate(self) -> None:
        ...
        self.assertEqual(runner.TICK_TELEMETRY_PATH, "")
        self.assertEqual(runner.TICK_TELEMETRY_SUMMARY, 0)
    ...
    def test_video_doom_loop_commands_and_background_presenter_are_wired(self) -> None:
        ...
        self.assertIn("VIDEO_DEMO_DOOMGENERIC_TICK_TELEMETRY_SUMMARY", text)
        self.assertIn("video_demo_doom_tick_telemetry_summary_print", text)
        self.assertIn("%s.tick_telemetry.summary=1", text)
        self.assertIn("%s.tick_telemetry.%s", text)
