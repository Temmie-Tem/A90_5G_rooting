...
runner = load_script("workspace/public/src/scripts/revalidation/build_native_init_boot_v3139_doomgeneric_demo_hud_sectioned.py")


class NativeDoomgenericDemoHudSectionedSourceV3139Tests(unittest.TestCase):
    def test_builder_contract_pins_v3139_candidate(self) -> None:
        self.assertEqual(runner.CYCLE, "V3139")
        self.assertEqual(runner.INIT_VERSION, "0.10.123")
        self.assertEqual(runner.INIT_BUILD, "v3139-doomgeneric-demo-hud-sectioned")
        self.assertEqual(runner.ENGINE_RAMDISK_PATH, "bin/a90_doomgeneric_private_engine_v3139")
        self.assertEqual(runner.ENGINE_NAME, "doomgeneric-private-link-v3139-demo-hud-sectioned")
        ...
        self.assertIn(b"video.demo.doom.dashboard.text_scale=section3-main3-small2", runner.REQUIRED_STRINGS)
        self.assertIn(b"video.demo.doom.dashboard.layout=top-frame-sectioned-hw-doom-input-footer", runner.REQUIRED_STRINGS)
    ...
    def test_native_hud_source_has_sectioned_layout_flag_and_strings(self) -> None:
        source = (REPO_ROOT / "workspace/public/src/native-init/v319/30_status_hud.inc.c").read_text(encoding="utf-8")

        self.assertIn("#define A90_DOOMGENERIC_NATIVE_DEMO_HUD_SECTIONED 0", source)
        self.assertIn("video.demo.doom.dashboard.sectioned_info=1", source)
        self.assertIn("video.demo.doom.dashboard.text_scale=section3-main3-small2", source)
        self.assertIn("top-frame-sectioned-hw-doom-input-footer", source)
        self.assertIn("sectioned HW / DOOM / key input HUD", source)
        self.assertIn("HW INFO", source)
        self.assertIn("DOOM INFO", source)
        self.assertIn("KEY INPUT", source)
        self.assertIn("CPU %s %s   GPU %s %s", source)

    def test_base_builders_expose_sectioned_compile_flag(self) -> None:
        for relative in (
            ...
            self.assertIn("NATIVE_DEMO_HUD_SECTIONED", source)

        base_source = (
            REPO_ROOT
            / "workspace/public/src/scripts/revalidation/build_native_init_boot_v3033_doomgeneric_visible_loop.py"
        ).read_text(encoding="utf-8")
        self.assertIn("A90_DOOMGENERIC_NATIVE_DEMO_HUD_SECTIONED", base_source)

    def test_adapter_source_rewrites_v3137_identity_to_v3139(self) -> None:
        source = runner.v3139_adapter_source()
    ...
    def test_configure_v3139_module_repoints_v3137_and_sectioned_flag(self) -> None:
        saved = runner.configure_v3139_module()
        try:
            ...
            self.assertIs(runner.v3137.v3137_adapter_source, runner.v3139_adapter_source)
    ...
    def test_report_template_records_sectioned_hud_delta(self) -> None:
        manifest = {
            "decision": runner.DECISION,
            "boot_image": "workspace/private/inputs/boot_images/boot_linux_v3139.img",
        ...
        self.assertIn("Native Init V3139 DOOMGENERIC Sectioned Demo HUD Source Build", report)
        self.assertIn("A90_DOOMGENERIC_NATIVE_DEMO_HUD_SECTIONED=1", report)
