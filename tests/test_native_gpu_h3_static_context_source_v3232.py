...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3232_gpu_h3_static_context_probe.py"
)


class NativeGpuH3StaticContextSourceV3232Tests(unittest.TestCase):
    def test_v3232_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3232")
        self.assertEqual(runner.INIT_VERSION, "0.11.43")
        self.assertEqual(runner.INIT_BUILD, "v3232-gpu-h3-static-context-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.43", required)
        self.assertIn(b"v3232-gpu-h3-static-context-probe", required)
        self.assertIn(
            b"gpu.h3.draw.scope=first-triangle-h3-static-context-mov-f32-shader",
            required,
        )
        self.assertIn(
        ...
        self.assertIn(
            b"gpu.h3.draw.static_context_source=mesa-freedreno-a6xx-fd6-emit-static-context-regs",
            required,
        )
    ...
    def test_dispatch_programs_mesa_static_context_registers(self) -> None:
    ...
    def test_builder_manifest_records_static_context_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"source_baseline": "v3230-gpu-h3-vpc-lm-siv-probe"', source)
        ...
        self.assertIn("preserve-v3230-ramdisk-overlay-v3232-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3232"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3232 boot image too large for boot partition", source)
