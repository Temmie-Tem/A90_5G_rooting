"""Host-only tests for the V2560 ACDB per-device manifest live wrapper."""
...
v2560 = load_revalidation("native_audio_acdb_per_device_manifest_live_handoff_v2560")
...
class AcdbPerDeviceManifestLiveV2560(unittest.TestCase):
    def test_to_v2490_args_forces_v2559_artifacts_and_fake_allocate(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2560-args-"))
        helper = root / "helper"
        ...
        args = Namespace(
            ...
            helper_timeout=90.0,
            android_root_recheck_attempts=v2560.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_ATTEMPTS,
            android_root_recheck_sleep_sec=v2560.v2490.v2396.DEFAULT_ANDROID_ROOT_RECHECK_SLEEP_SEC,
            android_settle_adb_retry_attempts=v2560.v2490.DEFAULT_SETTLE_ADB_RETRY_ATTEMPTS,
            android_settle_adb_retry_sleep_sec=v2560.v2490.DEFAULT_SETTLE_ADB_RETRY_SLEEP_SEC,
            readelf="readelf",
            file="file",
        )

        base = v2560.to_v2490_args(args, artifacts)
    ...
    def test_summarize_per_device_manifest_accepts_nonzero_non_topology_record(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2560-per-device-"))
        ...
        summary = v2560.summarize_per_device_manifest(artifact)

        self.assertEqual(summary["classification"], "v2560-per-device-manifest-captured")
    ...
    def test_real_audio_set_passthrough_is_boundary_violation(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2560-boundary-"))
        artifact = root / "device"
        acdbtap = artifact / "acdbtap"
        ...
        self.assertEqual(summary["classification"], "v2560-boundary-violation-real-audio-set-passthrough")
    ...
    def test_topology_recapture_without_helper_marker_is_not_per_device_success(self) -> None:
        root = Path(tempfile.mkdtemp(prefix="a90-v2560-topology-"))
        artifact = root / "device"
        acdbtap = artifact / "acdbtap"
        ...
        self.assertEqual(summary["classification"], "v2560-init-common-topology-recaptured-before-per-device")
        self.assertFalse(summary["full_success"])
