...
SCRIPT = Path("workspace/public/src/scripts/revalidation/s22plus_m34_s3_runtime_gadget_live_gate.py")
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location("s22plus_m34_s3_runtime_gadget_live_gate", SCRIPT)
...
class S22PlusM34S3RuntimeGadgetLiveGateTest(unittest.TestCase):
    def setUp(self):
        self.module = load_module()

    def test_policy_markers_include_hashes_tokens_and_stock_s3_semantics(self):
    ...
    def test_agents_exception_draft_satisfies_same_policy_markers(self):
        ...
        self.assertIn("authorize S1/S2 repeat, S3 repeat", draft)
    ...
    def test_verify_agents_exception_rejects_draft_only_block(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("draft-only M34 S3", str(caught.exception))
    ...
    def test_current_manifest_contract_matches_live_gate(self):
        with tempfile.TemporaryDirectory() as tmp:
            log_path = Path(tmp) / "m34_s3_manifest_check.log"
            self.module.verify_m34_manifest(MANIFEST, log_path)
            text = log_path.read_text(encoding="utf-8")
            self.assertIn("m34_s3_manifest_hashes=", text)
            self.assertIn("m34_manifest_safety=", text)
            self.assertIn("m34_s3_manifest_runtime_steps=", text)
            self.assertIn("m34_s3_manifest_closure=", text)
    ...
    def test_acm_match_requires_stock_samsung_vid_pid(self):
        self.assertTrue(self.module.is_m34_s3_acm({"vendor": "04E8", "product": "6860"}))
        self.assertFalse(self.module.is_m34_s3_acm({"vendor": "04e8", "product": "685d"}))
        self.assertFalse(self.module.is_m34_s3_acm({"vendor": "18d1", "product": "4ee7"}))
