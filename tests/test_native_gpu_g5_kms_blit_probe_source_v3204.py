...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3204_gpu_g5_kms_blit_probe.py"
)


class NativeGpuG5KmsBlitProbeSourceV3204Tests(unittest.TestCase):
    def test_v3204_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3204")
        self.assertEqual(runner.INIT_VERSION, "0.11.30")
        self.assertEqual(runner.INIT_BUILD, "v3204-gpu-g5-kms-blit-probe")

        required = b"\n".join(runner.REQUIRED_STRINGS)
        self.assertIn(b"0.11.30", required)
        self.assertIn(b"v3204-gpu-g5-kms-blit-probe", required)
        ...
        self.assertIn(b"kms-blit-probe", required)
        self.assertIn(b"gpu.g5.kms.version=1", required)
        self.assertIn(
            b"gpu.g5.kms.scope=kgsl-a2d-solid-fill-readback-to-kms-dumb-blit-probe",
            required,
        )
        self.assertIn(b"gpu.g5.kms.parent_enters_kgsl_open=0", required)
        self.assertIn(b"gpu.g5.kms.parent_enters_kgsl_ioctl=0", required)
        self.assertIn(
            b"gpu.g5.kms.blit_mode=kgsl-private-buffer-readback-to-kms-dumb-framebuffer",
            required,
        )
        self.assertIn(b"gpu.g5.kms.zero_copy_attempted=0", required)
        self.assertIn(b"gpu.g5.kms.kms_blit_attempted=1", required)
        self.assertIn(b"gpu.g5.kms.g4_readback_verified=%d", required)
        self.assertIn(b"gpu.g5.kms.blit_raw_pixel=0x%x", required)
        self.assertIn(b"gpu.g5.kms.present_rc=%d", required)
        self.assertIn(b"gpu.g5.kms.result=%s", required)
    ...
    def test_dispatch_exposes_bounded_child_only_solid_fill_probe(self) -> None:
        ...
        self.assertIn("static int gpu_g5_kms_blit_probe(int timeout_ms", source)
        self.assertIn("static int gpu_g5_blit_gpu_readback_to_kms", source)
        self.assertIn("gpu_g4_solid_fill_collect_child", source)
        self.assertIn("gpu_g4_solid_fill_result_ok", source)
        self.assertIn("a90_kms_begin_frame(0x050505)", source)
        self.assertIn("a90_kms_framebuffer()", source)
        self.assertIn("a90_kms_present(\"gpu-g5-kms-blit\", true)", source)
        self.assertIn('"gpu.g5.kms.parent_enters_kgsl_open=0', source)
        self.assertIn('"gpu.g5.kms.parent_enters_kgsl_ioctl=0', source)
        self.assertIn('"gpu.g5.kms.blit_mode=kgsl-private-buffer-readback-to-kms-dumb-framebuffer', source)
        self.assertIn('"gpu.g5.kms.zero_copy_attempted=0', source)
        self.assertIn('"gpu.g5.kms.kms_blit_attempted=1', source)
        self.assertIn('"gpu.g5.kms.blit_raw_pixel=0x%x', source)
        self.assertIn('"gpu.g5.kms.present_rc=%d', source)
    ...
    def test_help_lists_g0_g1_g2_g3_g4_and_g5_commands(self) -> None:
        source = DISPATCH.read_text(encoding="utf-8")
    ...
    def test_builder_manifest_records_g5_safety_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"gpu g5-kms-blit-probe --timeout-ms 5000 --materialize-devnode"', source)
        ...
        self.assertIn('"gpu_g5": {', source)
        self.assertIn('"display_path": "/dev/dri/card0"', source)
        self.assertIn('"zero_copy_attempted": False', source)
        self.assertIn('"expected_raw_pixel": "0xa5c3f00d"', source)
        self.assertIn('"parent_enters_kgsl_open": False', source)
        self.assertIn('"parent_enters_kgsl_ioctl": False', source)
        ...
        self.assertIn("NATIVE_INIT_V3203_GPU_G4_CCU_SEQNO_LIVE_PASS_2026-06-25.md", source)
