...
runner = load_script("workspace/public/src/scripts/server-distro/run_wsta79_persistent_operator_packet_status.py")
SOURCE = Path("workspace/public/src/scripts/server-distro/run_wsta79_persistent_operator_packet_status.py")


class ServerDistroWsta79PersistentOperatorPacketStatusTests(unittest.TestCase):
    def private_tmp(self):
        runner.DEFAULT_RUN_BASE.mkdir(parents=True, exist_ok=True)
        return tempfile.TemporaryDirectory(dir=runner.DEFAULT_RUN_BASE)

    def prepare_operator_packet(self, root: Path, ttl_sec: int = 300) -> dict[str, Path]:
    ...
    def test_default_run_is_fail_closed_and_device_inert(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            with mock.patch.object(runner.wsta78.wsta77.wsta76.wsta75.wsta74.wsta73.wsta71.wsta65.wsta64.wsta58.wsta55, "run", side_effect=AssertionError("unexpected live WSTA55")):
                result = runner.run(runner.build_arg_parser().parse_args([
                    "--run-dir",
                    str(root / "status"),
                ]))

        self.assertEqual(result["decision"], "wsta79-blocked-operator-packet-required")
        for key in (
            "device_action",
            "boot_flash",
            "native_reboot",
    ...
    def test_ready_operator_packet_status_rechecks_and_reports_ready(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            ...
            saved = json.loads((root / "status" / "wsta79_operator_packet_status.json").read_text(encoding="utf-8"))
            markdown = (root / "status" / "wsta79_operator_packet_status.md").read_text(encoding="utf-8")

        self.assertEqual(result["decision"], runner.PASS_DECISION)
        self.assertEqual(saved["decision"], runner.PASS_DECISION)
        status = result["operator_packet_status"]
        self.assertEqual(status["state"], "READY_TO_RUN_DEFAULT_OFF")
        self.assertTrue(status["ready_for_live"])
        self.assertEqual(status["wsta78_recheck_decision"], wsta78.PASS_DECISION)
        self.assertEqual(status["selected_wsta76_launch_brief"], runner.rel(artifacts["brief"]))
    ...
    def test_operator_packet_that_ages_stale_reports_not_ready_without_live_action(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            artifacts = self.prepare_operator_packet(root, ttl_sec=60)
            initial = json.loads(artifacts["initial"].read_text(encoding="utf-8"))
            expires = runner.wsta78.wsta77.wsta76.wsta75.wsta74.wsta73.wsta72.wsta67.wsta65.wsta64.parse_utc_stamp(initial["expires_utc"])
        ...
        self.assertEqual(status["wsta78_recheck_decision"], "wsta78-blocked-no-ready-brief")
        self.assertEqual(status["recommended_next_action"], "rerun-wsta72-through-wsta78")

    def test_drifted_operator_packet_reports_drift_without_live_action(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            artifacts = self.prepare_operator_packet(root)
            payload = json.loads(artifacts["operator"].read_text(encoding="utf-8"))
            payload["operator_packet"]["selected_wsta73_arming_packet"] = runner.rel(root / "drift" / "wsta73_arming_packet.json")
            drifted = root / "operator" / "wsta78_operator_packet_drifted.json"
    ...
    def test_nonpass_operator_packet_is_rejected(self) -> None:
        with self.private_tmp() as tmp:
            root = Path(tmp)
            packet = root / "bad" / "wsta78_operator_packet.json"
            packet.parent.mkdir(parents=True)
            packet.write_text(json.dumps({
                "decision": "wsta78-blocked-no-ready-brief",
                "operator_packet": {},
            }), encoding="utf-8")
            result = runner.run(runner.build_arg_parser().parse_args([
                "--run-dir",
        ...
        self.assertEqual(result["decision"], "wsta79-blocked-operator-packet-not-pass")
    ...
    def test_public_summary_markdown_and_template_are_redacted(self) -> None:
        ...
        for text in (summary_text, template_text, markdown):
            ...
            self.assertNotIn(runner.wsta78.wsta77.wsta76.wsta75.wsta74.wsta73.wsta72.wsta63.wsta58.wsta55.wsta45.wsta25.NATIVE_CONFIRM_TOKEN, text)
            self.assertNotIn(runner.wsta78.wsta77.wsta76.wsta75.wsta74.wsta73.wsta72.wsta63.wsta58.wsta55.wsta45.PUBLIC_CONFIRM_TOKEN, text)
    ...
    def test_print_template_exits_without_status(self) -> None:
        ...
        self.assertIn("WSTA79 host-only", payload)
        self.assertIn("--wsta78-operator-packet-json", payload)
