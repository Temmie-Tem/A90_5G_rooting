...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3192_gpu_g2_mmap_probe.py"
)


class NativeGpuG2MmapProbeSourceV3192Tests(unittest.TestCase):
    def test_v3192_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3192")
        self.assertEqual(runner.INIT_VERSION, "0.11.24")
        self.assertEqual(runner.INIT_BUILD, "v3192-gpu-g2-mmap-probe")

        required = b"\n".join(runner.REQUIRED_STRINGS)
        self.assertIn(b"0.11.24", required)
        self.assertIn(b"v3192-gpu-g2-mmap-probe", required)
        ...
        self.assertIn(b"gpu.g2.gpuobj.mmap_attempted=%d", required)
        self.assertIn(b"gpu.g2.gpuobj.mmap_offset_rule=id_times_4096", required)
        self.assertIn(b"gpu.g2.gpuobj.mmap_access_attempted=0", required)
        self.assertIn(b"gpu.g2.gpuobj.mmap_rc=%d", required)
        self.assertIn(b"gpu.g2.gpuobj.munmap_rc=%d", required)
        self.assertIn(b"gpu.g2.gpuobj.submit_attempted=0", required)
        self.assertIn(b"gpu.g2.gpuobj.power_write_attempted=0", required)

    def test_dispatch_exposes_bounded_child_only_mmap_probe(self) -> None:
        source = DISPATCH.read_text(encoding="utf-8")
        self.assertIn('strcmp(subcommand, "g2-mmap-probe")', source)
        self.assertIn('strcmp(subcommand, "mmap-probe")', source)
        self.assertIn("static int gpu_g2_gpuobj_probe_child(int write_fd, bool do_mmap)", source)
        ...
        self.assertIn("GPU_G2_MMAP_PAGE_SIZE 4096ULL", source)
        self.assertIn("mmap(NULL,", source)
        self.assertIn("PROT_READ | PROT_WRITE", source)
        self.assertIn("MAP_SHARED", source)
        self.assertIn("munmap(map", source)
        self.assertIn('"gpu.g2.gpuobj.parent_enters_open=0', source)
        self.assertIn('"gpu.g2.gpuobj.parent_enters_ioctl=0', source)
        self.assertIn('"gpu.g2.gpuobj.mmap_attempted=%d', source)
        self.assertIn('"gpu.g2.gpuobj.mmap_offset_rule=id_times_4096', source)
        self.assertIn('"gpu.g2.gpuobj.mmap_access_attempted=0', source)
    ...
    def test_help_lists_g0_g1_and_g2_commands(self) -> None:
        ...
        expected = "g2-mmap-probe [--timeout-ms N] [--materialize-devnode]"
    ...
    def test_builder_manifest_records_g2b_safety_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"gpu g2-mmap-probe --timeout-ms 5000 --materialize-devnode"', source)
        ...
        self.assertIn('"mmap_offset_rule"', source)
        self.assertIn('"mmap_access_attempted": False', source)
        self.assertIn('"kgsl-mapped-memory-read"', source)
        self.assertIn('"kgsl-mapped-memory-write"', source)
        self.assertIn('"kgsl-submit"', source)
        self.assertIn('"freedreno-render"', source)
        self.assertIn('"GDSC-write"', source)
        self.assertIn("NATIVE_INIT_V3191_GPU_G2_GPUOBJ_PROBE_LIVE_2026-06-25.md", source)
