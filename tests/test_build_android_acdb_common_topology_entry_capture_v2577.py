"""Host-only tests for the V2577 common-topology entry-arm ACDB capture build."""
...
v2577 = load_revalidation("build_android_acdb_common_topology_entry_capture_v2577")


def args(**overrides: object) -> Namespace:
    root = Path(tempfile.mkdtemp(prefix="a90-v2577-build-test-"))
...
class BuildAndroidAcdbCommonTopologyEntryCaptureV2577(unittest.TestCase):
    def test_source_state_requires_common_topology_entry_hook(self) -> None:
        state = v2577.source_state()
        ...
        self.assertTrue(state["required"]["helper_init_only_no_arm"])
        self.assertTrue(state["required"]["common_hook_exports_common_topology"])
        self.assertTrue(state["required"]["common_hook_arms_before_real"])
        self.assertTrue(state["required"]["common_hook_calls_real_common_topology"])

    def test_manifest_declares_common_topology_entry_arm_compile_flags(self) -> None:
        payload = v2577.manifest(args())
        ...
        self.assertTrue(payload["capture_contract"]["arm_point"].startswith("common-topology entry-arm"))

    @unittest.skipUnless(
        (v2577.TOOLCHAIN_ROOT / "bin/clang").exists() and (v2577.TOOLCHAIN_ROOT / "bin/ld.lld").exists(),
        "private Android clang/lld unavailable",
    )
    def test_build_outputs_helper_and_single_combined_preload(self) -> None:
        payload = v2577.manifest(args(build=True))
        build = payload["build"]
    ...
    def test_cli_writes_manifest(self) -> None:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "workspace/public/src/scripts/revalidation/build_android_acdb_common_topology_entry_capture_v2577.py",
                "--build-root",
