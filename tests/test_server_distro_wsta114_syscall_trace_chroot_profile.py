...
class ServerDistroWsta114SyscallTraceChrootProfileTests(unittest.TestCase):
    ...
    def test_trace_probe_script_runs_strace_around_smoke_service(self) -> None:
        ...
        self.assertIn("A90WSTA114_PROC_UNMOUNT_DEFERRED=1", script)
        self.assertIn("a90-dpublic-smoke-httpd", script)
        self.assertIn(': > "$TRACE"', script)
        self.assertIn('/bin/chmod 0666 "$TRACE"', script)
        self.assertIn('"$LAUNCHER" dpublic-smoke-httpd "$STRACE" -qq -f -s 96 -o "$TRACE" "$SMOKE" 127.0.0.1 8080', script)
        self.assertIn('"$HTTP_GET" 127.0.0.1 8080', script)
