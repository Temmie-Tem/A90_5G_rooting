...
runner = load_script(
    "workspace/public/src/scripts/revalidation/build_native_init_boot_v3236_gpu_h3_shader_footprint_probe.py"
)


class NativeGpuH3ShaderFootprintSourceV3236Tests(unittest.TestCase):
    def test_v3236_identity_and_required_markers(self) -> None:
        self.assertEqual(runner.CYCLE, "V3236")
        self.assertEqual(runner.INIT_VERSION, "0.11.45")
        self.assertEqual(runner.INIT_BUILD, "v3236-gpu-h3-shader-footprint-probe")
        self.assertEqual(
        ...
        self.assertIn(b"0.11.45", required)
        self.assertIn(b"v3236-gpu-h3-shader-footprint-probe", required)
        self.assertIn(
    ...
    def test_dispatch_programs_mesa_shader_footprint_registers(self) -> None:
    ...
    def test_builder_manifest_records_shader_footprint_boundary(self) -> None:
        source = Path(runner.__file__).read_text(encoding="utf-8")
        self.assertIn('"source_baseline": "v3234-r1-shader-output-source-on-v3232-ramdisk-base"', source)
        self.assertIn('"scope": "first-triangle-h3-shader-footprint-r1-footprint2-mov-f32-shader"', source)
        ...
        self.assertIn("preserve-v3232-ramdisk-overlay-v3236-init-helper-engine", source)
        self.assertIn('"bin/a90_doomgeneric_private_engine_v3236"', source)
        self.assertIn("BOOT_PARTITION_MAX_BYTES = 64 * 1024 * 1024", source)
        self.assertIn("V3236 boot image too large for boot partition", source)
