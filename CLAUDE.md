...
- **Active research cycle**: V1311 PASS → V1312 deploy helper v275 only — V1311 updated `a90_android_execns_probe.c` to helper marker `v275` and added `--pm-observer-late-per-proxy-lower-sequence-summary-sampler`. The new mode `late-per-proxy-lower-sequence-summary` keeps the bounded late-`per_proxy` route but outputs aggregate `response_summary.*` keys instead of repeated per-sample PMIC/GDSC blocks, targeting the full `80` samples at `50ms` without the `1MiB` stdout cap. Static aarch64 build PASS, sha256 `66e52e7507dd07bcb4071afd04bc60e51d1c6bb7b9cb7363205f1eb4f44d4677`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
