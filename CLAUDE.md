...
- **Active research cycle**: V1250 READ-ONLY LIVE PASS → V1251 debugfs read-surface preflight — V1249 deployed helper `a90_android_execns_probe v260` to `/cache/bin/a90_android_execns_probe` over serial fallback (`0313d613d95c56af5681871062b7fceb47ede3c3ef8fcff534d0eea3338eaa2f`). V1250 ran `wifi-companion-pmic-soft-reset-preflight` with `--allow-pmic-soft-reset-preflight` and confirmed all zero-action markers, but classified `read-only-incomplete`: global preflight saw `debugfs_pinctrl_present=0` and `debugfs_regulator_present=0`, so PMIC soft-reset/GDSC lines were not observable; `mdm3=OFFLINING` and GPIO142 IRQ count stayed `0`. Next V1251 should temporarily mount debugfs for read-only observation, run the same preflight, then unmount/cleanup and verify selftest. Still no PMIC/GPIO/debugfs/regulator write, eSoC ioctl, Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
