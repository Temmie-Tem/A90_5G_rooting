...
- **Active research cycle**: V1448 case-aligned micro endpoint classifier PASS (`v1448-case-aligned-micro-all-low-no-l0`). V1447 rollbackable live handoff of the V1445 image passed and returned to healthy v724/selftest. It still fails before `LTSSM L0` with no MHI/WLFW/BDF/FW-ready/`wlan0`. V1448 closes the sampler-alignment issue: all nine parent samples landed after the actual corrected RC1 `case=11` write returned, and GPIO135/GPIO142 stayed low from `0ms` through `150ms` after case completion. Next gate should be V1449 host-only provider-vs-RC1 timing analysis before another live mutation. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
