...
- **Active research cycle**: V1429 source/build-only PASS (`v1429-wifi-test-boot-endpoint-prereq-source-build-pass`). V1429 built rollbackable test image `tmp/wifi/v1429-wifi-test-boot-endpoint-prereq-sampler/boot_linux_v1429_wifi_test.img` with native init `0.9.78 (v1429-wifitest)`. It keeps the V1425 250ms corrected-RC1 watcher but removes retry widening and adds a read-only endpoint sampler around PERST release for GPIO102/PERST, GPIO103/CLKREQ, GPIO104/WAKE, `pcie_1_gdsc`, pcie1 refclk/pipe clocks, GPIO142/MDM2AP IRQ, pcie1 link state files, and LTSSM terminal state. Next gate should be V1430 local-only artifact sanity for this exact manifest before any live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
