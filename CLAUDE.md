...
- **Active research cycle**: V1244 HOST-ONLY PASS → V1245 PMIC/GDSC reachability planned — V1244 added `scripts/revalidation/native_wifi_android_power_surface_classifier_v1244.py` and compared Android-positive evidence against native V1243. Android claims PM8150L soft-reset `gpio9` as output and reaches PCIe RC1/WLAN-PD/ICNSS-QMI/FW-ready/`wlan0`; native V1243 reaches `pm-service` `/dev/subsys_esoc0` but keeps PM8150L soft-reset `gpio9` as `MUX UNCLAIMED`, PCIe GDSC lines at `0mV`, GPIO142 IRQ `0`, PCI/MHI absent, and `wlan0` absent. The remaining gap is below Binder/peripheral-manager delivery and inside/after `mdm_subsys_powerup`, before Android's PMIC/GDSC/GPIO142/PCIe RC1/SSCTL/MHI/WLFW response. V1245 should prove whether native `mdm_subsys_powerup` reaches PM8150L soft-reset/GDSC operations or reproduce Android PMIC pinctrl setup before another bounded `esoc0` trigger; still no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
