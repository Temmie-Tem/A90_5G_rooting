...
- **Active research cycle**: V1252 HOST-ONLY PASS → V1253 helper v261 source/build-only — V1249 deployed helper `a90_android_execns_probe v260` to `/cache/bin/a90_android_execns_probe` over serial fallback (`0313d613d95c56af5681871062b7fceb47ede3c3ef8fcff534d0eea3338eaa2f`). V1251 proved the native PMIC/GDSC reproduction candidate: PM8150L soft-reset GPIO remained `MUX UNCLAIMED`, PCIe GDSC lines remained `0mV`, `mdm3=OFFLINING`, GPIO142 IRQ count stayed `0`, and all zero-action markers passed. V1252 selected the next source/build gate: add helper `v261` mode `wifi-companion-pmic-power-surface-write-gate-preflight`, which must verify the V1251 read contract, locate a PM8150L `gpiochip` candidate without writing, and fail closed unless PMIC GPIO9 identity/offset is exact. The first later live proof may only hold PMIC GPIO9 low for a bounded window and must not open `/dev/subsys_esoc0`, start PM/CNSS/HAL, scan/connect, use credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
