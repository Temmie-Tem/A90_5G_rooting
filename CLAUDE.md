...
- **Active research cycle**: V1319 PASS → V1320 mdm_helper/ks/MHI response-contract classifier — V1319 superseded the older AP2MDM assertion gap: native now shows eSoC PIL notify, GPIO1270 soft-reset toggle, and GPIO135 high, but no GPIO142/PCIe/MHI/WLFW response; Android-positive evidence has GPIO142 IRQ, PCIe RC1, `ks`/MHI pipe, WLFW, and `wlan0`. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, direct GDSC write, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
