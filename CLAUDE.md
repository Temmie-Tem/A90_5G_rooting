...
- **Active research cycle**: V1264 HOST-ONLY PASS → V1265 source/build-only helper v264 — V1262 proved PMIC GPIO9 offset `7` line-info: flags `0x1`, `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. V1263 classified this with V1239/V1242/V1243 and rejected direct userspace PMIC GPIO9 line request/hold as the next safe path. V1264 fixed the next observer contract: extend the existing late `per_proxy` / PM-service response sampler with read-only PMIC GPIO9 `GPIO_GET_LINEINFO_IOCTL` snapshots in the same `/dev/subsys_esoc0` window. The blocker remains below the reproduced PM-service entry: native reaches `mdm_subsys_powerup`, but does not get Android's downstream GPIO142/PCIe/MHI/WLFW response. V1265 must stay source/build-only and preserve the hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
