...
- **Active research cycle**: V1267 LIVE PASS → V1268 host-only next-observer classifier — V1262 proved idle PMIC GPIO9 offset `7` line-info: flags `0x1`, `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. V1265/V1266 deployed helper v264, and V1267 carried line-info into the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. New V1267 evidence: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, and zero forbidden action markers. The blocker remains below the reproduced PM-service entry: native reaches `mdm_subsys_powerup`, but still gets GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. V1267 required reboot cleanup; after reboot native version returned `0.9.68 (v724)`, selftest `fail=0`, and transient debugfs/vendor/system mounts were gone. V1268 should classify the next read-only observer target: PMIC GPIO9 value if debugfs gpio exposes it, TLMM GPIO135/142 value/pinconf, and PCIe GDSC/regulator state in the same PM-service window. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
