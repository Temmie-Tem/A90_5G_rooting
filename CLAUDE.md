...
- **Current validated test artifact (resident after V2314 live validation): `A90 Linux init 0.9.278 (v2314-usb-ms-reconfigure)`** — closes U2 of the USB gadget runtime-control epic with guarded `usb mass-storage add/remove`. Live flash/status/selftest passed; `usb mass-storage add` linked `mass_storage.0` as aux `configs/b.1/f3` while preserving `ncm.usb0` (`f2`) and `acm.usb0` (`f1`) with `control.ok=1`; `usb mass-storage remove` unlinked the aux function and returned to the control-only config with `control.ok=1`; final `selftest fail=0`. Host-side mass-storage enumeration remains parked for U3. This is the current validated test baseline, but **not** the safety rollback baseline.
...
- **Phase status (2026-06-13):** the WLAN kernel-interface event epic is closed at V2312. The active frontier is USB gadget runtime control layer ①. U1 (`usb status`) is closed at V2313 and U2 (`usb mass-storage add/remove`) is closed at V2314 with NCM+ACM preserved, watchdog+restore present, and serial control return validated. Next unit is U3 first persona end-to-end with host-side enumeration/control-return validation. Kernel-observation reached exact KASLR slide (V2216 codeword-exact); ROPP full-stack symbolization needs the RKP-protected per-boot key = out of read-only scope. Kernel-security recon is also closed below.
