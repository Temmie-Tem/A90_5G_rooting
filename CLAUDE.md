...
- **Active research cycle**: V1258 DEPLOY PASS → V1259 bounded live devnode-open proof — V1256 proved PM8150L gpiochip devnode feasibility read-only: `/proc/devices` has `254 gpiochip`; `/sys/bus/gpio/devices/gpiochip2/dev=254:2`; `/sys/class/gpio/gpiochip1263` labels PM8150L with `ngpio=11`; debugfs confirms `gpiochip2` range `1263-1273` and PMIC GPIO9 offset `7`. V1257 built `a90_android_execns_probe v262` with `wifi-companion-pmic-gpiochip-devnode-open-preflight` / `--allow-pmic-gpiochip-devnode-open-preflight`. V1258 deployed v262 to `/cache/bin/a90_android_execns_probe` by serial fallback, remote SHA matched `17773e5bcdec090c061a962833d27a783439e1b718c96b47a504f625d79cc36d`, and postflight selftest stayed `fail=0`. Next V1259 may execute only the bounded temporary `/dev/gpiochip2` devnode-open proof: create temp char node for `254:2`, open read-only, run `GPIO_GET_CHIPINFO_IOCTL`, close, unlink. Still no GPIO line request, PMIC write, `/dev/subsys_esoc0`, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
