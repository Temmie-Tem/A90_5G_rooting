...
- **Active research cycle**: V1425 source/build-only PASS (`v1425-wifi-test-boot-rc1-retry-source-build-pass`). V1425 produced a rollbackable test boot artifact with native init `0.9.77 (v1425-wifitest)`, preserving the 250ms delayed corrected-RC1 path and adding two bounded corrected-RC1 retries at 500ms spacing after the initial write. This targets the post-PERST-release endpoint response gap classified in V1424. Next gate is V1426 artifact sanity, then V1427 rollbackable live handoff only if sanity passes. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
