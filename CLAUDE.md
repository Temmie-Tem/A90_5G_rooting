...
- **Active research cycle**: V1308 PASS → V1309 bounded no-write PMIC/GDSC transition sampler live — V1308 deployed helper `a90_android_execns_probe v274` to `/cache/bin/a90_android_execns_probe`, sha256 `eb96072631ca38c3296f5da1756a93765e198e8fdd4dc010d087bc4b3b5fc180`. V1309 should use the focused mode `late-per-proxy-focused-pmic-gdsc-transition` with `80` samples at `50ms`, emitting reduced no-write lower-surface fields: AP2MDM/MDM2AP target lines, PM8150L soft-reset source/line, PCIe0/PCIe1 GDSC source/line, PCI/MHI counts, MHI pipe, `ks`, `wlan0`, and safety zeros. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
