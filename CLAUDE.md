...
- **Active research cycle**: V1450 provider-trigger micro endpoint source/build PASS (`v1450-wifi-test-boot-provider-trigger-micro-endpoint-source-build-pass`). V1449 showed `__subsystem_get: esoc0` precedes explicit RC1 debugfs `case=11`, so V1447 sampled post-RC1-case state rather than provider transition state. V1450 builds a rollbackable Wi-Fi test boot that watches the provider-level PID1 kmsg trigger, uses `0ms` delay, does not write RC1 debugfs `rc_sel`/`case`, and samples endpoint GPIO/interrupt/link state at `0ms` through `150ms` around provider detection. Next gate should be V1451 local-only artifact sanity before any rollbackable live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
