...
- **Active research cycle**: V1317 PASS → V1318 critical-only lower trace collector — V1317 classified the V1316 trace-line sample as background-heavy: `260` stored lines, `10` critical sample lines, no SDX50M/PCIe/MHI/WLAN/CNSS target keywords, no target GPIO `135`/`142`/`1270`, and broad IRQ/clock noise consumed most saved lines. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, direct GDSC write, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
