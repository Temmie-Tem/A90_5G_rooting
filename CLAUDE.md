...
- **Active research cycle**: V1284 SOURCE/BUILD PASS → V1285 helper v269 deploy — V1279 confirmed TLMM `gpiochip0` range `0-174` is visible and GPIO135/GPIO142 pinmux ownership belongs to `soc:qcom,mdm3`, but exact GPIO135/GPIO142 debugfs value lines remain absent. V1283 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v268; PM-service reached eSoC, but GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`. V1284 repaired the helper kmsg collector by keeping `/dev/kmsg` first and adding a read-only `syslog(SYSLOG_ACTION_READ_ALL)`/klogctl fallback with `kmsg_source` reporting; helper v269 static aarch64 build passed (`dbb1f67652913ffe94b1f083a082d8f221820040b9f28e08b226eb1e0a50fc83`). Next selected gate is V1285 deploy-only for helper v269, then V1286 bounded PCIe/GDSC/klogctl live sampling. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
