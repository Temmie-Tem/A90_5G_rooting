...
- **Active research cycle**: V1283 LIVE PASS → V1284 kmsg collector repair — V1279 confirmed TLMM `gpiochip0` range `0-174` is visible and GPIO135/GPIO142 pinmux ownership belongs to `soc:qcom,mdm3`, but exact GPIO135/GPIO142 debugfs value lines remain absent. V1283 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v268; PM-service reached eSoC, but GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, and PCIe GDSC lines remain `0mV`. The new helper kmsg path failed because `/dev/kmsg` is absent (`errno=2`), while `/proc/kmsg` exists and `busybox dmesg` works from native shell. Next selected gate is V1284 source/build repair to use a read-only syslog/klogctl or wrapper-level `dmesg` fallback before retrying PCIe/GDSC/dmesg live sampling. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
