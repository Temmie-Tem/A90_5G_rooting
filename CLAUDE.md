...
- **Current validated test artifact (V2322): `A90 Linux init 0.9.286 (v2322-usb-named-lun-identity)`** — image `workspace/private/inputs/boot_images/boot_linux_v2322_usb_named_lun_identity.img`, SHA256 `81355888b6b19407c76463ee8d5ca045fd0f17294c3329ceda0afc1ab2a36f53`. Closes named multi-LUN mass-storage U-A only: parent USB descriptor remains V2321; `mass_storage.0/lun.0/inquiry_string` is `A90-LNX A90-INTERNAL    0001`, backing is a read-only `/cache` FAT16 image labeled `A90INTERNAL`, host `lsblk -S` sees vendor `A90-LNX` and model `A90-INTERNAL`, and host block view sees label `A90INTERNAL`, filesystem `vfat`, size `8M`, read-only `1`. `usb mass-storage remove` returns to NCM+ACM-only control with `selftest fail=0`. V2321 remains the rollback target until an explicit promotion decision.
...
- Baseline lineage (since the workspace reorg): `v726-wifi-lifecycle` → `v2169-transport-contract` → `v2174-wifi-urandom-connect` → `v2178-wifi-profile-autoconnect` → `v2182-hud-menu-cleanup` → `v2189-security-p0-stage-fix` → `v2232-service-object-fwclass-bridge` → `v2236-strict-wifi-connect` → `v2237-supplicant-terminate-poll` → `v2254-wifi-detail-surface` → `v2316-usb-linux-identity` → `v2317-usb-product-rodata` → `v2318-usb-full-identity-rodata` → `v2319-usb-product-overrun1-rodata` → `v2320-usb-product-overrun2-rodata` → `v2321-usb-clean-identity-rodata` → `v2322-usb-named-lun-identity` (test artifact; rollback still V2321).
