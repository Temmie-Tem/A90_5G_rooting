...
- **Active research cycle**: V1302 PASS → V1303 bounded compact dense live rerun — V1302 deployed `a90_android_execns_probe v273` to `/cache/bin/a90_android_execns_probe` with sha256 `dd1d15a5ef01189526720814c50b007f6dc9a0f25e9239caf0e9da34c65b6b46`. NCM was inactive, so serial fallback used chunk size `1800`, wrote `1010` chunks, and passed line check (`max_cmdv1_line_bytes=3788`, safe `3968`). Native selftest remained `pass=11 warn=1 fail=0`; no daemon start or Wi-Fi bring-up occurred. Next gate is V1303 bounded compact dense live rerun requiring `powerup_marker` evidence. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
