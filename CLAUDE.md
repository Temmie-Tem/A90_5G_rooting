...
- **Active research cycle**: V1316 PASS → V1317 lower trace-line classifier — V1316 proved selected tracefs lower events can capture provider-internal activity during the bounded late `per_proxy` PM-service path: `pm-service` reached `/dev/subsys_esoc0` / `mdm_subsys_powerup`, `18` events enabled, `81174` total events captured, `3936` critical events captured, and cleanup disabled all selected events. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, direct GDSC write, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
