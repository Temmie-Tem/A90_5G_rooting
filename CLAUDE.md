...
- **Active research cycle**: V1255 READ-ONLY LIVE PASS → V1256 gpiochip devnode feasibility — V1254 deployed helper `a90_android_execns_probe v261` to `/cache/bin/a90_android_execns_probe`. V1255 temporarily mounted debugfs and ran the v261 mapping preflight. Result: `v1255-pmic-gpio-mapping-incomplete`; PMIC debugfs identifies `gpiochip2` range `1263-1273`, PMIC GPIO9 global line `1270`, offset `7`, identity/offset match true, but `/dev/gpiochip*` scan count is `0`, so chardev mapping is not ready. All zero-action markers passed: no GPIO line request, eSoC ioctl, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write. Next V1256 should be read-only devnode feasibility from sysfs metadata; no mknod or line hold yet.
