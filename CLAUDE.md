...
- **Active research cycle**: V1233 SOURCE/BUILD PASS → V1234 deploy-only planned — helper `a90_android_execns_probe v257` adds a bounded non-ptrace `post_wait_branch.*` snapshot around the V1232 `ESOC_WAIT_FOR_REQ` transition. It records thread `wchan`, `/proc/<tid>/syscall`, selected syscall names/args/path strings, `/dev/esoc-0` and MHI pipe fd counts, and a 20-sample 10ms transition burst. Active blocker remains the immediate `mdm_helper` post-return branch and why Android's `ks` image-link path does not launch. No Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, `ESOC_NOTIFY`, or `ESOC_BOOT_DONE` in this gate.
