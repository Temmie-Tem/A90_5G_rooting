...
- **Active research cycle**: V1441 micro endpoint source/build PASS (`v1441-wifi-test-boot-micro-endpoint-source-build-pass`). V1440 showed the V1437 immediate sampler was still too slow because full debugfs regulator/clock scans overran the RC1 active window. V1441 builds a rollbackable Wi-Fi test boot with a concurrent corrected-RC1 writer plus minimal fast reader for selected interrupts, exact endpoint GPIOs, and pcie1 link-state files at `0ms` through `150ms`, with only one slower `post_micro_200ms` context sample. Next gate should be V1442 local-only artifact sanity before any rollbackable V1443 live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
