...
- **Active research cycle**: V1263 HOST-ONLY PASS → V1264 read-only ext-mdm/AP2MDM observer plan — V1262 proved PMIC GPIO9 offset `7` line-info: flags `0x1`, `GPIOLINE_FLAG_KERNEL=1`, consumer `AP2MDM_SOFT_RESET`. V1263 classified this with V1239/V1242/V1243 and rejected direct userspace PMIC GPIO9 line request/hold as the next safe path. The blocker remains below the reproduced PM-service `/dev/subsys_esoc0` entry: native reaches `mdm_subsys_powerup`, but does not get Android's downstream GPIO142/PCIe/MHI/WLFW response. Next V1264 should plan a read-only ext-mdm/AP2MDM contract observer that correlates kernel-owned line state, GPIO142 IRQ, PCIe RC1/MHI, and `mdm_subsys_powerup` timing without PMIC write, userspace GPIO line request, PM/CNSS/HAL start, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
