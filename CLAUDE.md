...
- **Active research cycle**: V1246 HOST-ONLY PASS → V1247 PMIC pinctrl reproduction planning — V1246 added `scripts/revalidation/native_wifi_same_run_power_stack_classifier_v1246.py` and proved V1243 has same-run evidence: 12 late-`per_proxy` phases simultaneously show `pm-service` Binder threads blocked in `mdm_subsys_powerup` and PM8150L soft-reset `gpio9` still `MUX UNCLAIMED`, PCIe GDSC lines still `0mV`, GPIO142 IRQ `0`, PCI/MHI absent, and `wlan0` absent. Android-positive evidence claims PM8150L `gpio9` as output and reaches PCIe RC1/WLAN-PD/ICNSS-QMI/FW-ready/`wlan0`. The remaining gap is not timing between separate runs; it is inside or immediately after the proprietary soft-reset/power path before Android-equivalent PMIC/GDSC/GPIO142/PCIe response becomes visible. V1247 should choose the first bounded PMIC pinctrl reproduction path via source/Android-contract planning before any write; still no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
