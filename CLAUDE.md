...
- **Active research cycle**: V1413 rollbackable live handoff PASS for procedure/downstream RC1, still BLOCKED for Wi-Fi connect readiness (`v1413-test-boot-downstream-progress-rollback-pass`). V1411 booted, PID1 mounted debugfs, the `/proc/kmsg` fallback watcher triggered corrected RC1 successfully (`write_rc=0`), and rollback verified v724/selftest fail=0. RC1 still failed before L0 with no MHI/WLFW/BDF/FW-ready/`wlan0`; `connect_ready=false`. Timing changed materially: `esoc0_to_test11` is about `0.032s`, earlier than Android's about `0.255s` reference. Next is V1414 source/build-only: add configurable PID1 watcher delay after the first `esoc0`/powerup marker, initially `250ms`, then rebuild/sanity-check before any live handoff. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
