...
- **Active research cycle**: V1291 HOST PASS → V1292 dynamic PCIe/GDSC/eSoC sequencing plan — V1291 classified V1290 against Android-positive evidence and closed static TLMM GPIO shape as the shortest blocker. Native GPIO135 is `gpio135 : out 0 16mA no pull` and GPIO142 is `gpio142 : in  0 8mA no pull`, matching Android. V1287 already demoted PMIC9 static shape. The active gap is now dynamic: after PM-service enters the eSoC path, PCIe GDSC stays `0mV`, GPIO142 IRQ remains `0`, and PCI/MHI/MHI-pipe/WLFW/SDX50M/`wlan0` remain absent. Next selected gate is V1292 host/source planning for dynamic PCIe/GDSC/eSoC power sequencing observability. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
