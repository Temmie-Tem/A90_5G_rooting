...
- **Active research cycle**: V1318 PASS → V1319 GPIO135-to-GPIO142/PCIe response classifier — V1318 captured critical-only lower tracefs evidence in the bounded late `per_proxy` PM-service path: eSoC PIL notification `fw=esoc0`, PMIC soft-reset GPIO `1270` toggles, AP2MDM GPIO `135` set high once, and no MDM2AP GPIO `142` lines across about `49.28s` after GPIO135 high. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, direct GDSC write, Wi-Fi HAL, scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
