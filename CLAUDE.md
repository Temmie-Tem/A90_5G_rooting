...
- **Active research cycle**: V1247 HOST-ONLY PASS → V1248 PMIC preflight helper planning — V1247 added `scripts/revalidation/native_wifi_pmic_pinctrl_repro_plan_v1247.py` and selected `v1247-select-fail-closed-pmic-preflight-before-write`. It rejects direct `/sys/class/gpio` export/write, debugfs pinctrl/regulator mutation, direct PCIe GDSC enable, and blind `/dev/subsys_esoc0` retry because Android does not expose the relevant GPIO class nodes, PM8150L `gpio9` is a PMIC pinctrl line, and V1246 already proved same-run `mdm_subsys_powerup` with PMIC/GDSC/downstream silence. Next V1248 should be source/build-only: add a fail-closed helper gate that validates the DTS/PMIC/native-state contract and refuses mutation unless a later cycle adds a separate explicit write approval path. Still no Wi-Fi HAL/scan/connect, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
