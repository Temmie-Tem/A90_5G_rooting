...
- **Active research cycle**: V1417 host-only classifier PASS (`v1417-delayed-rc1-timing-aligned-test11-semantics-gap`). V1416 aligned corrected RC1 timing with Android within `~20ms` (`0.275121s` vs `0.254929s`), but the test-boot debugfs `TEST: 11` path still lacks Android reset/release markers and fails before L0 in `LTSSM_POLL_COMPLIANCE`; no MHI/WLFW/BDF/FW-ready/`wlan0`, `connect_ready=false`. Next V1418 should be source/host-only: inspect the stock `msm_pcie` debugfs `TEST: 11` path against Android's normal RC1 bring-up path and decide whether the next test boot needs a different RC1 trigger rather than another blind delay retry. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
