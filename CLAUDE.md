...
- **Active research cycle**: V1435 focused endpoint live handoff PASS (`v1435-test-boot-downstream-progress-rollback-pass`). The V1433 test boot verified `A90 Linux init 0.9.79 (v1433-wifitest)`, collected focused endpoint window evidence, then rolled back to healthy v724 with selftest fail=0. The blocker remains below Wi-Fi userspace: RC1/LTSSM progresses but never reaches L0; MHI/WLFW/BDF/FW-ready/`wlan0` remain absent. Focused records removed summary truncation but exposed a same-window sampling race: broad `pre_rc1` showed pcie1 GDSC/clocks briefly enabled while later focused exact `pre_rc1` reads had them back off. Next gate should be V1436 host-only classification of V1435 evidence before another live mutation. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/BOOT_DONE spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
