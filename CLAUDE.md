...
- **Active research cycle**: V1290 LIVE PASS → V1291 static GPIO parity classifier — V1289 deployed helper v270 (`f1748fdc9c64a748c3270cd02a2b9bb796065b79632849e7384c2f37910f6e88`) by serial fallback. V1290 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v270. Exact TLMM target scans now work: GPIO135 is `gpio135 : out 0 16mA no pull`, and GPIO142 is `gpio142 : in  0 8mA no pull`, matching Android-positive static evidence. GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, PCIe/MHI/WLFW/SDX50M kmsg counts remain `0`, and post-reboot health passed on v724 (`selftest pass=11 warn=1 fail=0`). Next selected gate is V1291 host-only classification to move focus from static GPIO shape to dynamic PCIe/GDSC/eSoC power sequencing. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
