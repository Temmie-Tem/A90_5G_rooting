...
- **Active research cycle**: V1453 provider-trigger micro handoff classifier PASS (`v1453-provider-window-low-no-downstream`). V1452's rollbackable test boot safely sampled the provider window without explicit RC1 debugfs writes and rolled back to `A90 Linux init 0.9.68 (v724)`, but GPIO135/GPIO142 and endpoint IRQs stayed low, pcie1 GDSC/clocks stayed off in the context sample, and no RC1/MHI/WLFW/BDF/FW-ready/`wlan0` marker appeared. One measurement weakness remains: the PID1 kmsg reader stores the raw chunk, so the result line prefix can show an earlier cnss-daemon netlink message even when the chunk matched a provider line. Next gate should be V1454 source/build-only exact-line provider-trigger test boot with a longer read-only endpoint window. Preserve hard exclusions: no credential use, Wi-Fi scan/connect/DHCP/external ping, Wi-Fi HAL start, PMIC/GPIO/GDSC direct write, blind eSoC notify/`BOOT_DONE` spoof, global PCI rescan, platform bind/unbind, flash outside an explicit test-boot/rollback gate, boot image write outside an explicit test-boot/rollback gate, or partition write.
