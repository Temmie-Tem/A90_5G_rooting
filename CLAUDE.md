...
- **Active research cycle**: V1288 SOURCE/BUILD PASS → V1289 helper v270 deploy — V1285 deployed helper v269 (`dbb1f67652913ffe94b1f083a082d8f221820040b9f28e08b226eb1e0a50fc83`) by serial fallback. V1286 reran the bounded PM-service `/dev/subsys_esoc0` response sampler with helper v269; the repaired klog collector worked through `syslog-read-all` (`kmsg_open_seen=true`, `max_kmsg_filtered_count=32`). V1287 classified that evidence against Android-positive power evidence and demoted PM8150L gpio9 shape as the shortest blocker: native gpio9 already matches Android `out/high` shape, while GPIO142 IRQ remains `0`, `mdm3=OFFLINING`, PCI/MHI/MHI-pipe/`wlan0` remain absent, PCIe/MHI/WLFW/SDX50M kmsg counts remain `0`, and PCIe GDSC lines remain `0mV`. V1288 added helper v270 no-write exact `gpio135`/`gpio142` debugfs target scans and passed source/build (`f1748fdc9c64a748c3270cd02a2b9bb796065b79632849e7384c2f37910f6e88`). Next selected gate is V1289 deploy-only for helper v270, then V1290 bounded no-write TLMM/PCIe response sampling. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
