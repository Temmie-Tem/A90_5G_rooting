...
- **Active research cycle**: V1268 HOST-ONLY PASS → V1269 source/build-only helper v265 — V1267 carried PMIC GPIO9 line-info into the same late `per_proxy` / PM-service `/dev/subsys_esoc0` response window. V1267 evidence: 14/14 samples report PMIC GPIO9 flags `0x3` (`GPIOLINE_FLAG_KERNEL` + `GPIOLINE_FLAG_IS_OUT`) with consumer `AP2MDM_SOFT_RESET`, zero forbidden action markers, GPIO142 IRQ count `0`, `mdm3=OFFLINING`, PCI device count `0`, MHI bus count `0`, no MHI pipe, no WLFW/BDF, and no `wlan0`. V1267 required reboot cleanup; after reboot native version returned `0.9.68 (v724)`, selftest `fail=0`, and transient debugfs/vendor/system mounts were gone. V1268 selected the next target: source/build-only helper v265 should extend the same response sampler with read-only value/power snapshots — `/sys/kernel/debug/gpio` line for global GPIO1270 if present, PMIC GPIO9 pinmux/pinconf, TLMM GPIO135/142 pinmux/pinconf, PCIe GDSC regulator lines, and PCIe RC1 state. Preserve hard exclusions: no PMIC write, userspace GPIO line request/hold, direct eSoC ioctl, new daemon/HAL start beyond the existing bounded PM-service path, Wi-Fi bring-up, credentials, DHCP/routes, external ping, flash, boot image write, or partition write.
