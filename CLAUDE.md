...
- **Current validated test artifact (resident after V2315 live validation): `A90 Linux init 0.9.279 (v2315-usb-ms-persona)`** — closes U3 and layer ① of the USB gadget runtime-control epic. `usb mass-storage expose` creates a bounded read-only 8 MiB `/cache` backing image, links `mass_storage.0` as aux `configs/b.1/f3`, preserves `ncm.usb0` (`f2`) and `acm.usb0` (`f1`) with `control.ok=1`, and enumerates on the host as read-only `SAMSUNG File-Stor Gadget`; `usb mass-storage remove` unlinks the aux function, clears the LUN file, removes the host disk, and returns to control-only `control.ok=1`; final `selftest fail=0`. This is the current validated test baseline, but **not** the safety rollback baseline.
