# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: __sw_hweight16

- Date: 2026-06-30
- Decision: `a90-repl-live-call-proof-__sw_hweight16-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-sw-hweight16-20260630/proof/a90_repl_evidence.json`

## Candidate Selection

This unit continued the scalar bit-helper path after `__sw_hweight32`. The advisory sweep over the
adjacent helpers produced:

- `__sw_hweight16`: `SAFE-SCALAR`, source signature found, C1 verified by `export-recovery`.
...
The one-target proof selected `__sw_hweight16` because it has a verified scalar-only source/ABI
...
- `__sw_hweight16`: `0xffffff800856d87c`
- Resolution method: `export-recovery`
- Direct BL xrefs: `19`
- Shape: JOPP entry, leaf/no-BL, RET observed at offset `0x2c`.
...
- Source signature: `include/linux/bitops.h:11`, `extern unsigned int __sw_hweight16(unsigned int w)`
...
`__sw_hweight16` entry with fixed scalar low-16-bit words and checks a bounded population-count case
table.
...
PYTHONPATH=tests PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 -m unittest \
  ...
  tests.test_a90_repl.SelftestIntegrationTests.test_call_proof_sw_hweight16_passes_with_scalar_contract
...
- Full `tests.test_a90_repl`: `Ran 126 tests`, `OK`.
...
PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 \
  ...
  --evidence-dir workspace/private/runs/kernel/live-call-proof-sw-hweight16-20260630/proof \
  __sw_hweight16
```
...
| --- | --- | --- | --- |
| zero | `0x0000` | `0x0` | `0x0` |
| all-ones | `0xffff` | `0x10` | `0x10` |
| alternating-a | `0xaaaa` | `0x8` | `0x8` |
| single-high-bit | `0x8000` | `0x1` | `0x1` |
| a90d | `0xa90d` | `0x7` | `0x7` |

Checks:

- `static-c1-identity`: OK, `__sw_hweight16` resolved by `export-recovery`.
- `static-source-contract`: OK, signature `extern unsigned int __sw_hweight16(unsigned int w)`, no pointer args.
- `static-call-safety-contract`: OK, tier `SAFE-SCALAR`, no required pointer args.
- `__sw_hweight16-scalar-case-table`: OK, all 5 scalar case calls returned the expected values.
...
`__sw_hweight16` is now live-proven under a scalar unsigned 16-bit low-word contract. The proof
confirms the intended helper was reached, returned the expected population count for zero, all-ones,
...
outside the stated low-16-bit input contract, or mass calling. The device was rolled back to clean
