...
Status: host-only plan. The P3 boot candidate has now been rolled back to the
pinned stock boot AP, but P3 first-light remains unproven.
...
The operator later reached stock/general recovery, whose ADB endpoint was
unauthorized. That prevented `/proc/last_kmsg` or pstore collection. The
operator then entered download mode and the pinned stock boot-only rollback
completed:
...
No P4 candidate may be flashed until a new deterministic artifact and a new
SHA-pinned `AGENTS.md` boot-only exception exist.
...
This gate was attempted after P3. Because only stock/general recovery was
available before download mode, no pstore or last-kmsg evidence was collectable.
If this situation recurs on a future candidate, use:
...
The next boot artifact should assume recovery-based collection may fail. Current
preferred order:

1. **P4A pstore-first direct PID1 proof**:
   - direct `/init` writes high-priority kmsg/pstore markers,
   - uses pstore/ramoops as the primary proof if readable later,
   - also has a deliberately visible bounded terminal state that does not depend
     on Android, Magisk, USB gadget, or automatic recovery ADB,
   - remains recoverable by physical download mode.
...
The P3 incident is recovered to stock-boot Android, but the first-light proof is
still missing. This plan is preparation only. Any new boot flash requires:

1. Android or recovery health evidence,
2. a new deterministic host-built artifact,
3. a new SHA-pinned `AGENTS.md` S22+ boot-only exception.
