# Native Init V3333 GPU Z3 Overlay Linear Modifier Live EINVAL

- Cycle: `V3333`
- Decision: `v3333-gpu-z3-overlay-linear-modifier-live-einval`
- Init: `A90 Linux init 0.11.101 (v3333-gpu-z3-overlay-linear-modifier)`
- Boot image: `workspace/private/inputs/boot_images/boot_linux_v3333_gpu_z3_overlay_linear_modifier.img`
- Boot SHA256: `d374fb84dbd705aa22de1b960452d230b541ced6d68f63e1ed2d3cdd7c34f12f`

## Flash And Health

- Flash path: `workspace/public/src/scripts/revalidation/native_init_flash.py`
- Flash result: local SHA, remote pushed SHA, boot write, and boot readback SHA all matched the exact V3333 artifact.
- Post-flash version: `A90 Linux init 0.11.101 (v3333-gpu-z3-overlay-linear-modifier)`
- Post-flash health: `selftest pass=12 warn=1 fail=0`
- Bridge note: a host-side parallel serial command mistake caused one transaction-lock/prompt-noise incident; restarting `a90_bridge.py` restored clean V3333 `version` and `selftest` output.

## Live Probe

- Command: `gpu z3-imported-scanout-plane-probe --timeout-ms 60000 --hold-ms 12000 --materialize-devnode`
- Result marker: `gpu.z3.scanout.result=z3-imported-scanout-plane-present-failed`
- Result rc: `-22`
- Child status: `0x100`

## Passed Evidence

- KMS dumb target, PRIME export, `ADDFB2`, KGSL import/info, and GPU render all passed.
- Render semantic proof: `sample_count=64`, `match_count=64`, `exact_match_count=64`, `mismatch_count=0`, `output_other_count=0`
- Overlay plane selected: `plane_id=90`, `compatible=4`, `overlay=1`, `idle_xbgr=1`, `idle_xbgr_linear=1`, `selected_type=0`
- Modifier telemetry: `selected_linear=1`, `selected_tiled2=0`, `selected_tiled3=0`, `selected_compressed=0`, `in_formats_rc=-61`, `prop=0`, `blob=0`, `modifiers=0`
- Atomic optional telemetry: `atomic_optional_count=3`, `zpos_prop=61`, `alpha_prop=62`, `rotation_prop=91`, `pixel_blend_prop=0`
- Cleanup: `rmfb_rc=0`, `dumb_destroy_rc=0`, `close_prime_rc=0`, `close_drm_fd_rc=0`
- Post-probe health: `selftest pass=12 warn=1 fail=0`

## Failure

- Atomic plane commit returned `kms.atomic_commit_rc=-22`.
- Legacy fallback returned `kms.present_rc=-22`.
- This ruled out an `IN_FORMATS` modifier mismatch: the selected overlay has no `IN_FORMATS` blob, so the legacy format list is treated as LINEAR-capable.
- This also ruled out missing `pixel blend mode`: the property is absent on the selected plane.

## Follow-Up

V3334 sets `DRM_MODE_ATOMIC_ALLOW_MODESET` on the atomic commit to check whether this SDE overlay attach is classified as a modeset transition.

## Safety

- Device health stayed clean after the failed probe; no rollback was required.
- No PMIC/GDSC/regulator/GPIO/backlight write was introduced.
- No proprietary blob, EGL/GLES/OpenCL path, forbidden partition, or raw flash path was used.
