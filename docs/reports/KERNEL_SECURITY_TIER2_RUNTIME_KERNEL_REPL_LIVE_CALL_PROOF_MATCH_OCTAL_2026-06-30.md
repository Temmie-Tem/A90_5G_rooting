# Kernel Security Tier-2 Runtime Kernel REPL - Live Call Proof: match_octal

- Date: 2026-06-30
- Decision: `a90-repl-live-call-proof-match_octal-pass`
...
- Private evidence: `workspace/private/runs/kernel/live-call-proof-match-octal-20260630/proof/a90_repl_evidence.json`

## Static Gate

Target:

- `match_octal`: `0xffffff800855b83c`
...
- Disasm contract: x0 is the `substring_t *`, x1 is the `int *result`, the wrapper sets
  `w2 = 8`, then calls `match_number`.
- Source signature: `include/linux/parser.h:33`, `int match_octal(substring_t *, int *result)`
...
- The target was not called with host-supplied numeric pointers. The proof used one tool-owned
  layout containing `substring_t {from,to}`, bounded octal text `755`, an owned 4-byte result
...
PYTHONPYCACHEPREFIX=/tmp/a90_pycache python3 \
  ...
  --safe-op-retries 2 \
  --retry-delay-sec 0.3 \
  --evidence-dir workspace/private/runs/kernel/live-call-proof-match-octal-20260630/proof \
  match_octal
```
...
```json
{
  ...
  "expected_result": 493,
  "observed_result": 493,
  "expected_result_raw_hex": "0x000001ed",
  "observed_result_raw_hex": "0x000001ed",
...
- `static-c1-identity`: OK, `match_octal` resolved by `export-recovery`.
- `static-source-contract`: OK, signature `int match_octal(substring_t *, int *result)`.
- `static-call-safety-contract`: OK, tier `SAFE-WITH-VALID-PTR`, x0/x1 require verified owned pointers.
- `kmalloc-owned-match-octal-layout`: OK, allocated one owned kernel layout.
- `owned-match-octal-layout-poke-peek`: OK, substring slot, octal input, result slot, and canaries were written and read back.
- `match-octal-return-contract`: OK, returned `0`.
- `match-octal-result-layout-contract`: OK, stored signed `493`, preserved the substring slot, preserved the input text, and preserved the result-slot canary.
- `kfree-owned-match-octal-layout`: OK, cleanup succeeded.

Raw runtime slide, `match_octal` runtime address, owned layout pointer, substring pointer, input pointer,
...
`match_octal` is now live-proven under an owned `substring_t` plus owned `int *result` contract. The
proof confirms the intended wrapper was reached, parsed bounded octal input `755`, returned `0`,
stored signed `493` in the owned result slot with raw `0x000001ed`, preserved the input layout and
canaries, and cleaned up the owned allocation. It does not authorize arbitrary substring pointers,
user pointers, unterminated or unbounded parser state, invalid octal inputs, failure paths, overflow
cases, or mass calling.
