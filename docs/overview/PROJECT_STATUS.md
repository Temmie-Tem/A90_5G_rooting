...
- 최신 verified build: `stage3/boot_linux_v83.img` (`A90 Linux init 0.8.14 (v83)`)
- 최신 verified source: `stage3/linux_init/init_v83.c` + `stage3/linux_init/v83/*.inc.c` + `stage3/linux_init/a90_config.h` + `stage3/linux_init/a90_util.c/h` + `stage3/linux_init/a90_log.c/h` + `stage3/linux_init/a90_timeline.c/h` + `stage3/linux_init/a90_console.c/h`
...
- 상세 최신 verified 상태: `docs/reports/NATIVE_INIT_V83_CONSOLE_API_2026-04-29.md`
...
### 3-2. USB ACM serial console + 인터랙티브 셸 (v8~v83)

**현재 버전**: `init_v83` (`stage3/boot_linux_v83.img`) / `0.8.14 (v83)`
...
**확보된 관찰/제어 범위 (v83 verified build 기준):**
...
우선순위 순 (v83 verified build 이후):

1. **v84 shell + cmdproto boundary** — command dispatch와 `cmdv1/cmdv1x` framed protocol 책임 분리
2. **v85 run/service/netservice management** — timeout/cancel/zombie reap과 장기 실행 helper를 service 단위로 정리
