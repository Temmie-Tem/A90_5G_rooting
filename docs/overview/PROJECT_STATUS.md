...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1329 Android-only SDX50M response-prerequisite classifier PASS → V1330 focused Android read-only timing recapture plan` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1330** — focused Android read-only timing recapture plan
   (earliest `per_mgr`/`per_proxy`, `mdm_helper`, GPIO142, PCIe RC1, and `ks`/MHI를 한 monotonic timeline으로 재캡처하는 계획 수립)
2. **SDX50M eSoC response prerequisite gate** — native가 `mdm_subsys_powerup`까지 진입해도 Android에만 있는 GPIO142/PCIe/MHI/ks response sequence가 왜 absent인지 규명
