...
> Post-promotion observability candidate: `A90 Linux init 0.10.9 (v2835-audio-help-surface)`,
...
> Latest post-promotion validated candidate: `A90 Linux init 0.10.17 (v2853-audio-productization-marker-refresh)`,
> boot image `workspace/private/inputs/boot_images/boot_linux_v2853_audio_productization_marker_refresh.img`,
> SHA256 `780078e8932a98a87c6077c8622842511b65f3866cd226f5c3d1bd01ab93cc16`.
...
> PCM write/drain, and rollback to `v2321` selftest `fail=0`. V2843 then bundles the private
> SET-cal manifest/payloads under `/a90/audio`, and V2844 validates the same `audio chime --duration-ms 1200 --amplitude-milli 80 --execute`
> path on-device with host artifact deployment count `0`, card publication, manifest wait, SET-cal,
> route apply/reset, PCM write/drain, and rollback to `v2321` selftest `fail=0`. V2845/V2846 then enable and validate
...
> no refusal/error/write-failed markers, and rollback to `v2321` selftest `fail=0`. V2849/V2850 then add and validate read-only productization markers: `audio status` 30/30, `selftest verbose` audio 8/8, `screenapp audio-status` 6/6, and rollback to `v2321` selftest `fail=0`. V2851/V2852 then add and validate latest 0.10.x changelog/about visibility: `screenapp about-changelog` 6/6, `screenapp about-version` 6/6, unchanged `audio status` 30/30, unchanged `selftest verbose` audio 8/8, and rollback to `v2321` selftest `fail=0`. V2853/V2854 then refresh and validate device-visible productization markers for that evidence: `audio status` 33/33, `selftest verbose` audio 8/8, `screenapp audio-status` 6/6, `screenapp about-changelog` 6/6, and rollback to `v2321` selftest `fail=0`. V2855 then revalidates the latest-candidate `audio chime --duration-ms 1200 --amplitude-milli 80 --execute` path with host artifact deployment count `0`, SET-cal, route apply/reset, PCM write/drain, and rollback to `v2321` selftest `fail=0`. V2856 then revalidates the latest-candidate `audio stop internal-speaker-safe --execute` path after boot-chime settle with no-active PCM/SET-cal markers, core route reset write-done, no refusal/error/write-failed markers, and rollback to `v2321` selftest `fail=0`.
...
7. **오디오/ADSP 에픽 (V2324-V2815, CORE CLOSED; V2848 STOP EXECUTE LIVE PASS):** 내부 스피커 재생 경로를 닫음.
