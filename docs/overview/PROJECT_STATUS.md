...
> Post-promotion observability candidate: `A90 Linux init 0.10.5 (v2826-audio-menu-screens)`,
> boot image `workspace/private/inputs/boot_images/boot_linux_v2826_audio_menu_screens.img`,
> SHA256 `0662749d76651d50db837685b7778aedbbe83cc7071d16c703ea060e9e9d47a6`.
> V2827 validated the read-only `audio status` core/profile/safety fields, the `selftest verbose`
> PASS `audio` row, and display-only `screenapp audio-map` after adding the APPS/AUDIO menu page, then rolled back to `v2321` with `selftest fail=0`. The internal-speaker audio
...
7. **오디오/ADSP 에픽 (V2324-V2815, CORE CLOSED; V2827 OBSERVABILITY PASS):** 내부 스피커 재생 경로를 닫음.
   ...
   후보에서 SET-cal/route/PCM/cleanup/rollback을 검증했다. V2827은 `0.10.5` 관측성
   후보에서 `audio status`, `selftest verbose` PASS audio row, `screenapp audio-map`, APPS/AUDIO 메뉴 표면을 검증했다. 남은 작업은
