...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1283 LIVE PASS → V1284 kmsg collector repair` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1284** — source/build-only kmsg collector repair
   (`/dev/kmsg` absent, `/proc/kmsg` present, `busybox dmesg` works; helper should use
   read-only syslog/klogctl or wrapper-level dmesg fallback)
2. **V1285/V1286** — deploy and rerun bounded PCIe/GDSC/dmesg response sampler
