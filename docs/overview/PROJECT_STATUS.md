...
- 공식 숫자 버전: `0.9.68`
- 박힌 빌드 태그: `v724`
- 현재 진행 사이클: `V1333 early-CNSS observe-only helper support PASS → V1334 deploy helper v277 → V1335 bounded native early-CNSS WLFW parity observer` (native Wi-Fi bring-up; 디바이스 재flash 없음 — `CLAUDE.md` 기준)
...
1. **V1334/V1335** — deploy helper v277, then bounded native early-CNSS WLFW parity observer
   (`per_proxy`/eSoC trigger 전 native `cnss-daemon`이 Android처럼 early WLFW userspace state에 들어가는지 stdout/stderr, properties, fd, kmsg로 제한 관찰하고 `/dev/subsys_esoc0`는 observe-only로 닫아둔다)
2. **SDX50M eSoC response prerequisite gate** — V1335 결과에 따라 CNSS/provider state 보강 또는 eSoC trigger 재시도 범위를 결정
