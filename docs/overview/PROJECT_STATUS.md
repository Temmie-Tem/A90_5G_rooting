...
> Latest post-promotion build candidate: `A90 Linux init 0.10.10 (v2838-audio-chime-preset)`,
> boot image `workspace/private/inputs/boot_images/boot_linux_v2838_audio_chime_preset.img`,
> SHA256 `0772ef64e24ab863e50a646f710f5c4eb4059056c1ba9528dc38be870cc8bd86`.
> V2838 adds a manual `audio chime` preset that defaults to a bounded low-amplitude listen-mode tone over
> the proven `audio play` path; boot autoplay remains disabled and live validation is pending.
...
7. **오디오/ADSP 에픽 (V2324-V2815, CORE CLOSED; V2838 CHIME BUILD PENDING LIVE):** 내부 스피커 재생 경로를 닫음.
   ...
   스피커별 route map, 상태 UI, 부트차임 같은 post-promotion productization이며, V2838은 첫 수동 `audio chime` build candidate를 만들었고 라이브 검증은 다음 단계다.
