...
- 현재 official version은 `0.8.4`, build tag는 `v73`이다.
...
```bash
cp stage3/linux_init/init_v73.c stage3/linux_init/init_v74.c
```

반드시 바꿀 것:

- `#define INIT_BUILD "v74"`
- 예시가 patch 업데이트라면 `#define INIT_VERSION "0.8.5"`로 변경
- `A90v73` kmsg marker를 `A90v74`로 변경
- v49 번호는 재사용하지 않는다.
- `mark_step("..._v73\n")` 계열을 새 버전으로 변경
...
```bash
rg -n 'v73|A90v73|init_v73|boot_linux_v73|ramdisk_v73' stage3/linux_init/init_v74.c
```
...
aarch64-linux-gnu-gcc -static -Os -Wall -Wextra \
  -o stage3/linux_init/init_v74 stage3/linux_init/init_v74.c
aarch64-linux-gnu-strip stage3/linux_init/init_v74
file stage3/linux_init/init_v74
sha256sum stage3/linux_init/init_v74
strings stage3/linux_init/init_v74 | rg 'A90 Linux init .*\\(v74\\)|A90v74'
```
...
```bash
rm -rf /tmp/a90_boot_v74_unpack
mkdir -p /tmp/a90_boot_v74_unpack
python3 mkbootimg/unpack_bootimg.py \
  --boot_img stage3/boot_linux_v73.img \
  --out /tmp/a90_boot_v74_unpack \
  --format=mkbootimg \
  > /tmp/a90_boot_v74_mkbootimg_args.txt
```

ramdisk 생성:

```bash
rm -rf stage3/ramdisk_v74
mkdir -p stage3/ramdisk_v74/bin
cp stage3/linux_init/init_v74 stage3/ramdisk_v74/init
cp stage3/linux_init/a90_sleep stage3/ramdisk_v74/bin/a90sleep
chmod 755 stage3/ramdisk_v74/init stage3/ramdisk_v74/bin/a90sleep
(
  cd stage3/ramdisk_v74
  find . | LC_ALL=C sort | cpio -o -H newc > ../ramdisk_v74.cpio
)
```
...
args = shlex.split(Path('/tmp/a90_boot_v74_mkbootimg_args.txt').read_text())
for i, item in enumerate(args):
    if item == '--ramdisk':
        args[i + 1] = 'stage3/ramdisk_v74.cpio'
        break
else:
    raise SystemExit('missing --ramdisk')

cmd = ['python3', 'mkbootimg/mkbootimg.py', *args, '--output', 'stage3/boot_linux_v74.img']
...
```bash
ls -lh stage3/ramdisk_v74.cpio stage3/boot_linux_v74.img
sha256sum stage3/linux_init/init_v74 stage3/ramdisk_v74.cpio stage3/boot_linux_v74.img
strings stage3/boot_linux_v74.img | rg 'A90 Linux init .*\\(v74\\)|A90v74'
```
