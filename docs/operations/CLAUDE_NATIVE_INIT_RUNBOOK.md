...
- 현재 official version은 `0.8.7`, build tag는 `v76`이다.
...
```bash
cp stage3/linux_init/init_v76.c stage3/linux_init/init_v77.c
```

반드시 바꿀 것:

- `#define INIT_BUILD "v77"`
- 예시가 patch 업데이트라면 `#define INIT_VERSION "0.8.8"`로 변경
- `A90v76` kmsg marker를 `A90v77`로 변경
- v49 번호는 재사용하지 않는다.
- `mark_step("..._v76\n")` 계열을 새 버전으로 변경
...
```bash
rg -n 'v76|A90v76|init_v76|boot_linux_v76|ramdisk_v76' stage3/linux_init/init_v77.c
```
...
aarch64-linux-gnu-gcc -static -Os -Wall -Wextra \
  -o stage3/linux_init/init_v77 stage3/linux_init/init_v77.c
aarch64-linux-gnu-strip stage3/linux_init/init_v77
file stage3/linux_init/init_v77
sha256sum stage3/linux_init/init_v77
strings stage3/linux_init/init_v77 | rg 'A90 Linux init .*\\(v77\\)|A90v77'
```
...
```bash
rm -rf /tmp/a90_boot_v77_unpack
mkdir -p /tmp/a90_boot_v77_unpack
python3 mkbootimg/unpack_bootimg.py \
  --boot_img stage3/boot_linux_v76.img \
  --out /tmp/a90_boot_v77_unpack \
  --format=mkbootimg \
  > /tmp/a90_boot_v77_mkbootimg_args.txt
```

ramdisk 생성:

```bash
rm -rf stage3/ramdisk_v77
mkdir -p stage3/ramdisk_v77/bin
cp stage3/linux_init/init_v77 stage3/ramdisk_v77/init
cp stage3/linux_init/a90_sleep stage3/ramdisk_v77/bin/a90sleep
chmod 755 stage3/ramdisk_v77/init stage3/ramdisk_v77/bin/a90sleep
(
  cd stage3/ramdisk_v77
  find . | LC_ALL=C sort | cpio -o -H newc > ../ramdisk_v77.cpio
)
```
...
args = shlex.split(Path('/tmp/a90_boot_v77_mkbootimg_args.txt').read_text())
for i, item in enumerate(args):
    if item == '--ramdisk':
        args[i + 1] = 'stage3/ramdisk_v77.cpio'
        break
else:
    raise SystemExit('missing --ramdisk')

cmd = ['python3', 'mkbootimg/mkbootimg.py', *args, '--output', 'stage3/boot_linux_v77.img']
...
```bash
ls -lh stage3/ramdisk_v77.cpio stage3/boot_linux_v77.img
sha256sum stage3/linux_init/init_v77 stage3/ramdisk_v77.cpio stage3/boot_linux_v77.img
strings stage3/boot_linux_v77.img | rg 'A90 Linux init .*\\(v77\\)|A90v77'
```
