...
| `jiffies_to_clock_t` | `0xffffff800815857c`, `export-recovery`, direct BL xrefs `72`, `0x8`-byte identity leaf before `clock_t_to_jiffies` | scalar positive unsigned long jiffies value; current image leaf body is identity conversion; no pointer args | case table: `0x0 -> 0x0`, `0x1 -> 0x1`, `0x12345678 -> 0x12345678`, `0x7fffffff -> 0x7fffffff` | `n/a-scalar-only` | `a90-repl-live-call-proof-jiffies_to_clock_t-pass` |
| `jiffies_to_usecs` | `0xffffff8008158164`, `export-recovery`, direct BL xrefs `25`, `0x10`-byte multiply leaf before `timespec_trunc` | scalar unsigned long jiffies value bounded so `j * 10000` fits in unsigned int; no pointer args | case table: `0x0 -> 0x0`, `0x1 -> 0x2710`, `0x7b -> 0x12c4b0`, `0x68db8 -> 0xffffe380` | `n/a-scalar-only` | `a90-repl-live-call-proof-jiffies_to_usecs-pass` |
...
- NAPI context lookup proof: `get_current_napi_context` has crossed the live proof gate only under a
...
- Boot-stat timer proof: `get_boot_stat_time` has crossed the live proof gate only under a
  no-argument read-only MMIO counter contract. The proof covers three repeated calls returning
...
- DDR SMEM getter proof: `get_ddr_total_density` and `get_ddr_DSF_version` have crossed the live
  ...
  uint8 value `0x6`, and `get_ddr_DSF_version` returned uint32 value `0x650000`. These rows do not
  authorize arbitrary SMEM reads, logging-heavy debug getters, changing boot/stat state, or mass
  calling.
