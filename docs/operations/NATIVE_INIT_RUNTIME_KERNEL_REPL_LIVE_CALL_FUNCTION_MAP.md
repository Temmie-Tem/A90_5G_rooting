...
- Integer parser sweep: `simple_strtoull`, `kstrtouint`, `kstrtoint`, and `kstrtos16` have crossed
  the live proof gate only under owned NUL-terminated numeric strings plus their specific owned
  ...
  value, raw two's-complement representation, input immutability, and result-slot canary. `kstrtos16`
