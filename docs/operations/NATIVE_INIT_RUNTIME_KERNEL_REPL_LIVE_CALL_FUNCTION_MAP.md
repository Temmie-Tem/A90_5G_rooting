...
- Bitmap helper sweep: `__bitmap_weight`, `__bitmap_complement`, `__bitmap_subset`, `find_next_bit`, `find_next_zero_bit`, and
  `find_last_bit` have crossed the live proof gate only under owned unsigned-long bitmap buffers plus
  scalar count/size/offset bounded inside those bitmaps. `__bitmap_weight` covers zero count, low-tail
  ...
  cases while preserving the source bitmap and canaries. `__bitmap_subset` covers zero-size true,
  empty-source true, low-tail/first-word/second-word positive subset cases, a missing bit-90 negative
  case, full-size partial-mask negative, and full-size full-mask positive. `find_next_bit` covers low-word set hit,
  ...
  no-set-before-bound, and zero-size miss cases. All five proofs validated bitmap and canary
