...
- Read-I/O sweep: `filp_open`, cleanup-only `filp_close`, and `kernel_read` have crossed the live
  ...
  proven.
- String sweep: `strlen`, `strchr`, `strchrnul`, `strcmp`, `strnlen`, `strrchr`, `strscpy`,
  `strlcpy`, and
  ...
  and only proves first-occurrence hit plus missing-byte-returns-NUL-terminator cases; `strcmp`
  additionally requires two owned terminated strings and only proves equal/positive-sign compare
  cases; `strnlen` additionally requires the scalar `maxlen` contract; `strrchr` additionally
