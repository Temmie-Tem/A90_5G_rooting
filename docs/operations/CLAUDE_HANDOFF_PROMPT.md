...
- bridge에서 A90 Linux init 0.8.16 (v85)이 나오면 latest verified native init boot 상태다. `storage`/`mountsd status`로 SD 상태를 재확인한다.
...
latest v85 flash가 정말 필요할 때만 이 스크립트를 사용:
