...
후속 R4W1-E E1 F1은 exact candidate와 Magisk rollback을 각각 1회 전송하고
최종 Android/root health까지 복구했지만, 두 retained read에서 E1 carrier와
slot magic이 모두 0개였습니다. `NO_PROOF_F1_V2_CANDIDATE_ROLLED_BACK`
판정과 H0 원인 범위는
...
- 최신 V1137 helper build 결과는 `docs/reports/NATIVE_INIT_V1137_POST_PM_MDM_HELPER_COMPOSITE_HELPER_BUILD_2026-05-27.md`입니다.
- V1137에서 helper `v214`에 `wifi-companion-post-pm-mdm-helper-esoc-observer` guarded mode를 추가해 V1134 upper PM/CNSS path와 post-PM `mdm_helper`/eSoC observer를 결합했습니다.
- 최신 V1138 helper deploy 결과는 `docs/reports/NATIVE_INIT_V1138_EXECNS_HELPER_V214_DEPLOY_2026-05-27.md`입니다.
- V1138에서 helper `v214`를 `/cache/bin/a90_android_execns_probe`로 배포하고 remote SHA/mode/native health/post-deploy preflight를 확인했습니다.
- 최신 V1139 post-PM `mdm_helper`/eSoC live 결과는 `docs/reports/NATIVE_INIT_V1139_POST_PM_MDM_HELPER_ESOC_LIVE_2026-05-27.md`입니다.
...
- 최신 V1142 helper deploy 결과는 `docs/reports/NATIVE_INIT_V1142_EXECNS_HELPER_V215_DEPLOY_2026-05-27.md`입니다.
...
- 최신 V1149 Android `mdm_helper` strace handoff runner 결과는 `docs/reports/NATIVE_INIT_V1149_ANDROID_MDM_HELPER_STRACE_HANDOFF_RUNNER_2026-05-27.md`입니다.
- V1149 runner dry-run에서 Magisk module ZIP packaging, two-boot Android install/capture flow, cleanup/native rollback path, and guardrails를 검증했습니다. 아직 live Android boot/module install/capture는 실행하지 않았습니다.
- 최신 V1150 Magisk vendor overlay repair 결과는 `docs/reports/NATIVE_INIT_V1150_MAGISK_VENDOR_OVERLAY_REPAIR_2026-05-27.md`입니다.
- V1149 첫 live는 Android 두 번 부팅, module install, Magisk hook 실행, native v724 rollback까지는 성공했지만 `/system/vendor` wrapper가 `/vendor/bin/mdm_helper` 실행 경로를 잡지 못했고 host pull 디렉터리 생성도 누락됐습니다.
...
- 최신 V1159 Android PM thread sampler 결과는 `docs/reports/NATIVE_INIT_V1159_ANDROID_PM_THREAD_SAMPLER_CAPTURE_2026-05-27.md`입니다.
- V1159에서 `pm-service` `Binder:1554_2` thread가 `openat -> subsys_device_open -> __subsystem_get(esoc0) -> mdm_subsys_powerup` 경로에 D-state로 들어가는 것을 포착해, 다음 native gate를 PM-service Binder request 재현/분류로 좁혔습니다.
- 최신 V1160 PM eSoC trigger reconcile 결과는 `docs/reports/NATIVE_INIT_V1160_PM_ESOC_TRIGGER_RECONCILE_2026-05-27.md`입니다.
- V1160에서 Android-good 경로의 `vendor.per_proxy -> pm-service Binder esoc0 get` 타이밍과 native V1139의 `per_proxy` 스킵을 비교해, 다음 단위를 late-`per_proxy` eSoC trigger gate로 좁혔습니다.
- 최신 V1161 late `pm-proxy` helper build 결과는 `docs/reports/NATIVE_INIT_V1161_LATE_PER_PROXY_HELPER_BUILD_2026-05-27.md`입니다.
...
- 최신 V1165 late `pm-proxy` actionability live 결과는 `docs/reports/NATIVE_INIT_V1165_LATE_PM_PROXY_ACTIONABILITY_2026-05-27.md`입니다.
...
- 최신 V1168 PM-service callback dispatch live 결과는 `docs/reports/NATIVE_INIT_V1168_PM_CALLBACK_DISPATCH_LIVE_2026-05-27.md`입니다.
- V1168에서 state helper가 client record를 순회해 callback pointer `0x7f9a0eca5c`로 branch함을 확인했지만 `/dev/subsys_esoc0`은 열리지 않았고, maps 캡처는 post-child 시점이라 비어 다음 단위를 live sample-loop maps mapping으로 좁혔습니다.
- 최신 V1169 PM-service callback maps live 결과는 `docs/reports/NATIVE_INIT_V1169_PM_CALLBACK_MAPS_LIVE_2026-05-27.md`입니다.
- V1169에서 callback pointer가 `libperipheral_client.so+0x8a5c`로 매핑됐고, 해당 경로는 state를 Parcel에 써 Binder transact를 호출하는 notification stub이므로 다음 blocker를 Binder callback delivery 또는 수신 client action으로 좁혔습니다.
- 최신 V1170 PM-service callback transact live 결과는 `docs/reports/NATIVE_INIT_V1170_PM_CALLBACK_TRANSACT_LIVE_2026-05-27.md`입니다.
- V1170에서 primary `state=2` Binder transact는 `0x0`으로 성공했고 `/dev/subsys_esoc0`은 여전히 열리지 않아, 다음 blocker를 수신 client-side Binder callback handler/action으로 좁혔습니다.
- 최신 V1171 PM receiver callback live 결과는 `docs/reports/NATIVE_INIT_V1171_PM_RECEIVER_CALLBACK_LIVE_2026-05-27.md`입니다.
...
- 최신 V1174 PM ack body live 결과는 `docs/reports/NATIVE_INIT_V1174_PM_ACK_BODY_LIVE_2026-05-27.md`입니다.
- V1174에서 PM-service ack body가 `state=2`에서 device open을 수행해 fd `8`을 얻고 state `3`으로 전환하지만 mdm3/WLFW/`wlan0`는 나오지 않음을 확인해, 다음 blocker를 opened path/fd target 또는 state-3 이후 Android actor로 좁혔습니다.
- 최신 V1175 PM ack fd-target live 결과는 `docs/reports/NATIVE_INIT_V1175_PM_ACK_FD_TARGET_LIVE_2026-05-27.md`입니다.
...
- 최신 private firmware PM observer live 결과는 `docs/reports/NATIVE_INIT_V1124_PRIVATE_FIRMWARE_PM_OBSERVER_LIVE_2026-05-27.md`입니다.
- 다음 private firmware PM-service early-exit trace 계획은 `docs/plans/NATIVE_INIT_V1125_PRIVATE_FIRMWARE_PM_SERVICE_EARLY_EXIT_TRACE_PLAN_2026-05-27.md`입니다.
...
- 최신 PM observer classifier는 `docs/reports/NATIVE_INIT_V1114_PM_SERVICE_LIFETIME_CLASSIFIER_2026-05-27.md`입니다.
- 이전 PM observer live gate는 `docs/reports/NATIVE_INIT_V1113_GLOBAL_FIRMWARE_PM_CONNECT_LIVE_2026-05-27.md`입니다.
...
- V1114에서 `pm-service`가 현재 1000 ms post-start probe 전에 `exit_code=0`으로 사라져 CNSS PM client path가 zero-hit임을 host-only로 분류했습니다.
...
- V1124에서 helper-private firmware mounts는 성공했지만 provider가 여전히 보이지 않아, 다음 blocker를 private-firmware namespace 안의 `pm-service` early clean-exit branch로 좁혔습니다.
- 아직 Wi-Fi HAL, scan/connect/link-up, DHCP, route, external ping은 실행하지 않았습니다.
- 다음 블로커는 V1125에서 private-firmware PM observer namespace 안의 `pm-service` early clean-exit branch를 trace하는 것입니다.
...
- `plans/NATIVE_INIT_V1168_PM_CALLBACK_DISPATCH_LIVE_PLAN_2026-05-27.md` – V1167이 증명한 `state=2` helper 아래 client callback dispatch와 callback target pointer를 추적하는 V1168 계획
- `plans/NATIVE_INIT_V1169_PM_CALLBACK_MAPS_LIVE_PLAN_2026-05-27.md` – V1168 callback pointer를 live sample-loop `/proc/<pm-service>/maps`로 매핑하는 V1169 계획
...
- `plans/NATIVE_INIT_V1033_PM_SELINUX_DOMAIN_PROOF_PLAN_2026-05-26.md` – current-boot V490 policy-load 뒤 PM SELinux domain proof를 시도하고 helper allowlist blocker를 분류하는 계획
- `plans/NATIVE_INIT_V1034_PM_SELINUX_DOMAIN_ALLOWLIST_SUPPORT_PLAN_2026-05-26.md` – helper `v176`에 PM SELinux domain-proof contexts를 추가하는 source/build-only 계획
- `plans/NATIVE_INIT_V1035_HELPER_V176_DEPLOY_PLAN_2026-05-26.md` – helper `v176`을 `/cache/bin/a90_android_execns_probe`로 deploy-only 배포하고 원격 sha/marker parity를 확인하는 계획
- `plans/NATIVE_INIT_V1036_PM_SELINUX_DOMAIN_PROOF_V176_PLAN_2026-05-26.md` – helper `v176`과 current-boot V490 policy-load로 PM SELinux domain proof를 재실행하는 계획
- `plans/NATIVE_INIT_V1037_PM_RUNTIME_DOMAIN_GUARD_LIVE_V176_PLAN_2026-05-26.md` – helper `v176` PM runtime-domain guard를 live로 재검증하고 PM fd contract blocker를 분리하는 계획
- `plans/NATIVE_INIT_V1038_PM_FD_AFTER_DOMAIN_CLASSIFIER_PLAN_2026-05-26.md` – V1037 이후 PM fd gap을 Android V1024/V1036 evidence와 비교해 `pm-proxy` context parity bug를 분류하는 계획
- `plans/NATIVE_INIT_V1039_PM_PROXY_CONTEXT_PARITY_SUPPORT_PLAN_2026-05-26.md` – helper `v177`에서 `pm-proxy`를 Android의 `vendor_per_proxy` domain으로 분리하고 PM fd/wchan gap capture를 추가하는 source/build 계획
- `plans/NATIVE_INIT_V1040_HELPER_V177_DEPLOY_PLAN_2026-05-26.md` – helper `v177`을 `/cache/bin/a90_android_execns_probe`로 deploy-only 설치하고 remote sha/usage parity를 확인하는 계획
- `plans/NATIVE_INIT_V1041_PM_FULL_CONTRACT_V177_LIVE_PLAN_2026-05-26.md` – helper `v177`로 PM full-contract live proof를 재실행해 current-boot SELinux exec attr blocker를 fail-closed로 분류하는 계획
- `plans/NATIVE_INIT_V1042_PM_SELINUX_DOMAIN_PROOF_V177_PLAN_2026-05-26.md` – helper `v177` current-boot V401/V490 refresh 후 PM SELinux domain proof를 재검증하는 계획
- `plans/NATIVE_INIT_V1043_PM_FULL_CONTRACT_AFTER_V1042_PLAN_2026-05-26.md` – V1042 fresh policy/domain proof 직후 PM full-contract live gate를 재실행하는 계획
- `plans/NATIVE_INIT_V1044_PM_PIL_ANDROID_GPIO_ESOC_CLASSIFIER_PLAN_2026-05-26.md` – V1043 PM fd blocker를 Android GPIO/eSoC timing 양성 증거와 비교해 lower PIL prerequisite 문제인지 host-only로 분류하는 계획
- `plans/NATIVE_INIT_V1048_HELPER_V178_DEPLOY_PLAN_2026-05-26.md` – helper `v178`을 `/cache/bin/a90_android_execns_probe`로 deploy-only 설치하고 remote sha/native health/no-Wi-Fi guard를 검증하는 계획
- `plans/NATIVE_INIT_V1049_PM_FULL_CONTRACT_WITH_MODEM_HOLDER_PLAN_2026-05-26.md` – helper `v178` modem pre-holder가 PM fd contract를 형성하는지 current-boot SELinux refresh 후 bounded live로 검증하는 계획
- `plans/NATIVE_INIT_V1050_PM_MODEM_PRE_HOLDER_PRIVATE_ROOT_REPAIR_PLAN_2026-05-26.md` – V1049 `ENOENT` 원인을 helper private-root 미진입으로 보고 `v179` source/build-only로 수정하는 계획
- `plans/NATIVE_INIT_V1051_HELPER_V179_DEPLOY_PLAN_2026-05-26.md` – helper `v179`을 `/cache/bin/a90_android_execns_probe`로 deploy-only 설치하고 remote sha/usage/native health/no-Wi-Fi guard를 검증하는 계획
- `plans/NATIVE_INIT_V1052_PM_FULL_CONTRACT_WITH_MODEM_HOLDER_V179_PLAN_2026-05-26.md` – helper `v179`로 private-root modem pre-holder PM full-contract live gate를 재실행하는 계획
...
- `plans/NATIVE_INIT_V1058_FIRST_OPEN_RUNTIME_PREREQ_PLAN_2026-05-27.md` – V1056 이후 current native boot의 firmware_class path, firmware mounts, modem PIL blob visibility, device-node presence, lower perturbation markers를 read-only로 분류하는 계획
- `plans/NATIVE_INIT_V1060_CURRENT_CNSS_ONLY_OBSERVER_PLAN_2026-05-27.md` – V1059 firmware mount refresh 이후 current-boot SELinux/policy를 갱신하고 helper v180 CNSS-only observer로 QRTR/sysmon/WLFW/MHI/wlan0 marker를 확인하는 계획
- `plans/NATIVE_INIT_V1061_GLOBAL_FIRMWARE_PM_FULL_CONTRACT_PLAN_2026-05-27.md` – global firmware mount와 global `/dev/subsys_modem` holder를 유지한 상태에서 helper v180 PM full-contract-with-modem-holder gate를 재검증하는 계획
- `plans/NATIVE_INIT_V1062_PM_CONTRACT_GAP_CLASSIFIER_PLAN_2026-05-27.md` – V1024 Android-positive PM fd evidence와 V1061 native PM full-contract gap/eSoC warning을 host-only로 비교 분류하는 계획
...
- `plans/NATIVE_INIT_V1114_PM_SERVICE_LIFETIME_CLASSIFIER_PLAN_2026-05-27.md` – V1113 evidence에서 `pm-service` lifetime/readiness와 CNSS PM client zero-hit 원인을 host-only로 분류하는 계획
- `plans/NATIVE_INIT_V1115_IMMEDIATE_CNSS_HELPER_BUILD_PLAN_2026-05-27.md` – V1114가 선택한 immediate-CNSS-after-`per_mgr` observer order를 helper `v210`에 source/build-only로 추가하는 계획
...
- `reports/NATIVE_INIT_V2190_V2189_SECURITY_P0_STAGE_FIX_BASELINE_PROMOTION_2026-06-10.md` – V2189 security P0 stage-fix artifact를 현재 native-init baseline으로 승격한 V2190 promotion report
...
- `reports/NATIVE_INIT_V1033_PM_SELINUX_DOMAIN_PROOF_2026-05-26.md` – V1033 live 결과 current-boot V490 policy-load는 통과했지만 helper `v175` domain-proof allowlist가 PM contexts를 차단해 V1034 source/build repair를 선택한 결과
- `reports/NATIVE_INIT_V1034_PM_SELINUX_DOMAIN_ALLOWLIST_SUPPORT_2026-05-26.md` – V1034 source/build 결과 helper `v176`에 PM SELinux domain-proof contexts가 추가되고 static build가 통과한 결과
- `reports/NATIVE_INIT_V1039_PM_PROXY_CONTEXT_PARITY_SUPPORT_2026-05-26.md` – V1039 source/build 결과 helper `v177`이 `pm-proxy`를 `vendor_per_proxy`로 매핑하고 PM fd/wchan gap capture marker를 포함한 결과
- `reports/NATIVE_INIT_V1040_HELPER_V177_DEPLOY_2026-05-26.md` – V1040 deploy-only 결과 helper `v177` remote sha/usage parity가 일치하고 daemon/Wi-Fi bring-up 없이 설치 완료된 결과
- `reports/NATIVE_INIT_V1041_PM_FULL_CONTRACT_V177_LIVE_2026-05-26.md` – V1041 live 결과 helper `v177`은 배포됐지만 current-boot SELinux exec attr가 `kernel`로 남아 PM actors를 fail-closed 차단한 결과
- `reports/NATIVE_INIT_V1042_PM_SELINUX_DOMAIN_PROOF_V177_2026-05-26.md` – V1042 결과 V401/V490 current-boot refresh 뒤 helper `v177` PM domains가 4/4 post-exec proof를 통과한 결과
- `reports/NATIVE_INIT_V1043_PM_FULL_CONTRACT_AFTER_V1042_2026-05-26.md` – V1043 결과 PM domains는 4/4 matched지만 `pm_proxy_helper`가 `pil_boot/subsys_powerup`에서 D-state로 막혀 PM fd contract가 미형성된 결과
- `reports/NATIVE_INIT_V1044_PM_PIL_ANDROID_GPIO_ESOC_CLASSIFIER_2026-05-26.md` – V1044 결과 V1043 blocker가 SELinux parity가 아니라 Android-only lower PM/PIL prerequisite delta임을 host-only로 분류한 결과
- `reports/NATIVE_INIT_V1048_HELPER_V178_DEPLOY_2026-05-26.md` – V1048 결과 helper `v178` remote sha가 일치하고 native health/actor-clean/Wi-Fi-link-clean을 유지한 deploy-only 결과
- `reports/NATIVE_INIT_V1049_PM_FULL_CONTRACT_WITH_MODEM_HOLDER_2026-05-26.md` – V1049 결과 modem pre-holder가 `ENOENT`로 `/dev/subsys_modem`을 열지 못해 PM fd contract가 미형성됨을 검증한 결과
- `reports/NATIVE_INIT_V1050_PM_MODEM_PRE_HOLDER_PRIVATE_ROOT_REPAIR_2026-05-26.md` – V1050 결과 helper `v179`가 private root에서 `/dev/subsys_modem` pre-holder를 열도록 source/build-only 수정한 결과
- `reports/NATIVE_INIT_V1051_HELPER_V179_DEPLOY_2026-05-26.md` – V1051 결과 helper `v179` remote sha/usage contract가 일치하고 native health/no-Wi-Fi guard를 유지한 deploy-only 결과
- `reports/NATIVE_INIT_V1052_PM_FULL_CONTRACT_WITH_MODEM_HOLDER_V179_2026-05-26.md` – V1052 결과 private-root pre-holder가 경로 문제를 통과했지만 `/dev/subsys_modem` open이 `errno=14`로 실패해 PM fd contract가 미형성된 결과
...
- `reports/NATIVE_INIT_V1058_FIRST_OPEN_RUNTIME_PREREQ_2026-05-27.md` – V1058 결과 current native boot에서 `firmware_class.path`는 맞지만 global firmware mounts와 modem PIL blob visibility가 없어 다음을 bounded firmware-mount prerequisite refresh로 라우팅한 결과
- `reports/NATIVE_INIT_V1059_FIRMWARE_MOUNT_REFRESH_2026-05-27.md` – V1059 결과 `apnhlos`/`modem` firmware partitions를 read-only global mount로 검증하고 cleanup까지 통과했지만 mount-only로는 QRTR/WLFW marker delta가 없어 다음을 bounded companion start-only와 결합하도록 라우팅한 결과
- `reports/NATIVE_INIT_V1060_CURRENT_CNSS_ONLY_OBSERVER_2026-05-27.md` – V1060 결과 current-boot SELinux/policy refresh 후 firmware mounts, modem holder, lower companions, `cnss_diag`, `cnss-daemon`은 통과했지만 WLFW/MHI/BDF/`wlan0` service publication은 여전히 비어 있음을 분류한 결과
- `reports/NATIVE_INIT_V1061_GLOBAL_FIRMWARE_PM_FULL_CONTRACT_2026-05-27.md` – V1061 결과 global firmware mount가 helper modem pre-holder를 해결했지만 `pm-service`/`per_mgr`의 `/dev/subsys_modem` fd가 없어 PM full contract가 미완성이고 eSoC refcount warning을 별도 blocker로 분류한 결과
- `reports/NATIVE_INIT_V1062_PM_CONTRACT_GAP_CLASSIFIER_2026-05-27.md` – V1062 결과 Android는 `pm-service` `/dev/subsys_modem` fd와 WLFW chain이 있지만 native V1061은 `per_mgr` fd가 없고 eSoC refcount warning이 있어 다음을 PM service trigger/input 분석으로 라우팅한 결과
...
- `reports/NATIVE_INIT_V1082_PM_SERVICE_INSTRUCTION_TRACE_LIVE_2026-05-27.md` – V1082 live 결과 `get_system_info` failure branch와 pipe close path가 hit되고 success/Binder path가 0임을 instruction-level tracefs로 확정한 결과
- `reports/NATIVE_INIT_V1083_MDMDETECT_SYSTEM_INFO_CLASSIFIER_2026-05-27.md` – V1083 host-only 결과 `libmdmdetect.so`가 `/sys/bus/msm_subsys/devices`와 `/sys/bus/esoc/devices`를 요구하며 V1084 instruction trace 후보를 분류한 결과
- `reports/NATIVE_INIT_V1084_MDMDETECT_INSTRUCTION_TRACE_LIVE_2026-05-27.md` – V1084 live 결과 `get_system_info()`가 private namespace의 `/sys/bus/msm_subsys/devices` 부재로 실패함을 instruction-level trace로 확정한 결과
- `reports/NATIVE_INIT_V1085_PM_SERVICE_MODEM_DETECT_SYSFS_PARITY_2026-05-27.md` – V1085 결과 helper `v197`에서 PM observer에 modem detect sysfs surface를 materialize해 `get_system_info()` success path를 통과시킨 결과
- `reports/NATIVE_INIT_V1086_PM_SERVICE_SUCCESS_PATH_TRACE_LIVE_2026-05-27.md` – V1086 live 결과 `pm-service`가 mdmdetect success 뒤 Binder `addService("vendor.qcom.PeripheralManager")`에서 실패하고 QMI thread 전에 clean return 0으로 종료됨을 확정한 결과
...
- `reports/NATIVE_INIT_V1114_PM_SERVICE_LIFETIME_CLASSIFIER_2026-05-27.md` – V1114 결과 `pm-service`가 1000 ms post-start probe 전에 `exit_code=0`으로 종료되어 V1115 immediate-CNSS-after-per_mgr gate를 선택한 결과
- `reports/NATIVE_INIT_V1115_IMMEDIATE_CNSS_HELPER_BUILD_2026-05-27.md` – V1115 결과 helper `v210`에 immediate-CNSS-after-`per_mgr` PM observer order를 추가하고 static helper build/string 검증을 통과한 결과
...
- `reports/NATIVE_INIT_V1124_PRIVATE_FIRMWARE_PM_OBSERVER_LIVE_2026-05-27.md` – V1124 결과 helper-private firmware mounts는 성공했지만 provider가 보이지 않아 다음을 private-firmware namespace의 `pm-service` early clean-exit trace로 좁힌 결과
- `reports/NATIVE_INIT_V1128_POST_POLICY_PRIVATE_FIRMWARE_CNSS_PM_2026-05-27.md` – V1128 결과 V490 policy-load 뒤 provider와 CNSS PM register/connect가 통과하고 `/dev/subsys_modem` `__subsystem_get` blocker로 이동한 결과
- `reports/NATIVE_INIT_V1129_POST_POLICY_GLOBAL_FIRMWARE_MOUNT_ONLY_2026-05-27.md` – V1129 결과 global firmware mount-only가 충분하지 않아 bounded `/dev/subsys_modem` first-opener contract로 라우팅한 결과
...
- `reports/NATIVE_INIT_V1134_OUTER_HOLDER_POST_POLICY_CNSS_LIVE_2026-05-27.md` – V1134 live 결과 CNSS PM register/connect는 성공했지만 `mdm3`/service69/WLFW/wlan0 lower publication delta가 없음을 확인한 결과
- `reports/NATIVE_INIT_V1135_LOWER_PUBLICATION_GAP_CLASSIFIER_2026-05-27.md` – V1135 결과 남은 blocker를 PM provider가 아닌 `mdm3`/eSoC/MDM2AP/WLFW lower publication gap으로 고정한 결과
- `reports/NATIVE_INIT_V1136_POST_PM_ESOC_GATE_PLANNER_2026-05-27.md` – V1136 결과 helper `v213`에 post-PM `mdm_helper`/eSoC composite mode가 없어 V1137 source/build-only를 선택한 결과
- `reports/NATIVE_INIT_V1137_POST_PM_MDM_HELPER_COMPOSITE_HELPER_BUILD_2026-05-27.md` – V1137 결과 helper `v214`에 post-PM `mdm_helper`/eSoC observer composite mode를 추가하고 static ARM64 build/marker 검증을 통과한 결과
