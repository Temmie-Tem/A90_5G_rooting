...
- 2026-05-27 기준 최신 PM observer proof는 `docs/reports/NATIVE_INIT_V1111_TAGGED_PM_CONNECT_PATH_CAPTURE_2026-05-27.md`입니다.
...
- V1107에서 확인된 pre-CNSS `per_proxy` mutex wait blocker는 ordering 문제로 닫혔고, 현재 블로커는 native init에서 CNSS PM connect 이후 `/dev/subsys_modem` open이 왜 반환되지 않는지입니다.
- 아직 Wi-Fi HAL, scan/connect/link-up, DHCP, route, external ping은 실행하지 않았습니다.
- 다음 블로커는 Wi-Fi HAL 전에 `/dev/subsys_modem` open precondition을 Android-good/native-bad로 비교하는 것입니다.
