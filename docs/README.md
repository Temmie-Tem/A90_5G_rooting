...
- 2026-05-27 기준 최신 PM observer proof는 `docs/reports/NATIVE_INIT_V1104_PM_SERVER_CONNECT_MUTEX_TRACEFS_2026-05-27.md`입니다.
- V1104에서 `pm-proxy`는 PM register/connect 모두 `0x0`으로 반환되고 connect helper가 modem mutex를 unlock하지만, 이후 `cnss-daemon`은 같은 modem record의 `pthread_mutex_lock(entry + 0x18)`에서 멈춤을 확인했습니다.
- 아직 Wi-Fi HAL, scan/connect/link-up, DHCP, route, external ping은 실행하지 않았습니다.
- 다음 블로커는 raw `pthread_mutex_lock`/`unlock` 또는 Binder thread state 기반 modem mutex owner/waiter 분류입니다.
