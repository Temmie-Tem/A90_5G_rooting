...
- 2026-05-27 기준 최신 PM observer proof는 `docs/reports/NATIVE_INIT_V1105_PM_SERVER_RAW_MUTEX_TRACEFS_2026-05-27.md`입니다.
- V1105에서 `pm-proxy`는 PM register/connect 모두 `0x0`으로 반환되고 connect helper가 modem mutex를 unlock하지만, 이후 `cnss-daemon`은 같은 modem record의 raw `pthread_mutex_lock@plt` call에서 반환하지 않음을 확인했습니다.
- 아직 Wi-Fi HAL, scan/connect/link-up, DHCP, route, external ping은 실행하지 않았습니다.
- 다음 블로커는 Binder thread `wchan`/futex state 기반 modem mutex owner/waiter 분류입니다.
