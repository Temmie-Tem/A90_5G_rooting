...
| S011 | PASS | active host scripts do not set known root SSH credentials | No active `scripts/revalidation` or `mkbootimg/gki/certify_bootimg.py` match for default root SSH credential patterns. | Legacy archived docs/scripts are excluded from active v143 runtime/tooling scope. |
...
| S013 | PASS | volume hold repeat timer clears when a screen cannot consume repeats | Retained v131-v143 auto-HUD loops clear `menu_hold_code` and `menu_hold_next_ms` when a timed repeat is not consumed. | Covers F032 zero-timeout poll/redraw spin in non-repeat screens. |
...
```bash
python3 scripts/revalidation/local_security_rescan.py --out docs/security/scans/SECURITY_FRESH_SCAN_V143_2026-05-08.md
git diff --check
```
