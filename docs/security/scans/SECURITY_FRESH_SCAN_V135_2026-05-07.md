# v135 Fresh Local Security Rescan

Date: 2026-05-07
Baseline: `A90 Linux init 0.9.35 (v135)`
Git HEAD: `349f641`
Scope: active v135 native-init source, shared modules, current revalidation host tools, and known root-control surfaces.

This is a local targeted rescan, not a Codex Cloud scanner replacement. It checks the previously imported F001-F033 pattern families, exposure guardrails, and v135 controller policy matrix wiring against the current repository state.
...
| S011 | PASS | active host scripts do not set known root SSH credentials | No active `scripts/revalidation` or `mkbootimg/gki/certify_bootimg.py` match for default root SSH credential patterns. | Legacy archived docs/scripts are excluded from active v135 runtime/tooling scope. |
...
| S013 | PASS | volume hold repeat timer clears when a screen cannot consume repeats | Retained v131-v135 auto-HUD loops clear `menu_hold_code` and `menu_hold_next_ms` when a timed repeat is not consumed. | Covers F032 zero-timeout poll/redraw spin in non-repeat screens. |
...
| S017 | WARN | accepted local root-control channels remain intentionally present | USB ACM root shell and localhost serial bridge are still present by design. | Matches F021/F030 accepted-lab-boundary; do not expose bridge or ACM control over LAN/Wi-Fi without new auth. |

## Interpretation

The local targeted scan found no new implementation blocker in the active v135 code path. The remaining warning is the already accepted trusted-lab boundary for physical USB ACM/local serial bridge control.
...
```bash
python3 scripts/revalidation/local_security_rescan.py --out docs/security/scans/SECURITY_FRESH_SCAN_V135_2026-05-07.md
git diff --check
```
