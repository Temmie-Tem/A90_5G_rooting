# F046. NCM preflight may run untrusted cache tcpctl as root
...
- Valid finding.
- Initial class: host harness selects persistent `/cache/bin` helper using weak existence-only validation.
- Related to helper trust and tcpctl exposure findings: F002, F003, F004, F005, F042, F044.
- Treat as P1 before using NCM preflight as serverization evidence.
- Preferred first remediation is to remove the `/cache/bin/a90_tcpctl` preflight fallback and require `/bin/a90_tcpctl`.

## Local Remediation

- Implemented in the F045-F046 host harness batch.
- `ncm_tcp_preflight.py` now defines `TRUSTED_TCPCTL_BINARY = "/bin/a90_tcpctl"`.
- The preflight probes only `/bin/a90_tcpctl` and refuses `/cache/bin/a90_tcpctl` fallback.
- If the ramdisk helper is unavailable, the module fails with an explicit trusted-helper-missing message instead of executing a persistent cache helper.
- Local validation:
