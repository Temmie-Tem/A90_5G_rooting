# F045. Predictable /tmp root dd target permits symlink overwrite
...
- Valid finding.
- Initial class: device-side root write path under shared `/tmp`.
- Related to previous path/symlink guard findings: F011, F013, F035, F038, F041.
- Treat as P0 before broader long-running mixed-soak use.
- Preferred remediation is a root-created private device temp directory, not just a less predictable filename.

## Local Remediation

- Implemented in the F045-F046 host harness batch.
- `cpu_memory_profiles.py` now adds a `secrets.token_hex(8)` suffix to the run id.
- Each profile creates a root-owned private temp directory under `/tmp/a90-cpumem.*` with `toybox mkdir -m 700`.
- Memory workload files are written inside that private directory and cleanup removes the private directory, not a predictable shared `/tmp/<run-id>-<profile>-mem.bin` file.
- Local validation:
