...
- Exact phrase plus only `--allow-device-mutation` fails closed.
- Exact phrase plus only `--assume-yes` fails closed.
- Missing-one-flag cases fail closed for both V351 `run` and `cleanup`.
