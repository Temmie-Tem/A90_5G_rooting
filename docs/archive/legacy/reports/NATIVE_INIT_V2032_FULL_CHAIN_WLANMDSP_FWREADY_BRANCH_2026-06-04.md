...
- Decision: `v2032-full-chain-wlanmdsp-open-oack-fwmem-only-no-fwready-rollback-pass`
- Label: `full-chain-wlanmdsp-open-oack-fwmem-only-no-fwready`
- Result: PASS
- Reason: native now reaches WLAN-PD UP, requests/opens `wlanmdsp.mbn`, and completes WLFW cap/BDF/cal, but the TFTP trace only proves RRQ/OACK/open, not ACK/DATA payload transfer; only the non-queued fw-mem indication (`msg_id=0x2b`) follows, with no queueable FW-ready/status indication (`0x28`/`0x2a`/`0x41`), WLFW service 69, FW-ready, or `wlan0`.
...
| wlanmdsp TFTP | open-only | `V2029` saw RRQ `/readonly/vendor/firmware_mnt/image/wlanmdsp.mbn`, successful open of `/vendor/rfs/msm/mpss/readonly/vendor/firmware_mnt/image/wlanmdsp.mbn`, and OACKs, but `ACK=0`, `DATA=0`, and no transfer-complete stats |
...
- The requested full-chain branch is `wlan_pd UP + cnss-daemon running + wlanmdsp requested/opened + cap/BDF/cal complete`, but `WLFW 69` remains absent.
- This no longer proves the second requested branch (`requested+served+loaded yet no WLAN-PD completion`) because V2029 captured no ACK/DATA payload transfer. The safe label is open/OACK-only until transfer completion is measured.
...
- The next useful measurement is the TFTP transfer-completion edge: prove whether native completes the Android-good fallback `wlanmdsp.mbn` transfer before treating the missing queueable WLFW FW-ready/status indication (`0x28`/`0x2a`/`0x41`) as the next blocker.
- If a live run is needed, use the rollbackable lower-window route with Android-parity fallback readonly bridge, readwrite tmpfs bridge, full downstream consumer chain, and the least intrusive TFTP transfer stat observer available; do not add boot-time QRTR matrix or multi-strace.
