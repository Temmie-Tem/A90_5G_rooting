...
- V423 Android hwservice inventory result: read-only ADB collector is implemented and V424 corrected its classifier to match full private `lshal` command evidence instead of truncated manifest preview text. Plan: `docs/plans/NATIVE_INIT_V423_ANDROID_HWSERVICE_INVENTORY_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V423_ANDROID_HWSERVICE_INVENTORY_2026-05-20.md`.
- V424 Android hwservice handoff result: bounded Android boot handoff, V423 read-only inventory, and native v319 rollback completed with `v424-handoff-pass`; Wi-Fi bring-up remained false. Android-side V423 evidence reports `v423-android-hwservice-targets-present` for the three Samsung `ISehWifi/default` fqinstances, but `sys.boot_completed` was still empty and `lshal` emitted fetch warnings/N/A rows. Plan: `docs/plans/NATIVE_INIT_V424_ANDROID_HWSERVICE_HANDOFF_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V424_ANDROID_HWSERVICE_HANDOFF_2026-05-20.md`.
- V425 boot-complete Android hwservice handoff result: bounded handoff waited for `sys.boot_completed=1`, reran V423, restored native v319, and compared against V422/V407. Decision `v425-bootcomplete-targets-present-native-gap`: all three Samsung `ISehWifi/default` fqinstances are present in boot-complete Android, while native V422 targeted wait still times out and V407 HAL start-only remains clean. Android boot-complete also shows framework Wi-Fi binder services and `wpa_supplicant` present as part of normal Android boot. Plan: `docs/plans/NATIVE_INIT_V425_BOOTCOMPLETE_HWSERVICE_HANDOFF_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V425_BOOTCOMPLETE_HWSERVICE_HANDOFF_2026-05-20.md`.
- V426 service-surface mapper result: host-only mapper parsed V425/V423 boot-complete Android evidence and reported `v426-native-registration-surface-gap`. Android has boot-complete framework Wi-Fi services, `vendor.wifi_hal_ext`, `wificond`, `wpa_supplicant`, AOSP/Samsung Wi-Fi HAL processes, VINTF Wi-Fi declarations, and all three Samsung `ISehWifi/default` fqinstances. Native V407 keeps the HAL alive, but V422 still cannot observe the fqinstances. Plan: `docs/plans/NATIVE_INIT_V426_SERVICE_SURFACE_MAPPER_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V426_SERVICE_SURFACE_MAPPER_2026-05-20.md`.
- V427 query improvement planner result: host-only planner parsed V425/V426 target rows and AOSP `lshal` semantics, then reported `v427-explicit-status-query-needed`. All three Samsung `ISehWifi/default` rows are present, but all have `cannot be fetched from service manager (null)` warnings and the current columns do not prove explicit live registration. Plan: `docs/plans/NATIVE_INIT_V427_QUERY_IMPROVEMENT_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V427_QUERY_IMPROVEMENT_2026-05-20.md`.
- V428 lshal status-column probe result: helper v29 added `wifi-hal-lshal-vintf-status-list` and `wifi-hal-composite-lshal-status-list`, deployed successfully, restored SELinuxfs runtime surface through the existing bounded V401 executor, and ran the explicit status-column query. Decision `v428-lshal-status-query-runtime-gap`: native VINTF-only evidence declares `vendor.samsung.hardware.wifi@2.2::ISehWifi/default`, but the bounded composite `lshal list --types=binderized,vintf --neat -V -S -i -p -e -c` query timed out. All composite children were observable, cleanup/postflight were clean, and Wi-Fi bring-up remained false. Plan: `docs/plans/NATIVE_INIT_V428_LSHAL_STATUS_COLUMNS_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V428_LSHAL_STATUS_COLUMNS_2026-05-20.md`.
- V429 minimal lshal status split result: helper v30 added `wifi-hal-composite-lshal-binderized-status-list`, deployed successfully, and ran VINTF-only control plus bounded binderized-only `lshal list --types=binderized --neat -S`. Decision `v429-lshal-minimal-split-runtime-gap`: VINTF-only native evidence again declares `vendor.samsung.hardware.wifi@2.2::ISehWifi/default`, but binderized-only native query still timed out. All composite children were observable, cleanup/postflight were clean, and Wi-Fi bring-up remained false. Plan: `docs/plans/NATIVE_INIT_V429_LSHAL_MINIMAL_SPLIT_PLAN_2026-05-20.md`. Report: `docs/reports/NATIVE_INIT_V429_LSHAL_MINIMAL_SPLIT_2026-05-20.md`.
- next execution item: V430 Android explicit-column mirror. Boot Android to `sys.boot_completed=1`, run read-only VINTF/binderized minimal `lshal` commands, restore native v319, and compare Android rows with V429 native timeout before deciding Android-managed runtime pivot.
...
### V397. SELinux Surface Proof — PASS / NATIVE STATUS MISSING

- plan: `docs/plans/NATIVE_INIT_V397_SELINUX_SURFACE_PROOF_PLAN_2026-05-20.md`
- report: `docs/reports/NATIVE_INIT_V397_SELINUX_SURFACE_PROOF_2026-05-20.md`
- tool: `scripts/revalidation/wifi_service_manager_selinux_surface_proof.py`
- evidence: `tmp/wifi/v397-selinux-surface-final-20260520-075153/`
- result:
  - decision `service-manager-selinux-status-native-missing`
  - `/proc/filesystems` has `selinuxfs`
  - `/proc/mounts` has no `selinuxfs` at `/sys/fs/selinux`
  - `/sys/fs/selinux/status` and `/sys/fs/selinux/enforce` are absent
  - Android `plat_service_contexts` and `system_ext_service_contexts` are visible under `/mnt/system`
  - `device_mutations=False`, `daemon_start_executed=False`, `wifi_bringup_executed=False`
...
### V400. Toybox SELinuxfs Mount Approval Packet — PASS / READY FOR V401 APPROVAL

- plan: `docs/plans/NATIVE_INIT_V400_TOYBOX_SELINUXFS_MOUNT_APPROVAL_PACKET_PLAN_2026-05-20.md`
- report: `docs/reports/NATIVE_INIT_V400_TOYBOX_SELINUXFS_MOUNT_APPROVAL_PACKET_2026-05-20.md`
- tools:
...
### V454. Wi-Fi Operator Strict Post-route Packet — PASS / NEXT OPERATOR COMMAND
...
- generator: `scripts/revalidation/wifi_operator_strict_postroute_packet_v454.py`
- router updates:
  - `scripts/revalidation/wifi_handoff_result_router_v449.py`
  - `scripts/revalidation/wifi_operator_preflight_readiness_v450.py`
- evidence:
...
- result:
  ...
  - generated host preflight/live scripts include automatic V449/V450/V452 post-route calls.
  - if V447 succeeds but any post-route command fails, the generated script exits with the post-route failure.
  - generated scripts passed `bash -n` and fail-closed empty/cancel probes.
  - V449 and V450 now recommend the V454 host preflight script.
  - no device commands or mutations ran.
  - `wifi_bringup_executed=False`.
