...
이 문서는 `A90 Linux init 0.9.9 (v109)` verified 이후 바로 실행할 작업 큐다.
큰 방향은 “보이는 부팅 → 복구 가능한 로그 → 단독 조작 → 작은 userland → USB networking” 순서다.

## 현재 고정 기준점

- latest verified build: `A90 Linux init 0.9.9 (v109)`
- official version: `0.9.9`
- build tag: `v109`
...
- local artifact retention: `v109` latest, `v108` rollback, `v48` known-good만 보존하고 나머지 ignored stage3 산출물은 정리 가능
...
- next execution item: v110 menu/app controller cleanup
