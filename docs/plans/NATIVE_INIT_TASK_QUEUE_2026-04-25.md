...
- next execution item: V428 explicit `lshal -V -S` status-column probe. Add a native VINTF-only control plus bounded composite binderized/vintf query, optionally mirrored by Android boot-complete read-only capture, before any Wi-Fi bring-up attempt.
