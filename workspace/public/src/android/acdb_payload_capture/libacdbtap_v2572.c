/*
 * V2572 ARM32 ACDB interposer with generic indirect-output capture.
 *
...
static __attribute__((unused)) void a90_exit_group(int code)
{
...
static __attribute__((unused)) void a90_write_str(int fd, const char *s)
{
...
static int a90_log_capture(uint32_t seq, uint32_t cmd, uint32_t in_len, uint32_t out_len,
                           int32_t ret, const uint8_t *out, const char *kind)
{
    uint8_t digest[32];
    char digest_hex[64];
    char raw_path[160];
    ...
    a90_build_raw_path(raw_path, seq, cmd, out_len, kind);
...
static int a90_log_generic_indirect_capture(uint32_t seq, uint32_t cmd, uint32_t in_len,
                                            int32_t ret, const uint8_t *in)
{
    uint32_t indirect_len;
    const uint8_t *indirect_out;

    if (ret != 0 || !in || in_len < 8U)
        return 0;

    indirect_len = a90_load_le32(in);
    indirect_out = (const uint8_t *)(unsigned long)a90_load_le32(in + 4U);
    if (!a90_is_probable_user_read_ptr(indirect_out, indirect_len))
        return 0;

    return a90_log_capture(seq, cmd, in_len, indirect_len, ret, indirect_out, "indirect");
}
...
acdb_ioctl(uint32_t cmd, const uint8_t *in, uint32_t in_len, uint8_t *out, uint32_t out_len)
{
    ...
    if (a90_log_generic_indirect_capture(seq, cmd, in_len, ret, in))
        a90_exit_group(0);
#else
    (void)a90_log_generic_indirect_capture(seq, cmd, in_len, ret, in);
#endif
#if A90_ACDBTAP_EXIT_ON_TARGET
    if (a90_log_capture(seq, cmd, in_len, out_len, ret, out, "direct"))
        a90_exit_group(0);
#else
    (void)a90_log_capture(seq, cmd, in_len, out_len, ret, out, "direct");
#endif
    a90_in_hook = 0;
    return ret;
}
