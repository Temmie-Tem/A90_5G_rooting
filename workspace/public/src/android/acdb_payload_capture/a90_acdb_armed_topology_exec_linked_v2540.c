/*
 * V2540 ARM32 own-process ACDB topology caller.
 *
 * Built as a 32-bit Android PIE with direct DT_NEEDED dependencies on the
 * staged stock ACDB libraries.  It initializes ACDB, arms the LD_PRELOAD
 * acdb_ioctl dumper, and calls acdb_loader_send_common_custom_topology().
 *
 * The helper itself does not open /dev/msm_audio_cal, does not issue audio
 * calibration ioctls, and does not handle raw topology bytes.  The V2540
 * combined preload captures the acdb_ioctl GET output and exits the process
 * immediately after a real non-zero 4916-byte payload is banked.
 */
...
void _start(void)
{
    ...
    a90_write_event("topology_helper", "init_v3_return", init_ret);
    if (init_ret != 0)
        a90_exit(29);

    if (!a90_arm_capture) {
        a90_write_event("topology_helper", "arm_capture_missing", -1);
        a90_exit(30);
    }
    a90_arm_capture();
    a90_write_event("topology_helper", "armed_before_common_topology", 0);

    topology_ret = acdb_loader_send_common_custom_topology();
    a90_write_event("topology_helper", "send_common_custom_topology_return", topology_ret);
    a90_exit(topology_ret == 0 ? 0 : 31);
}
