/*
 * V2553 ARM32 own-process ACDB full-manifest caller.
 *
 * This helper initializes the stock ACDB loader, arms the acdb_ioctl dump
 * interposer, then asks libacdbloader for the same speaker topology and
 * per-device playback calibration edges observed in Android-good runs.
 *
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-full-manifest-events.jsonl"
...
void _start(void)
{
    ...
    a90_write_event("full_manifest_helper", "init_v3_return", init_ret);
    if (init_ret != 0)
        a90_exit(29);

    if (!a90_arm_capture) {
        a90_write_event("full_manifest_helper", "arm_capture_missing", -1);
        a90_exit(30);
    }

    a90_arm_capture();
    a90_write_event("full_manifest_helper", "armed_before_common_topology", 0);

    topology_ret = acdb_loader_send_common_custom_topology();
    a90_write_event("full_manifest_helper", "send_common_custom_topology_return", topology_ret);

    a90_write_event("full_manifest_helper", "before_send_audio_cal_v5", 0);
    ...
    a90_write_event("full_manifest_helper", "send_audio_cal_v5_return", audio_cal_ret);
