/*
 * V2617 ARM32 own-process ACDB direct GET matrix driver.
 *
 ...
 * returns.  This helper then issues the V2616-pinned per-device GET geometry
 * directly.  It does not call the send-audio-cal dispatcher, does not open the
 * native audio calibration device, and does not issue any calibration SET.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2617-direct-matrix-events.jsonl"
...
#define A90_VOL_MAX_GAIN_STEP 15U
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2617_direct_matrix\",\"stage\":\"");
                                 ...
                                 int32_t ret, uint32_t out_word)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2617_direct_matrix\",\"stage\":\"case_return\",\"case\":\"");
...
static void a90_run_base_matrix(void)
{
    uint32_t words[5];

    words[0] = A90_APP_TYPE_MEDIA;
    (void)a90_call("preget-app", 0x0001122eU, words, 1U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = A90_APP_TYPE_MEDIA;
    (void)a90_call("device-app-meta", 0x0001122dU, words, 2U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = A90_SAMPLE_RATE_48K;
    words[2] = A90_APP_TYPE_MEDIA;
    (void)a90_call("audproc-common-size", 0x00013267U, words, 3U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = A90_SAMPLE_RATE_48K;
    words[2] = A90_APP_TYPE_MEDIA;
    words[3] = A90_MATRIX_CAPACITY;
    words[4] = (uint32_t)(uintptr_t)a90_out;
    (void)a90_call("audproc-common-data", 0x00013265U, words, 5U, 4U, 0U);

    words[0] = A90_APP_TYPE_MEDIA;
    (void)a90_call("audproc-stream-size", 0x00013268U, words, 1U, 4U, 0U);

    words[0] = A90_APP_TYPE_MEDIA;
    words[1] = A90_MATRIX_CAPACITY;
    words[2] = (uint32_t)(uintptr_t)a90_out;
    (void)a90_call("audproc-stream-data", 0x00013269U, words, 3U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    (void)a90_call("afe-meta", 0x000130d8U, words, 1U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = A90_SAMPLE_RATE_48K;
    (void)a90_call("afe-common-size", 0x00013271U, words, 2U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = A90_SAMPLE_RATE_48K;
    words[2] = A90_MATRIX_CAPACITY;
    words[3] = (uint32_t)(uintptr_t)a90_out;
    (void)a90_call("afe-common-data", 0x0001326fU, words, 4U, 4U, 0U);

    words[0] = A90_SPEAKER_ACDB_ID;
    words[1] = 1U;
    words[2] = 0xccU;
    words[3] = 0xffc1a3c4U;
    (void)a90_call("tail-meta", 0x00012eebU, words, 4U, 4U, 0U);
}

static void a90_run_vol_sweep(void)
{
    uint32_t words[5];
    uint32_t step;

    for (step = 0; step <= A90_VOL_MAX_GAIN_STEP; step++) {
        words[0] = A90_SPEAKER_ACDB_ID;
        words[1] = A90_APP_TYPE_MEDIA;
        words[2] = step;
        (void)a90_call("vol-size", 0x0001326dU, words, 3U, 4U, step);
        ...
        words[3] = A90_MATRIX_CAPACITY;
        words[4] = (uint32_t)(uintptr_t)a90_out;
        (void)a90_call("vol-data", 0x0001326eU, words, 5U, 4U, step);
    }
}
...
void _start(void)
{
    ...
    a90_write_event("before_base_matrix", 0);
    a90_run_base_matrix();
    a90_write_event("before_vol_sweep", 0);
    a90_run_vol_sweep();
