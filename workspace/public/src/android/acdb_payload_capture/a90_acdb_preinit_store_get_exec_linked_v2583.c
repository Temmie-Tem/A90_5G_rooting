/*
 * V2583 ARM32 ACDB pre-init store-get driver.
 *
 * This helper only enters acdb_loader_init_v3().  The V2583 preload owns the
 * common-topology hook, skips the already-pinned topology public call, runs the
 * bounded store_get metadata cases inside that hook, and exits before the known
 * libacdbloader init-tail crash can run.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2583-init-driver-events.jsonl"
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2583_init_driver\",\"stage\":\"");
...
void _start(void)
{
    ...
    a90_write_event("start", 0);
    a90_write_event("before_init_v3", 0);
    init_ret = acdb_loader_init_v3(A90_ACDB_FILES_PATH, A90_DELTA_DIR, 0U);
    a90_write_event("init_v3_return_unexpected", init_ret);
    a90_exit(init_ret == 0 ? 0 : 29);
}
