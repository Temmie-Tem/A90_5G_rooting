/*
 * V2692 ARM32 ACDB interposer with lower custom-topology pointer-target dumps.
 *
...
#define A90_CMD_LOWER_AFE_CUSTOM_TOPO 0x000130daU
#define A90_CMD_LOWER_ADM_CUSTOM_TOPO 0x00011394U
#define A90_CMD_LOWER_ASM_CUSTOM_TOPO 0x00012e01U
#define A90_CMD_LOWER_AFE_CUSTOM_TOPO_SUPP 0x000130dcU
...
#define A90_MAPS_PATH "/proc/self/maps"
#define A90_PTRTARGET_MAX_BYTES 4096U
...
static int a90_open_read(const char *path)
{
    return (int)a90_syscall4(A90_NR_OPENAT, A90_AT_FDCWD, (long)path, A90_O_RDONLY, 0);
}

static long a90_read_fd(int fd, void *buf, size_t len)
{
    return a90_syscall3(A90_NR_READ, fd, (long)buf, (long)len);
}
...
static int a90_hex_digit_value(char c)
{
    if (c >= '0' && c <= '9')
        return c - '0';
    if (c >= 'a' && c <= 'f')
...
static const char *a90_parse_hex_u32(const char *p, uint32_t *out)
{
    uint32_t value = 0;
    int digit;
    int seen = 0;

    while ((digit = a90_hex_digit_value(*p)) >= 0) {
...
static int a90_maps_range_readable(uint32_t addr, uint32_t len, uint32_t *map_start,
                                   uint32_t *map_end)
{
    ...
    fd = a90_open_read(A90_MAPS_PATH);
    if (fd < 0)
        return 0;

    nr = a90_read_fd(fd, maps_buf, sizeof(maps_buf) - 1U);
    a90_close(fd);
    if (nr <= 0)
        return 0;
    maps_buf[nr] = 0;
    ...
    while (p < end && *p) {
        uint32_t start = 0;
        uint32_t stop = 0;
        const char *next = a90_parse_hex_u32(p, &start);
        ...
        if (!next || *next != '-')
            continue;
        next = a90_parse_hex_u32(next + 1, &stop);
        if (!next || *next != ' ')
            continue;
        next++;
        if (*next != 'r')
            continue;
        if (addr >= start && range_end <= stop) {
            if (map_start)
                *map_start = start;
            if (map_end)
                *map_end = stop;
...
static int a90_is_probable_android32_read_ptr(const uint8_t *buf, uint32_t len)
{
    uint32_t addr = (uint32_t)(unsigned long)buf;

    if (!buf || len == 0 || len > A90_MAX_CAPTURE_LEN)
        return 0;
    if (addr < 0x00010000U || addr >= 0xffff0000U)
        return 0;
    if (addr + len < addr || addr + len >= 0xffff0000U)
        return 0;
    return 1;
}

static uint32_t a90_out_word0(const uint8_t *out, uint32_t out_len)
{
    if (!out || out_len < 4U)
        return 0;
    return a90_load_le32(out);
}

static int a90_log_known_indirect_capture(uint32_t seq, uint32_t cmd, uint32_t in_len,
                                          int32_t ret, const uint8_t *in,
                                          const uint8_t *out, uint32_t out_len,
                                          const char *kind, uint32_t ptr_word,
                                          uint32_t cap_word)
{
    uint32_t words;
    uint32_t dump_len;
    ...
    if (ret != 0 || !in || !out || out_len < 4U)
        return 0;

    words = in_len / 4U;
    if (ptr_word >= words || cap_word >= words)
        return 0;

    dump_len = a90_out_word0(out, out_len);
    cap_len = a90_load_le32(in + (cap_word * 4U));
    payload = (const uint8_t *)(unsigned long)a90_load_le32(in + (ptr_word * 4U));

    if (dump_len == 0 || cap_len == 0 || dump_len > cap_len)
        return 0;
    if (!a90_is_probable_android32_read_ptr(payload, dump_len))
        return 0;

    return a90_log_capture_kind(seq, cmd, in_len, dump_len, ret, payload, kind, 0);
}

static int a90_log_command_layout_indirect_captures(uint32_t seq, uint32_t cmd,
                                                    uint32_t in_len, int32_t ret,
                                                    const uint8_t *in,
                                                    const uint8_t *out,
                                                    uint32_t out_len)
{
    switch (cmd) {
    case A90_CMD_AUDPROC_INSTANCE_COMMON_TABLE:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-ap-common", 4U, 3U);
    case A90_CMD_AUDPROC_INSTANCE_STREAM_TABLE:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-ap-stream", 2U, 1U);
    ...
    case A90_CMD_LOWER_AFE_CUSTOM_TOPO:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-lower-afe-custom-topology", 1U, 0U);
    case A90_CMD_LOWER_ADM_CUSTOM_TOPO:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-lower-adm-custom-topology", 1U, 0U);
    case A90_CMD_LOWER_ASM_CUSTOM_TOPO:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-lower-asm-custom-topology", 1U, 0U);
    case A90_CMD_LOWER_AFE_CUSTOM_TOPO_SUPP:
        return a90_log_known_indirect_capture(seq, cmd, in_len, ret, in, out, out_len,
                                              "ind-lower-afe-supp-custom-topology", 1U, 0U);
...
static int a90_is_lower_custom_topology_cmd(uint32_t cmd)
{
    return cmd == A90_CMD_LOWER_AFE_CUSTOM_TOPO ||
           cmd == A90_CMD_LOWER_ADM_CUSTOM_TOPO ||
           cmd == A90_CMD_LOWER_ASM_CUSTOM_TOPO ||
           cmd == A90_CMD_LOWER_AFE_CUSTOM_TOPO_SUPP;
}

static void a90_log_ptrtarget_status(uint32_t seq, uint32_t cmd, uint32_t in_len,
                                     uint32_t ptr, uint32_t requested_len,
                                     uint32_t dump_len, uint32_t map_start,
                                     uint32_t map_end, const char *status)
{
    char line[768];
    char *p;
    int event_fd;
    ...
    p = line;
    p = a90_append_str(p, "{\"event\":\"ptrtarget_status\",");
    p = a90_append_hex_field(p, "seq", seq);
    *p++ = ',';
    p = a90_append_hex_field(p, "pid", a90_getpid());
    *p++ = ',';
    ...
    *p++ = ',';
    p = a90_append_hex_field(p, "in_len", in_len);
    *p++ = ',';
    p = a90_append_hex_field(p, "ptr", ptr);
    *p++ = ',';
    p = a90_append_hex_field(p, "requested_len", requested_len);
    *p++ = ',';
    p = a90_append_hex_field(p, "dump_len", dump_len);
    *p++ = ',';
    p = a90_append_hex_field(p, "map_start", map_start);
    *p++ = ',';
    p = a90_append_hex_field(p, "map_end", map_end);
    *p++ = ',';
    p = a90_append_str(p, "\"status\":\"");
    p = a90_append_str(p, status);
    *p++ = '"';
    *p++ = '}';
    *p++ = '\n';
...
static int a90_log_lower_custom_ptrtarget_pre(uint32_t seq, uint32_t cmd, uint32_t in_len,
                                              const uint8_t *in)
{
    ...
    if (!a90_is_lower_custom_topology_cmd(cmd) || !in || in_len < 8U)
        return 0;

    requested_len = a90_load_le32(in);
    ptr = a90_load_le32(in + 4U);
    dump_len = requested_len;
    if (dump_len > A90_PTRTARGET_MAX_BYTES)
        dump_len = A90_PTRTARGET_MAX_BYTES;

    if (!dump_len) {
        a90_log_ptrtarget_status(seq, cmd, in_len, ptr, requested_len, dump_len, 0, 0,
                                 "ptrtarget_zero_len");
        return 0;
    }

    if (!a90_maps_range_readable(ptr, dump_len, &map_start, &map_end)) {
        a90_log_ptrtarget_status(seq, cmd, in_len, ptr, requested_len, dump_len, 0, 0,
                                 "ptrtarget_unmapped");
        return 0;
    }

    payload = (const uint8_t *)(unsigned long)ptr;
    a90_log_ptrtarget_status(seq, cmd, in_len, ptr, requested_len, dump_len, map_start, map_end,
                             "ptrtarget_maps_verified");
    (void)a90_log_capture_kind(seq, cmd, in_len, dump_len, 0, payload, "ptrtarget-pre", 0);
    return 1;
}
...
#endif
    (void)a90_log_lower_custom_ptrtarget_pre(seq, cmd, in_len, in);
    ret = a90_real_acdb_ioctl(cmd, in, in_len, out, out_len);
...
#if A90_ACDBTAP_EXIT_ON_TARGET
    if (a90_log_command_layout_indirect_captures(seq, cmd, in_len, ret, in, out, out_len))
        a90_exit_group(0);
#else
    (void)a90_log_command_layout_indirect_captures(seq, cmd, in_len, ret, in, out, out_len);
#endif
