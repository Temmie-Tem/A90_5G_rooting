/*
 * V2717 ARM32 own-process ACDB route-first common-topology caller.
 *
 * This helper initializes the stock ACDB loader, arms the acdb_ioctl dump
 * interposer, asks libacdbloader for the observed speaker per-device playback
 * calibration edge first, then asks for the common custom topology.
 *
 * Safety boundary: this helper itself does not open /dev/msm_audio_cal and does
 * not issue any ioctl directly.  Live use must pair it with the combined preload
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-route-first-common-events.jsonl"
...
void _start(void)
{
    ...
    a90_write_event("route_first_common_helper", "init_v3_return", init_ret);
    if (init_ret != 0)
        a90_exit(29);

    if (!a90_arm_capture) {
        a90_write_event("route_first_common_helper", "arm_capture_missing", -1);
        a90_exit(30);
    }

    a90_arm_capture();
    a90_write_event("route_first_common_helper", "armed_before_send_audio_cal_v5", 0);

    a90_write_event("route_first_common_helper", "before_send_audio_cal_v5", 0);
    ...
    a90_write_event("route_first_common_helper", "send_audio_cal_v5_return", audio_cal_ret);

    topology_ret = acdb_loader_send_common_custom_topology();
    a90_write_event("route_first_common_helper", "send_common_custom_topology_return", topology_ret);
