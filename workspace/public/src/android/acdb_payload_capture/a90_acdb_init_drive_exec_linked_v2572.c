/*
 * V2572 ARM32 own-process ACDB per-device indirect init driver.
 *
 * This helper only starts acdb_loader_init_v3().  The V2572 preload owns the
 * common-topology hook, drives the bounded per-device capture path, and exits
 * the process before the known init-tail null dereference can run.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2572-init-driver-events.jsonl"
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2572_init_driver\",\"stage\":\"");
