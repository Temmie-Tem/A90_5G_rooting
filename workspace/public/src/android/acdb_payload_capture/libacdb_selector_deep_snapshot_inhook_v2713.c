/*
 * V2713 lower hidden-node ACDB selector deep-snapshot preload.
 *
 ...
 * create_cal_node(base+0xfd45), allocate_cal_block(base+0xfbbd), and the pinned
 * acdb_ioctl GET command.  Unlike V2674/V2692, this unit does not call the
 * fake AUDIO_SET_CALIBRATION shim; it supplies an own-process 64 KiB output
 * buffer to the GET so the acdbtap indirect capture can dump selected topology
 * bytes after a successful ret==0.
 *
 * V2704 showed the shallow block snapshot is insufficient: cal_type 24/10/14
 * share the same visible node selector seed and only differ in output buffer /
 * mem_handle.  V2713 adds a bounded node/block memory word snapshot around the
 * loader-internal objects before the forced large GET.  The output remains
 * private JSONL only; no AUDIO_SET_CALIBRATION call is made by this path.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2713-selector-deep-snapshot-events.jsonl"
...
#define A90_V2713_NODE_WORDS 16U
#define A90_V2713_BLOCK_WORDS 32U
...
static void a90_write_block_snapshot(uint32_t cal_type, const struct a90_cal_node *node,
                                     const struct a90_cal_block *block)
{
    int fd = a90_open_append(A90_EVENTS_PATH);
    if (fd < 0)
        return;

    a90_write_str(fd, "{\"event\":\"v2692_lower_block_snapshot\",\"cal_type\":");
    a90_write_dec(fd, cal_type);
    a90_write_str(fd, ",\"node\":\"");
    a90_write_hex32(fd, (uint32_t)(uintptr_t)node);
    a90_write_str(fd, "\",\"node_word0\":\"");
    a90_write_hex32(fd, node ? node->word0 : 0);
    a90_write_str(fd, "\",\"node_word4\":\"");
    a90_write_hex32(fd, node ? node->word4 : 0);
    a90_write_str(fd, "\",\"block\":\"");
    a90_write_hex32(fd, (uint32_t)(uintptr_t)block);
    a90_write_str(fd, "\",\"get_arg0\":\"");
    a90_write_hex32(fd, block ? block->get_arg0 : 0);
    a90_write_str(fd, "\",\"word4\":\"");
    a90_write_hex32(fd, block ? block->word4 : 0);
    a90_write_str(fd, "\",\"get_arg1\":\"");
    a90_write_hex32(fd, block ? block->get_arg1 : 0);
    a90_write_str(fd, "\",\"mem_handle\":");
    a90_write_sdec(fd, block ? block->mem_handle : -1);
    a90_write_str(fd, ",\"word16\":\"");
    a90_write_hex32(fd, block ? block->word16 : 0);
    a90_write_str(fd, "\",\"word20\":\"");
    a90_write_hex32(fd, block ? block->word20 : 0);
    a90_write_str(fd, "\",\"pid\":");
    a90_write_dec(fd, a90_getpid());
    a90_write_str(fd, ",\"tid\":");
    a90_write_dec(fd, a90_gettid());
...
static void a90_write_words_json_array(int fd, const uint32_t *words, uint32_t count)
{
    uint32_t i;
    ...
    for (i = 0; i < count; i++) {
        if (i)
            a90_write_str(fd, ",");
        a90_write_str(fd, "\"");
        a90_write_hex32(fd, words ? words[i] : 0);
        a90_write_str(fd, "\"");
    }
    a90_write_str(fd, "]");
}

static void a90_write_selector_deep_snapshot(uint32_t cal_type, const struct a90_cal_node *node,
                                             const struct a90_cal_block *block)
{
    int fd = a90_open_append(A90_EVENTS_PATH);
    if (fd < 0)
    ...
    a90_write_str(fd, "{\"event\":\"v2713_selector_deep_snapshot\",\"cal_type\":");
    a90_write_dec(fd, cal_type);
    a90_write_str(fd, ",\"node\":\"");
    a90_write_hex32(fd, (uint32_t)(uintptr_t)node);
    a90_write_str(fd, "\",\"block\":\"");
    a90_write_hex32(fd, (uint32_t)(uintptr_t)block);
    a90_write_str(fd, "\",\"node_words\":");
    a90_write_words_json_array(fd, (const uint32_t *)node, A90_V2713_NODE_WORDS);
    a90_write_str(fd, ",\"block_words\":");
    a90_write_words_json_array(fd, (const uint32_t *)block, A90_V2713_BLOCK_WORDS);
    a90_write_str(fd, ",\"pid\":");
    a90_write_dec(fd, a90_getpid());
    a90_write_str(fd, ",\"tid\":");
    a90_write_dec(fd, a90_gettid());
...
#define A90_V2703_LARGE_GET_BYTES 65536U
static uint8_t a90_v2703_large_get_buf[A90_V2703_LARGE_GET_BYTES];

static void a90_zero_bytes(uint8_t *buf, uint32_t count)
{
    uint32_t i;
    for (i = 0; i < count; i++)
        buf[i] = 0;
...
static int32_t a90_run_one_lower_target(const struct a90_target_cal *target)
{
    ...
    a90_write_block_snapshot(target->cal_type, node, block);
    a90_write_selector_deep_snapshot(target->cal_type, node, block);

    a90_zero_bytes(a90_v2703_large_get_buf, A90_V2703_LARGE_GET_BYTES);
    get_in[0] = A90_V2703_LARGE_GET_BYTES;
    get_in[1] = (uint32_t)(uintptr_t)a90_v2703_large_get_buf;
    a90_write_event("v2703_large_get_request", 0, target->cal_type, get_in[0]);
    get_ret = acdb_ioctl(target->get_cmd, (const uint8_t *)get_in, 8U, (uint8_t *)&get_out, 4U);
    a90_write_event("v2703_large_get_return", get_ret, target->cal_type, get_out);
    if (get_ret != 0)
        return -34;
