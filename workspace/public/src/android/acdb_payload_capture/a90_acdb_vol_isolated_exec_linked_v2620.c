/*
 * V2620 ARM32 own-process ACDB VOL-isolated direct GET driver.
 *
 * V2618 proved the V2617 matrix captures AFE/AUDPROC payloads but SIGSEGVs
 * after entering the late tail metadata command, before the VOL sweep starts.
 * This helper keeps the proven init/manual-arm/fake-allocate path, skips the
 * crashing late-tail command entirely, and runs only the minimal safe preget
 * probes plus the VOL direct GET sweep.  It does not call send_audio_cal_v5,
 * does not open /dev/msm_audio_cal, and does not issue calibration SET.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2620-vol-isolated-events.jsonl"
...
static void a90_write_event(const char *stage, int code)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2620_vol_isolated\",\"stage\":\"");
                                 ...
                                 int32_t ret, uint32_t out_word)
{
    ...
    a90_write_str(fd, "{\"event\":\"v2620_vol_isolated\",\"stage\":\"case_return\",\"case\":\"");
...
static void a90_run_safe_prelude(void)
{
    uint32_t words[2];
...
void _start(void)
{
    ...
    a90_write_event("before_safe_prelude", 0);
    a90_run_safe_prelude();
