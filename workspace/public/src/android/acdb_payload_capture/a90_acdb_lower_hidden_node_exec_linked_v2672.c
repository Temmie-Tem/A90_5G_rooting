/*
 * V2672 ARM32 own-process ACDB lower hidden-node caller.
 *
 * Host/build-only unit. Live use must pair this helper with the V2672 preload,
 * libacdbtap indirect-layout tap, and the V2630 ioctl shim. The helper only
 * initializes libacdbloader, arms capture after init returns, and delegates the
 * lower hidden-node sequence to a preload export. It does not open the audio-calibration device, does not issue ioctls,
 * and does not write speaker audio.
 */
...
extern int32_t a90_run_lower_hidden_nodes(void) __attribute__((weak, visibility("default")));

#define A90_ACDB_FILES_PATH "/vendor/etc/audconf/OPEN"
#define A90_DELTA_DIR "/data/local/tmp/a90-acdb-ownget/delta"
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2672-lower-hidden-helper-events.jsonl"
...
void _start(void)
{
    ...
    if (!a90_run_lower_hidden_nodes) {
        a90_write_event("lower_runner_missing", -1);
        a90_exit(31);
    }

    a90_arm_capture();
    a90_write_event("armed_before_lower_hidden_nodes", 0);
    lower_ret = a90_run_lower_hidden_nodes();
    a90_write_event("lower_hidden_nodes_return", lower_ret);
