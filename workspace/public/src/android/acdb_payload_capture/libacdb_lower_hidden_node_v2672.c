/*
 * V2672 lower hidden-node ACDB SET capture preload.
 *
 * This preload keeps the V2608 init common-topology policy: skip the public
 * common path during acdb_loader_init_v3, patch the initialized flag, and return
 * to init. After the helper arms capture post-init, a90_run_lower_hidden_nodes()
 * recreates the pinned V2671 lower sequence using libacdbloader internal
 * offsets: create_cal_node(base+0xfd45), allocate_cal_block(base+0xfbbd), the
 * pinned acdb_ioctl GET command, and a fake AUDIO_SET_CALIBRATION ioctl. The
 * V2630 ioctl shim linked into the same shared object always fake-successes SET
 * and dumps the SET argument/dma-buf before returning.
 */
...
#define A90_EVENTS_PATH "/data/local/tmp/a90-acdb-ownget/acdb-v2672-lower-hidden-events.jsonl"
...
__attribute__((visibility("default"))) int32_t acdb_loader_send_common_custom_topology(void)
{
    ...
    a90_write_event("return_to_init_v3_no_arm_no_send", common_ret, 0, 0);
