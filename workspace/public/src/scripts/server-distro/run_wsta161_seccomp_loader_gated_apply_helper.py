...
def emit_helper_source(manifest: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        *table,
        ...
        "      if (i + 1 >= (size_t)argc) {",
        "        printf(\"a90_seccomp_loader_decision=blocked-exec-command-required\\n\");",
        "        return 64;",
        "      }",
        "      exec_argv = &argv[i + 1];",
        "      break;",
        ...
        "    return 65;",
        "  }",
        "  if (exec_after_load && !apply) {",
        "    printf(\"a90_seccomp_loader_decision=blocked-exec-requires-apply\\n\");",
        "    return 65;",
        "  }",
        "  if (service != NULL) {",
        ...
        "      {",
        "        int rc = a90_wsta161_load_profile(profile);",
        "        if (rc != 0) {",
        "          return rc;",
        "        }",
        "        if (exec_after_load) {",
...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, bool]:
    return {
        ...
        "exec_after_load_compiled": manifest.get("exec_after_load_compiled") is True,
