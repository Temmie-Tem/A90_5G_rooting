#!/usr/bin/env python3
"""WSTA159 host-only seccomp loader check-only rootfs proof.

Stages the WSTA153 policy, WSTA156 non-loaded filter artifact, and WSTA158
check-only loader helper into a private rootfs directory.  The launcher is then
run with ``A90_SERVICE_LAUNCH_SECCOMP_ENFORCE=1`` and a qemu wrapper around the
staged ARM64 helper, proving the helper check-only path executes before the
launcher still blocks actual seccomp load/enforcement.
"""
...
PASS_DECISION = "wsta159-seccomp-loader-checkonly-rootfs-proof-pass"
SUMMARY_NAME = "wsta159_result.json"
DRY_RUN_STDOUT_NAME = "launcher_checkonly_helper_dry_run_stdout.txt"
ENFORCE_STDOUT_NAME = "launcher_checkonly_helper_enforce_stdout.txt"
QEMU_WRAPPER_NAME = "a90-seccomp-loader-checkonly-qemu-wrapper"
...
def make_qemu_wrapper(fakebin: Path, qemu: str, helper: Path) -> Path:
    wrapper = fakebin / QEMU_WRAPPER_NAME
    wrapper.write_text(
        "#!/bin/sh\n"
        f"exec {qemu} {helper} \"$@\"\n",
        encoding="utf-8",
...
def run_launcher(rootfs: Path, fakebin: Path, helper_wrapper: Path, *, enforce: bool) -> dict[str, Any]:
    env = os.environ.copy()
    ...
    env["A90_SERVICE_LAUNCH_SECCOMP_LOADER_HELPER"] = str(helper_wrapper)
    if enforce:
...
def validate_proof(dry_run: dict[str, Any], enforce_run: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "dry_run_helper_check_only": marker_value(dry_stdout, "A90WSTA159_SECCOMP_HELPER_CHECK_ONLY") == "1",
        ...
        "enforce_helper_check_only": marker_value(enforce_stdout, "A90WSTA159_SECCOMP_HELPER_CHECK_ONLY") == "1",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ts = utc_stamp()
    run_id = args.run_id or f"wsta159-seccomp-loader-checkonly-rootfs-proof-{ts}"
    ...
    result: dict[str, Any] = {
        "scope": "WSTA159 host-only seccomp loader check-only rootfs proof",
        "started_utc": ts,
        "run_dir": rel(run_dir),
        "safety": safety_flags(),
        "checks": {
            "explicit_gate": bool(args.emit_seccomp_loader_checkonly_rootfs_proof),
            ...
            "filter_manifest_private": is_under(filter_manifest, PRIVATE_ROOT),
            "filter_manifest_present": filter_manifest.is_file(),
            "filter_object_private": is_under(filter_object, PRIVATE_ROOT),
            "filter_object_present": filter_object.is_file(),
            "helper_manifest_private": is_under(helper_manifest, PRIVATE_ROOT),
            "helper_manifest_present": helper_manifest.is_file(),
            "helper_binary_private": is_under(helper_binary, PRIVATE_ROOT),
            "helper_binary_present": helper_binary.is_file(),
            "qemu_aarch64_present": bool(qemu_path),
        },
    }
    for key, decision in (
        ("explicit_gate", "wsta159-blocked-explicit-gate-required"),
        ("private_run_dir", "wsta159-blocked-nonprivate-run-dir"),
        ("policy_json_private", "wsta159-blocked-policy-json-nonprivate"),
        ("policy_json_present", "wsta159-blocked-policy-json-missing"),
        ("filter_manifest_private", "wsta159-blocked-filter-manifest-nonprivate"),
        ("filter_manifest_present", "wsta159-blocked-filter-manifest-missing"),
        ("filter_object_private", "wsta159-blocked-filter-object-nonprivate"),
        ("filter_object_present", "wsta159-blocked-filter-object-missing"),
        ("helper_manifest_private", "wsta159-blocked-helper-manifest-nonprivate"),
        ("helper_manifest_present", "wsta159-blocked-helper-manifest-missing"),
        ("helper_binary_private", "wsta159-blocked-helper-binary-nonprivate"),
        ("helper_binary_present", "wsta159-blocked-helper-binary-missing"),
        ("qemu_aarch64_present", "wsta159-blocked-qemu-aarch64-missing"),
    ):
    ...
    stage = stage_rootfs(rootfs, policy_path, filter_manifest, filter_object, helper_manifest, helper_binary)
    helper_wrapper = make_qemu_wrapper(fakebin, args.qemu_aarch64, rootfs / wsta3.TARGET_SECCOMP_LOADER_HELPER)
    dry_run = run_launcher(rootfs, fakebin, helper_wrapper, enforce=False)
    enforce_run = run_launcher(rootfs, fakebin, helper_wrapper, enforce=True)
    proof_checks = validate_proof(dry_run, enforce_run)
    result["stage"] = stage
    result["proof"] = {
        "service": "dpublic-hud",
        "qemu_wrapper": rel(helper_wrapper),
    ...
    result["decision"] = PASS_DECISION if all(proof_checks.values()) else "wsta159-blocked-proof-invalid"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--emit-seccomp-loader-checkonly-rootfs-proof", action="store_true")
...
def main_with_args(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        payload = {"decision": "wsta159-runner-error", "error": str(exc)}
