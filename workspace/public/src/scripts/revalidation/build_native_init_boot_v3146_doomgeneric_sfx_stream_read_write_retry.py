#!/usr/bin/env python3
"""Build V3146 DOOM real SFX PCM stream over the V3141 demo HUD stack."""
...
CYCLE = "V3146"
INIT_VERSION = "0.10.128"
INIT_BUILD = "v3146-doomgeneric-sfx-stream-read-write-retry"
BUILD_TAG = INIT_BUILD
DECISION = "v3146-doomgeneric-sfx-stream-read-write-retry-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3146_DOOMGENERIC_SFX_STREAM_READ_WRITE_RETRY_SOURCE_BUILD_2026-06-24.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v3146_doomgeneric_sfx_stream_read_write_retry.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v3146_doomgeneric_sfx_stream_read_write_retry"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3146_doomgeneric_sfx_stream_read_write_retry.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v531_doomgeneric_sfx_stream_read_write_retry"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3146"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3146.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3146.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3146"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3146-sfx-stream-read-write-retry"

FRAME_PATH = "/tmp/a90-doomgeneric-v3146-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3146-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3146-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3146-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3146-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3146-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3146-sfx.pcmstream"
...
FRAME_SCALE = "1:1-demo-hud-large-groups-sfx-stream-read-write-retry"
SCALE_PATH = v3141.SCALE_PATH
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-sfx-stream-read-write-retry"

INPUT_THREAD_MARKER = v3141.INPUT_THREAD_MARKER.replace("v3141", "v3146")
TIME_MODEL_MARKER = v3141.TIME_MODEL_MARKER.replace("v3141", "v3146")
DEMO_HUD_MARKER = v3141.DEMO_HUD_MARKER.replace("v3141", "v3146")
PACED_TIME_MARKER = v3141.PACED_TIME_MARKER.replace("v3141", "v3146")
TICK_TELEMETRY_MARKER = v3141.TICK_TELEMETRY_MARKER.replace("v3141", "v3146")
SCALE_MARKER = v3141.SCALE_MARKER.replace("v3141", "v3146")
PHASE_TELEMETRY_MARKER = v3141.PHASE_TELEMETRY_MARKER.replace("v3141", "v3146")
GAMETIC_FRAME_TELEMETRY_MARKER = v3141.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3141", "v3146")
SFX_STREAM_MARKER = "a90.doomgeneric.v3146.audio=real-sfx-pcm-stream-read-write-retry-music-disabled"
SOUND_MODE = "native-doom-sfx-pcm-stream-read-write-retry-v3146"
AUDIO_CORUN = 1
...
SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3146.c"
...
def replace_required(text: str, old: str, new: str) -> str:
    if old not in text:
        raise RuntimeError(f"missing V3146 source fragment: {old[:100]!r}")
...
def replace_loop_required(text: str, old: str, new: str) -> str:
    ...
    if start < 0:
        raise RuntimeError("missing V3146 loop function")
    end = text.find("\nint main(int argc, char **argv) {", start)
    if end < 0:
        raise RuntimeError("missing V3146 main after loop function")
    loop = text[start:end]
    if old not in loop:
        raise RuntimeError(f"missing V3146 loop fragment: {old[:100]!r}")
...
def _rewrite_required_string(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3141": b"a90-doomgeneric-v3146",
        b"a90.doomgeneric.v3141": b"a90.doomgeneric.v3146",
        b"v3141": b"v3146",
    }
...
static void sfx_update(void) {{
    ...
    if (stream_fd >= 0 && write_full(stream_fd, mix, frames * 2U * sizeof(mix[0])) < 0) {{
        close(stream_fd);
        stream_fd = -1;
    }}
}}
...
def v3146_adapter_source() -> str:
    source = _V3141_ADAPTER_SOURCE_TEXT.replace("v3141", "v3146").replace("V3141", "V3146")
    source = source.replace(
        "a90.doomgeneric.v3146.scale=producer-960x600-1to1-demo-hud-large-groups",
        SCALE_MARKER,
    )
    source = source.replace(
        "a90.doomgeneric.v3146.demo_hud=large-grouped-hw-doom-input",
        DEMO_HUD_MARKER,
    )
    source = replace_required(
        source,
        'const char a90_doomgeneric_v3146_demo_hud_policy[] =\n'
        '    "a90.doomgeneric.v3146.demo_hud=large-grouped-hw-doom-input";',
        'const char a90_doomgeneric_v3146_demo_hud_policy[] =\n'
        f'    "{DEMO_HUD_MARKER}";\n'
        'const char a90_doomgeneric_v3146_audio_policy[] =\n'
        f'    "{SFX_STREAM_MARKER}";',
    )
    source = replace_required(
        source,
        "        marker_checksum(a90_doomgeneric_v3146_demo_hud_policy) == 0U) {",
        "        marker_checksum(a90_doomgeneric_v3146_demo_hud_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3146_audio_policy) == 0U) {",
    )
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3146 DOOMGENERIC SFX Stream Read Write Retry Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "- Starts a bounded 10-second PCM stream co-run for loop-start and reports `real_doom_sfx=1`.",
        "",
        "## Runtime Contract",
        "",
        ...
        "## Validation",
        "",
        "- `py_compile`: V3146 builder and focused tests.",
        "- `unittest`: V3146 source contract.",
        "- Build: AArch64 helper/native-init compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3146 identity, real SFX stream markers, stream path, and inherited V3141 HUD/input markers.",
        "- `git diff --check`: PASS.",
        "",
        "## Metadata",
        "",
        ...
        "- Candidate type: `doomgeneric-sfx-stream-read-write-retry-candidate`.",
    ]) + "\n"
...
def configure_v3146_modules() -> dict[str, Any]:
    ...
    v3141.v3141_adapter_source = v3146_adapter_source
...
def restore_v3146_modules(saved: dict[str, Any]) -> None:
    v3133 = v3141.v3137.v3135.v3133
    for name in _PATCHED_V3141_ATTRS:
...
def main() -> int:
    ...
    saved = configure_v3146_modules()
    try:
        rc = v3141.main()
    finally:
        restore_v3146_modules(saved)
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-sfx-stream-read-write-retry-candidate",
        "adoption_state": "pending-sfx-stream-read-write-retry-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-sfx-stream-read-write-retry-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-sfx-stream-read-write-retry-candidate",
