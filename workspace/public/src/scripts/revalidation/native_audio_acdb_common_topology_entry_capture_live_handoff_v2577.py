#!/usr/bin/env python3
"""V2577 live handoff for common-topology entry-arm ACDB topology capture.

This wrapper reuses the V2490 checked Android boot/stage/pull/rollback engine
with V2577 artifacts:

- helper only starts acdb_loader_init_v3();
- combined preload keeps acdb_ioctl silent until the common-topology hook is
  entered during init, arms capture immediately before calling the real
  acdb_loader_send_common_custom_topology(), dumps every out_len>0 ACDB call,
  fakes audio calibration ioctls when A90_ACDB_FAKE_ALLOCATE=1, and exits after
  the first ret==0 non-zero 4916-byte topology buffer.
"""
...
import build_android_acdb_common_topology_entry_capture_v2577 as v2577
import native_audio_acdb_ownprocess_get_live_handoff_v2490 as v2490

RUN_ID = "V2577"
BUILD_TAG = "v2577-audio-acdb-common-topology-entry-capture-live"
ROOT = v2577.ROOT
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2577_AUDIO_ACDB_COMMON_TOPOLOGY_ENTRY_CAPTURE_LIVE_HANDOFF_2026-06-16.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2577-acdb-common-topology-entry-capture-{stamp}"
...
def read_v2577_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    common_topology_entry_arm = (
        "-DA90_ACDBTAP_AUTO_ARM_ON_INITIALIZE=0" in cflags
        and "-DA90_ACDBTAP_CUSTOM_TOPOLOGY_ONLY=0" in cflags
    )
    return {
        "ok": bool(payload.get("ok") and helper.get("ok") and preload.get("ok") and common_topology_entry_arm),
        "path": rel(path),
        "manifest": payload,
        "helper": helper,
        "preload": preload,
        "common_topology_entry_arm": common_topology_entry_arm,
        "common_hook_arms_before_real": bool(required.get("common_hook_arms_before_real")),
...
def build_v2577_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        build_root=args.v2577_build_root,
        manifest_path=args.v2577_manifest_path,
        clang=v2577.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2577.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    return v2577.manifest(build_args)


def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2577_artifacts:
        build_v2577_artifacts(args)
    manifest = read_v2577_manifest(args.v2577_manifest_path)
...
def summarize_common_topology_entry_capture(artifact_dir: Path) -> dict[str, Any]:
    base_summary = v2490.parse_ownget_artifacts(artifact_dir)
    helper_events_path = artifact_dir / "acdb-v2572-init-driver-events.jsonl"
    common_events_path = artifact_dir / "acdb-v2577-common-topology-arm-events.jsonl"
    ...
    common_events = read_jsonl(common_events_path)
    helper_stages = [event.get("stage") for event in helper_events if event.get("event") == "v2572_init_driver"]
    common_stages = [event.get("stage") for event in common_events if event.get("event") == "v2577_common_topology_arm"]
    init_returns = [
        int32_or_none(event.get("code"))
        for event in helper_events
        if event.get("event") == "v2572_init_driver" and event.get("stage") == "init_v3_return"
    ]
    init_ok = bool(init_returns and init_returns[-1] == 0)
    common_hook_armed = "armed_before_real_common_topology" in common_stages
    ...
    if pass_through_set_events:
        classification = "v2577-boundary-violation-real-audio-set-passthrough"
    elif topology:
        classification = "v2577-common-topology-entry-capture-captured"
    elif common_hook_armed and not rows:
        classification = "v2577-common-topology-entry-armed-no-acdbtap-events"
    elif acdb_log_has_common_topology and acdb_log_has_topology_get and helper_sigsegv and not common_hook_armed:
        classification = "v2577-init-internal-topology-before-common-topology-entry-sigsegv"
    elif not common_hook_armed and not init_ok:
        classification = "v2577-init-v3-failed-before-arm"
    elif not rows:
        classification = "v2577-common-topology-entry-no-acdbtap-events"
    elif size_queries:
        classification = "v2577-common-topology-entry-capture-size-query-only"
    elif successful:
        classification = "v2577-common-topology-entry-capture-partial-nonzero-capture"
    elif not common_hook_armed:
        classification = "v2577-init-internal-common-topology-entry-no-topology-capture"
    else:
        classification = f"v2577-{base_summary.get('classification', 'no-acdbtap-capture')}"
    ...
    return {
        ...
        "init_v3_ok": init_ok,
        "common_hook_armed_before_real_common_topology": common_hook_armed,
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2577-acdb-common-topology-entry-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2577_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "common_topology_entry_arm": bool(artifacts.get("manifest", {}).get("common_topology_entry_arm")),
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2577 common-topology entry-arm helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2577 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2577_artifacts"]
    base_args = to_v2490_args(args, artifacts)
    result = v2490.run_live(base_args)
    pulled_dir = select_pulled_dir_from_result(result)
    capture_summary = summarize_common_topology_entry_capture(pulled_dir) if pulled_dir else {
        "classification": "v2577-no-pulled-artifacts",
        "success": False,
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2577-result.json", wrapper)
    return wrapper
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- v2490_engine_decision: `{payload.get('v2490_engine_result', {}).get('decision')}`",
        f"- classification: `{summary.get('classification')}`",
        f"- init_v3_ok: `{summary.get('init_v3_ok')}`",
        f"- common_hook_armed_before_real_common_topology: `{summary.get('common_hook_armed_before_real_common_topology')}`",
        f"- common_hook_event_count: `{summary.get('common_hook_event_count')}`",
        f"- common_hook_stages: `{summary.get('common_hook_stages')}`",
    ...
    lines.extend([
        ...
        "## Artifacts",
        "",
        f"- helper_sha256: `{payload.get('v2577_artifacts', {}).get('helper', {}).get('sha256')}`",
        f"- preload_sha256: `{payload.get('v2577_artifacts', {}).get('preload', {}).get('sha256')}`",
        "",
        "## Boundary",
        "",
    ...
    if summary.get("classification") == "v2577-common-topology-entry-armed-no-acdbtap-events":
        lines.extend([
            ...
            f"- The underlying V2490 engine decision was `{payload.get('v2490_engine_result', {}).get('decision')}`, so the real common-topology call did not complete inside the bounded helper window.",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2577-artifacts", action="store_true")
    parser.add_argument("--v2577-build-root", type=Path, default=v2577.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2577-manifest-path", type=Path, default=v2577.DEFAULT_MANIFEST)
