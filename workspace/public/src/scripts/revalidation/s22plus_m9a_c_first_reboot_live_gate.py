#!/usr/bin/env python3
"""Guarded S22+ M9A C first-action reboot native-init live gate.
...
M9A package and rollback APs without touching any device.

M9A is a lower-layer discriminator, not a module or USB proof. The expected
live proof is automatic return to Samsung download mode after a freestanding C
PID1 performs exactly one reboot("download") syscall. The helper must not count
the original candidate Odin endpoint as proof: it waits for that endpoint to
disconnect, then treats a later Odin endpoint as the candidate's self-download
result.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M9A-C-FIRST-REBOOT-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M9A-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M9A_AP_SHA256 = "c953f74fe7e3cdc226ebd3e1f0bac2142ee39e14483d87022714ae98e336d6b1"
EXPECTED_M9A_BOOT_SHA256 = "4c998680a1ccdbd5017053d7da58858ab818fc0644f08ef5bb0fc5d0dcc2d981"
EXPECTED_M9A_BASE_BOOT_SHA256 = "2e541703951dc725bad35850faf7028c2d910dd5f21166449b63f1248c29967e"
EXPECTED_M9A_KERNEL_SHA256 = "bceca73edbfca3499148e16741c939779157925949ef6bc8a8e31d6b68fc2cff"
EXPECTED_M9A_INIT_SHA256 = "46dfc4ecf92457260484d38360c70c0a45a1b7aead3a5cac567ec21ab2c7d97f"
EXPECTED_M9A_SOURCE_SHA256 = "6248617a4d2fe077768aef1324937659d33a0c93a453d0ecf9cd8cc3d3ec34a8"
EXPECTED_M9A_RETAINED_MARKER = "S22_NATIVE_INIT_M9A_NO_RETAINED_MARKER_EXPECTED"

DEFAULT_M9A_AP = Path("workspace/private/outputs/s22plus_native_init/inplace_m9a_c_first_reboot_v0_1/odin4/AP.tar.md5")
DEFAULT_M9A_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/inplace_m9a_c_first_reboot_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    ...
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m9a_c_first_reboot_live_gate_{stamp}")
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    required = [
        "S22+ M9A C first-action reboot native-init boot-only",
        EXPECTED_M9A_AP_SHA256,
        EXPECTED_M9A_BOOT_SHA256,
        EXPECTED_M9A_BASE_BOOT_SHA256,
        EXPECTED_M9A_KERNEL_SHA256,
        EXPECTED_M9A_INIT_SHA256,
        EXPECTED_M9A_SOURCE_SHA256,
        LIVE_ACK_TOKEN,
        ROLLBACK_ACK_TOKEN,
        "freestanding C raw-syscall runtime",
        "one direct `reboot(2)` syscall",
        "no marker",
        "no VFS",
        "no kmsg",
        "no mount",
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M9A live authorization markers: {missing}")


def verify_m9a_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M9A manifest missing: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    hashes = data.get("hashes", {})
    safety = data.get("safety", {})
    ramdisk = data.get("ramdisk", {})
    init_info = data.get("m9a_init", {})
    tar_members_seen = data.get("tar_members")
    append_log(log_path, f"m9a_manifest_path={path}")
    append_log(log_path, f"m9a_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    append_log(log_path, f"m9a_manifest_safety={json.dumps(safety, sort_keys=True)}")
    append_log(log_path, f"m9a_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")

    expected_hashes = {
        "ap_tar_md5": EXPECTED_M9A_AP_SHA256,
        "boot_img": EXPECTED_M9A_BOOT_SHA256,
        "base_boot": EXPECTED_M9A_BASE_BOOT_SHA256,
        "kernel": EXPECTED_M9A_KERNEL_SHA256,
        "m9a_init": EXPECTED_M9A_INIT_SHA256,
        "source": EXPECTED_M9A_SOURCE_SHA256,
        "nochange_repack_boot": EXPECTED_M9A_BASE_BOOT_SHA256,
    }
    for key, expected in expected_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M9A manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M9A manifest tar members mismatch: {tar_members_seen!r}")
    ...
    expected_safety: dict[str, Any] = {
        ...
        "first_externally_observable_action": "reboot-download",
        "intended_syscall_count": 1,
        "auto_reboot": "download-immediately",
        "marker_write": False,
        "kmsg_write": False,
        "vfs_setup": False,
        "mounts": False,
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise SystemExit(f"M9A manifest safety {key} mismatch: {safety.get(key)!r} != {expected!r}")
    if safety.get("intended_syscalls") != ["reboot"]:
        raise SystemExit(f"M9A intended syscalls mismatch: {safety.get('intended_syscalls')!r}")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M9A manifest did not replace /init")
    if ramdisk.get("only_intended_entry_change") != "init":
        raise SystemExit("M9A manifest intended ramdisk change is not only /init")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M9A must not inject vendor module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M9A must not inject module-list files into boot ramdisk")

    required_strings = set(init_info.get("required_strings", []))
    if "download" not in required_strings:
        raise SystemExit("M9A required download string missing from manifest")
    if init_info.get("svc_count") != 1:
        raise SystemExit(f"M9A expected one svc instruction, got {init_info.get('svc_count')!r}")
    ...
    for forbidden in [
        ...
        if forbidden not in forbidden_strings:
            raise SystemExit(f"M9A forbidden-string gate missing from manifest: {forbidden}")
...
def observe_until_odin(run_dir: Path, log_path: Path, seconds: int, odin: Path) -> str | None:
    ...
    while time.monotonic() < deadline:
        iteration += 1
        label = f"m9a_self_download_{iteration:03d}"
        host_snapshot(run_dir, log_path, label, odin)
        devices = odin_devices(odin, log_path, f"{label}-extra")
        if len(devices) == 1:
            append_log(log_path, f"m9a_self_download_seen=1 device={devices[0]}")
            return devices[0]
        if len(devices) > 1:
            raise SystemExit(f"refusing ambiguous Odin devices during M9A observation: {devices}")
        rows = adb_rows(log_path, f"{label}-extra")
        if rows:
            append_log(log_path, f"m9a_candidate_adb_rows={rows}")
        time.sleep(1.0)
    append_log(log_path, "m9a_self_download_seen=0")
    return None
...
def rollback_from_download(
    ...
    if serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", serial, marker=EXPECTED_M9A_RETAINED_MARKER)
    return 0


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--m9a-ap", type=Path, default=DEFAULT_M9A_AP)
    parser.add_argument("--m9a-manifest", type=Path, default=DEFAULT_M9A_MANIFEST)
    ...
    log_path = run_dir / "s22plus_m9a_c_first_reboot_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus m9a c first-reboot live gate ===")
    append_log(log_path, f"target={EXPECTED_TARGET}")

    odin = resolve(root, args.odin)
    m9a_ap = resolve(root, args.m9a_ap)
    m9a_manifest = resolve(root, args.m9a_manifest)
    ...
    verify_ap(m9a_ap, EXPECTED_M9A_AP_SHA256, "m9a_candidate", log_path)
    verify_m9a_manifest(m9a_manifest, log_path)
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M9A candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M9A rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M9A candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    if odin_device is None:
        print("download mode did not appear for M9A candidate flash", file=sys.stderr)
        return 2

    candidate_rc = flash_ap(odin, m9a_ap, odin_device, log_path, "candidate")
    if candidate_rc != 0:
        print(f"M9A candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        print(
            "M9A candidate flash completed but the original Odin device did not disconnect; "
        ...
        if post_rollback_serial is None:
            return 6
        append_log(log_path, "m9a_result=no-proof-original-download-never-disconnected")
        collect_android_pstore(
            run_dir,
            log_path,
            "post_rollback_no_disconnect",
            post_rollback_serial,
            marker=EXPECTED_M9A_RETAINED_MARKER,
        )
        return 7

    print("M9A candidate flashed. Waiting for C first-reboot self-return.")
    rollback_device = observe_until_odin(run_dir, log_path, args.self_download_wait_sec, odin)
    if rollback_device is None:
        print("M9A self-download did not appear; enter download mode manually and run --rollback-from-download.", file=sys.stderr)
        return 4
    ...
    if post_rollback_serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", post_rollback_serial, marker=EXPECTED_M9A_RETAINED_MARKER)
    print(f"M9A live gate completed with C first-reboot self-return and rollback ok; log={log_path}")
    return 0
