#!/usr/bin/env python3
"""Build S22+ M26 HS-only prefix/download discriminator artifacts.

Host-only. This script does not reboot, flash, or touch a connected device.

M26 uses the M25 HS-only USB2 module list and DTBO high-speed cap context. Each generated
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/m26_hs_prefix_download_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_m26_hs_prefix_download.c")
DEFAULT_M25_MANIFEST = DEFAULT_M25_OUT / "manifest.json"
M26_MODULES_RAMDISK = "s22plus_m26_hs_only_usb2.modules"
MARKER = "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD"
DOWNLOAD_ARG = "download"
EXPECTED_HS_ONLY_COUNT = 40
DEFAULT_PREFIXES = (0, 24, 25, 27, 28, 30, 33, 40)
...
def load_m25_hs_context(root: Path, m25_manifest_path: Path) -> dict[str, Any]:
    ...
    if len(module_text.encode("ascii")) >= 8192:
        raise SystemExit("M26 HS-only module list exceeds runtime parser buffer")
...
def compile_init(source: Path, out_path: Path, build_dir: Path, prefix_count: int) -> dict[str, str | list[str]]:
    ...
    result = run(
        [
            ...
            "-Wl,-e,_start",
            "-Wl,-z,noexecstack",
            f"-DM26_PREFIX_LIMIT={prefix_count}",
            f'-DM26_PREFIX_LABEL="{label}"',
            "-o",
            out_path,
            source,
        ]
    )
    require_ok(result, f"compile M26 {label} checkpoint init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, f"strip M26 {label} checkpoint init")

    file_info = run(["file", out_path])
    require_ok(file_info, f"file M26 {label} init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, f"readelf M26 {label} init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, f"objdump M26 {label} init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit(f"M26 {label} init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit(f"M26 {label} init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit(f"M26 {label} init disassembly does not contain svc")
    if not any("mov" in line and "x8" in line and "#0x8e" in line for line in objdump_text.splitlines()):
        raise SystemExit(f"M26 {label} init disassembly does not load arm64 __NR_reboot (142)")
    if prefix_count > 0 and not any("#0x111" in line and "// #273" in line for line in objdump_text.splitlines()):
        raise SystemExit(f"M26 {label} init disassembly does not load arm64 __NR_finit_module (273)")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M26 {label} /init: {required}")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"ttyGS0", b"ss_acm.0", b"/config"):
        if forbidden in binary:
            raise SystemExit(f"M26 {label} /init contains forbidden string: {forbidden!r}")
    (build_dir / f"m26_{label}_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / f"m26_{label}_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / f"m26_{label}_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def build_prefix(
    *,
    ...
    module_list = build_dir / M26_MODULES_RAMDISK
    module_list.write_text(str(hs_context["module_text"]), encoding="ascii")
    init_out = build_dir / f"s22plus_init_m26_{label.lower()}_hs_prefix_download"
    init_info = compile_init(source, init_out, build_dir, prefix_count)

    run_in_dir([magiskboot, "unpack", "-h", base_boot], nochange_dir, f"M26 {label} no-change unpack")
    run_in_dir([magiskboot, "repack", base_boot, prefix_dir / "boot_nochange_repack.img"], nochange_dir, f"M26 {label} no-change repack")
    nochange_sha = sha256_file(prefix_dir / "boot_nochange_repack.img")
    if nochange_sha != base_sha:
        raise SystemExit(f"M26 {label} no-change repack is not byte-identical: {nochange_sha} != {base_sha}")

    unpack_text = run_in_dir([magiskboot, "unpack", "-h", base_boot], work_dir, f"M26 {label} unpack")
    ramdisk = work_dir / "ramdisk.cpio"
    kernel = work_dir / "kernel"
    header = work_dir / "header"
    original_init = build_dir / "init.magisk.original"
    run_in_dir([magiskboot, "cpio", ramdisk, f"extract init {original_init}"], work_dir, f"M26 {label} extract original init")
    ...
    patch_init_text = run_in_dir([magiskboot, "cpio", ramdisk, f"add 750 init {init_out}"], work_dir, f"M26 {label} replace /init")
    patch_modules_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {M26_MODULES_RAMDISK} {module_list}"],
        work_dir,
        f"M26 {label} add module list",
    )
    cpio_test_after = run([magiskboot, "cpio", ramdisk, "test"], cwd=work_dir).returncode
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M26 {label} patch: {cpio_test_after}")

    extracted_init = build_dir / "init.replaced"
    run_in_dir([magiskboot, "cpio", ramdisk, f"extract init {extracted_init}"], work_dir, f"M26 {label} extract replaced init")
    if sha256_file(extracted_init) != sha256_file(init_out):
        raise SystemExit(f"replaced /init does not match compiled M26 {label} init")
    extracted_modules = build_dir / f"{M26_MODULES_RAMDISK}.extracted"
    run_in_dir([magiskboot, "cpio", ramdisk, f"extract {M26_MODULES_RAMDISK} {extracted_modules}"], work_dir, f"M26 {label} extract module list")
    if sha256_file(extracted_modules) != sha256_file(module_list):
        raise SystemExit(f"replaced M26 {label} module list does not match builder output")

    ramdisk_after = build_dir / "ramdisk.after.cpio"
    shutil.copy2(ramdisk, ramdisk_after)
    boot_img = prefix_dir / "boot.img"
    repack_text = run_in_dir([magiskboot, "repack", base_boot, boot_img], work_dir, f"M26 {label} repack patched boot")
    if boot_img.stat().st_size != BOOT_PARTITION_SIZE:
        raise SystemExit(f"M26 {label} patched boot size mismatch: {boot_img.stat().st_size} != {BOOT_PARTITION_SIZE}")
    patched_unpack_dir = prefix_dir / "patched-unpack"
    patched_unpack_dir.mkdir()
    run_in_dir([magiskboot, "unpack", "-h", boot_img], patched_unpack_dir, f"M26 {label} unpack patched boot")
    if sha256_file(patched_unpack_dir / "kernel") != sha256_file(kernel):
        raise SystemExit(f"M26 {label} patched boot kernel changed")
    ...
    if members != ["boot.img.lz4"]:
        raise SystemExit(f"M26 {label} AP tar member mismatch: {members}")
    ...
    hashes = {
        ...
        "m26_hs_only_modules": sha256_file(module_list),
        "m26_init": sha256_file(init_out),
    ...
    sizes = {
        ...
        "m26_hs_only_modules": module_list.stat().st_size,
        "m26_init": init_out.stat().st_size,
    ...
    manifest: dict[str, Any] = {
        ...
        "purpose": f"M26 {label} HS-only prefix/download: load first {prefix_count} of 40 M25 HS-only modules, then reboot-download",
        "prefix": {
            "label": label,
            "count": prefix_count,
        ...
        "safety": {
            ...
            "module_list_path": f"/{M26_MODULES_RAMDISK}",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": M26_MODULES_RAMDISK,
        ...
        "future_live_interpretation": {
            "download_mode_returns": f"M26 {label} reached checkpoint after first {prefix_count} modules",
            "no_download_and_loop": f"reset occurred before M26 {label} checkpoint or checkpoint-download path failed",
        },
    }
...
def main(argv: list[str]) -> int:
    ...
    (out_dir / M26_MODULES_RAMDISK).write_text(str(hs_context["module_text"]), encoding="ascii")
    manifests = [
        build_prefix(
            root=root,
            out_dir=out_dir,
    ...
    top_manifest = {
        "generated_at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "target": "SM-S906N/g0q/S906NKSS7FYG8",
        "purpose": "M26 HS-only prefix/download discriminator matrix",
        "safety": {
            "host_only_build": True,
            "live_flash_authorized": False,
        ...
        "prefixes": [
            {
                ...
                "init_sha256": manifest["hashes"]["m26_init"],
