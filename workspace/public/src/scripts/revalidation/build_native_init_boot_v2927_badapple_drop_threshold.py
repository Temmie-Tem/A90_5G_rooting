#!/usr/bin/env python3
"""Build V2927 native-init candidate with Bad Apple A/V sync offset and one-frame drop threshold."""
...
CYCLE = "V2927"
INIT_VERSION = "0.10.42"
INIT_BUILD = "v2927-badapple-drop-threshold"
BUILD_TAG = INIT_BUILD
DECISION = "v2927-badapple-drop-threshold-source-build-pass"

OUT_DIR = workspace_private_build_path("native-init", BUILD_TAG)
REPORT_PATH = REPO_ROOT / "docs/reports/NATIVE_INIT_V2927_BADAPPLE_DROP_THRESHOLD_SOURCE_BUILD_2026-06-20.md"
BOOT_IMAGE = workspace_private_input_path("boot_images", "boot_linux_v2927_badapple_drop_threshold.img", legacy_fallback=False)
INIT_BINARY = OUT_DIR / "init_v2927_badapple_drop_threshold"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2927_badapple_drop_threshold.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v470_badapple_drop_threshold"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.42 (v2927-badapple-drop-threshold)",
...
def require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V2927 boot-image markers: {missing}")
...
def render_report(manifest: dict[str, Any], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    ...
    return "\n".join([
        "# Native Init V2927 Bad Apple A/V Drop Threshold Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "- Parent live unit: V2925 proved the start-offset parser and exposed the remaining small pageflip jitter drops; V2927 keeps the offset and raises the late-frame drop threshold to one full frame.",
        "",
        "## Included Delta",
        "",
        "- Packages the Bad Apple A/V sync start-offset and one-frame late-drop threshold into a flashable test image.",
        ...
        *marker_lines,
        ...
        "## Validation",
        "",
        "- `py_compile`: V2927 builder.",
        "- Build: AArch64 static native-init compile, helper compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V2927 identity, real Bad Apple preset SHA, Player HUD sync-offset and drop-threshold strings, read-only telemetry HUD strings, and retained pageflip/mono1 markers.",
        ...
        "- Candidate type: `badapple-drop-threshold-candidate`.",
        "",
    ])
...
def main() -> int:
    ...
    manifest.update({
        ...
        "candidate_type": "badapple-drop-threshold-candidate",
        "parent_test_artifact": "v2925-badapple-sync-offset-flash-live-partial",
        "badapple_player_hud": {
            "version": 1,
            "source_unit": "V2927",
            "parent_unit": "V2925",
    ...
    (OUT_DIR / "badapple-drop-threshold-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "badapple-drop-threshold-candidate",
