#!/usr/bin/env python3
"""V2231 rollbackable handoff runner for the V2230 service-object-visible post-BDF hold boot.

Default mode is host-only dry-run. The live path flashes V2230, collects the
long post-BDF service-object-visible result, runs the V2220 parser over the
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2231_POST_BDF_HOLD_HANDOFF_RUNNER_2026-06-12.md"
)
V2230_BUILD_DIR = REPO_ROOT / "workspace" / "private" / "builds" / "native-init" / "v2230-service-object-visible-post-bdf-hold"
V2230_MANIFEST = V2230_BUILD_DIR / "manifest.json"
TEST_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v2230_service_object_visible_post_bdf_hold.img",
...
TEST_EXPECT_VERSION = "A90 Linux init 0.9.265 (v2230-service-object-visible-post-bdf-hold)"
ROLLBACK_EXPECT_VERSION = "A90 Linux init 0.9.261 (v2189-security-p0-stage-fix)"
HELPER_REMOTE_PATHS = {
    "helper_result": "/cache/native-init-wifi-test-boot-v2230-helper.result",
    "summary": "/cache/native-init-wifi-test-boot-v2230.summary",
    "log": "/cache/native-init-wifi-test-boot-v2230.log",
}
REQUIRED_CONFIRM = "I_UNDERSTAND_V2231_FLASHES_V2230_AND_ROLLS_BACK_TO_V2189"
REPORT_KEY_EVENTS = [
    "uprobe:wlfw_start",
...
def load_build_manifest() -> dict[str, Any]:
    if not V2230_MANIFEST.exists():
        return {}
    return read_bounded_json(V2230_MANIFEST)
...
def preflight() -> dict[str, Any]:
    ...
    return {
        "v2230_manifest": rel(V2230_MANIFEST),
        "v2230_manifest_exists": V2230_MANIFEST.exists(),
        "v2230_manifest_pass": bool(build_manifest.get("pass")),
...
def classify(manifest: dict[str, Any]) -> dict[str, Any]:
    if not manifest.get("execute"):
        ...
        return {
            "decision": "v2231-post-bdf-hold-handoff-dry-run-ready" if ok else "v2231-post-bdf-hold-handoff-dry-run-blocked",
            "pass": bool(ok),
            "reason": "host-only flash/capture/rollback plan is complete; live execution still requires explicit approval" if ok else "required boot image or V2230 build manifest is missing/mismatched",
        }
    if manifest.get("live_block") == "v2222-preflight-failed":
        return {
            "decision": "v2231-post-bdf-hold-handoff-preflight-failed-no-flash",
            "pass": False,
            "reason": "V2222 preflight failed before the test boot flash, so no live post-BDF hold capture was attempted",
        }
    ...
    if not rollback_result.get("selftest_ok"):
        return {
            "decision": "v2231-post-bdf-hold-handoff-rollback-selftest-failed",
            "pass": False,
            "reason": "rollback did not finish with selftest fail=0",
        }
    if manifest.get("live_block") == "test-flash-failed":
        return {
            "decision": "v2231-post-bdf-hold-handoff-test-flash-failed-rollback-pass",
            "pass": False,
            "reason": "V2230 test boot flash failed, but rollback selftest fail=0 passed",
        }
    diagnosis = collect.get("diagnosis") or {}
    if diagnosis.get("kind") == "property-root-missing":
        return {
            "decision": "v2231-post-bdf-hold-helper-property-root-missing-rollback-pass",
            "pass": False,
            "reason": "V2230 booted and rollback passed, but the helper exited before tracing because its configured property root was missing",
        }
    ...
    return {
        "decision": "v2231-post-bdf-hold-helper-parse-incomplete-rollback-pass",
        "pass": False,
    ...
    if manifest.get("execute"):
        ...
        if diagnosis.get("kind") == "property-root-missing":
            lines.extend([
                "## Live Diagnosis",
                "",
                "- The V2230 helper did not reach WLFW/CNSS trace collection.",
                "- Recovered helper result shows `helper_status=setup-error` and `setup_error=lstat property root: No such file or directory`.",
                "- Root cause: the configured V2230 property root was missing on the device.",
                "- Next unit: inspect the V2230 property-root staging before repeating the service-object-visible run.",
                "",
            ])
            ...
            if wlfw_request_hits and cap_hits and bdf_hits and str(diagnosis.get("wlan0_present")) != "1":
                ...
                if nonlog.get("label") == "peripheral-default-service-manager-call-no-return":
                    lines.extend([
                        "## Live Diagnosis",
                        "",
                        "- V2230 kept the property-root setup fix and the helper completed normally.",
                        ...
                        "- This branch would mean the V2230 service-object-visible route still failed before service-name/get-service despite starting service-manager/PM components.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", default="v2231-post-bdf-hold-handoff")
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "safety": {
            ...
            "partition_write_scope": "boot image flash to V2230 and rollback to V2189 only" if args.execute else "none",
        },
    }
    if not args.execute:
        ...
        manifest["result"] = {
            "decision": "v2231-post-bdf-hold-handoff-live-confirmation-missing",
            "pass": False,
        ...
        if current_window_a90_absent:
            manifest["preflight_warning"] = {
                "decision": "v2231-current-window-a90-events-absent-nonblocking",
                "reason": "V2222 current-window collector found no active a90 events after rollback; V2230 test boot is expected to recreate the helper-owned boot-window events.",
            }
        elif current_window_collector_busy:
            manifest["preflight_warning"] = {
                "decision": "v2231-current-window-collector-busy-nonblocking",
                "reason": "V2222 bridge/status/version/helpers checks passed with fail=0, but the optional current-window collector hit auto-menu busy before the test boot; V2230 test boot is expected to create fresh helper-owned boot-window events.",
            }
        if not v2222.get("ok") and not (current_window_a90_absent or current_window_collector_busy):
            ...
            write_step(store, steps, "flash-v2230-from-native", test_flash)
