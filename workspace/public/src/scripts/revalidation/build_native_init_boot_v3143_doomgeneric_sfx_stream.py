#!/usr/bin/env python3
"""Build V3143 DOOM real SFX PCM stream over the V3141 demo HUD stack."""
...
CYCLE = "V3143"
INIT_VERSION = "0.10.125"
INIT_BUILD = "v3143-doomgeneric-sfx-stream"
BUILD_TAG = INIT_BUILD
DECISION = "v3143-doomgeneric-sfx-stream-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3143_DOOMGENERIC_SFX_STREAM_SOURCE_BUILD_2026-06-24.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v3143_doomgeneric_sfx_stream.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v3143_doomgeneric_sfx_stream"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3143_doomgeneric_sfx_stream.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v531_doomgeneric_sfx_stream"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3143"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3143.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3143.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3143"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3143-sfx-stream"

FRAME_PATH = "/tmp/a90-doomgeneric-v3143-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3143-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3143-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3143-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3143-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3143-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3143-sfx.pcmstream"
...
FRAME_SCALE = "1:1-demo-hud-large-groups-sfx-stream"
SCALE_PATH = v3141.SCALE_PATH
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-sfx-stream"

INPUT_THREAD_MARKER = v3141.INPUT_THREAD_MARKER.replace("v3141", "v3143")
TIME_MODEL_MARKER = v3141.TIME_MODEL_MARKER.replace("v3141", "v3143")
DEMO_HUD_MARKER = v3141.DEMO_HUD_MARKER.replace("v3141", "v3143")
PACED_TIME_MARKER = v3141.PACED_TIME_MARKER.replace("v3141", "v3143")
TICK_TELEMETRY_MARKER = v3141.TICK_TELEMETRY_MARKER.replace("v3141", "v3143")
SCALE_MARKER = v3141.SCALE_MARKER.replace("v3141", "v3143")
PHASE_TELEMETRY_MARKER = v3141.PHASE_TELEMETRY_MARKER.replace("v3141", "v3143")
GAMETIC_FRAME_TELEMETRY_MARKER = v3141.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3141", "v3143")
SFX_STREAM_MARKER = "a90.doomgeneric.v3143.audio=real-sfx-pcm-stream-music-disabled"
SOUND_MODE = "native-doom-sfx-pcm-stream-v3143"
AUDIO_CORUN = 1
...
SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3143.c"
...
def replace_required(text: str, old: str, new: str) -> str:
    if old not in text:
        raise RuntimeError(f"missing V3143 source fragment: {old[:100]!r}")
...
def replace_loop_required(text: str, old: str, new: str) -> str:
    ...
    if start < 0:
        raise RuntimeError("missing V3143 loop function")
    end = text.find("\nint main(int argc, char **argv) {", start)
    if end < 0:
        raise RuntimeError("missing V3143 main after loop function")
    loop = text[start:end]
    if old not in loop:
        raise RuntimeError(f"missing V3143 loop fragment: {old[:100]!r}")
...
def _rewrite_required_string(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3141": b"a90-doomgeneric-v3143",
        b"a90.doomgeneric.v3141": b"a90.doomgeneric.v3143",
        b"v3141": b"v3143",
    }
...
def v3143_adapter_source() -> str:
    source = _V3141_ADAPTER_SOURCE_TEXT.replace("v3141", "v3143").replace("V3141", "V3143")
    source = source.replace(
        "a90.doomgeneric.v3143.scale=producer-960x600-1to1-demo-hud-large-groups",
        SCALE_MARKER,
    )
    source = source.replace(
        "a90.doomgeneric.v3143.demo_hud=large-grouped-hw-doom-input",
        DEMO_HUD_MARKER,
    )
    source = replace_required(
        source,
        'const char a90_doomgeneric_v3143_demo_hud_policy[] =\n'
        '    "a90.doomgeneric.v3143.demo_hud=large-grouped-hw-doom-input";',
        'const char a90_doomgeneric_v3143_demo_hud_policy[] =\n'
        f'    "{DEMO_HUD_MARKER}";\n'
        'const char a90_doomgeneric_v3143_audio_policy[] =\n'
        f'    "{SFX_STREAM_MARKER}";',
    )
    source = replace_required(
        source,
        "        marker_checksum(a90_doomgeneric_v3143_demo_hud_policy) == 0U) {",
        "        marker_checksum(a90_doomgeneric_v3143_demo_hud_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3143_audio_policy) == 0U) {",
    )
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3143 DOOMGENERIC SFX Stream Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "## Validation",
        "",
        "- `py_compile`: V3143 builder and focused tests.",
        "- `unittest`: V3143 source contract.",
        "- Build: AArch64 helper/native-init compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3143 identity, real SFX stream markers, stream path, and inherited V3141 HUD/input markers.",
        "- `git diff --check`: PASS.",
        "",
        "## Metadata",
        "",
        ...
        "- Candidate type: `doomgeneric-sfx-stream-candidate`.",
    ]) + "\n"
...
def configure_v3143_modules() -> dict[str, Any]:
    ...
    v3141.v3141_adapter_source = v3143_adapter_source
...
def restore_v3143_modules(saved: dict[str, Any]) -> None:
    v3133 = v3141.v3137.v3135.v3133
    for name in _PATCHED_V3141_ATTRS:
...
def main() -> int:
    ...
    saved = configure_v3143_modules()
    try:
        rc = v3141.main()
    finally:
        restore_v3143_modules(saved)
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-sfx-stream-candidate",
        "adoption_state": "pending-sfx-stream-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-sfx-stream-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-sfx-stream-candidate",
