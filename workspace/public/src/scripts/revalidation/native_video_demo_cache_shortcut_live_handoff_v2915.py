#!/usr/bin/env python3
"""V2915 live validation for the cached video demo shortcut with audio sync.

V2913 added ``video demo badapple-scale`` and V2914 built it
into a flashable image. This runner flashes V2914, verifies the existing
V2900 Bad-Apple-scale SD cache through the demo shortcut, starts bounded
native audio playback, then plays a 10 second page-flip slice from the
trusted demo cache anchored to the audio worker status file. It then
rolls back to v2321.
"""
...
RUN_ID = "V2915"
BUILD_TAG = "v2915-video-demo-cache-shortcut-live"
REPORT_TITLE = "Native Init V2915 Video Demo Cache Shortcut Live Validation"
DECISION_PREFIX = "v2915-video-demo-cache-shortcut"
CANDIDATE_IMAGE = video_live.ROOT / "workspace/private/inputs/boot_images/boot_linux_v2914_video_demo_cache_shortcut.img"
CANDIDATE_VERSION = "0.10.37"
CANDIDATE_TAG = "v2914-video-demo-cache-shortcut"
CANDIDATE_SHA256 = "4fa35892b070e3158c99aabb785f655000e9baf047e35dda174e37103af0ab8b"
REPORT_PATH = video_live.ROOT / "docs/reports/NATIVE_INIT_V2915_VIDEO_DEMO_CACHE_SHORTCUT_LIVE_2026-06-20.md"
...
def configure_globals() -> None:
    ...
    video_live.REMOTE_DIR = "/mnt/sdext/a90/runtime/video/v2915"
...
def demo_summary(text: str) -> dict[str, Any]:
    return {
        ...
        "asset_id": "video.demo.asset_id=v2874-synthetic-mono1-checker-6501f" in text,
...
def render_report(result: dict[str, Any]) -> str:
    ...
    demo = play.get("demo", {}) if isinstance(play.get("demo"), dict) else {}
    ...
    return "\n".join([
        ...
        "## Command Results",
        "",
        f"- `video demo {PRESET_NAME} status`: rc=`{result.get('cache_status_rc')}`, summary=`{status}`",
        f"- `video demo {PRESET_NAME} verify`: rc=`{result.get('cache_verify_rc')}`, summary=`{verify}`",
        ...
        f"- `video demo {PRESET_NAME} play --trust-cache --sync-audio-status`: rc=`{result.get('cache_play_rc')}`, pass=`{int(bool(play.get('pass')))}`",
        f"- Demo markers: `{demo}`",
...
def run_live(args: Any, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        status = video_live.base.run_serial_step(out_dir, steps, "candidate-video-demo-cache-shortcut-status", ["video", "demo", PRESET_NAME, "status"], timeout=120.0, allow_error=True, retry_unsafe=False)
        ...
        if (status.get("rc") != 0 or
                not all_true(result["cache_status_summary"]) or
                not all_true(result["cache_status_preset_summary"]) or
                not all_true(result["cache_status_demo_summary"])):
            result["decision"] = f"{DECISION_PREFIX}-cache-status-failed-before-rollback"
            raise RuntimeError("video demo status did not emit required markers")

        verify = video_live.base.run_serial_step(out_dir, steps, "candidate-video-demo-cache-shortcut-verify", ["video", "demo", PRESET_NAME, "verify"], timeout=420.0, allow_error=True, retry_unsafe=False)
        ...
        if (verify.get("rc") != 0 or
                not all_true(result["cache_verify_summary"]) or
                not all_true(result["cache_verify_preset_summary"]) or
                not all_true(result["cache_verify_demo_summary"])):
            result["decision"] = f"{DECISION_PREFIX}-cache-verify-failed-before-rollback"
            raise RuntimeError("video demo verify did not emit required markers")
        ...
        play = video_live.base.run_serial_step(
            out_dir,
            steps,
            "candidate-video-demo-cache-shortcut-play-trust-audio-sync",
            [
...
def dry_run_payload(args: Any, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"video demo {PRESET_NAME} play --trust-cache --frames {FIXTURE_SYNC_FRAMES} --present pageflip --sync-audio-status {SYNC_STATUS_PATH} --sync-wait-ms {SYNC_WAIT_MS}",
        ],
    }
