#!/usr/bin/env python3
"""V2782 live validation for the native-init audio stage module split.
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2782_AUDIO_STAGE_MODULE_DEVICE_VALIDATION_2026-06-19.md"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2782-audio-stage-module-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2782-audio-stage-module-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if (
            ...
            result["decision"] = "v2782-audio-stage-module-device-pass"
        elif result["prereq_error_observed"]:
            result["decision"] = "v2782-audio-stage-module-profile-error"
        elif not result["prereq_version_observed"]:
            result["decision"] = "v2782-audio-stage-module-missing-command"
        else:
            result["decision"] = "v2782-audio-stage-module-device-validation-incomplete"
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2782-audio-stage-module-device-validation-dry-run",
