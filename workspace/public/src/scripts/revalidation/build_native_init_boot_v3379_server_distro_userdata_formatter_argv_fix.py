#!/usr/bin/env python3
"""Build V3379 native-init boot image for the server-distro D4C formatter argv fix surface."""
...
CYCLE = "V3379"
INIT_VERSION = "0.11.137"
INIT_BUILD = "v3379-server-distro-userdata-formatter-argv-fix"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3379_SERVER_DISTRO_D4C_FORMATTER_ARGV_FIX_SOURCE_BUILD_2026-07-03.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3379_server_distro_userdata_formatter_argv_fix.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3379_server_distro_userdata_formatter_argv_fix"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3379_server_distro_userdata_formatter_argv_fix.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v629_server_distro_switchroot"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3379"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3379.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3379.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3379"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3379-server-distro-userdata-formatter-argv-fix"

FRAME_PATH = "/tmp/a90-doomgeneric-v3379-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3379-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3379-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3379-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3379-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3379-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3379-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3379.c"
...
def _rewrite_v3379_text(text: str) -> str:
    ...
    replacements = (
        ...
        ("hot-reload-autohud", "server-distro-userdata-formatter-argv-fix"),
        ("v3368", "v3379"),
        ("V3368", "V3379"),
        ("a90-doomgeneric-v3368", "a90-doomgeneric-v3379"),
        ("a90.doomgeneric.v3368", "a90.doomgeneric.v3379"),
    )
...
def _rewrite_v3379_bytes(item: bytes) -> bytes:
    return _rewrite_v3379_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3379_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3379_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3379_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3379_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3379_bytes(marker) for marker in previous.REQUIRED_STRINGS)

REQUIRED_STRINGS = PREVIOUS_REQUIRED_STRINGS + (
    b"0.11.137",
    b"v3379-server-distro-userdata-formatter-argv-fix",
...
def _boot_audit_manifest() -> dict[str, Any]:
    ...
    manifest["rung"] = "server-distro-d4c-formatter-argv-fix"
    manifest["scope"] = "D4C-entry-formatter-argv-fix-surface"
...
def v3379_adapter_source() -> str:
    return _rewrite_v3379_text(ORIG_PREVIOUS_ADAPTER_SOURCE())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "server-distro-userdata-formatter-argv-fix.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "server-distro-d4c-formatter-argv-fix",
...
def _normalize_manifest_for_v3379(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest.update({
        "decision": DECISION,
        "cycle": CYCLE,
        "candidate_tag": INIT_BUILD,
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest = _normalize_manifest_for_v3379(json.loads(manifest_path.read_text(encoding="utf-8")))
    if base_main_error:
...
def _postprocess_manifest() -> dict[str, Any]:
    manifest = _normalize_manifest_for_v3379(ORIG_PREVIOUS_POSTPROCESS())
    manifest_path = OUT_DIR / "manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_PATH.write_text(
        render_report(
...
def _overlay_preserved_v3379_ramdisk() -> dict[str, Any]:
    overlay = ORIG_PREVIOUS_OVERLAY()
    overlay["mode"] = "preserve-v3335-ramdisk-overlay-v3379-init-helper-engine"
    return overlay


def _patch_v3368_module_for_v3379() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "v3368_adapter_source": v3379_adapter_source,
        "_rewrite_v3368_text": _rewrite_v3379_text,
        "_rewrite_v3368_bytes": _rewrite_v3379_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_overlay_preserved_v3368_ramdisk": _overlay_preserved_v3379_ramdisk,
