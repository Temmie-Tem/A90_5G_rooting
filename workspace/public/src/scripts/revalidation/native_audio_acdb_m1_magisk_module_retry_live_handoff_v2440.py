#!/usr/bin/env python3
"""V2440 source/test M1 Magisk-module retry runner.
...
It also carries forward the V2435 cleanup discipline and fixes the V2437
transfer bug: adb pushes payload files only into a shell-owned incoming
directory, then Magisk root validates exact SHA-256 values before copying those
files into the final module namespace.  `magisk --install-module` remains
deferred.

V2440 additionally hardens the post-module-reboot settle window from V2439:
after the module activation reboot, ADB/root readiness is retried as a bounded
sequence instead of treating one transient `adb: no devices/emulators found` as
...
RUN_ID = "V2440"
BUILD_TAG = "v2440-audio-acdb-m1-magisk-module-retry"
ROOT = v2429.ROOT
...
REMOTE_INCOMING_DIR = f"{v2429.REMOTE_DIR}/incoming"
...
def remote_stage_path(name: str) -> str:
    if name == v2429.HELPER_NAME:
        return f"{REMOTE_INCOMING_DIR}/bin/{v2429.HELPER_NAME}"
...
def local_module_manifest(args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    ...
    for name, path in local_module_paths(args).items():
        ...
        if path.exists():
            entry["sha256"] = v2429.sha256(path)
...
cp "$INCOMING_DIR/bin/{v2429.HELPER_NAME}" "$HELPER"
...
def post_module_reboot_settle_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "classification": "bounded adb reacquire plus Magisk-root retry after module activation reboot",
    }
...
def dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "module_lifecycle": {
            ...
            "v2440_post_module_reboot_retry": True,
        },
        "commands": commands,
    ...
    if not safety.get("ok"):
        blockers.append("V2440 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2440 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2440 command safety failed: {plan.get('command_safety')}")
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dry-run", action="store_true", help="emit the V2440 live plan; no device action")
    mode.add_argument("--run-live", action="store_true", help="run the exact-gated V2440 M1 Magisk-module retry")
