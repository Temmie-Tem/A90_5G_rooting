#!/usr/bin/env python3
"""V2875 live handoff for the V2874 A90VSTR1 video stream reader.

This runner flashes the V2874 candidate, stages a private synthetic full-geometry
A90VSTR1 stream under /cache/a90-runtime/pkg/video/v2875, runs the bounded
`video stream --manifest ... --video-only` command, then rolls back to v2321 and
...
ROOT = repo_root()
RUN_ID = "V2875"
BUILD_TAG = "v2875-video-stream-reader-live"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2874_video_stream_reader.img"
CANDIDATE_VERSION = "0.10.23"
CANDIDATE_TAG = "v2874-video-stream-reader"
CANDIDATE_SHA256 = "da69a7e1402b5eb5a84e4a62560f291af5373c2d63aca817263f7a841bbcbfa7"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2875_VIDEO_STREAM_READER_LIVE_2026-06-19.md"
PREPARE_STREAM = ROOT / "workspace/public/src/scripts/revalidation/prepare_video_stream_v2874.py"
REMOTE_DIR = "/cache/a90-runtime/pkg/video/v2875"
...
def classify_stream_output(text: str, expected_frames: int) -> dict[str, Any]:
    ...
    return {
        ...
        and "video.stream.sha256_match=1" in text
        and "video.stream.pixel_format=xbgr8888" in text
        and "video.stream.path=kms-dumb-buffer" in text,
    }
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        *installed_lines,
        ...
        f"- KMS path marker: `{int(bool(stream_summary.get('path_ok')))}`",
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        "decision": "v2875-video-stream-reader-live-dry-run" if preflight_ok(state) else "v2875-video-stream-reader-live-blocked",
        ...
        "commands": {
            ...
            "stream": ["video", "stream", "--manifest", REMOTE_MANIFEST, "--video-only", "--frames", str(args.frames)],
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "decision": "v2875-video-stream-reader-live-started",
        "pass": False,
        "preflight": state,
        "steps": steps,
    ...
    try:
        ...
        flash = base.run_step(
            out_dir,
            steps,
            "flash-v2874-video-stream-reader",
        ...
        if not (result["candidate_version_ok"] and result["candidate_selftest_fail0"] and result["candidate_video_status_ok"]):
            result["decision"] = "v2875-video-stream-reader-candidate-health-failed-before-stream"
        ...
        stream = base.run_serial_step(
            ...
            ["video", "stream", "--manifest", REMOTE_MANIFEST, "--video-only", "--frames", str(args.frames)],
        ...
        if stream.get("rc") != 0 or not result["stream_summary"].get("pass"):
            result["decision"] = "v2875-video-stream-reader-stream-failed-before-rollback"
        ...
        if not result["candidate_selftest_after_stream_fail0"]:
            result["decision"] = "v2875-video-stream-reader-post-stream-selftest-failed"
            raise RuntimeError("candidate post-stream selftest did not report fail=0")
        result["decision"] = "v2875-video-stream-reader-live-pass-before-rollback"
        result["pass"] = True
    except Exception as exc:
        result.setdefault("decision", "v2875-video-stream-reader-live-blocked")
        if result["decision"] == "v2875-video-stream-reader-live-started":
            result["decision"] = "v2875-video-stream-reader-live-blocked"
...
def main() -> int:
    args = parse_args()
    out_dir = ROOT / f"workspace/private/runs/video/v2875-video-stream-reader-live-{now_slug()}"
    ...
    if not preflight_ok(state):
        payload = {
            "decision": "v2875-video-stream-reader-live-preflight-failed-no-flash",
            "pass": False,
            "preflight": state,
        }
