#!/usr/bin/env python3
"""Build V2319 USB product overrun+1 rodata test boot.

V2319 keeps V2318 native-init behavior and live-tests the smallest product-string
overrun: ``SAMSUNG_Android\0`` is replaced by ``A90 Linux ARM64X\0``, which
writes one byte beyond the original 16-byte product slot. The manufacturer patch
...
OUT_DIR = workspace_private_build_path("native-init", "v2319-usb-product-overrun1-rodata")
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2319_USB_PRODUCT_OVERRUN1_RODATA_SOURCE_BUILD_2026-06-14.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2319_usb_product_overrun1_rodata.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2319_usb_product_overrun1_rodata"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2319_usb_product_overrun1_rodata.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v435_usb_product_overrun1_rodata"
PATCHED_KERNEL = OUT_DIR / "kernel_v2319_usb_product_overrun1_rodata"
...
PRODUCT_NEW = b"A90 Linux ARM64X\x00"
MANUFACTURER_OFFSET = 0x2346D6C
MANUFACTURER_OLD = b"SAMSUNG\x00"
MANUFACTURER_NEW = b"A90-LNX\x00"

USB_PRODUCT_OVERRUN1_RODATA_PATCH = {
    "mode": "product-overrun-plus-one",
    "reason": "V2318 proved fixed-length full identity patching works; V2319 overwrites exactly one byte past the product slot to test the smallest overrun boundary.",
    ...
    "product_expected_overwritten_after_slot": "0x01 -> 0x00",
...
def patch_kernel_identity_literals(source_kernel: Path, patched_kernel: Path) -> dict[str, object]:
    ...
    if product_overrun != 1:
        raise RuntimeError(f"product replacement must overrun by exactly one byte, got {product_overrun}")
    after_slot_offset = PRODUCT_OFFSET + len(PRODUCT_OLD)
    if data[after_slot_offset] != 0x01:
        raise RuntimeError(
            f"unexpected byte after product slot: {data[after_slot_offset]:#x}; "
            "expected 0x01 for the bounded overrun test"
        )
...
def configure_base() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--cycle-label": "v2319",
        "--init-version": "0.9.283",
        "--init-build": "v2319-usb-product-overrun1-rodata",
        "--out-dir": str(OUT_DIR),
        "--init-binary": str(INIT_BINARY),
        "--helper-binary": str(HELPER_BINARY),
        "--ramdisk-cpio": str(RAMDISK_CPIO),
        "--boot-image": str(BOOT_IMAGE),
        "--wifi-test-klog-prefix": "A90v2319",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2319.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2319.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2319.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2319-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2319.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2319-supervisor.pid",
...
def patch_mkbootimg_tools(base_wrapper) -> None:
    ...
    def build_boot_image(args) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="a90-v2319-unpack-") as temp_name:
            ...
            args.product_overrun1_rodata_patch_info = patch_info
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...]) -> str:
    ...
    patch_info = manifest["usb_product_overrun1_rodata_patch"]
    return "\n".join([
        ...
        f"- Product replacement length delta: `{patch_info['product_overrun_bytes']}` byte.",
...
def main() -> int:
    ...
    def render_with_patch_info(manifest: dict[str, object]) -> str:
        if "usb_product_overrun1_rodata_patch" not in manifest:
            if LAST_PATCH_INFO is None:
                raise RuntimeError("product overrun+1 rodata patch info missing before report render")
            manifest["usb_product_overrun1_rodata_patch"] = LAST_PATCH_INFO
    ...
    manifest["candidate_tag"] = "v2319-usb-product-overrun1-rodata"
    manifest["parent_baseline"] = "v2318-usb-full-identity-rodata"
    manifest["rollback_baseline"] = "v2237-supplicant-terminate-poll"
    manifest["helper_flags"] = list(helper_flags)
    manifest["usb_product_overrun1_rodata_patch"] = patch_info
    ...
    promotion_path.write_text(json.dumps({
        "candidate_tag": "v2319-usb-product-overrun1-rodata",
        ...
        "product_overrun1_rodata_patch": patch_info,
        "note": "V2319 tests a one-byte product rodata overrun on top of the V2318 full identity patch: host iManufacturer should remain A90-LNX and iProduct should become A90 Linux ARM64X.",
