#!/usr/bin/env python3
"""V2799 live handoff for native-init audio native ioctl width.

This is the audio core closure gate: flash the V2799 native-init image, stage the
...
V2799 specifically verifies that the SET-cal executor uses native 64-bit
`_IOWR('a', n, void *)` request codes, so execution can progress beyond the
V2798 first `AUDIO_ALLOCATE_CALIBRATION` errno=14 blocker.
"""
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2799_AUDIO_NATIVE_IOCTL_WIDTH_LIVE_2026-06-19.md"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2799-audio-native-ioctl-width-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2799-audio-native-ioctl-width-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2799-audio-native-ioctl-width-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio native ioctl width command did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-native-ioctl-width-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2799-audio-native-ioctl-width-live-worker-failed-before-rollback"
            raise RuntimeError("native audio native ioctl width command did not emit all required pass markers")
        ...
        result["decision"] = "v2799-audio-native-ioctl-width-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2799-audio-native-ioctl-width-live-blocked")
        if result["decision"] == "v2799-audio-native-ioctl-width-live-started":
            result["decision"] = "v2799-audio-native-ioctl-width-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2799-audio-native-ioctl-width-live-dry-run",
