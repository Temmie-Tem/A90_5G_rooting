#!/usr/bin/env python3
"""Build V2717 route-first common-topology ACDB SET capture artifacts.

This host-only build wrapper keeps the V2630 fake-success
AUDIO_SET_CALIBRATION capture shim and the V2659 phase-aware common-topology
hook, but swaps the helper order:
...
* the helper first runs the route-specific send_audio_cal_v5 edge;
* the helper then calls the post-init common-topology path, which invokes the
...
ROOT = v2630.ROOT
RUN_ID = "V2717"
BUILD_TAG = "v2717-acdb-route-first-common-setcal-capture-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2717_AUDIO_ACDB_ROUTE_FIRST_COMMON_SETCAL_CAPTURE_BUILD_2026-06-18.md"

HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_route_first_common_exec_linked_v2717.c"
PREINIT_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_phase_common_v2717.c"
HELPER_ARTIFACT_NAME = "a90_acdb_route_first_common_setcal_capture_exec_linked_v2717"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_route_first_common_setcal_capture_combined_preload_v2717.so"
...
def source_state() -> dict[str, Any]:
    ...
    required = {
        ...
        "helper_calls_send_v5_before_common_topology": send_pos > arm_pos and common_pos > send_pos,
        ...
        "helper_event_path_v2717": "acdb-route-first-common-events.jsonl" in helper,
        ...
        "phase_hook_event_path_v2717": "acdb-v2717-route-first-phase-common-events.jsonl" in preinit,
    ...
    return {
        ...
        "v2717_delta": {
            "basis": "V2716 exhausted existing real-HAL traces and V2715 showed the lower hidden selector state is stale; V2717 tests the one untried public order: route-specific send_v5 before post-init common topology.",
            ...
            "helper": "Route-first helper drives send_audio_cal_v5 before acdb_loader_send_common_custom_topology.",
            ...
            "acceptance": "future live capture must show the route-first helper stages and byte-exact SET records for cal_types 10, 14, and 24.",
        },
    }
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    if isinstance(preload_checks, dict) and "soname_v2630" in preload_checks:
        preload_checks["soname_v2717"] = preload_checks.pop("soname_v2630")
...
def make_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            "call_order": "acdb_loader_init_v3 -> init-short common hook -> a90_arm_capture -> send_audio_cal_v5 -> post-init real common topology",
            "target_cal_types": [10, 14, 24],
            "supplemental_cal_types": [20, 39],
            "success_discriminator": "future live run must capture route-first helper markers plus byte-exact SET records for 10/14/24",
...
def write_report(payload: dict[str, Any], report_path: Path) -> None:
    ...
    lines = [
        ...
        f"- route_first_phase_common_compile_ok: `{build_state.get('compile', {}).get('preinit_no_send', {}).get('ok') if isinstance(build_state, dict) else None}`",
