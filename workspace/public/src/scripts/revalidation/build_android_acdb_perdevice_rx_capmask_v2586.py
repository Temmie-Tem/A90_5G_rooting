#!/usr/bin/env python3
"""Build V2586 ARM32 ACDB per-device capture with send_audio_cal_v5 arg2=1.

Host-only unit.  V2574/V2575 closed only the arg2=0 public-send route.  The
operator RE update pins arg2 as a capability/path bitmask and requires bit0 set
for RX, so this build reuses the V2572 per-device indirect capture shape with
`A90_SPEAKER_RX_PATH=1` injected at compile time.
"""
...
ROOT = v2572.ROOT
RUN_ID = "V2586"
BUILD_TAG = "v2586-acdb-perdevice-rx-capmask-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2586_AUDIO_ACDB_PERDEVICE_RX_CAPMASK_BUILD_2026-06-16.md"
HELPER_ARTIFACT_NAME = "a90_acdb_perdevice_rx_capmask_exec_linked_v2586"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_perdevice_rx_capmask_capture_v2586.so"
...
def source_state() -> dict[str, Any]:
    ...
    base["v2586_delta"] = {
        ...
        "compile_override": CAP_MASK_CFLAG,
        "reason": "operator RE: send_audio_cal_v5 bails when (arg2 & 0x3) == 0; RX bit0 must be set",
    }
    return base
...
def binary_state(path: Path, *, readelf: str, file_cmd: str, kind: str) -> dict[str, Any]:
    ...
    if kind == "helper":
        ...
        checks = {
            ...
            "soname_v2586": f"Library soname: [{PRELOAD_ARTIFACT_NAME}]" in dyn,
            "mode_0600": (path.stat().st_mode & 0o777) == 0o600,
        }
    state["checks"] = checks
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    preinit_obj = obj_dir / "libacdb_preinit_perdevice_v2572_rx_capmask.o"
...
def make_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            "delta_from_v2572": "send_audio_cal_v5 arg2/path/capability mask is compiled as 1 instead of 0",
            "per_device_call": "acdb_loader_send_audio_cal_v5(15, 1, 0x11135, 48000, 48000, 0, 1)",
            "operator_re_basis": "arg2 is a capability/path mask; (arg2 & 0x3)==0 bails before per-device GETs",
