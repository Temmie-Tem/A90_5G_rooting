#!/usr/bin/env python3
"""Build V3270 GPU H3 SP const/output-control probe."""
...
CYCLE = "V3270"
INIT_VERSION = "0.11.61"
INIT_BUILD = "v3270-gpu-h3-sp-const-fs-output-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3270_GPU_H3_SP_CONST_FS_OUTPUT_SOURCE_BUILD_2026-06-26.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3270_gpu_h3_sp_const_fs_output_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3270_gpu_h3_sp_const_fs_output_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3270_gpu_h3_sp_const_fs_output_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v600_gpu_h3_sp_const_fs_output_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3270"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3270.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3270.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3270"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3270-gpu-h3-sp-const-fs-output-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3270-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3270-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3270-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3270-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3270-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3270-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3270-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-sp-const-fs-output-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-sp-const-fs-output-probe"

INPUT_THREAD_MARKER = previous.INPUT_THREAD_MARKER.replace("v3268", "v3270")
TIME_MODEL_MARKER = previous.TIME_MODEL_MARKER.replace("v3268", "v3270")
DEMO_HUD_MARKER = previous.DEMO_HUD_MARKER.replace("v3268", "v3270")
PACED_TIME_MARKER = previous.PACED_TIME_MARKER.replace("v3268", "v3270")
TICK_TELEMETRY_MARKER = previous.TICK_TELEMETRY_MARKER.replace("v3268", "v3270")
SCALE_MARKER = previous.SCALE_MARKER.replace("v3268", "v3270")
PHASE_TELEMETRY_MARKER = previous.PHASE_TELEMETRY_MARKER.replace("v3268", "v3270")
GAMETIC_FRAME_TELEMETRY_MARKER = previous.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3268", "v3270")
SFX_STREAM_MARKER = "a90.doomgeneric.v3270.audio=real-sfx-pcm-stream-gpu-h3-sp-const-fs-output-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-sp-const-fs-output-probe-v3270"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3270.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"

SCOPE = "first-triangle-h3-sp-const-fs-output-cntl-raster-mode-cp-set-mode-window-offset-visibility-packets-vpc-so-override-off-sysmem-bin-control-sp-update-cntl-compiler-vs-instrlen-cache-invalidate-rb-render-cntl-r0-output-shader"
...
def _rewrite_v3270_bytes(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3268": b"a90-doomgeneric-v3270",
        b"a90.doomgeneric.v3268": b"a90.doomgeneric.v3270",
        b"v3268": b"v3270",
        b"V3268": b"V3270",
        b"gpu-h3-raster-mode-probe": b"gpu-h3-sp-const-fs-output-probe",
...
def _rewrite_v3270_text(text: str) -> str:
    for old, new in (
        ...
        ("a90-doomgeneric-v3268", "a90-doomgeneric-v3270"),
        ("a90.doomgeneric.v3268", "a90.doomgeneric.v3270"),
        ("v3268", "v3270"),
        ("V3268", "V3270"),
        ("gpu-h3-raster-mode-probe", "gpu-h3-sp-const-fs-output-probe"),
...
GPU_H3_SP_CONST_FS_OUTPUT_MARKERS = (
...
REQUIRED_STRINGS = tuple(_rewrite_v3270_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        ...
        "state_reg_writes_expected": 100,
        "pm4_dwords_expected": 270,
        "shader_payload": SHADER_PAYLOAD,
        "readback": "expect changed pixels if missing SP const enable or invalid FS depth/sampmask/stencil regids blocked color MRT output",
        "next_live_validation": [
            "flash-v3270-through-native-init-flash",
            "post-flash-health-check",
            "gpu-h3-sp-const-fs-output-timeout-guard",
...
def v3270_adapter_source() -> str:
    return _rewrite_v3270_text(previous.v3268_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-sp-const-fs-output-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-sp-const-fs-output-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-sp-const-fs-output-live-validation",
...
def _overlay_preserved_v3270_ramdisk() -> dict[str, Any]:
    ...
    overlay["mode"] = "preserve-v3268-ramdisk-overlay-v3270-init-helper-engine"
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-h3-sp-const-fs-output-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-sp-const-fs-output-live-validation"
...
def _apply_v3270_overrides() -> None:
    ...
    replacements = {
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3270_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3270_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3270_ramdisk,
