#!/usr/bin/env python3
"""Inert-until-bound R4W1-C direct-PID1 boot-only live gate.

The offline mode is host-only.  Live and recovery modes require a complete,
separately committed policy clause.  The helper reopens the immutable R4W1-C
connected PASS, uses generation-bound Odin endpoint receipts, consumes the
candidate exception before one boot-only transfer, and performs mandatory
Magisk-first rollback before retained-marker classification.
"""
...
SCHEMA = "s22plus_fyg8_r4w1c_live_gate_v1"
TARGET = connected.TARGET
SCRIPT_RELATIVE = Path(
    "workspace/public/src/scripts/revalidation/s22plus_fyg8_r4w1c_live_gate.py"
)
TEST_RELATIVE = Path("tests/test_s22plus_fyg8_r4w1c_live_gate.py")
POLICY_DRAFT = Path(
...
ODIN_CORE_SHA256 = connected.EXPECTED_ODIN_CORE_SHA256

RUN_ROOT = Path("workspace/private/runs")
CONSUMED_STATE = Path(
    "workspace/private/state/s22plus_fyg8_r4w1c_live_exception_consumed.json"
)
PASS_VERDICT = "PASS_R4W1C_DIRECT_PID1_EXEC_ACCEPTED_AND_ROLLED_BACK"
NO_PROOF_VERDICT = "NO_PROOF_R4W1C_EXEC_OR_RETENTION_UNRESOLVED"
...
def policy_template(root: Path) -> tuple[str, dict[str, Any]]:
    ...
    if identity != {
        ...
        raise GateError("R4W1-C live policy template identity mismatch")
    try:
        text = payload.decode("utf-8")
    except UnicodeError as exc:
        raise GateError("R4W1-C live policy template is not UTF-8") from exc
    placeholders = (
        "LIVE_HELPER_SHA256",
        "LIVE_TEST_SHA256",
    ...
    if "DRAFT_INACTIVE" not in text or any(
        ...
        raise GateError("R4W1-C live policy template placeholder contract mismatch")
    extract_policy_clause(text)
    if core.read_stable_file(path, maximum=128 * 1024) != payload:
        raise GateError("R4W1-C live policy template changed while reopening")
...
def render_policy_clause(
    ...
    if "{{" in text or "}}" in text:
        raise GateError("R4W1-C live policy rendering left a placeholder")
...
def policy_clause(root: Path) -> str:
    ...
    if clause != expected:
        raise GateError("R4W1-C live policy is not the exact reviewed rendering")
    return clause
...
def parse_connected_binding(clause: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise GateError("R4W1-C live clause lacks one exact connected binding")
    raw = matches[0].groupdict()
    if any(
        re.fullmatch(r"[A-Za-z0-9._/-]+", raw[key]) is None
        for key in ("pass_path", "result_path")
    ):
        raise GateError("R4W1-C connected path binding contains unsafe bytes")
    if raw["pass_path"] != str(connected.PASS_STATE):
        raise GateError("R4W1-C connected PASS path binding mismatch")
    if re.fullmatch(
        r"[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{6}Z",
        raw["created_at"],
    ) is None:
        raise GateError("R4W1-C connected timestamp binding is malformed")
    ...
    if (
        ...
        raise GateError("R4W1-C connected result path binding is not canonical")
    return {
...
def verify_artifacts(root: Path, args: argparse.Namespace) -> dict[str, Any]:
    return connected.verify_artifacts(root, args)
...
def reopen_connected_evidence(
    ...
    result_path = connected.require_direct_path(
        root, root / Path(str(record["result_path"])), "R4W1-C connected result"
    )
    ...
    if binding != expected:
        raise GateError("R4W1-C live clause connected evidence mismatch")
    if (
        ...
        raise GateError("R4W1-C connected source or clause identity mismatch")
    if (
        core.read_stable_file(pass_path, maximum=1024 * 1024) != pass_before
        or core.read_stable_file(result_path, maximum=8 * 1024 * 1024)
        != result_before
    ):
        raise GateError("R4W1-C connected evidence changed while reopening")
...
def _direct_state_parent(root: Path) -> Path:
    return connected.require_direct_path(
        ...
        "R4W1-C consumed-state directory",
        directory=True,
    )
...
def prepared_binding(
    ...
    if len(phases) != 1 or phases[0].get("phase") != "prepared":
        raise GateError("R4W1-C consumption requires exactly one prepared receipt")
    expected_payload = connected.expected_phase_payload(baseline)
    if phase_payload(root, run_dir, "prepared") != expected_payload:
        raise GateError("R4W1-C prepared receipt does not bind the live baseline")
    record = phases[0]
    return {
        "path": str(
            connected.require_direct_path(
                root, Path(str(record["path"])), "R4W1-C prepared receipt"
            ).relative_to(root)
        ),
        "size": record["size"],
...
def require_consumed(
    ...
    _direct_state_parent(root)
    path = connected.require_direct_path(
        root, root / CONSUMED_STATE, "R4W1-C consumed state"
    )
    ...
    if (
        ...
        raise GateError("R4W1-C consumed state contract mismatch")
    ...
    if (
        ...
        raise GateError("R4W1-C consumed run path is not canonical")
    run_dir = connected.require_direct_path(
        root, root / relative, "R4W1-C consumed run directory", directory=True
    )
    connected_evidence = reopen_connected_evidence(root, artifacts, clause)
    if record["connected_binding"] != connected_evidence["binding"]:
        raise GateError("R4W1-C consumed connected evidence binding mismatch")
    phases = odin_core.list_phase_receipts(run_dir)
    if not phases or phases[0].get("phase") != "prepared":
        raise GateError("R4W1-C consumed transaction lacks prepared evidence")
    prepared = phases[0]
    prepared_path = connected.require_direct_path(
        root, Path(str(prepared["path"])), "R4W1-C consumed prepared receipt"
    )
    prepared_summary = {
    ...
    if record["prepared"] != prepared_summary:
        raise GateError("R4W1-C consumed prepared evidence mismatch")
    ...
    if (
        ...
        raise GateError("R4W1-C consumed Android baseline mismatch")
    if core.read_stable_file(path, maximum=1024 * 1024) != consumed_bytes:
        raise GateError("R4W1-C consumed state changed while reopening")
...
def next_snapshot_sequence(run_dir: Path) -> int:
    ...
    if sequences != list(range(len(sequences))):
        raise GateError("R4W1-C Odin snapshot sequence is not contiguous")
...
def reopen_rollback_observer(
    ...
    summary_path = connected.require_direct_path(
        root, root / Path(str(summary["path"])), "R4W1-C rollback observer summary"
    )
...
def find_completed_rollback_observer(
    ...
    for path in candidates:
        ...
        direct = connected.require_direct_path(
            root, path, "R4W1-C completed rollback observer summary"
        )
        summary = {
...
def classify_verdict(
    *,
    ...
    if rollback_target != "magisk" or not rollback_ok:
        return "FAIL_R4W1C_ROLLBACK_NOT_VERIFIED_RECOVERY_REQUIRED", 20
    if observer is None:
        return "FAIL_R4W1C_OBSERVER_CAPTURE", 21
    marker = observer.get("marker")
    if not isinstance(marker, dict) or marker.get("integrity_issue") is True:
        return "FAIL_R4W1C_MARKER_INTEGRITY", 22
    if not candidate_transfer_ok:
        if marker.get("acceptance_present") is True:
            return "FAIL_R4W1C_MARKER_INTEGRITY", 23
        return "FAIL_R4W1C_CANDIDATE_TRANSFER_AND_ROLLED_BACK", 24
    if (
        ...
        return "FAIL_R4W1C_CANDIDATE_OBSERVATION_REQUIRED", 25
    if marker.get("acceptance_present") is True:
...
def phase_payload(root: Path, run_dir: Path, phase: str) -> dict[str, Any] | None:
    ...
    if not path.exists() and not path.is_symlink():
        return None
    path = connected.require_direct_path(
        root, path, f"R4W1-C phase receipt {phase}"
    )
    payload = core.read_stable_file(path, maximum=odin_core.MAX_RECEIPT_BYTES)
    try:
        value = json.loads(payload)
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise GateError(f"R4W1-C phase receipt is invalid: {phase}") from exc
    if (
        ...
        raise GateError(f"R4W1-C phase receipt contract mismatch: {phase}")
    records = {
    ...
    if record is None or {
        ...
        raise GateError(f"R4W1-C phase receipt identity mismatch: {phase}")
...
def transaction_evidence(root: Path, run_dir: Path) -> dict[str, Any]:
    ...
    if not isinstance(records, list) or not isinstance(segments, list) or not segments:
        raise GateError("R4W1-C transaction index evidence is incomplete")
    receipt_summaries: list[dict[str, Any]] = []
    known: dict[str, dict[str, Any]] = {}
    for record in [*snapshots, *phases]:
        path = connected.require_direct_path(
            root, Path(str(record["path"])), "R4W1-C transaction receipt"
        )
        payload = core.read_stable_file(path, maximum=odin_core.MAX_RECEIPT_BYTES)
        if {"size": len(payload), "sha256": core.sha256_bytes(payload)} != {
            ...
            raise GateError("R4W1-C transaction receipt identity mismatch")
        absolute = str(path)
        receipt_summaries.append({**record, "path": str(path.relative_to(root))})
        known[absolute] = record
    if len(records) != len(known):
        raise GateError("R4W1-C transaction index record count mismatch")
    seen: set[str] = set()
    for record in records:
        if not isinstance(record, dict):
            raise GateError("R4W1-C transaction index record is malformed")
        receipt = record.get("receipt")
        if not isinstance(receipt, str) or receipt in seen:
            raise GateError("R4W1-C transaction index receipt reference is invalid")
        expected = known.get(receipt)
        if expected is None:
            raise GateError("R4W1-C transaction index references an unknown receipt")
        if (
            record.get("receipt_size") != expected["size"]
            or record.get("receipt_sha256") != expected["sha256"]
        ):
            raise GateError("R4W1-C transaction index receipt identity mismatch")
        seen.add(receipt)
    if seen != set(known):
        raise GateError("R4W1-C transaction index omits a receipt")
    segment_summaries: list[dict[str, Any]] = []
    for segment in segments:
        if segment.get("complete") is not True or segment.get("partial_tail_bytes") != 0:
            raise GateError("R4W1-C transaction index has an incomplete segment")
        path = connected.require_direct_path(
            root, Path(str(segment["path"])), "R4W1-C transaction index"
        )
    ...
    if (
        ...
        raise GateError("R4W1-C transaction evidence changed while reopening")
    return evidence
...
def append_timeline_event(
    ...
    *,
    ...
    if name not in core.TIMELINE_NAMES:
        raise GateError(f"unknown R4W1-C timeline event: {name}")
    ...
    if events and index <= core.TIMELINE_NAMES.index(events[-1]["name"]):
        raise GateError("R4W1-C timeline events are not strictly ordered")
    stamp = timestamp_utc or core.utc_now()
    if UTC_RE.fullmatch(stamp) is None:
        raise GateError("R4W1-C timeline timestamp is malformed")
...
def _finish(
    ...
    *,
    ...
    if verdict == PASS_VERDICT and [event["name"] for event in timeline] != list(
        ...
        verdict = "FAIL_R4W1C_PASS_TIMELINE_INCOMPLETE"
        rc = 26
...
def next_recovery_attempt(root: Path, run_dir: Path) -> tuple[int, Path, Path]:
    ...
    for path in run_dir.glob("recovery-attempt-*-timeline.json"):
        connected.require_direct_path(root, path, "R4W1-C recovery timeline")
        match = re.fullmatch(r"recovery-attempt-([0-9]{2})-timeline\.json", path.name)
        if match is None:
            raise GateError("R4W1-C recovery timeline name is malformed")
        attempts.append(int(match.group(1)))
    attempts.sort()
    if attempts != list(range(1, len(attempts) + 1)):
        raise GateError("R4W1-C recovery attempt sequence is not contiguous")
    attempt = len(attempts) + 1
    if attempt > MAX_RECOVERY_ATTEMPTS:
        raise GateError("R4W1-C recovery failed twice; automatic recovery is stopped")
    ...
    if result.exists() or result.is_symlink():
        raise GateError("R4W1-C recovery result already exists")
    ...
    *,
    ...
    try:
        ...
        return _finish(
            ...
            timeline,
            result,
            verdict="FAIL_R4W1C_ROLLBACK_NOT_VERIFIED_RECOVERY_REQUIRED",
            rc=20,
            result_name=result_name,
            error=str(exc),
        )
...
def _live_run_with_odin(
    ...
    if not policy_active(root) or policy.get("active") is not True:
        raise GateError("R4W1-C live policy is inactive")
    if args.ack != LIVE_ACK_TOKEN:
        raise GateError("R4W1-C live acknowledgement mismatch")
    clause = policy_clause(root)
    connected_evidence = reopen_connected_evidence(root, artifacts, clause)
    _direct_state_parent(root)
    if (root / CONSUMED_STATE).exists() or (root / CONSUMED_STATE).is_symlink():
        raise GateError("R4W1-C candidate exception is already consumed")
    run_dir = core.allocate_run_dir(root, RUN_ROOT, "s22plus-r4w1c-live", args.run_dir)
    ...
    with odin_core.transaction_session(run_dir) as lease:
        ...
        try:
            ...
            if current_policy != policy or policy_clause(root) != clause:
                raise GateError("R4W1-C live policy changed before consumption")
            current_connected = reopen_connected_evidence(root, artifacts, clause)
            if current_connected != connected_evidence:
                raise GateError("R4W1-C connected evidence changed before consumption")
...
def rollback_from_download(
    ...
    if args.ack != ROLLBACK_ACK_TOKEN:
        raise GateError("R4W1-C rollback acknowledgement mismatch")
    if not policy_active(root) or policy.get("active") is not True:
        raise GateError("R4W1-C recovery requires the exact ACTIVE live policy")
    clause = policy_clause(root)
    ...
    if phase_payload(root, run_dir, "classified") is not None:
        raise GateError("R4W1-C transaction is already classified")
                ...
                json.dumps(
                    {
                        ...
                        "odin_transfer": False,
                        "flash": False,
                        "verdict": "PASS_R4W1C_LIVE_GATE_OFFLINE_CHECK",
                    },
                    indent=2,
                )
            )
