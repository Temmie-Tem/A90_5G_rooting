#!/usr/bin/env python3
"""Build V2787 native-init audio speaker descriptor API boot image.

V2787 adds a first-class speaker descriptor API for the audio speaker-map surface.
...
CYCLE = "V2787"
INIT_VERSION = "0.9.301"
INIT_BUILD = "v2787-audio-speaker-descriptor-api"
BUILD_TAG = "v2787-audio-speaker-descriptor-api"
DECISION = "v2787-audio-speaker-descriptor-api-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2787_AUDIO_SPEAKER_DESCRIPTOR_API_SOURCE_BUILD_2026-06-19.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2787_audio_speaker_descriptor_api.img", legacy_fallback=False
)
BASE_BOOT = workspace_private_input_path(
    "boot_images", "boot_linux_v2334_audio_snd_nodes_preflight.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2787_audio_speaker_descriptor_api"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2787_audio_speaker_descriptor_api.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v444_audio_speaker_descriptor_api"
...
def configure() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--wifi-test-klog-prefix": "A90v2787",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2787.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2787.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2787.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2787-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2787.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2787-supervisor.pid",
    }
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    return "\n".join([
        "# Native Init V2787 Audio Speaker Descriptor API Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        "- Track: audio speaker descriptor API source build.",
        ...
        "## Purpose",
        "",
        "- This compiles the V2787 source change that moves speaker-map role/channel/hardware/safety metadata into the route API as `struct audio_speaker_map_entry`.",
        "- Expected future live validation should confirm `audio speaker-map internal-speaker-safe` exposes descriptor fields while remaining read-only.",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "parent_baseline": "v2786-audio-route-boolean-core",
        ...
        "audio_speaker_descriptor_api_source_validation": {
            "speaker_descriptor_api_compiled": True,
            "commands_to_probe_live_after_flash": [
                "audio status",
                "audio profiles",
                "audio speaker-map internal-speaker-safe",
                "audio stages internal-speaker-safe",
                "audio prereq internal-speaker-safe",
                "audio play internal-speaker-safe --mode probe --dry-run",
            ],
            "known_source_boundary": "speaker descriptor/query helpers are owned by a90_audio_route.{h,c}; profile and route data remain in a90_audio_profile.{h,c}; stage contract data remain in a90_audio_stage.{h,c}; command orchestration remains in a90_audio.c",
        },
    ...
    (OUT_DIR / "promotion-candidate.json").write_text(json.dumps({
        ...
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "note": "V2787 is a test image for the audio speaker descriptor API split, not a promoted rollback baseline.",
