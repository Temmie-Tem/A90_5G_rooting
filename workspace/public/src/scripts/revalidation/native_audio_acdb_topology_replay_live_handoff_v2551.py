#!/usr/bin/env python3
"""V2551 exact-gated native ACDB topology replay live handoff.
...
RUN_ID = "V2551"
BUILD_TAG = "v2551-audio-acdb-topology-replay-live-handoff"
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload.update(
        {
            "decision": "v2551-acdb-topology-replay-live-handoff-dry-run",
            "run_id": RUN_ID,
            "build_tag": BUILD_TAG,
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not state.get("ok"):
        raise SystemExit("refusing live run: V2551 dry-run preflight failed")
    out_dir = snd.ROOT / f"workspace/private/runs/audio/v2551-acdb-topology-replay-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2551-acdb-topology-replay-live-started",
        "out_dir": rel(out_dir),
        "preflight": state,
        "steps": steps,
        "rolled_back": False,
    ...
    try:
        ...
        result["decision"] = "v2551-acdb-topology-replay-live-pass-before-rollback"
    except Exception as exc:  # noqa: BLE001
        result["decision"] = "v2551-acdb-topology-replay-live-blocked"
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if result.get("decision") == "v2551-acdb-topology-replay-live-pass-before-rollback" else 1
