#!/usr/bin/env python3
"""Build V3226 GPU H3 shader-mode register probe."""
...
import build_native_init_boot_v3224_gpu_h3_mrt_component_mask_probe as previous

base = previous.base

CYCLE = "V3226"
INIT_VERSION = "0.11.40"
INIT_BUILD = "v3226-gpu-h3-shader-mode-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3226_GPU_H3_SHADER_MODE_SOURCE_BUILD_2026-06-25.md"
)
...
INIT_BINARY = OUT_DIR / "init_v3226_gpu_h3_shader_mode_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3226_gpu_h3_shader_mode_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v571_gpu_h3_shader_mode_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3226"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3226.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3226.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3226"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3226-gpu-h3-shader-mode-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3226-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3226-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3226-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3226-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3226-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3226-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3226-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-shader-mode-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-shader-mode-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3226")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3226")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3226")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3226")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3226")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3226")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3226")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3226")
SFX_STREAM_MARKER = "a90.doomgeneric.v3226.audio=real-sfx-pcm-stream-gpu-h3-shader-mode-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-shader-mode-probe-v3226"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3226.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3226_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3224": b"a90-doomgeneric-v3226",
        b"a90.doomgeneric.v3224": b"a90.doomgeneric.v3226",
        b"v3224": b"v3226",
        b"V3224": b"V3226",
        b"first-triangle-h3-mrt-component-mask-mov-f32-shader":
...
REQUIRED_STRINGS = tuple(_rewrite_v3226_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3224-gpu-h3-mrt-component-mask-probe",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-shader-mode-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3226-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-shader-mode-mov-f32-shader-timeout-guard",
...
def v3226_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-shader-mode-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-shader-mode-probe")
        .replace("v3208", "v3226")
        .replace("V3208", "V3226")
    )


def _overlay_preserved_v3226_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3224 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3226 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3226 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3226 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3226-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            if item == "--ramdisk":
                ...
                raise RuntimeError("V3224 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3226 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3226 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3224-ramdisk-overlay-v3226-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-shader-mode-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-shader-mode-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-shader-mode-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3224-ramdisk-overlay-v3226-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-shader-mode-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-shader-mode-live-validation"
...
def _apply_v3226_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-shader-mode-probe")
        ...
        "v3210_adapter_source": v3226_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3226_ramdisk,
