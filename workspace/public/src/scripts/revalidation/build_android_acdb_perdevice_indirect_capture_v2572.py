#!/usr/bin/env python3
"""Build V2572 ARM32 ACDB per-device generic-indirect capture artifacts.
...
The preload keeps acdb_ioctl quiet until the interposed common-topology hook
manually arms capture, skips the already-pinned real common-topology public call
by default, patches the known init flag, then calls acdb_loader_send_audio_cal_v5()
with generic direct and indirect acdb_ioctl capture enabled.
...
RUN_ID = "V2572"
BUILD_TAG = "v2572-acdb-perdevice-indirect-capture-host-only"
HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_init_drive_exec_linked_v2572.c"
ACDBTAP_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdbtap_v2572.c"
IOCTL_SOURCE_REL = preload_base.IOCTL_SOURCE_REL
PREINIT_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_preinit_perdevice_v2572.c"
...
HELPER_ARTIFACT_NAME = "a90_acdb_perdevice_indirect_exec_linked_v2572"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_perdevice_indirect_capture_v2572.so"
...
def source_state() -> dict[str, Any]:
    ...
    combined = "\n".join(texts.values())
    preinit_patch_pos = texts["preinit"].find("patch_initialized_flag_return")
    preinit_arm_pos = texts["preinit"].find("if (a90_arm_capture)")
    preinit_send_pos = texts["preinit"].find("before_send_audio_cal_v5")
    required = {
        ...
        "preinit_skips_real_common_topology_by_default": (
            "#define A90_V2572_CALL_REAL_COMMON_TOPOLOGY 0" in texts["preinit"]
            and "skip_real_common_topology" in texts["preinit"]
        ),
        "preinit_patches_init_flag": "A90_LOADER_INIT_FLAG_OFF" in texts["preinit"] and "*flag = 1U" in texts["preinit"],
        "preinit_arms_after_patch_before_send_audio_cal": (
            preinit_patch_pos >= 0
            and preinit_arm_pos > preinit_patch_pos
            and preinit_send_pos > preinit_arm_pos
        ),
        ...
        "tap_has_generic_indirect_capture": "a90_log_generic_indirect_capture" in texts["tap"] and "\"indirect\"" in texts["tap"],
        "tap_filters_probable_user_pointer": "a90_is_probable_user_read_ptr" in texts["tap"] and "0x10000000U" in texts["tap"],
        "tap_has_nonzero_guard": "a90_is_all_zero" in texts["tap"] and "all_zero" in texts["tap"],
        "tap_distinguishes_direct_and_indirect_raw": "\"direct\"" in texts["tap"] and "\\\"buffer\\\"" in texts["tap"],
...
def vendor_lib_state(readelf: str) -> dict[str, Any]:
    ...
    state["required_for_v2572"] = required
    state["required_for_v2572_ok"] = all(required.values())
    return state
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    preinit_obj = obj_dir / "libacdb_preinit_perdevice_v2572.o"
...
def main() -> int:
    ...
    payload["ok"] = bool(source["required_ok"] and source["prohibited_ok"] and vendor["required_for_v2572_ok"] and build_state.get("ok", True))
