#!/usr/bin/env python3
"""Guarded S22+ M10A1 stat-dev reboot native-init live gate.
...
M10A1 package and rollback APs without touching any device.

M10A1 is a lower-layer discriminator, not a module or USB proof. It performs a
read-only newfstatat("/dev") pathname VFS probe, then reboot("download"). A
later Odin endpoint is still manually ambiguous: the helper can observe and
rollback from download mode, but only the operator can confirm whether download
mode was automatic or manually entered.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M10A1-STAT-DEV-REBOOT-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M10A1-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M10A1_AP_SHA256 = "68a7f1f5b336a32d882e7cdde73f299815d689b6885b724a6b6c7672bdda00bf"
EXPECTED_M10A1_BOOT_SHA256 = "2fe6b3270f7d493f677f126594061eea33d22de7abe98dc2210fe8050961ecb2"
EXPECTED_M10A1_BASE_BOOT_SHA256 = "2e541703951dc725bad35850faf7028c2d910dd5f21166449b63f1248c29967e"
EXPECTED_M10A1_KERNEL_SHA256 = "bceca73edbfca3499148e16741c939779157925949ef6bc8a8e31d6b68fc2cff"
EXPECTED_M10A1_INIT_SHA256 = "477583121c6c29f5eb31866c034352abb2f03c8fe97ec71e2f63ecbddd6f1642"
EXPECTED_M10A1_SOURCE_SHA256 = "a60b66ec5d07f93bb9e29ac96c342e57621815630c29f31653b104e19f7ff86b"
EXPECTED_M10A1_RETAINED_MARKER = "S22_NATIVE_INIT_M10A1_NO_RETAINED_MARKER_EXPECTED"

DEFAULT_M10A1_AP = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a1_stat_dev_reboot_v0_1/odin4/AP.tar.md5")
DEFAULT_M10A1_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a1_stat_dev_reboot_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    ...
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m10a1_stat_dev_reboot_live_gate_{stamp}")
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    required = [
        "S22+ M10A1 stat-dev reboot native-init boot-only",
        EXPECTED_M10A1_AP_SHA256,
        EXPECTED_M10A1_BOOT_SHA256,
        EXPECTED_M10A1_BASE_BOOT_SHA256,
        EXPECTED_M10A1_KERNEL_SHA256,
        EXPECTED_M10A1_INIT_SHA256,
        EXPECTED_M10A1_SOURCE_SHA256,
        LIVE_ACK_TOKEN,
        ROLLBACK_ACK_TOKEN,
        "freestanding C raw-syscall runtime",
        "`newfstatat(AT_FDCWD, \"/dev\", ...)` read-only pathname VFS probe",
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M10A1 live authorization markers: {missing}")


def verify_m10a1_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M10A1 manifest missing: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    hashes = data.get("hashes", {})
    safety = data.get("safety", {})
    ramdisk = data.get("ramdisk", {})
    init_info = data.get("m10a1_init", {})
    tar_members_seen = data.get("tar_members")
    append_log(log_path, f"m10a1_manifest_path={path}")
    append_log(log_path, f"m10a1_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    append_log(log_path, f"m10a1_manifest_safety={json.dumps(safety, sort_keys=True)}")
    append_log(log_path, f"m10a1_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")

    expected_hashes = {
        "ap_tar_md5": EXPECTED_M10A1_AP_SHA256,
        "boot_img": EXPECTED_M10A1_BOOT_SHA256,
        "base_boot": EXPECTED_M10A1_BASE_BOOT_SHA256,
        "kernel": EXPECTED_M10A1_KERNEL_SHA256,
        "m10a1_init": EXPECTED_M10A1_INIT_SHA256,
        "source": EXPECTED_M10A1_SOURCE_SHA256,
        "nochange_repack_boot": EXPECTED_M10A1_BASE_BOOT_SHA256,
    }
    for key, expected in expected_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M10A1 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M10A1 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    expected_safety: dict[str, Any] = {
        ...
        "first_externally_observable_action": "newfstatat-dev-then-reboot-download",
        "intended_syscall_count": 2,
        "auto_reboot": "download-after-newfstatat-dev",
        "marker_write": False,
        "kmsg_write": False,
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise SystemExit(f"M10A1 manifest safety {key} mismatch: {safety.get(key)!r} != {expected!r}")
    if safety.get("intended_syscalls") != ["newfstatat", "reboot"]:
        raise SystemExit(f"M10A1 intended syscalls mismatch: {safety.get('intended_syscalls')!r}")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M10A1 manifest did not replace /init")
    if ramdisk.get("only_intended_entry_change") != "init":
        raise SystemExit("M10A1 manifest intended ramdisk change is not only /init")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A1 must not inject vendor module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A1 must not inject module-list files into boot ramdisk")

    required_strings = set(init_info.get("required_strings", []))
    if "/dev" not in required_strings:
        raise SystemExit("M10A1 required /dev string missing from manifest")
    if "download" not in required_strings:
        raise SystemExit("M10A1 required download string missing from manifest")
    if init_info.get("svc_count") != 2:
        raise SystemExit(f"M10A1 expected two svc instructions, got {init_info.get('svc_count')!r}")
    ...
    for forbidden in [
        ...
        if forbidden not in forbidden_strings:
            raise SystemExit(f"M10A1 forbidden-string gate missing from manifest: {forbidden}")
...
def observe_until_odin(run_dir: Path, log_path: Path, seconds: int, odin: Path) -> str | None:
    ...
    while time.monotonic() < deadline:
        iteration += 1
        label = f"m10a1_download_observation_{iteration:03d}"
        host_snapshot(run_dir, log_path, label, odin)
        devices = odin_devices(odin, log_path, f"{label}-extra")
        if len(devices) == 1:
            append_log(log_path, "m10a1_download_endpoint_seen=1")
            append_log(log_path, "m10a1_manual_download_ambiguity=operator-confirmation-required")
            return devices[0]
        if len(devices) > 1:
            raise SystemExit(f"refusing ambiguous Odin devices during M10A1 observation: {devices}")
        rows = adb_rows(log_path, f"{label}-extra")
        if rows:
            append_log(log_path, f"m10a1_candidate_adb_rows={rows}")
        time.sleep(1.0)
    append_log(log_path, "m10a1_download_endpoint_seen=0")
    return None
...
def rollback_from_download(
    ...
    if serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", serial, marker=EXPECTED_M10A1_RETAINED_MARKER)
    return 0


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--m10a1-ap", type=Path, default=DEFAULT_M10A1_AP)
    parser.add_argument("--m10a1-manifest", type=Path, default=DEFAULT_M10A1_MANIFEST)
    ...
    log_path = run_dir / "s22plus_m10a1_stat_dev_reboot_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus m10a1 stat-dev reboot live gate ===")
    ...
    m10a1_ap = resolve(root, args.m10a1_ap)
    m10a1_manifest = resolve(root, args.m10a1_manifest)
    ...
    verify_ap(m10a1_ap, EXPECTED_M10A1_AP_SHA256, "m10a1_candidate", log_path)
    verify_m10a1_manifest(m10a1_manifest, log_path)
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M10A1 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M10A1 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M10A1 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    if odin_device is None:
        print("download mode did not appear for M10A1 candidate flash", file=sys.stderr)
        return 2

    candidate_rc = flash_ap(odin, m10a1_ap, odin_device, log_path, "candidate")
    if candidate_rc != 0:
        print(f"M10A1 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        print(
            "M10A1 candidate flash completed but the original Odin device did not disconnect; "
        ...
        if post_rollback_serial is None:
            return 6
        append_log(log_path, "m10a1_result=no-proof-original-download-never-disconnected")
        collect_android_pstore(
            run_dir,
            log_path,
            "post_rollback_no_disconnect",
            post_rollback_serial,
            marker=EXPECTED_M10A1_RETAINED_MARKER,
        )
        return 7

    print("M10A1 candidate flashed. Waiting for later download endpoint; operator manual entry would make it ambiguous.")
    rollback_device = observe_until_odin(run_dir, log_path, args.download_observation_wait_sec, odin)
    if rollback_device is None:
        print("M10A1 download endpoint did not appear; enter download mode manually and run --rollback-from-download.", file=sys.stderr)
        return 4
    ...
    if post_rollback_serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", post_rollback_serial, marker=EXPECTED_M10A1_RETAINED_MARKER)
    print(
        "M10A1 live gate observed a later download endpoint and rollback ok; "
