#!/usr/bin/env python3
"""Build V2672 lower hidden-node ACDB SET capture artifacts.

Host-only build unit. V2671 proved the common-topology lower blocks for cal_types
10/14/24 are interior blocks and must not be jumped into directly. V2672 instead
builds a small own-process helper plus preload that recreates the pinned lower
sequence with libacdbloader internal helpers:
...
ROOT = v2659.ROOT
RUN_ID = "V2672"
BUILD_TAG = "v2672-acdb-lower-hidden-node-setcal-capture-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2672_AUDIO_ACDB_LOWER_HIDDEN_NODE_CAPTURE_BUILD_2026-06-18.md"

HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_lower_hidden_node_exec_linked_v2672.c"
PREINIT_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_lower_hidden_node_v2672.c"
HELPER_ARTIFACT_NAME = "a90_acdb_lower_hidden_node_setcal_capture_exec_linked_v2672"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_lower_hidden_node_setcal_capture_combined_preload_v2672.so"
...
def source_state() -> dict[str, Any]:
    ...
    lower_pos = helper.find("a90_run_lower_hidden_nodes()")
    patch_pos = preinit.find("patch_initialized_flag_return")
    return_pos = preinit.find("return_to_init_v3_no_arm_no_send")
    ...
    required = {
        ...
        "helper_imports_arm_capture": "extern void a90_arm_capture" in helper,
        "helper_imports_lower_runner": "extern int32_t a90_run_lower_hidden_nodes" in helper,
        "helper_calls_init_v3_open_audconf": 'A90_ACDB_FILES_PATH "/vendor/etc/audconf/OPEN"' in helper,
        "helper_arms_after_init_before_lower": init_pos >= 0 and arm_pos > init_pos and lower_pos > arm_pos,
        ...
        "preload_returns_to_init_without_send": patch_pos >= 0 and return_pos > patch_pos,
    ...
    return {
        ...
        "v2672_contract": {
            "basis": "V2671 lower blocks are interior and not directly callable; recreate node/allocate/GET/SET sequence instead",
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    if isinstance(helper, dict):
        ...
        if isinstance(checks, dict):
            ...
            checks["weak_a90_run_lower_hidden_nodes"] = " a90_run_lower_hidden_nodes" in helper_symbols
            ...
            helper["ok"] = bool(
                ...
                and checks["weak_a90_run_lower_hidden_nodes"]
    ...
    if isinstance(preload, dict):
        ...
        if isinstance(checks, dict):
            ...
            checks["soname_v2672"] = f"Library soname: [{PRELOAD_ARTIFACT_NAME}]" in preload_dynamic
...
def make_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            "call_order": "acdb_loader_init_v3 -> init common skip hook -> a90_arm_capture -> a90_run_lower_hidden_nodes",
