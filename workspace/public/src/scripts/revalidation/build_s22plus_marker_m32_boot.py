#!/usr/bin/env python3
"""Build the S22+ M3.2 marker-only direct-PID1 boot candidate.
...
M3.2 keeps M3.1's marker-only direct `/init` behavior but compresses the boot
ramdisk with the stock-style legacy LZ4 format before `mkbootimg`.  This removes
the uncompressed-newc ramdisk variable shared by P3/M3/M3.1 failures.
"""
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/marker_m32_lz4ramdisk_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_marker_m32.c")
DEFAULT_RAMDISK_LZ4 = Path("workspace/private/tools/lz4-src/programs/lz4")
MARKER = "S22_NATIVE_INIT_MARKER_ONLY_M32"
LEGACY_LZ4_MAGIC = b"\x02\x21\x4c\x18"


def write_ramdisk_lz4_legacy(cpio_path: Path, out_lz4: Path, lz4_bin: Path, scratch_dir: Path) -> None:
    result = run([lz4_bin, "-l", "-9", "-f", cpio_path, out_lz4])
    require_ok(result, "legacy-LZ4 ramdisk compression")
    data = out_lz4.read_bytes()
    if not data.startswith(LEGACY_LZ4_MAGIC):
        raise SystemExit(f"ramdisk legacy-LZ4 magic mismatch: {data[:4].hex()}")

    roundtrip = scratch_dir / "ramdisk_m32_roundtrip.cpio"
    dec = run([lz4_bin, "-d", "-f", out_lz4, roundtrip])
    require_ok(dec, "legacy-LZ4 ramdisk decompression gate")
    if sha256_file(roundtrip) != sha256_file(cpio_path):
        raise SystemExit("legacy-LZ4 ramdisk roundtrip SHA mismatch")
...
def main(argv: list[str]) -> int:
    ...
    marker_init = build_dir / "s22plus_init_marker_m32"
    ...
    ramdisk_cpio = out_dir / "ramdisk_marker_m32.cpio"
    ramdisk_lz4 = out_dir / "ramdisk_marker_m32.lz4"
    ...
    boot_unpadded = out_dir / "boot_marker_m32_unpadded.img"
    ...
    manifest = {
        ...
        "purpose": "M3.2 marker-only direct native-init candidate with stock-format legacy-LZ4 ramdisk",
        "safety": {
            "boot_only": True,
