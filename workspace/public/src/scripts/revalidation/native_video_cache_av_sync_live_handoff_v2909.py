#!/usr/bin/env python3
"""V2909 live validation for trusted SD video-cache playback with audio sync.

V2907 added ``video cache play --trust-cache`` and V2908 built it into a
flashable image. This runner flashes V2908, verifies the existing V2900
Bad-Apple-scale SD cache once, starts bounded native audio playback, then plays
a 10 second page-flip slice from the trusted cache anchored to the audio worker
status file. It then rolls back to v2321.
"""
...
RUN_ID = "V2909"
BUILD_TAG = "v2909-video-cache-av-sync-live"
REPORT_TITLE = "Native Init V2909 Trusted Video Cache A/V Sync Live Validation"
DECISION_PREFIX = "v2909-video-cache-av-sync"
CANDIDATE_IMAGE = video_live.ROOT / "workspace/private/inputs/boot_images/boot_linux_v2908_video_cache_trust_play.img"
CANDIDATE_VERSION = "0.10.35"
CANDIDATE_TAG = "v2908-video-cache-trust-play"
CANDIDATE_SHA256 = "3d0f12ce17245de8649841d44a382e11291f9e8a0b0df527ff96bdc4e1b0fc78"
REPORT_PATH = video_live.ROOT / "docs/reports/NATIVE_INIT_V2909_VIDEO_CACHE_AV_SYNC_LIVE_2026-06-20.md"
...
def configure_globals() -> None:
    ...
    video_live.REMOTE_DIR = "/mnt/sdext/a90/runtime/video/v2909"
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "## Command Results",
        "",
        f"- `video cache status`: rc=`{result.get('cache_status_rc')}`, summary=`{status}`",
        f"- `video cache verify`: rc=`{result.get('cache_verify_rc')}`, summary=`{verify}`",
        ...
        f"- `video cache play --trust-cache --sync-audio-status`: rc=`{result.get('cache_play_rc')}`, pass=`{int(bool(play.get('pass')))}`",
...
def run_live(args: Any, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        status = video_live.base.run_serial_step(out_dir, steps, "candidate-video-cache-status", ["video", "cache", "status", FIXTURE_SHA256], timeout=120.0, allow_error=True, retry_unsafe=False)
        ...
        verify = video_live.base.run_serial_step(out_dir, steps, "candidate-video-cache-verify", ["video", "cache", "verify", FIXTURE_SHA256], timeout=420.0, allow_error=True, retry_unsafe=False)
        ...
        play = video_live.base.run_serial_step(
            out_dir,
            steps,
            "candidate-video-cache-play-trust-audio-sync",
            [
                "video", "cache", "play", FIXTURE_SHA256,
                "--trust-cache",
...
def dry_run_payload(args: Any, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"video cache play {FIXTURE_SHA256} --trust-cache --frames {FIXTURE_SYNC_FRAMES} --present pageflip --sync-audio-status {SYNC_STATUS_PATH} --sync-wait-ms {SYNC_WAIT_MS}",
        ],
    }
