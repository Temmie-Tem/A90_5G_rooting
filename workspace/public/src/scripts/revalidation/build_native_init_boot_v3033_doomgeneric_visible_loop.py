...
def patch_ramdisk_with_doomgeneric_helper() -> None:
    ...
    def patch_with_doomgeneric_helper(ramdisk_files: dict[str, Path]) -> None:
        ...
        if HW_PLANE_SCALE:
            doomgeneric_flags = (
                *doomgeneric_flags,
                numeric_define("VIDEO_DEMO_DOOMGENERIC_HW_PLANE_SCALE", 1),
            )
        if PRE_SCALED_LARGE_FRAME:
            doomgeneric_flags = (
                *doomgeneric_flags,
                numeric_define("VIDEO_DEMO_DOOMGENERIC_PRE_SCALED_LARGE_FRAME", 1),
            )
        if NO_FULL_CLEAR:
            doomgeneric_flags = (
                *doomgeneric_flags,
                numeric_define("VIDEO_DEMO_DOOMGENERIC_NO_FULL_CLEAR", 1),
            )
