#!/usr/bin/env python3
"""Build V3153 DOOM physical-button exit fast-menu-live-hud candidate."""
...
CYCLE = "V3153"
INIT_VERSION = "0.10.135"
INIT_BUILD = "v3153-doomgeneric-physical-exit-fast-menu-live-hud"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3153_DOOMGENERIC_PHYSICAL_EXIT_FAST_MENU_LIVE_HUD_SOURCE_BUILD_2026-06-24.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3153_doomgeneric_physical_exit_fast_menu_live_hud.img",
    legacy_fallback=False,
)
INIT_BINARY = OUT_DIR / "init_v3153_doomgeneric_physical_exit_fast_menu_live_hud"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3153_doomgeneric_physical_exit_fast_menu_live_hud.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v532_doomgeneric_physical_exit_fast_menu_live_hud"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3153"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3153.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3153.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3153"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3153-physical-exit-fast-menu-live-hud"

FRAME_PATH = "/tmp/a90-doomgeneric-v3153-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3153-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3153-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3153-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3153-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3153-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3153-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-sfx-long-window-physical-exit-fast-menu-live-hud"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-sfx-long-window-physical-exit-fast-menu-live-hud"

INPUT_THREAD_MARKER = v3151.INPUT_THREAD_MARKER.replace("v3151", "v3153")
TIME_MODEL_MARKER = v3151.TIME_MODEL_MARKER.replace("v3151", "v3153")
DEMO_HUD_MARKER = v3151.DEMO_HUD_MARKER.replace("v3151", "v3153")
PACED_TIME_MARKER = v3151.PACED_TIME_MARKER.replace("v3151", "v3153")
TICK_TELEMETRY_MARKER = v3151.TICK_TELEMETRY_MARKER.replace("v3151", "v3153")
SCALE_MARKER = v3151.SCALE_MARKER.replace("v3151", "v3153")
PHASE_TELEMETRY_MARKER = v3151.PHASE_TELEMETRY_MARKER.replace("v3151", "v3153")
GAMETIC_FRAME_TELEMETRY_MARKER = v3151.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3151", "v3153")
SFX_STREAM_MARKER = "a90.doomgeneric.v3153.audio=real-sfx-pcm-stream-long-window-physical-exit-fast-menu-live-hud"
SOUND_MODE = "native-doom-sfx-pcm-stream-long-window-v3153"
...
SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3153.c"
...
def _rewrite_required_string(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3151": b"a90-doomgeneric-v3153",
        b"a90.doomgeneric.v3151": b"a90.doomgeneric.v3153",
        b"v3151": b"v3153",
        b"V3151": b"V3153",
    }
...
def _v3153_values() -> dict[str, Any]:
...
def _v3153_adapter_source_from_patched_v3148() -> str:
    return (
        _ORIGINAL_V3151_ADAPTER_SOURCE_FROM_PATCHED_V3148()
        .replace("real-sfx-pcm-stream-long-window-physical-exit-menu-return",
                 "real-sfx-pcm-stream-long-window-physical-exit-fast-menu-live-hud")
        .replace("v3151", "v3153")
        .replace("V3151", "V3153")
    )


def _apply_v3153_globals() -> dict[str, Any]:
    saved = {name: getattr(v3151, name) for name in _v3153_values()}
    for name, value in _v3153_values().items():
    ...
    v3151._v3151_adapter_source_from_patched_v3148 = _v3153_adapter_source_from_patched_v3148
    v3151.render_report = render_report
    v3151._postprocess_manifest = _postprocess_manifest
    return saved


def _restore_v3153_globals(saved: dict[str, Any]) -> None:
    for name, value in saved.items():
        setattr(v3151, name, value)


def v3153_adapter_source() -> str:
    saved = _apply_v3153_globals()
    try:
        return _v3153_adapter_source_from_patched_v3148()
    finally:
        _restore_v3153_globals(saved)
...
def _postprocess_manifest() -> dict[str, Any]:
    ...
    doom.update({
        "physical_button_exit": {
            ...
            "action": "exit-doom-loop-fast-menu-live-hud-then-stop-audio",
            "return_reason": "physical-button-exit",
            "menu_return": True,
            "fast_menu_live_hud": True,
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-physical-exit-fast-menu-live-hud-candidate",
        "adoption_state": "pending-physical-exit-fast-menu-live-hud-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-physical-exit-fast-menu-live-hud-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-physical-exit-fast-menu-live-hud-candidate",
        ...
        "fast_menu_live_hud_on_physical_exit": True,
        "start_new_hud_from_doom_exit_path": True,
        "sound_mode": SOUND_MODE,
