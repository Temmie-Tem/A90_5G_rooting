#!/usr/bin/env python3
"""V2788 live validation for the native-init audio speaker descriptor API.

This validates the V2787 behavior change: speaker-map role/channel/hardware/safety descriptors are exposed by the native audio command on device, then rolls back to V2321.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2787-audio-speaker-descriptor-api/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2787_audio_speaker_descriptor_api.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2788_AUDIO_SPEAKER_DESCRIPTOR_API_DEVICE_VALIDATION_2026-06-19.md"

CANDIDATE_VERSION = "0.9.301"
CANDIDATE_TAG = "v2787-audio-speaker-descriptor-api"
...
def preflight_state() -> dict[str, Any]:
    ...
    return {
        ...
        "live_scope": [
            ...
            "run read-only audio status/profiles/profile commands",
            "run read-only audio speaker-map descriptor command",
            "do not run ADSP boot, /dev/snd materialization, route apply/reset, ACDB SET, PCM open, or playback",
            "rollback to v2321",
        ],
    }
...
def summarize_step(step: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_profile": f"audio.profile.id={PROFILE}" in text,
        "has_speaker_map_version": "audio.speaker_map.version=1" in text,
        "has_speaker_map_profile": f"audio.speaker_map.profile={PROFILE}" in text,
        ...
        "has_speaker_map_count": "audio.speaker_map.speaker.count=6" in text,
        "has_speaker_map_shared_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.0.id=shared",
            "audio.speaker_map.speaker.0.role=stream-route",
            "audio.speaker_map.speaker.0.channel=both",
            "audio.speaker_map.speaker.0.hardware=SLIMBUS_0_RX shared speaker path",
            "audio.speaker_map.speaker.0.safety=safe-observed-core",
        ]),
        "has_speaker_map_left_vi_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.1.id=SPKR_VI_1",
            "audio.speaker_map.speaker.1.role=feedback",
            "audio.speaker_map.speaker.1.channel=left",
            "audio.speaker_map.speaker.1.hardware=AIF4_VI speaker feedback input 1",
            "audio.speaker_map.speaker.1.safety=speaker-protection-observed",
        ]),
        "has_speaker_map_right_vi_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.2.id=SPKR_VI_2",
            "audio.speaker_map.speaker.2.role=feedback",
            "audio.speaker_map.speaker.2.channel=right",
            "audio.speaker_map.speaker.2.hardware=AIF4_VI speaker feedback input 2",
            "audio.speaker_map.speaker.2.safety=speaker-protection-observed",
        ]),
        "has_speaker_map_vi_format_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.3.id=SPKR_VI",
            "audio.speaker_map.speaker.3.role=feedback-format",
            "audio.speaker_map.speaker.3.channel=both",
            "audio.speaker_map.speaker.3.hardware=SLIM_4_TX VI feedback format",
            "audio.speaker_map.speaker.3.safety=speaker-protection-observed",
        ]),
        "has_speaker_map_left": "audio.speaker_map.speaker.4.id=SpkrLeft" in text,
        "has_speaker_map_left_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.4.id=SpkrLeft",
            "audio.speaker_map.speaker.4.role=endpoint",
            "audio.speaker_map.speaker.4.channel=left",
        ...
        "has_speaker_map_right_descriptor": all(marker in text for marker in [
            "audio.speaker_map.speaker.5.id=SpkrRight",
            "audio.speaker_map.speaker.5.role=endpoint",
            "audio.speaker_map.speaker.5.channel=right",
...
def render_report(result: dict[str, Any]) -> str:
    ...
    summary_lines = [
        "- `{name}`: ok={ok} rc={rc} audio={audio} profile={profile} "
        "speaker_map={speaker_map} descriptors={descriptors} read_only={read_only}".format(
            name=item["name"],
            ok=item["ok"],
            rc=item["rc"],
            audio=item["has_audio_output"],
            profile=item["has_profile"],
            ...
            descriptors=int(bool(
                item["has_speaker_map_shared_descriptor"]
                and item["has_speaker_map_left_vi_descriptor"]
                and item["has_speaker_map_right_vi_descriptor"]
                and item["has_speaker_map_vi_format_descriptor"]
                and item["has_speaker_map_left_descriptor"]
                and item["has_speaker_map_right_descriptor"]
            )),
            read_only=int(bool(
                item["has_speaker_map_read_only"]
                and item["has_speaker_map_route_write0"]
                and item["has_speaker_map_playback0"]
            )),
        )
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2788-audio-speaker-descriptor-api-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2788-audio-speaker-descriptor-api-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        profile_summary = next((item for item in command_summaries if item.get("name") == "audio-profile"), {})
        speaker_summary = next((item for item in command_summaries if item.get("name") == "audio-speaker-map"), {})
        result["profile_observed"] = bool(profile_summary.get("has_profile"))
        result["speaker_map_read_only_observed"] = bool(
            speaker_summary.get("has_speaker_map_version")
            and speaker_summary.get("has_speaker_map_profile")
            ...
            and speaker_summary.get("has_speaker_map_count")
        ...
        result["speaker_descriptor_observed"] = bool(
            speaker_summary.get("has_speaker_map_shared_descriptor")
            and speaker_summary.get("has_speaker_map_left_vi_descriptor")
            and speaker_summary.get("has_speaker_map_right_vi_descriptor")
            and speaker_summary.get("has_speaker_map_vi_format_descriptor")
            and speaker_summary.get("has_speaker_map_left_descriptor")
            and speaker_summary.get("has_speaker_map_right_descriptor")
        )
        if (
            result["profile_observed"]
            and result["speaker_map_read_only_observed"]
            and result["speaker_descriptor_observed"]
        ):
            result["decision"] = "v2788-audio-speaker-descriptor-api-device-pass"
        elif not result["speaker_map_read_only_observed"]:
            result["decision"] = "v2788-audio-speaker-descriptor-api-missing-command"
        else:
            result["decision"] = "v2788-audio-speaker-descriptor-api-device-validation-incomplete"
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2788-audio-speaker-descriptor-api-device-validation-dry-run",
...
def main() -> int:
    ...
    return 0 if (
        result.get("decision") == "v2788-audio-speaker-descriptor-api-device-pass"
