#!/usr/bin/env python3
"""Build V3200 GPU G4 A2D solid-fill/readback probe with CCU color flush."""
...
CYCLE = "V3200"
INIT_VERSION = "0.11.28"
INIT_BUILD = "v3200-gpu-g4-solid-fill-ccu-flush-probe"
BUILD_TAG = INIT_BUILD
DECISION = "v3200-gpu-g4-solid-fill-ccu-flush-probe-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3200_GPU_G4_SOLID_FILL_CCU_FLUSH_PROBE_SOURCE_BUILD_2026-06-25.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3200_gpu_g4_solid_fill_ccu_flush_probe.img",
    legacy_fallback=False,
)
INIT_BINARY = OUT_DIR / "init_v3200_gpu_g4_solid_fill_ccu_flush_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3200_gpu_g4_solid_fill_ccu_flush_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v560_gpu_g4_solid_fill_ccu_flush_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3200"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3200.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3200.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3200"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3200-gpu-g4-solid-fill-ccu-flush-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3200-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3200-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3200-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3200-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3200-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3200-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3200-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-g4-solid-fill-ccu-flush-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-g4-solid-fill-ccu-flush-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3194", "v3200")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3194", "v3200")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3194", "v3200")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3194", "v3200")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3194", "v3200")
SCALE_MARKER = base.SCALE_MARKER.replace("v3194", "v3200")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3194", "v3200")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3194", "v3200")
SFX_STREAM_MARKER = "a90.doomgeneric.v3200.audio=real-sfx-pcm-stream-gpu-g4-solid-fill-ccu-flush-probe"
SOUND_MODE = "native-doom-sfx-gpu-g4-solid-fill-ccu-flush-probe-v3200"
...
SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3200.c"
...
def _rewrite_required_string(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3194": b"a90-doomgeneric-v3200",
        b"a90.doomgeneric.v3194": b"a90.doomgeneric.v3200",
        b"v3194": b"v3200",
        b"V3194": b"V3200",
    }
...
GPU_G4_SOLID_FILL_MARKERS = (
    ...
    b"gpu.g4.fill.post_blit_event=pc_ccu_flush_color_ts",
...
REQUIRED_STRINGS = tuple(
    ...
    b"0.11.28",
    b"v3200-gpu-g4-solid-fill-ccu-flush-probe",
) + GPU_G4_SOLID_FILL_MARKERS


def _v3200_require_strings(path: Path) -> list[str]:
    ...
    if missing:
        raise RuntimeError(f"missing V3200 boot-image markers: {missing}")
...
SFX_BACKEND_SOURCE_TEXT = (
    ...
    .replace("gpu-g3-noop-submit-probe", "gpu-g4-solid-fill-ccu-flush-probe")
    .replace("real-sfx-pcm-stream-gpu-g3-noop-submit-probe",
             "real-sfx-pcm-stream-gpu-g4-solid-fill-ccu-flush-probe")
    .replace("v3194", "v3200")
    .replace("V3194", "V3200")
...
def _v3200_overrides() -> dict[str, Any]:
...
def _v3200_values() -> dict[str, Any]:
    values = dict(ORIG_V3194_VALUES())
    values.update(_v3200_overrides())
    return values


def v3200_adapter_source() -> str:
    return (
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3200 GPU G4 Solid Fill CCU Flush Probe Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        "- Track: GPU G4 KGSL A6xx A2D solid-fill render plus CPU readback verification, with only the minimal post-blit CCU color flush restored.",
        ...
        "## Included Delta",
        "",
        "- Keeps `gpu g4-solid-fill-probe` as the user-visible command and restores only one post-blit `CP_EVENT_WRITE`: `PC_CCU_FLUSH_COLOR_TS`.",
        ...
        "- Keeps V3198's exclusion of `DEBUG_LABEL`, `CACHE_FLUSH_TS`, and `CACHE_INVALIDATE`; V3197 implicated the `CACHE_INVALIDATE` tail in the CP fault path.",
        ...
        "- V3198 removed all post-blit events and avoided the V3196 dmesg fault but left readback at the sentinel pattern. V3200 restores only the Mesa-derived CCU color clean event required to make CP_BLIT output visible to sysmem.",
        "",
        "## Source Basis",
        "",
        ...
        "- Local live evidence: `docs/reports/NATIVE_INIT_V3197_GPU_G4_SOLID_FILL_PROBE_LIVE_INCIDENT_2026-06-25.md` identified the V3196 post-blit event-write tail as unsafe; `docs/reports/NATIVE_INIT_V3199_GPU_G4_SOLID_FILL_NOEVENT_LIVE_2026-06-25.md` showed that removing all post-blit events avoids faults but leaves the target buffer unchanged.",
        ...
        "## Validation",
        "",
        "- `py_compile`: V3200 builder and focused tests.",
        "- `unittest`: V3200 GPU G4 source contract plus V3198/V3196/V3194/V3192/V3190 regression contracts.",
        "- Build: AArch64 helper/native-init compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3200 identity, G0/G1/G2/G3 markers, and G4 solid-fill markers.",
        "- `git diff --check`: PASS.",
        "",
        "## Metadata",
        "",
        ...
        "- Candidate type: `gpu-g4-solid-fill-ccu-flush-probe-candidate`.",
    ]) + "\n"
...
def _postprocess_manifest() -> dict[str, Any]:
    ...
    manifest.update({
        ...
        "candidate_type": "gpu-g4-solid-fill-ccu-flush-probe-candidate",
        "adoption_state": "pending-gpu-g4-solid-fill-ccu-flush-live-validation",
        "gpu_g4": {
            "source_baseline": "v3198-gpu-g4-solid-fill-noevent-probe",
            ...
            "command_stream": {
                ...
                "cp_set_marker": "0x65",
                "cp_blit": "0x2c",
                "cp_event_write_tail": "minimal-pc-ccu-flush-color-ts-only",
                "post_blit_events": [
            ...
            "next_live_validation": [
                "flash-v3200-through-native-init-flash",
    ...
    (OUT_DIR / "gpu-g4-solid-fill-ccu-flush-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-g4-solid-fill-ccu-flush-probe-candidate",
...
def _patch_base_module() -> list[tuple[Any, str, Any, bool]]:
    ...
    for name, value in _v3200_overrides().items():
        existed = hasattr(base, name)
        saved.append((base, name, getattr(base, name, None), existed))
        setattr(base, name, value)
    for name, value in (
        ("_v3194_overrides", _v3200_overrides),
        ("_v3194_values", _v3200_values),
        ("v3194_adapter_source", v3200_adapter_source),
        ("_v3194_require_strings", _v3200_require_strings),
