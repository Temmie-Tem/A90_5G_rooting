#!/usr/bin/env python3
"""V2587 Android handoff wrapper for V2586 send_audio_cal_v5 arg2=1 artifacts.

Host-only by default.  Live mode reuses the V2490 Android boot/stage/pull/rollback
engine through the V2573 per-device indirect classifier, but selects the V2586
private helper/preload where send_audio_cal_v5 arg2 is compiled as RX cap mask 1.
"""
...
import build_android_acdb_perdevice_rx_capmask_v2586 as v2586
import native_audio_acdb_perdevice_indirect_capture_live_handoff_v2573 as v2573

ROOT = v2586.ROOT
RUN_ID = "V2587"
BUILD_TAG = "v2587-audio-acdb-perdevice-rx-capmask-live"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2587_AUDIO_ACDB_PERDEVICE_RX_CAPMASK_RUNNER_2026-06-16.md"
EXACT_GATE = (
    "AUD-ACDB-V2587-perdevice-rx-capmask go: one-shot send_audio_cal_v5 arg2=1 "
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2587-acdb-perdevice-rx-capmask-{stamp}"
...
def build_v2586_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        build_root=args.v2586_build_root,
        readelf=args.readelf,
        file=args.file,
        clang=args.clang,
        lld=args.lld,
    )
    payload = v2586.make_payload(build_args)
    args.v2586_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2586_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2586_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    return {
        ...
        "helper": helper,
        "preload": preload,
        "arg2_one_contract": "send_audio_cal_v5(15, 1," in str(contract.get("per_device_call", "")),
        "source_override_ok": bool(sources.get("required", {}).get("preinit_rx_path_compile_override_guard")),
        "capture_contract": contract,
    }


def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2586_artifacts:
        build_v2586_artifacts(args)
    manifest = read_v2586_manifest(args.v2586_manifest_path)
    ...
    return {
        ...
        "ok": bool(
            manifest.get("ok")
            and manifest.get("arg2_one_contract")
            and manifest.get("source_override_ok")
            and helper.get("ok")
            and preload.get("ok")
        ),
    }
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2586 arg2=1 helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    if args.exact_gate != EXACT_GATE:
        raise RuntimeError("exact V2587 live gate mismatch")
    if args.out_dir is None:
        args.out_dir = default_live_out_dir()
    dry = dry_run_payload(args)
    if not dry.get("ok"):
        raise RuntimeError(f"V2587 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2586_artifacts"]
    ...
    per_device_summary = v2573.summarize_perdevice_indirect_capture(pulled_dir) if pulled_dir else {
        "classification": "v2587-no-pulled-artifacts",
        "full_success": False,
        "per_device_success": False,
        "partial_success": False,
    }
    classification = str(per_device_summary.get("classification", "unknown")).replace("v2573-", "v2587-")
    wrapper = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
    ...
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2587-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    artifacts = payload.get("v2586_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2586-artifacts", action="store_true")
    parser.add_argument("--v2586-build-root", type=Path, default=v2586.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2586-manifest-path", type=Path, default=v2586.DEFAULT_MANIFEST)
    ...
    parser.add_argument("--clang", type=Path, default=v2586.v2572.TOOLCHAIN_ROOT / "bin/clang")
    parser.add_argument("--lld", type=Path, default=v2586.v2572.TOOLCHAIN_ROOT / "bin/ld.lld")
