#!/usr/bin/env python3
"""Build V3363 native-init boot image adding the hot-reload (`reload`) command.
...
import build_native_init_boot_v3360_self_dd_f3_self_rollback as previous

base = previous.base
ORIG_PREVIOUS_REWRITE_TEXT = previous._rewrite_v3360_text
ORIG_PREVIOUS_ADAPTER_SOURCE = previous.v3360_adapter_source
ORIG_PREVIOUS_BOOT_AUDIT_MANIFEST = previous._boot_audit_manifest
ORIG_PREVIOUS_OVERLAY = previous._overlay_preserved_v3360_ramdisk
ORIG_PREVIOUS_FINALIZE = previous._finalize_manifest_after_overlay
ORIG_PREVIOUS_POSTPROCESS = previous._postprocess_manifest

CYCLE = "V3363"
INIT_VERSION = "0.11.124"
INIT_BUILD = "v3363-init-hot-reload"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3363_INIT_HOT_RELOAD_SOURCE_BUILD_2026-07-02.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3363_init_hot_reload.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BASE_BOOT
INIT_BINARY = OUT_DIR / "init_v3363_init_hot_reload"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3363_init_hot_reload.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v629_init_hot_reload"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3363"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3363.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3363.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3363"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3363-init-hot-reload"

FRAME_PATH = "/tmp/a90-doomgeneric-v3363-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3363-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3363-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3363-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3363-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3363-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3363-sfx.pcmstream"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3363.c"
...
def _rewrite_v3363_text(text: str) -> str:
    text = ORIG_PREVIOUS_REWRITE_TEXT(text)
    replacements = (
        (previous.INIT_VERSION, INIT_VERSION),
        (previous.INIT_BUILD, INIT_BUILD),
        ("0.11.123", INIT_VERSION),
        ("v3360", "v3363"),
        ("V3360", "V3363"),
        ("a90-doomgeneric-v3360", "a90-doomgeneric-v3363"),
        ("a90.doomgeneric.v3360", "a90.doomgeneric.v3363"),
    )
...
def _rewrite_v3363_bytes(item: bytes) -> bytes:
    return _rewrite_v3363_text(item.decode("utf-8")).encode("utf-8")


FRAME_SCALE = _rewrite_v3363_text(previous.FRAME_SCALE)
FRAME_IPC = _rewrite_v3363_text(previous.FRAME_IPC)
SFX_STREAM_MARKER = _rewrite_v3363_text(previous.SFX_STREAM_MARKER)
SOUND_MODE = _rewrite_v3363_text(previous.SOUND_MODE)

PREVIOUS_REQUIRED_STRINGS = tuple(_rewrite_v3363_bytes(item) for item in previous.REQUIRED_STRINGS)

REQUIRED_STRINGS = PREVIOUS_REQUIRED_STRINGS + (
    b"0.11.124",
    b"v3363-init-hot-reload",
    b"A90RELOAD",
...
def render_report(
    ...
    return "\n".join([
        ...
        "## Validation Contract",
        "",
        "- Static PASS requires the V3363 version strings plus the reload markers (`A90RELOAD`, "
        "`INIT-RELOAD-EXECVE`, usage) to be present while preserving the prior command surface.",
        "- Live H0 PASS, separately gated, will require: flash V3363, stage its own init ELF, "
...
def v3363_adapter_source() -> str:
    return _rewrite_v3363_text(ORIG_PREVIOUS_ADAPTER_SOURCE())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "init-hot-reload.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "init-hot-reload",
...
def _normalize_manifest_for_v3363(manifest: dict[str, Any]) -> dict[str, Any]:
    manifest.update({
        "decision": DECISION,
        "cycle": CYCLE,
        "candidate_tag": INIT_BUILD,
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest = _normalize_manifest_for_v3363(json.loads(manifest_path.read_text(encoding="utf-8")))
    if base_main_error:
...
def _postprocess_manifest() -> dict[str, Any]:
    manifest = _normalize_manifest_for_v3363(ORIG_PREVIOUS_POSTPROCESS())
    manifest_path = OUT_DIR / "manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    REPORT_PATH.write_text(
        render_report(
...
def _overlay_preserved_v3363_ramdisk() -> dict[str, Any]:
    overlay = ORIG_PREVIOUS_OVERLAY()
    overlay["mode"] = "preserve-v3335-ramdisk-overlay-v3363-init-helper-engine"
    return overlay


def _patch_v3360_module_for_v3363() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "v3360_adapter_source": v3363_adapter_source,
        "_rewrite_v3360_text": _rewrite_v3363_text,
        "_rewrite_v3360_bytes": _rewrite_v3363_bytes,
        "_boot_audit_manifest": _boot_audit_manifest,
        "_write_candidate_manifest": _write_candidate_manifest,
        "_overlay_preserved_v3360_ramdisk": _overlay_preserved_v3363_ramdisk,
