#!/usr/bin/env python3
"""V2665 Android handoff wrapper for V2664 common-only ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2664 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2664
...
The own-process send path is the V2664 common-only helper:

    acdb_loader_init_v3("/vendor/etc/audconf/OPEN", delta, meta_head)
      -> a90_arm_capture()
      -> acdb_loader_send_common_custom_topology()

The V2664 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
SET records needed after V2648: cal_types 10, 14, and 24. Cal_type 25 is useful
...
import build_android_acdb_common_only_setcal_capture_v2664 as v2664
...
ROOT = v2664.ROOT
RUN_ID = "V2665"
BUILD_TAG = "v2665-audio-acdb-common-only-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2665_AUDIO_ACDB_COMMON_ONLY_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"

# Gate-4 custom topology records needed by the downstream ADM/ASM/AFE open path.
CUSTOM_TOPOLOGY_CAL_TYPES = {10, 14, 24}
SUPPLEMENTAL_CAL_TYPES = {25}
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2665-acdb-common-only-setcal-capture-{stamp}"
...
def build_v2664_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2664_build_root,
        manifest=args.v2664_manifest_path,
        report=v2664.DEFAULT_REPORT,
        clang=v2664.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2664.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    payload = v2664.make_payload(build_args)
    args.v2664_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2664_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2664_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    setcal_contract_ok = bool(
        ...
        and required.get("helper_skips_send_audio_cal_v5")
        and required.get("helper_arms_after_init")
        and required.get("helper_calls_common_after_arm")
        and required.get("phase_hook_init_short_success")
        and required.get("phase_hook_calls_real_common_postinit")
        and required.get("phase_hook_neutralizes_reentry")
        and required.get("phase_hook_patches_init_flag")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2664_artifacts:
        build_v2664_artifacts(args)
    manifest = read_v2664_manifest(args.v2664_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2665-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2665-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    phase_path = artifact_dir / "acdb-v2659-phase-common-events.jsonl"
    setcal_events = [
    ...
    phase_stages = [
        ...
        if event.get("event") == "v2659_phase_common"
    ]
    ...
    phase_common_return_codes = [code for code in phase_common_return_codes if code is not None]
    ...
    if real_set_events:
        classification = "v2665-boundary-violation-real-audio-set-passthrough"
    elif reentry_neutralized and not rows:
        classification = "v2665-phase-common-reentry-neutralized-no-setcal"
    elif postinit_real_common_called and not rows:
        classification = "v2665-postinit-real-common-returned-before-setcal-no-setcal"
    elif init_short_success and not rows:
        classification = "v2665-init-short-success-sigsegv-before-postinit-common-no-setcal"
    elif not rows:
        classification = f"v2665-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2665-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2665-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2665-setcal-records-no-custom-topology"
    else:
        classification = "v2665-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2665-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2665-custom-topology-setcal-partial",
            "v2665-setcal-records-no-custom-topology",
            "v2665-phase-common-reentry-neutralized-no-setcal",
            "v2665-postinit-real-common-returned-before-setcal-no-setcal",
            "v2665-init-short-success-sigsegv-before-postinit-common-no-setcal",
        },
        "operator_valuable": bool(rows or init_short_success or postinit_real_common_called or reentry_neutralized),
        "counts_toward_fails_twice": (
            False
            if init_short_success or postinit_real_common_called or reentry_neutralized
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2665-acdb-common-only-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2664_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "send_path": "init_v3 -> a90_arm_capture -> send_common_custom_topology() only",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2664 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2665 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2664_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2665-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2664_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
        "# NATIVE_INIT V2665 — ACDB common-only SET capture live handoff",
        "",
        "Date: 2026-06-18",
        "",
        "## Scope",
        ...
        "boot/stage/pull/rollback engine and the V2664 helper/preload artifacts. This is",
        "measurement-only: the V2664 ioctl shim always fake-successes",
        ...
        f"- phase_common_return_codes: `{summary.get('phase_common_return_codes')}`",
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2664 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2665-init-short-success-sigsegv-before-postinit-common-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2664 init-short phase succeeded:",
    ...
    if summary.get("classification") == "v2665-phase-common-reentry-neutralized-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2664 phase hook neutralized nested common-topology",
    ...
    if summary.get("classification") == "v2665-postinit-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the corrected V2664 preload reached the real common-topology",
            ...
            "- this is useful frontier evidence: V2664 passed the init-short phase and reached",
    ...
    lines.extend([
        ...
        "## Contract",
        "",
        "- stages the V2664 helper/preload through the V2490 Android-good handoff engine;",
        ...
        "- runs only `acdb_loader_send_common_custom_topology()` after init so the",
        "  missing common custom-topology SET ioctls fire and are intercepted;",
        ...
        "## Validation",
        "",
        "- `python3 -m py_compile workspace/public/src/scripts/revalidation/native_audio_acdb_common_only_setcal_capture_live_handoff_v2665.py tests/test_native_audio_acdb_common_only_setcal_capture_live_handoff_v2665.py`",
        "- `PYTHONPATH=tests:workspace/public/src/scripts/revalidation python3 -m unittest tests.test_native_audio_acdb_common_only_setcal_capture_live_handoff_v2665 -v`",
        "- `PYTHONPATH=workspace/public/src/scripts/revalidation python3 workspace/public/src/scripts/revalidation/native_audio_acdb_common_only_setcal_capture_live_handoff_v2665.py --dry-run --write-report`",
        "- live run, if present: `PYTHONPATH=workspace/public/src/scripts/revalidation python3 workspace/public/src/scripts/revalidation/native_audio_acdb_common_only_setcal_capture_live_handoff_v2665.py --run-live --write-report`",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2664-artifacts", action="store_true")
    parser.add_argument("--v2664-build-root", type=Path, default=v2664.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2664-manifest-path", type=Path, default=v2664.DEFAULT_MANIFEST)
