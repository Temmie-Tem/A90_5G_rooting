#!/usr/bin/env python3
"""V2229 rollbackable handoff runner for the V2228 service-object-visible observer boot.

Default mode is host-only dry-run. The live path flashes V2228, collects the
helper-owned service-object-visible result, runs the V2220 parser over the
collected artifacts, then rolls back to V2189 and verifies selftest fail=0.
"""
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2229_SERVICE_OBJECT_VISIBLE_HANDOFF_RUNNER_2026-06-12.md"
)
V2228_BUILD_DIR = REPO_ROOT / "workspace" / "private" / "builds" / "native-init" / "v2228-service-object-visible-observer"
V2228_MANIFEST = V2228_BUILD_DIR / "manifest.json"
TEST_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v2228_service_object_visible_observer.img",
...
TEST_EXPECT_VERSION = "A90 Linux init 0.9.264 (v2228-service-object-visible-observer)"
ROLLBACK_EXPECT_VERSION = "A90 Linux init 0.9.261 (v2189-security-p0-stage-fix)"
HELPER_REMOTE_PATHS = {
    "helper_result": "/cache/native-init-wifi-test-boot-v2228-helper.result",
    "summary": "/cache/native-init-wifi-test-boot-v2228.summary",
    "log": "/cache/native-init-wifi-test-boot-v2228.log",
}
REQUIRED_CONFIRM = "I_UNDERSTAND_V2229_FLASHES_V2228_AND_ROLLS_BACK_TO_V2189"
REPORT_KEY_EVENTS = [
    "uprobe:wlfw_start",
...
def load_build_manifest() -> dict[str, Any]:
    if not V2228_MANIFEST.exists():
        return {}
    return read_bounded_json(V2228_MANIFEST)
...
def preflight() -> dict[str, Any]:
    ...
    return {
        "v2228_manifest": rel(V2228_MANIFEST),
        "v2228_manifest_exists": V2228_MANIFEST.exists(),
        "v2228_manifest_pass": bool(build_manifest.get("pass")),
...
def run_parser(store: EvidenceStore, steps: list[dict[str, Any]], inputs: list[Path]) -> dict[str, Any]:
    ...
    command: list[object] = [
        ...
        parser_out,
        "--label",
        "v2229-service-object-visible-helper",
        "--allow-nohit",
    ]
    for path in inputs:
...
def classify(manifest: dict[str, Any]) -> dict[str, Any]:
    if not manifest.get("execute"):
        ...
        ok = all([
            pre.get("v2228_manifest_pass"),
            pre.get("test_image_exists"),
            pre.get("test_image_sha_matches_manifest"),
            pre.get("rollback_image_exists"),
        ])
        return {
            "decision": "v2229-service-object-visible-handoff-dry-run-ready" if ok else "v2229-service-object-visible-handoff-dry-run-blocked",
            "pass": bool(ok),
            "reason": "host-only flash/capture/rollback plan is complete; live execution still requires explicit approval" if ok else "required boot image or V2228 build manifest is missing/mismatched",
        }
    if manifest.get("live_block") == "v2222-preflight-failed":
        return {
            "decision": "v2229-service-object-visible-handoff-preflight-failed-no-flash",
            "pass": False,
            "reason": "V2222 preflight failed before the test boot flash, so no live service-object-visible capture was attempted",
        }
    ...
    if not rollback_result.get("selftest_ok"):
        return {
            "decision": "v2229-service-object-visible-handoff-rollback-selftest-failed",
            "pass": False,
            "reason": "rollback did not finish with selftest fail=0",
        }
    if manifest.get("live_block") == "test-flash-failed":
        return {
            "decision": "v2229-service-object-visible-handoff-test-flash-failed-rollback-pass",
            "pass": False,
            "reason": "V2228 test boot flash failed, but rollback selftest fail=0 passed",
        }
    diagnosis = collect.get("diagnosis") or {}
    if diagnosis.get("kind") == "property-root-missing":
        return {
            "decision": "v2229-service-object-visible-helper-property-root-missing-rollback-pass",
            "pass": False,
            "reason": "V2228 booted and rollback passed, but the helper exited before tracing because its configured property root was missing",
        }
    ...
    return {
        "decision": "v2229-service-object-visible-helper-parse-incomplete-rollback-pass",
        "pass": False,
    ...
    if manifest.get("execute"):
        ...
        if diagnosis.get("kind") == "property-root-missing":
            lines.extend([
                "## Live Diagnosis",
                "",
                "- The V2228 helper did not reach WLFW/CNSS trace collection.",
                "- Recovered helper result shows `helper_status=setup-error` and `setup_error=lstat property root: No such file or directory`.",
                "- Root cause: the configured V2228 property root was missing on the device.",
                "- Next unit: inspect the V2228 property-root staging before repeating the service-object-visible run.",
                "",
            ])
            ...
            if wlfw_request_hits and cap_hits and bdf_hits and str(diagnosis.get("wlan0_present")) != "1":
                service_object = diagnosis.get("service_object_visible") or {}
                lines.extend([
                    "## Live Diagnosis",
                    "",
                ...
                if nonlog.get("label") == "peripheral-default-service-manager-call-no-return":
                    lines.extend([
                        "## Live Diagnosis",
                        "",
                        "- V2228 kept the property-root setup fix and the helper completed normally.",
                        ...
                        "- This branch would mean the V2228 service-object-visible route still failed before service-name/get-service despite starting service-manager/PM components.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", default="v2229-service-object-visible-handoff")
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "safety": {
            ...
            "partition_write_scope": "boot image flash to V2228 and rollback to V2189 only" if args.execute else "none",
        },
    }
    ...
    if not args.execute:
        ...
        manifest["result"] = {
            "decision": "v2229-service-object-visible-handoff-live-confirmation-missing",
            "pass": False,
        ...
        if current_window_a90_absent:
            manifest["preflight_warning"] = {
                "decision": "v2229-current-window-a90-events-absent-nonblocking",
                "reason": "V2222 current-window collector found no active a90 events after rollback; V2228 test boot is expected to recreate the helper-owned boot-window events.",
            }
        elif current_window_collector_busy:
            manifest["preflight_warning"] = {
                "decision": "v2229-current-window-collector-busy-nonblocking",
                "reason": "V2222 bridge/status/version/helpers checks passed with fail=0, but the optional current-window collector hit auto-menu busy before the test boot; V2228 test boot is expected to create fresh helper-owned boot-window events.",
            }
        if not v2222.get("ok") and not (current_window_a90_absent or current_window_collector_busy):
            ...
            write_step(store, steps, "flash-v2228-from-native", test_flash)
