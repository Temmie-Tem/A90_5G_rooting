#!/usr/bin/env python3
"""Build V3240 GPU H3 Sample Location probe."""
...
CYCLE = "V3240"
INIT_VERSION = "0.11.47"
INIT_BUILD = "v3240-gpu-h3-sample-location-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3240_GPU_H3_SAMPLE_LOCATION_SOURCE_BUILD_2026-06-25.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3240_gpu_h3_sample_location_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3240_gpu_h3_sample_location_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3240_gpu_h3_sample_location_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v578_gpu_h3_sample_location_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3240"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3240.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3240.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3240"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3240-gpu-h3-sample-location-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3240-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3240-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3240-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3240-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3240-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3240-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3240-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-sample-location-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-sample-location-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3240")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3240")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3240")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3240")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3240")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3240")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3240")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3240")
SFX_STREAM_MARKER = "a90.doomgeneric.v3240.audio=real-sfx-pcm-stream-gpu-h3-sample-location-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-sample-location-probe-v3240"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3240.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3240_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3232": b"a90-doomgeneric-v3240",
        b"a90.doomgeneric.v3232": b"a90.doomgeneric.v3240",
        b"v3232": b"v3240",
        b"V3232": b"V3240",
        b"first-triangle-h3-static-context-mov-f32-shader":
            b"first-triangle-h3-sample-location-disable-r1-footprint2-mov-f32-shader",
...
GPU_H3_SAMPLE_LOCATION_MARKERS = (
    ...
    b"gpu.h3.draw.scope=first-triangle-h3-sample-location-disable-r1-footprint2-mov-f32-shader",
...
REQUIRED_STRINGS = tuple(_rewrite_v3240_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3238-fresh-boot-fwclass-materialize-prep-plus-v3236-r1-footprint2-h3-candidate-on-v3232-ramdisk-base",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-sample-location-disable-r1-footprint2-mov-f32-shader",
        ...
        "state_reg_writes_expected": 91,
        "pm4_dwords_expected": 229,
        ...
        "next_live_validation": [
            "flash-v3240-through-native-init-flash",
            "post-flash-health-check",
            "verify-fresh-boot-fwclass-path-before-h3",
            "gpu-h3-sample-location-disable-r1-footprint2-mov-f32-shader-timeout-guard",
...
def v3240_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-sample-location-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-sample-location-probe")
        .replace("v3208", "v3240")
        .replace("V3208", "V3240")
    )


def _overlay_preserved_v3240_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3232 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3240 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3240 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3240 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3240-overlay-") as temp_name:
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3240 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3240 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3232-ramdisk-overlay-v3240-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-sample-location-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-sample-location-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-sample-location-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3232-ramdisk-overlay-v3240-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-sample-location-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-sample-location-live-validation"
...
def _apply_v3240_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-sample-location-probe")
        ...
        "v3210_adapter_source": v3240_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3240_ramdisk,
