#!/usr/bin/env python3
"""Build V3312 GPU 2D D3 video texture present probe."""
...
import build_native_init_boot_v3311_gpu_2d_d2_realframe_texture_probe as previous

base = previous.base

CYCLE = "V3312"
INIT_VERSION = "0.11.84"
INIT_BUILD = "v3312-gpu-2d-d3-video-texture-present-probe"
BUILD_TAG = INIT_BUILD
DECISION = "v3312-gpu-2d-d3-video-texture-present-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3312_GPU_2D_D3_VIDEO_TEXTURE_PRESENT_SOURCE_BUILD_2026-06-27.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3312_gpu_2d_d3_video_texture_present_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3312_gpu_2d_d3_video_texture_present_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3312_gpu_2d_d3_video_texture_present_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v608_gpu_2d_d3_video_texture_present_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3312"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3312.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3312.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3312"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3312-gpu-2d-d3-video-texture-present-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3312-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3312-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3312-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3312-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3312-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3312-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3312-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-2d-d3-video-texture-present-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-2d-d3-video-texture-present-probe"

SFX_STREAM_MARKER = "a90.doomgeneric.v3312.audio=real-sfx-pcm-stream-gpu-2d-d3-video-texture-present-probe"
SOUND_MODE = "native-doom-sfx-gpu-2d-d3-video-texture-present-probe-v3312"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3312.c"
...
D3_PRESET_SHA256 = previous.D2_PRESET_SHA256


def _rewrite_v3312_text(text: str) -> str:
    replacements = (
        ...
        ("a90-doomgeneric-v3311", "a90-doomgeneric-v3312"),
        ("a90.doomgeneric.v3311", "a90.doomgeneric.v3312"),
        ("v3311", "v3312"),
        ("V3311", "V3312"),
        ("gpu-2d-d2-realframe-texture-probe", "gpu-2d-d3-video-texture-present-probe"),
    )
...
def _rewrite_v3312_bytes(item: bytes) -> bytes:
    return _rewrite_v3312_text(item.decode("utf-8")).encode("utf-8")
...
REQUIRED_STRINGS = tuple(_rewrite_v3312_bytes(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_d3_manifest() -> dict[str, Any]:
    return {
        ...
        "scope": SCOPE,
        "command": D3_COMMAND,
        "candidate_type": "gpu-2d-d3-video-texture-present-probe-candidate",
        ...
        "next_live_validation": [
            "flash-v3312-through-native-init-flash",
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3312 GPU 2D D3 Video Texture Present Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "## Validation",
        "",
        "- `py_compile`: V3312 builder and focused source test.",
        "- `unittest`: V3312 D3 source contract plus V3311 D2 baseline coverage.",
        "- Build: AArch64 helper/native-init compile, preserved-ramdisk overlay, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3312 identity plus D3 video texture present telemetry.",
        ...
        "- Candidate type: `gpu-2d-d3-video-texture-present-probe-candidate`.",
    ]) + "\n"


def v3312_adapter_source() -> str:
    return _rewrite_v3312_text(previous.v3311_adapter_source())


def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-2d-d3-video-texture-present-probe-candidate.json").write_text(json.dumps({
...
def _overlay_preserved_v3312_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3312 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3312 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3312 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3312 DOOM engine binary: {ENGINE_BINARY}")

    with tempfile.TemporaryDirectory(prefix="a90-v3312-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            ...
            raise RuntimeError("V3312 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3312 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3312 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3311-ramdisk-overlay-v3312-init-helper-engine",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    manifest["candidate_type"] = "gpu-2d-d3-video-texture-present-probe-candidate"
...
def _apply_v3312_overrides() -> None:
    previous._apply_v3311_overrides()
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": _rewrite_v3312_text(base.SFX_BACKEND_SOURCE_TEXT),
        "REQUIRED_STRINGS": REQUIRED_STRINGS,
        "render_report": render_report,
        "v3210_adapter_source": v3312_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3312_ramdisk,
