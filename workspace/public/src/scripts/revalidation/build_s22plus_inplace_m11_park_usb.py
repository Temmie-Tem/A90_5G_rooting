#!/usr/bin/env python3
"""Build the S22+ M11 park-USB USB-ACM native-init candidate.

Host-only. This script does not reboot, flash, or touch a connected device.

M11 keeps the freestanding raw-syscall PID1 runtime and pivots away from the
unreliable reboot-download beacon. The builder derives a park-based USB subset
from stock vendor_boot's modules.load.recovery + modules.dep, removes
watchdog/reset-prone and glink/EUD/debug-anomaly modules, injects only that
small text list into the boot ramdisk, and leaves all module binaries in stock
...
DEFAULT_OUT = Path("workspace/private/outputs/s22plus_native_init/inplace_m11_park_usb_v0_1")
DEFAULT_SOURCE = Path("workspace/public/src/native-init/s22plus_init_usb_acm_m11_park_usb.c")
...
MARKER = "S22_NATIVE_INIT_USB_ACM_M11"
...
RUNTIME_MODULES_PARK_USB_BUF = 8192
RUNTIME_MODULE_NAME_BUF = 128
MODULES_PARK_USB_RAMDISK = "s22plus_m11_park_usb.modules"
...
def is_blocked_module(name: str) -> bool:
    if name in EXPLICIT_M11_BLOCKLIST:
        return True
    lower = name.lower()
...
def derive_park_usb(recovery_lines: list[str], modules_dep_lines: list[str]) -> dict[str, object]:
    recovery_unique: list[str] = []
    seen_recovery: set[str] = set()
    for name in recovery_lines:
        if name not in seen_recovery:
            recovery_unique.append(name)
            seen_recovery.add(name)
    recovery_set = set(recovery_unique)
    dep_map = parse_modules_dep(modules_dep_lines)

    missing_recovery = sorted(module for module in REQUIRED_USB_MODULES if module not in recovery_set)
    missing_dep = sorted(module for module in REQUIRED_USB_MODULES if module not in dep_map)
    if missing_recovery:
        raise SystemExit(f"required USB module missing from modules.load.recovery: {missing_recovery}")
    if missing_dep:
        raise SystemExit(f"required USB module missing from modules.dep: {missing_dep}")

    closure: set[str] = set()
    stack = list(REQUIRED_USB_MODULES)
    while stack:
        name = stack.pop()
        if name in closure:
            continue
        ...
        for dep_name in dep_map.get(name, []):
            if dep_name not in dep_map:
                raise SystemExit(f"dependency missing from modules.dep: {name} -> {dep_name}")
            if dep_name not in closure:
                stack.append(dep_name)

    closure_not_recovery = sorted(closure - recovery_set)
    if closure_not_recovery:
        raise SystemExit(f"USB dependency closure is not wholly in recovery order: {closure_not_recovery[:10]}")

    blocked_from_closure = sorted(name for name in closure if is_blocked_module(name))
    blocked_explicit_present = sorted(name for name in closure if name in EXPLICIT_M11_BLOCKLIST)
    ordered = [name for name in recovery_unique if name in closure and not is_blocked_module(name)]
    if not ordered:
        raise SystemExit("derived M11 park USB is empty")
    if any(name in EXPLICIT_M11_BLOCKLIST for name in ordered):
        raise SystemExit("explicitly blocked module survived M11 park USB blocklist")
    if any("audio" in name.lower() or "snd" in name.lower() for name in ordered):
        raise SystemExit("audio module survived M11 park USB blocklist")
    missing_after_block = sorted(module for module in REQUIRED_USB_MODULES if module not in ordered)
    if missing_after_block:
        raise SystemExit(f"blocklist removed a required USB module: {missing_after_block}")

    subset_text = "".join(f"{name}\n" for name in ordered)
    if len(subset_text.encode("utf-8")) >= RUNTIME_MODULES_PARK_USB_BUF:
        raise SystemExit(
            "M11 park USB does not fit runtime parser buffer: "
            f"{len(subset_text.encode('utf-8'))} >= {RUNTIME_MODULES_PARK_USB_BUF}"
        )
    too_long = [name for name in ordered if len(name) >= RUNTIME_MODULE_NAME_BUF]
    if too_long:
        raise SystemExit(f"M11 park USB basename exceeds runtime parser buffer: {too_long[:5]}")

    return {
        "required_usb_modules": REQUIRED_USB_MODULES,
        "required_usb_positions": {module: recovery_lines.index(module) + 1 for module in REQUIRED_USB_MODULES},
        "dependency_closure_count": len(closure),
        "dependency_closure": sorted(closure),
        "blocked_from_closure": blocked_from_closure,
        "blocked_explicit_present_in_closure": blocked_explicit_present,
        "explicit_m11_blocklist": sorted(EXPLICIT_M11_BLOCKLIST),
        ...
        "runtime_modules_park_usb_buffer": RUNTIME_MODULES_PARK_USB_BUF,
...
def extract_vendor_ramdisk_metadata(vendor_ramdisk: Path, lz4_tool: Path, build_dir: Path) -> dict[str, object]:
    ...
    park_usb = derive_park_usb(recovery_lines, modules_dep_lines)

    missing_ko = sorted(module for module in REQUIRED_USB_MODULES if module not in ko_names)
    missing_recovery = sorted(module for module in REQUIRED_USB_MODULES if module not in recovery_lines)
    if missing_ko:
        raise SystemExit(f"required vendor .ko missing from vendor ramdisk: {missing_ko}")
    if missing_recovery:
        raise SystemExit(f"required module missing from modules.load.recovery: {missing_recovery}")
    ...
    if recovery_too_long:
        raise SystemExit(
            "modules.load.recovery basename exceeds M11 runtime parser buffer: "
    ...
    positions = {module: recovery_lines.index(module) + 1 for module in REQUIRED_USB_MODULES}
    ...
    (build_dir / MODULES_PARK_USB_RAMDISK).write_text(str(park_usb["subset_text"]), encoding="ascii")
    (build_dir / "m11_park_usb.txt").write_text(str(park_usb["subset_text"]), encoding="ascii")
    return {
        ...
        "m11_park_usb": {key: value for key, value in park_usb.items() if key != "subset_text"},
...
def compile_init(source: Path, out_path: Path, build_dir: Path) -> dict[str, str | list[str]]:
    ...
    require_ok(result, "compile M11 park-usb init")
    strip = run(["aarch64-linux-gnu-strip", "-s", out_path])
    require_ok(strip, "strip M11 park-usb init")

    file_info = run(["file", out_path])
    require_ok(file_info, "file M11 park-usb init")
    readelf = run(["aarch64-linux-gnu-readelf", "-h", "-l", out_path])
    require_ok(readelf, "readelf M11 park-usb init")
    objdump = run(["aarch64-linux-gnu-objdump", "-d", out_path])
    require_ok(objdump, "objdump M11 park-usb init")
    ...
    if "INTERP" in readelf_text or "Requesting program interpreter" in readelf_text:
        raise SystemExit("M11 init unexpectedly has a program interpreter")
    if "AArch64" not in readelf_text:
        raise SystemExit("M11 init is not AArch64")
    if "svc" not in objdump_text:
        raise SystemExit("M11 init disassembly does not contain svc")
    ...
    for required in required_strings:
        if required.encode("ascii") not in binary:
            raise SystemExit(f"required marker missing from M11 /init: {required}")
    reboot_nr_lines = [
        line
    ...
    if reboot_nr_lines:
        raise SystemExit("M11 init unexpectedly contains arm64 __NR_reboot (142)")
    for forbidden in (b"ld-linux", b"libc.so", b"/vendor_dlkm", b"s22plus-m5", b"modules.load.recovery", b"download"):
        if forbidden in binary:
            raise SystemExit(f"M11 /init contains forbidden string: {forbidden!r}")

    (build_dir / "m11_init_file.txt").write_bytes(file_info.stdout + file_info.stderr)
    (build_dir / "m11_init_readelf.txt").write_text(readelf_text, encoding="utf-8")
    (build_dir / "m11_init_objdump.txt").write_text(objdump_text, encoding="utf-8")
    return {
...
def main(argv: list[str]) -> int:
    ...
    m11_init = build_dir / "s22plus_init_usb_acm_m11"
    m11_init_info = compile_init(source, m11_init, build_dir)
    ...
    subset_file = build_dir / MODULES_PARK_USB_RAMDISK
    if not subset_file.exists():
        raise SystemExit(f"M11 subset file missing: {subset_file}")
    patch_init_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 750 init {m11_init}"],
        work_dir,
        "replace /init with M11 init",
    )
    patch_subset_text = run_in_dir(
        [magiskboot, "cpio", ramdisk, f"add 640 {MODULES_PARK_USB_RAMDISK} {subset_file}"],
        work_dir,
        "add M11 park USB list",
    )
    ...
    if cpio_test_after not in (1, 2):
        raise SystemExit(f"unexpected ramdisk cpio test rc after M11 patch: {cpio_test_after}")
    ...
    if sha256_file(extracted_replaced) != sha256_file(m11_init):
        raise SystemExit("replaced /init does not match compiled M11 init")
    extracted_subset = build_dir / MODULES_PARK_USB_RAMDISK
    run_in_dir(
        [magiskboot, "cpio", ramdisk, f"extract {MODULES_PARK_USB_RAMDISK} {extracted_subset}.extracted"],
        work_dir,
        "extract M11 park USB list",
    )
    if sha256_file(Path(f"{extracted_subset}.extracted")) != sha256_file(subset_file):
        raise SystemExit("replaced M11 park USB list does not match builder output")
    ...
    sizes = {
        ...
        "m11_park_usb": subset_file.stat().st_size,
        "m11_init": m11_init.stat().st_size,
    ...
    manifest = {
        ...
        "purpose": "M11 in-place park-based USB-ACM native-init using vendor_boot recovery-order USB subset minus reset-prone modules",
        "safety": {
            "boot_only": True,
            ...
            "module_list_path": f"/{MODULES_PARK_USB_RAMDISK}",
            "module_subset": "modules.dep closure of M11 USB/platform/Max77705 seeds in recovery order, with debug/glink/eud/watchdog/audio/display/WLAN/etc blocklist",
            ...
            "watchdog": "not-touched-by-init-source; watchdog modules blocklisted from M11 subset",
        ...
        "ramdisk": {
            ...
            "added_subset_entry": MODULES_PARK_USB_RAMDISK,
