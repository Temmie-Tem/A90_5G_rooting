#!/usr/bin/env python3
"""Build V2796 native-init audio ION devnode boot image.

V2796 materializes `/dev/ion` from `/sys/class/misc/ion/dev` immediately before
SET-cal ION allocation. V2795 proved the integrated native `audio play --execute`
path reached the execute stage, but failed because `/dev/ion` was absent even
though the kernel misc device was registered.
"""
...
CYCLE = "V2796"
INIT_VERSION = "0.9.309"
INIT_BUILD = "v2796-audio-ion-devnode"
BUILD_TAG = "v2796-audio-ion-devnode"
DECISION = "v2796-audio-ion-devnode-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2796_AUDIO_ION_DEVNODE_SOURCE_BUILD_2026-06-19.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2796_audio_ion_devnode.img", legacy_fallback=False
)
BASE_BOOT = workspace_private_input_path(
    "boot_images", "boot_linux_v2334_audio_snd_nodes_preflight.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2796_audio_ion_devnode"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2796_audio_ion_devnode.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v445_audio_ion_devnode"
...
def configure() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--wifi-test-klog-prefix": "A90v2796",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2796.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2796.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2796.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2796-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2796.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2796-supervisor.pid",
    }
...
def render_report(manifest: dict[str, object], helper_flags: tuple[str, ...], init_extra_flags: tuple[str, ...]) -> str:
    return "\n".join([
        "# Native Init V2796 Audio ION Devnode Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "## Purpose",
        "",
        "- Compiles the V2796 source change that creates `/dev/ion` from `/sys/class/misc/ion/dev` just before the SET-cal executor opens ION.",
        "- Keeps the V2795 direct execute behavior: integrated play verifies the SET-cal manifest without the extra preload pass, then the execute stage owns payload loading, ION allocation, `/dev/msm_audio_cal` ioctl progress, route apply, bounded PCM, route reset, and SET deallocate.",
        ...
        "- The next live unit must flash this image, deploy the private SET-cal manifest bundle at `/cache/a90-acdb-setcal-replay-v2725`, run `audio play --execute`, confirm `/dev/ion` materialization, poll `audio play-status`, confirm audible sound if playback completes, and rollback to `v2321`.",
        "",
        "## Metadata",
        "",
...
def main() -> int:
    ...
    manifest.update({
        ...
        "parent_baseline": "v2795-audio-setcal-direct-execute",
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "deeper_fallback_baseline": "v2237-supplicant-terminate-poll",
        "audio_ion_devnode_source_validation": {
            ...
            "v2795_blocker": "audio.setcal.execute.ion.open_ok=0 errno=2",
    ...
    (OUT_DIR / "promotion-candidate.json").write_text(json.dumps({
        ...
        "rollback_baseline": "v2321-usb-clean-identity-rodata",
        "note": "V2796 is a test image for integrated native audio playback, not a promoted rollback baseline.",
