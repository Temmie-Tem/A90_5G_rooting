#!/usr/bin/env python3
"""Build V3092 native-init DOOM wall-clock tick candidate."""
...
CYCLE = "V3092"
INIT_VERSION = "0.10.102"
INIT_BUILD = "v3092-doomgeneric-wall-clock-ticks"
BUILD_TAG = INIT_BUILD
DECISION = "v3092-doomgeneric-wall-clock-ticks-source-build-pass"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3092_DOOMGENERIC_WALL_CLOCK_TICKS_SOURCE_BUILD_2026-06-23.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v3092_doomgeneric_wall_clock_ticks.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v3092_doomgeneric_wall_clock_ticks"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3092_doomgeneric_wall_clock_ticks.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v514_doomgeneric_wall_clock_ticks"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3092"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3092.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3092.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3092"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3092-wall-clock-ticks"
...
FRAME_PATH = "/tmp/a90-doomgeneric-v3092-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3092-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3092-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3092-input.sock"
INPUT_UDP_PORT = v3086.INPUT_UDP_PORT
DEVICE_NCM_HOST = v3086.DEVICE_NCM_HOST
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3092-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3092-tick-telemetry.txt"
...
NATIVE_DASHBOARD_LARGE_FRAME = v3086.NATIVE_DASHBOARD_LARGE_FRAME
NATIVE_DOOM_PRESENT_PAGEFLIP = v3086.NATIVE_DOOM_PRESENT_PAGEFLIP
FRAME_SCALE = v3086.FRAME_SCALE
SCALE_PATH = v3086.SCALE_PATH
...
TICK_TELEMETRY_MARKER = "a90.doomgeneric.v3092.tick_telemetry=wall-clock-time-vs-game-tic-summary"
WALL_CLOCK_MARKER = "a90.doomgeneric.v3092.time_model=clock-monotonic-elapsed"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.102 (v3092-doomgeneric-wall-clock-ticks)",
    b"v3092-doomgeneric-wall-clock-ticks",
    b"doomgeneric-private-link-v3092-wall-clock-ticks",
    b"/bin/a90_doomgeneric_private_engine_v3092",
    ...
    WALL_CLOCK_MARKER.encode("ascii"),
...
def _replace_required(source: str, old: str, new: str) -> str:
    if old not in source:
        raise RuntimeError(f"missing V3092 adapter source anchor: {old[:96]!r}")
    return source.replace(old, new, 1)


def v3092_adapter_source() -> str:
    ...
    source = _replace_required(
        ...
        'const char a90_doomgeneric_v3081_frame_ipc_policy[] =\n'
        '    "a90.doomgeneric.v3081.frame_ipc=shared-mmap-seq";\n'
        'const char a90_doomgeneric_v3092_tick_telemetry_policy[] =\n'
        f'    "{TICK_TELEMETRY_MARKER}";\n'
        'const char a90_doomgeneric_v3092_wall_clock_policy[] =\n'
        f'    "{WALL_CLOCK_MARKER}";\n',
    )
    source = _replace_required(
        source,
        ...
        "static uint32_t fake_ticks_ms;\n"
        "static uint32_t wall_clock_base_ms;\n"
        "static int wall_clock_base_set;\n"
        ...
        "static uint32_t tick_telemetry_last_ticks_ms;\n",
    )
    source = _replace_required(
        source,
    ...
    source = _replace_required(
        ...
        "    frame_checksum = 0;\n",
        "    fake_ticks_ms = 0;\n"
        "    wall_clock_base_ms = a90_doomgeneric_monotonic_ms();\n"
        "    wall_clock_base_set = 1;\n"
        "    tick_telemetry_sleep_calls = 0;\n"
        "    tick_telemetry_sleep_ms_total = 0;\n"
        "    tick_telemetry_getticks_calls = 0;\n"
        "    tick_telemetry_last_ticks_ms = 0;\n"
        "    frame_checksum = 0;\n",
    )
    source = _replace_required(
        source,
        ...
        "}\n\n"
        "uint32_t DG_GetTicksMs(void) {\n"
        "    uint32_t now = a90_doomgeneric_monotonic_ms();\n"
        "\n"
        "    ++tick_telemetry_getticks_calls;\n"
        "    if (!wall_clock_base_set) {\n"
        "        wall_clock_base_ms = now;\n"
        "        wall_clock_base_set = 1;\n"
        "    }\n"
        "    tick_telemetry_last_ticks_ms = now - wall_clock_base_ms;\n"
        "    return tick_telemetry_last_ticks_ms;\n"
        "}\n",
    )
    source = _replace_required(
        source,
        ...
        "        marker_checksum(a90_doomgeneric_v3079_pace_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3081_frame_ipc_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3092_tick_telemetry_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3092_wall_clock_policy) == 0U) {\n",
    )
    source = _replace_required(
        source,
        "    unsigned char key = 0;\n\n"
    ...
    source = _replace_required(
        ...
        "    DG_SleepMs(16U);\n"
        "    DG_DrawFrame();\n"
        "    probe_ticks = DG_GetTicksMs();\n"
        "    if (probe_ticks < 1U || probe_ticks > 250U ||\n"
        "        a90_doomgeneric_presented_frames() != 1U) {\n"
        "        return 25;\n"
        "    }\n",
    )
    source = _replace_required(
        ...
        """    }\n"""
        """    i_time = I_GetTime();\n"""
        """    observed_ticks = tick_telemetry_last_ticks_ms;\n"""
        ...
        """    ok = ok && fprintf(fp, "ticks_ms=%u\\n", observed_ticks) >= 0;\n"""
        """    ok = ok && fprintf(fp, "sleep_requested_ms=%u\\n", fake_ticks_ms) >= 0;\n"""
        ...
        """    ok = ok && fprintf(fp, "time_model=clock-monotonic-elapsed\\n") >= 0;\n"""
        """    ok = ok && fprintf(fp, "fake_time_model=disabled-wall-clock-active\\n") >= 0;\n"""
        """    ok = ok && fprintf(fp, "sleep_policy=usleep-requested-ms\\n") >= 0;\n"""
...
def apply_v3092_globals() -> None:
    ...
    v3086.v3084.v3083.v3081.v3081_adapter_source = v3092_adapter_source
    v3086.apply_v3086_globals()
    V3059.v3059_adapter_source = v3092_adapter_source
...
def render_report(
    ...
    return "\n".join([
        "# Native Init V3092 DOOMGENERIC Wall-Clock Ticks Source Build",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        "- Track: active Video playback / DOOM smoothness root-cause isolation.",
        ...
        "## Included Delta",
        "",
        "- Keeps V3086 pageflip cadence, V3084 serial-preserve, V3083 fast 3:2 dashboard scaling, V3081 shared-frame IPC, V3079 presenter-paced helper, timing probe, audio corun, and UDP/NCM input.",
        "- Changes the DOOM port clock from `DG_SleepMs`-accumulated fake time to `CLOCK_MONOTONIC` elapsed time.",
        "- Keeps helper-side DOOM time/tic telemetry written atomically at the end of each visible loop.",
        "- Purpose: compare wall-clock DOOM ticks against V3091 fake-time telemetry before touching scaling or direct-present behavior.",
        "",
        "## Telemetry Contract",
        "",
        f"- Telemetry marker: `{TICK_TELEMETRY_MARKER}`",
        f"- Wall-clock marker: `{WALL_CLOCK_MARKER}`",
        f"- Telemetry path: `{TICK_TELEMETRY_PATH}`",
        "- Captured fields: `ticks_ms`, `sleep_requested_ms`, `sleep_calls`, `sleep_ms_total`, `getticks_calls`, `i_get_time`, `gametic`, `presented_frames`, and loop result.",
        "- Time model: `clock-monotonic-elapsed`.",
        "- Sleep policy: `usleep-requested-ms`.",
        "- DOOM ticrate: `35`.",
        "",
        "## Runtime Contract",
        "",
        ...
        *marker_lines,
        ...
        "## Validation",
        "",
        "- `py_compile`: V3092 builder and focused tests.",
        "- `unittest`: V3092 source contract plus V3090/V3086/V3084/V3083/V3081/V3079 lineage regressions.",
        "- Build: AArch64 helper compile/link, native-init compile, ramdisk pack, boot image pack, SHA256 capture.",
        "- Marker check: generated boot image contains V3092 identity, wall-clock marker, tick telemetry marker/path, serial-preserve marker, fast-scale marker, shared-frame markers, pace/pageflip markers, timing probe, audio marker, and UDP input markers.",
        "- `git diff --check`: PASS.",
        "",
        "## Next Unit",
        "",
        ...
        f"- Scope: flash exact V3092 boot image via `native_init_flash.py`, health-check, run bounded DOOM loop, then read `{TICK_TELEMETRY_PATH}` to compare wall-clock ticks, game tics, presented frames, and pageflip timing.",
        "",
        "## Metadata",
        "",
        f"- Helper flags: `{', '.join(helper_flags)}`",
        f"- Init extra flags: `{', '.join(init_extra_flags)}`",
        "- Candidate type: `doomgeneric-wall-clock-ticks-candidate`.",
    ]) + "\n"
...
def main() -> int:
    ...
    doom.update({
        ...
        "cadence_experiment": "wall-clock-ticks-vs-fake-time",
        "tick_telemetry_marker": TICK_TELEMETRY_MARKER,
        "wall_clock_marker": WALL_CLOCK_MARKER,
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-wall-clock-ticks-candidate",
        "adoption_state": "pending-wall-clock-ticks-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-wall-clock-ticks-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-wall-clock-ticks-candidate",
