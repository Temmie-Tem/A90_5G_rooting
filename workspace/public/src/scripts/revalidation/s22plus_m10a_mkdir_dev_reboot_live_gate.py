#!/usr/bin/env python3
"""Guarded S22+ M10A mkdir-dev reboot native-init live gate.
...
M10A package and rollback APs without touching any device.

M10A is a lower-layer discriminator, not a module or USB proof. The expected
live proof is automatic return to Samsung download mode after a freestanding C
PID1 performs mkdirat("/dev", 0755), then reboot("download"). The helper must
not count the original candidate Odin endpoint as proof: it waits for that
endpoint to disconnect, then treats a later Odin endpoint as the candidate's
...
LIVE_ACK_TOKEN = "S22PLUS-M10A-MKDIR-DEV-REBOOT-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M10A-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_M10A_AP_SHA256 = "d71c8c82d2703892802228dd61ded561a9b4f90c678db15452014f2477170105"
EXPECTED_M10A_BOOT_SHA256 = "c62fce5e444bad47e2b934f6e9e82bc731058a0c9494629f0eb9044ff92e8b24"
EXPECTED_M10A_BASE_BOOT_SHA256 = "2e541703951dc725bad35850faf7028c2d910dd5f21166449b63f1248c29967e"
EXPECTED_M10A_KERNEL_SHA256 = "bceca73edbfca3499148e16741c939779157925949ef6bc8a8e31d6b68fc2cff"
EXPECTED_M10A_INIT_SHA256 = "8f954dfcd5d5887f8c1659e7e658617561627d9c7fecc518972a795ac20422b3"
EXPECTED_M10A_SOURCE_SHA256 = "c12b710f93b957313ad1018de40ebe2dec53883c5de6d018c9d5577b1a426cf0"
EXPECTED_M10A_RETAINED_MARKER = "S22_NATIVE_INIT_M10A_NO_RETAINED_MARKER_EXPECTED"

DEFAULT_M10A_AP = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a_mkdir_dev_reboot_v0_1/odin4/AP.tar.md5")
DEFAULT_M10A_MANIFEST = Path("workspace/private/outputs/s22plus_native_init/inplace_m10a_mkdir_dev_reboot_v0_1/manifest.json")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    ...
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m10a_mkdir_dev_reboot_live_gate_{stamp}")
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    required = [
        "S22+ M10A mkdir-dev reboot native-init boot-only",
        EXPECTED_M10A_AP_SHA256,
        EXPECTED_M10A_BOOT_SHA256,
        EXPECTED_M10A_BASE_BOOT_SHA256,
        EXPECTED_M10A_KERNEL_SHA256,
        EXPECTED_M10A_INIT_SHA256,
        EXPECTED_M10A_SOURCE_SHA256,
        LIVE_ACK_TOKEN,
        ROLLBACK_ACK_TOKEN,
        "freestanding C raw-syscall runtime",
        "exactly one `mkdirat(\"/dev\", 0755)` side effect",
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M10A live authorization markers: {missing}")


def verify_m10a_manifest(path: Path, log_path: Path) -> None:
    if not path.is_file():
        raise SystemExit(f"M10A manifest missing: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    hashes = data.get("hashes", {})
    safety = data.get("safety", {})
    ramdisk = data.get("ramdisk", {})
    init_info = data.get("m10a_init", {})
    tar_members_seen = data.get("tar_members")
    append_log(log_path, f"m10a_manifest_path={path}")
    append_log(log_path, f"m10a_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    append_log(log_path, f"m10a_manifest_safety={json.dumps(safety, sort_keys=True)}")
    append_log(log_path, f"m10a_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")

    expected_hashes = {
        "ap_tar_md5": EXPECTED_M10A_AP_SHA256,
        "boot_img": EXPECTED_M10A_BOOT_SHA256,
        "base_boot": EXPECTED_M10A_BASE_BOOT_SHA256,
        "kernel": EXPECTED_M10A_KERNEL_SHA256,
        "m10a_init": EXPECTED_M10A_INIT_SHA256,
        "source": EXPECTED_M10A_SOURCE_SHA256,
        "nochange_repack_boot": EXPECTED_M10A_BASE_BOOT_SHA256,
    }
    for key, expected in expected_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M10A manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M10A manifest tar members mismatch: {tar_members_seen!r}")
    ...
    expected_safety: dict[str, Any] = {
        ...
        "first_externally_observable_action": "mkdirat-dev-then-reboot-download",
        "intended_syscall_count": 2,
        "auto_reboot": "download-after-mkdirat-dev",
        "marker_write": False,
        "kmsg_write": False,
    ...
    for key, expected in expected_safety.items():
        if safety.get(key) != expected:
            raise SystemExit(f"M10A manifest safety {key} mismatch: {safety.get(key)!r} != {expected!r}")
    if safety.get("intended_syscalls") != ["mkdirat", "reboot"]:
        raise SystemExit(f"M10A intended syscalls mismatch: {safety.get('intended_syscalls')!r}")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M10A manifest did not replace /init")
    if ramdisk.get("only_intended_entry_change") != "init":
        raise SystemExit("M10A manifest intended ramdisk change is not only /init")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A must not inject vendor module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M10A must not inject module-list files into boot ramdisk")

    required_strings = set(init_info.get("required_strings", []))
    if "/dev" not in required_strings:
        raise SystemExit("M10A required /dev string missing from manifest")
    if "download" not in required_strings:
        raise SystemExit("M10A required download string missing from manifest")
    if init_info.get("svc_count") != 2:
        raise SystemExit(f"M10A expected two svc instructions, got {init_info.get('svc_count')!r}")
    ...
    for forbidden in [
        ...
        if forbidden not in forbidden_strings:
            raise SystemExit(f"M10A forbidden-string gate missing from manifest: {forbidden}")
...
def observe_until_odin(run_dir: Path, log_path: Path, seconds: int, odin: Path) -> str | None:
    ...
    while time.monotonic() < deadline:
        iteration += 1
        label = f"m10a_self_download_{iteration:03d}"
        host_snapshot(run_dir, log_path, label, odin)
        devices = odin_devices(odin, log_path, f"{label}-extra")
        if len(devices) == 1:
            append_log(log_path, f"m10a_self_download_seen=1 device={devices[0]}")
            return devices[0]
        if len(devices) > 1:
            raise SystemExit(f"refusing ambiguous Odin devices during M10A observation: {devices}")
        rows = adb_rows(log_path, f"{label}-extra")
        if rows:
            append_log(log_path, f"m10a_candidate_adb_rows={rows}")
        time.sleep(1.0)
    append_log(log_path, "m10a_self_download_seen=0")
    return None
...
def rollback_from_download(
    ...
    if serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", serial, marker=EXPECTED_M10A_RETAINED_MARKER)
    return 0


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--m10a-ap", type=Path, default=DEFAULT_M10A_AP)
    parser.add_argument("--m10a-manifest", type=Path, default=DEFAULT_M10A_MANIFEST)
    ...
    parser.add_argument("--self-download-wait-sec", type=int, default=150)
    ...
    log_path = run_dir / "s22plus_m10a_mkdir_dev_reboot_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus m10a mkdir-dev reboot live gate ===")
    append_log(log_path, f"target={EXPECTED_TARGET}")

    odin = resolve(root, args.odin)
    m10a_ap = resolve(root, args.m10a_ap)
    m10a_manifest = resolve(root, args.m10a_manifest)
    ...
    verify_ap(m10a_ap, EXPECTED_M10A_AP_SHA256, "m10a_candidate", log_path)
    verify_m10a_manifest(m10a_manifest, log_path)
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M10A candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M10A rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M10A candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    if odin_device is None:
        print("download mode did not appear for M10A candidate flash", file=sys.stderr)
        return 2

    candidate_rc = flash_ap(odin, m10a_ap, odin_device, log_path, "candidate")
    if candidate_rc != 0:
        print(f"M10A candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        print(
            "M10A candidate flash completed but the original Odin device did not disconnect; "
        ...
        if post_rollback_serial is None:
            return 6
        append_log(log_path, "m10a_result=no-proof-original-download-never-disconnected")
        collect_android_pstore(
            run_dir,
            log_path,
            "post_rollback_no_disconnect",
            post_rollback_serial,
            marker=EXPECTED_M10A_RETAINED_MARKER,
        )
        return 7

    print("M10A candidate flashed. Waiting for mkdir-dev reboot self-return.")
    rollback_device = observe_until_odin(run_dir, log_path, args.self_download_wait_sec, odin)
    if rollback_device is None:
        print("M10A self-download did not appear; enter download mode manually and run --rollback-from-download.", file=sys.stderr)
        return 4
    ...
    if post_rollback_serial is None:
        return 6
    collect_android_pstore(run_dir, log_path, "post_rollback", post_rollback_serial, marker=EXPECTED_M10A_RETAINED_MARKER)
    print(f"M10A live gate completed with mkdir-dev reboot self-return and rollback ok; log={log_path}")
    return 0
