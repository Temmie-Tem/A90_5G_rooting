#!/usr/bin/env python3
"""Build V3230 GPU H3 VPC LM/SIV probe."""
...
import build_native_init_boot_v3228_gpu_h3_fragment_input_probe as previous

base = previous.base

CYCLE = "V3230"
INIT_VERSION = "0.11.42"
INIT_BUILD = "v3230-gpu-h3-vpc-lm-siv-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3230_GPU_H3_VPC_LM_SIV_SOURCE_BUILD_2026-06-25.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3230_gpu_h3_vpc_lm_siv_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3230_gpu_h3_vpc_lm_siv_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3230_gpu_h3_vpc_lm_siv_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v573_gpu_h3_vpc_lm_siv_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3230"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3230.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3230.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3230"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3230-gpu-h3-vpc-lm-siv-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3230-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3230-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3230-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3230-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3230-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3230-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3230-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-vpc-lm-siv-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-vpc-lm-siv-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3230")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3230")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3230")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3230")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3230")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3230")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3230")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3230")
SFX_STREAM_MARKER = "a90.doomgeneric.v3230.audio=real-sfx-pcm-stream-gpu-h3-vpc-lm-siv-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-vpc-lm-siv-probe-v3230"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3230.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3230_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3228": b"a90-doomgeneric-v3230",
        b"a90.doomgeneric.v3228": b"a90.doomgeneric.v3230",
        b"v3228": b"v3230",
        b"V3228": b"V3230",
        b"first-triangle-h3-fragment-input-state-mov-f32-shader":
            b"first-triangle-h3-vpc-lm-siv-mov-f32-shader",
    }
...
GPU_H3_VPC_LM_SIV_MARKERS = (
    b"gpu.h3.draw.scope=first-triangle-h3-vpc-lm-siv-mov-f32-shader",
    ...
    b"gpu.h3.draw.vpc_varying_lm_transfer_cntl0=0x%x",
    b"gpu.h3.draw.vpc_varying_lm_transfer_cntl1=0x%x",
    b"gpu.h3.draw.vpc_varying_lm_transfer_cntl2=0x%x",
    b"gpu.h3.draw.vpc_varying_lm_transfer_cntl3=0x%x",
    b"gpu.h3.draw.vpc_vs_siv_cntl=0x%x",
    b"gpu.h3.draw.vpc_vs_siv_cntl_v2=0x%x",
    b"gpu.h3.draw.gras_su_vs_siv_cntl=0x%x",
...
REQUIRED_STRINGS = tuple(_rewrite_v3230_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        "source_baseline": "v3228-gpu-h3-fragment-input-probe",
        "command": "gpu h3-draw-envelope-probe --timeout-ms 5000 --materialize-devnode",
        "scope": "first-triangle-h3-vpc-lm-siv-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3230-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-vpc-lm-siv-mov-f32-shader-timeout-guard",
...
def v3230_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-vpc-lm-siv-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-vpc-lm-siv-probe")
        .replace("v3208", "v3230")
        .replace("V3208", "V3230")
    )


def _overlay_preserved_v3230_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3228 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3230 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3230 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3230 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3230-overlay-") as temp_name:
        ...
        for index, item in enumerate(mkboot_args):
            if item == "--ramdisk":
                ...
                raise RuntimeError("V3228 base boot mkbootimg args did not include --ramdisk")
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3230 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3230 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3228-ramdisk-overlay-v3230-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-vpc-lm-siv-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-vpc-lm-siv-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-vpc-lm-siv-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3228-ramdisk-overlay-v3230-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-vpc-lm-siv-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-vpc-lm-siv-live-validation"
...
def _apply_v3230_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-vpc-lm-siv-probe")
        ...
        "v3210_adapter_source": v3230_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3230_ramdisk,
