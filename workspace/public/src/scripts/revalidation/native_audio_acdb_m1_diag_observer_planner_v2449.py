...
def source_state() -> dict[str, Any]:
    ...
    if HELPER_SOURCE.exists():
        ...
        state.update({
            ...
            "contains_source_buffer_capture": "source_buffer_capture" in text and "ok-source-read" in text and "ok-source-mmap" in text,
            "contains_acdb_source_target_filter": "is_acdb_source_target" in text and "/vendor/etc/acdbdata" in text and "/vendor/lib/" in text,
...
def command_safety(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    required = [
        ...
        "ok-early-dup",
        "ok-remote-mmap",
        "source_buffer_capture",
        "ok-source-read",
        "ok-source-mmap",
        "A90_COMPAT_ARM_NR_PREAD64",
        "ioctl_unmatched",
        "syscall_stop_count",
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "diagnostic_contract": {
            ...
            "adds_early_dmabuf_fd_duplication": True,
            ...
            "dmabuf_capture_policy": "matching custom-topology set-cal dmabuf bytes are stored only in private binary artifacts; public summaries may include length and SHA-256 only; if proc-fd duplication fails, V2467 may copy from a previously observed traced-process mmap of the same fd, and V2470 may use an early duplicate of that dmabuf fd captured at mmap-entry time",
            "source_buffer_capture_policy": "ACDB regular-file read/pread and mmap source bytes are stored only in private sourcebuf artifacts; public summaries may include length and SHA-256 only; this is a lower-risk candidate path before owner-context instrumentation",
        },
        "boundaries": [
