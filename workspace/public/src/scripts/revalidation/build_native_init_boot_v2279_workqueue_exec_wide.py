#!/usr/bin/env python3
"""Build V2279 wide workqueue execute_start + codeword sampler test boot.

This source/build wrapper keeps the V2237 Wi-Fi route and packages a higher-capacity
workqueue execute_start sampler plus the perf-regs/codeword sampler in the same
post-FWREADY boot_wlan / firmware_class fallback window.
"""
...
OUT_DIR = workspace_private_build_path("native-init", "v2279-workqueue-exec-wide")
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2279_WORKQUEUE_EXEC_WIDE_SOURCE_BUILD_2026-06-12.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v2279_workqueue_exec_wide.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v2279_workqueue_exec_wide"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v2279_workqueue_exec_wide.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v434_workqueue_exec_wide"
BPF_HELPER_BINARY = OUT_DIR / "a90_bpf_workqueue_exec_wide_sample_ring"
BPF_HELPER_SOURCE = REPO_ROOT / "workspace/public/src/native-init/helpers/a90_bpf_workqueue_exec_wide_sample_ring.c"
BPF_HELPER_RAMDISK_PATH = "bin/a90_bpf_workqueue_exec_wide_sample_ring"
BPF_HELPER_RUNTIME_PATH = "/bin/a90_bpf_workqueue_exec_wide_sample_ring"
...
EXPECTED_HELPER_MARKER = "a90_android_execns_probe v434"
EXTRA_INIT_FLAGS = v2237.EXTRA_INIT_FLAGS
HELPER_MODE = v2237.HELPER_MODE
HELPER_RUNTIME_MODE = v2237.HELPER_RUNTIME_MODE
WORKQUEUE_LOG_PATH = "/cache/native-init-v2279-workqueue-exec-wide.log"
WORKQUEUE_DURATION_MS = 45000
WORKQUEUE_PRINT_LIMIT = 8192
WORKQUEUE_WAIT_MS = 60000
TAIL_LOG_PATH = "/cache/native-init-v2279-tail-perf-regs-codeword.log"
...
def configure_base() -> tuple[str, ...]:
    ...
    replacements = {
        ...
        "--init-build": "v2279-workqueue-exec-wide",
        "--out-dir": str(OUT_DIR),
        "--init-binary": str(INIT_BINARY),
        "--helper-binary": str(HELPER_BINARY),
        "--ramdisk-cpio": str(RAMDISK_CPIO),
        "--boot-image": str(BOOT_IMAGE),
        "--wifi-test-klog-prefix": "A90v2279",
        "--wifi-test-disable": "/cache/native-init-wifi-test-boot-v2279.disable",
        "--wifi-test-log": "/cache/native-init-wifi-test-boot-v2279.log",
        "--wifi-test-summary": "/cache/native-init-wifi-test-boot-v2279.summary",
        "--wifi-test-helper-result": "/cache/native-init-wifi-test-boot-v2279-helper.result",
        "--wifi-test-pid": "/cache/native-init-wifi-test-boot-v2279.pid",
        "--wifi-test-watcher-pid": "/cache/native-init-wifi-test-boot-v2279-supervisor.pid",
...
def main() -> int:
    ...
    manifest["candidate_tag"] = "v2279-workqueue-exec-wide"
    ...
    manifest["workqueue_sampler"] = {
        ...
        "tracepoint_fields": ["work", "function"],
        "stack_depth": 64,
        "stack_print_limit": 512,
    }
    ...
    live_candidate_path.write_text(json.dumps({
        "candidate_tag": "v2279-workqueue-exec-wide",
        ...
        "next_live_cycle": "V2280",
        "note": "V2279 keeps the V2237 WLAN route and adds read-only wide workqueue execute_start and codeword samplers around the post-FWREADY boot_wlan/firmware_class window.",
