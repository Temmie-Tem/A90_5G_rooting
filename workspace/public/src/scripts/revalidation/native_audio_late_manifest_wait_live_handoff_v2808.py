#!/usr/bin/env python3
"""V2808 live handoff for V2807 late-manifest native audio play.

This flashes the V2807 late-manifest-wait candidate, starts native
`audio play --execute` before deploying private ACDB artifacts, waits for
ADSP/ASoC card publication, deploys the bundle into the default
`/cache/a90-runtime` manifest namespace while the worker is waiting, and then
requires SET-cal, route, bounded PCM, cleanup, and rollback to V2321.
"""
...
ROOT = repo_root()
CYCLE = "V2808"
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2808_AUDIO_LATE_MANIFEST_WAIT_LIVE_2026-06-19.md"
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2807-audio-late-manifest-wait/manifest.json"
...
def configure_base_for_v2808() -> None:
    ...
    base.CANDIDATE_TAG = CANDIDATE_TAG
    base.REMOTE_NATIVE_MANIFEST = DEFAULT_REMOTE_MANIFEST


configure_base_for_v2808()
...
def remap_remote_path(path: str | None) -> str | None:
    ...
    if path.startswith(OLD_REMOTE_ROOT + "/"):
        return DEFAULT_REMOTE_ROOT + path[len(OLD_REMOTE_ROOT):]
    return path


def remap_deploy_plan_to_default_runtime(deploy_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    remapped["remote_dir"] = DEFAULT_REMOTE_ROOT
    remapped["remote_native_manifest"] = DEFAULT_REMOTE_MANIFEST
    remapped["v2808_remote_remap"] = {
        "old_remote_root": OLD_REMOTE_ROOT,
        "default_remote_root": DEFAULT_REMOTE_ROOT,
        "default_remote_manifest": DEFAULT_REMOTE_MANIFEST,
    }
    return remapped
...
def preflight_state() -> dict[str, Any]:
    ...
    state["discriminator"] = "audio-play-before-late-default-runtime-acdb-deploy"
    state["candidate_expect_version"] = CANDIDATE_VERSION
    state["candidate_expect_tag"] = CANDIDATE_TAG
    state["remote_native_manifest"] = DEFAULT_REMOTE_MANIFEST
    ...
    state["live_scope"] = [
        ...
        "require worker manifest_wait to complete and then SET-cal/route/PCM",
...
def capture_audio_status(out_dir: Path, steps: list[dict[str, Any]], label: str) -> dict[str, Any]:
    ...
    summary = parse_audio_status("\n".join([stdout_of(adsp), stdout_of(snd)]))
    return {
...
def classify_play_output(text: str) -> dict[str, Any]:
    summary = base.classify_play_output(text)
    summary.update({
        "manifest_wait_started": "audio.play.worker.manifest_wait_started=1" in text
        or "audio.play.integrated.wait.manifest.path=" in text,
        "manifest_ready": "audio.play.worker.manifest_ready=1" in text
        or "audio.play.integrated.wait.manifest.ready=1" in text,
        "manifest_timeout": "audio.play.worker.manifest_ready=0" in text
        or "audio.play.integrated.wait.manifest.ready=0" in text,
    })
    return summary
...
def run_play_sequence(args: argparse.Namespace,
    ...
    command = play_command(args)
    result: dict[str, Any] = {"play_command": " ".join(command)}

    hide_auto_menu(out_dir, steps, "before-play")
    play = base.run_serial_step(
        out_dir,
        steps,
        "candidate-audio-play-execute-listen-before-deploy",
        command,
        timeout=90.0,
        retry_unsafe=False,
        allow_error=True,
    ...
    result["status_after_deploy"] = capture_audio_status(out_dir, steps, "after-late-deploy-before-worker-done")
    ...
    log_step = base.run_serial_step(
        out_dir,
        steps,
        "candidate-audio-late-manifest-play-log",
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    write_json(out_dir / "remapped-deploy-plan-public-metadata.json", {
        "remote_root": DEFAULT_REMOTE_ROOT,
        "remote_native_manifest": DEFAULT_REMOTE_MANIFEST,
        "artifact_count": len(base.deploy_artifacts_for_native_manifest(deploy_plan)),
    ...
    try:
        ...
        result["status_before_play"] = capture_audio_status(out_dir, steps, "before-play")
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "",
        f"- Native command started before ACDB deploy: `{result.get('play_command')}`",
        ...
        f"- Card/control after late deploy before worker done: `{int(bool(status_after_deploy.get('has_sound_card')))} / {int(bool(status_after_deploy.get('has_sound_control')))}`",
        ...
        f"- Remapped remote root: `{DEFAULT_REMOTE_ROOT}`",
        f"- Native manifest remote path: `{DEFAULT_REMOTE_MANIFEST}`",
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": {
            ...
            "audio_play_start": play_command(args),
            "card_poll": ["audio", "adsp-status"],
            "late_install_count": state.get("deploy_artifact_count", 0) + 1,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--play-timeout", type=float, default=260.0)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "card_ready_after_play_start": result.get("card_ready_after_play_start"),
