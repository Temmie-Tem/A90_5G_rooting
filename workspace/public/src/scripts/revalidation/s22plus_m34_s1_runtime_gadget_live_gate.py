#!/usr/bin/env python3
"""Guarded S22+ M34 S1 stock configfs runtime-gadget native-init live gate.

Dry-run is the default. Live mode requires a fresh SHA-pinned AGENTS.md
exception and an explicit ack token.

M34 S1 starts from the P30-proven full ACM module closure, creates the stock-
ordered configfs gadget with `ss_acm.0`, writes `UDC=none`, and parks before the
two off-stock pullup knobs (`max_speed=high-speed`, `usb_role=device`) and
before the final `UDC=a600000.dwc3` pullup.
"""
...
LIVE_ACK_TOKEN = "S22PLUS-M34-S1-STOCK-CONFIGFS-LIVE-GATE"
ROLLBACK_ACK_TOKEN = "S22PLUS-M34-S1-STOCK-CONFIGFS-ROLLBACK-FROM-DOWNLOAD"

EXPECTED_TARGET = "SM-S906N/g0q/S906NKSS7FYG8"
EXPECTED_STAGE = "S1"
EXPECTED_M34_MARKER = "S22_NATIVE_INIT_M34_RUNTIME_GADGET_SPLIT_S1"
EXPECTED_M34_AP_SHA256 = "77e8858ea6becc3e988232d464f97827f55594f16ed6edebd23c3529c972d237"
EXPECTED_M34_BOOT_SHA256 = "bb46233068890bb6849c63b4dab845ca48b65a9ffeac9e24ad08e81416b63f85"
EXPECTED_M34_INIT_SHA256 = "5339170f3138843a8f8da6cfd5f20f85696d3a9d18ae22bda439e21d0dd259cd"
...
EXPECTED_MODULE_ENTRY = "s22plus_m34_s1_runtime_gadget_split.modules"
...
DEFAULT_M34_AP = Path("workspace/private/outputs/s22plus_native_init/m34_runtime_gadget_split_v0_2/S1/odin4/AP.tar.md5")
...
def resolve_run_dir(root: Path, requested: Path | None) -> Path:
    if requested is not None:
        ...
        return run_dir
    base = resolve(root, DEFAULT_RUN_ROOT / f"s22plus_m34_s1_runtime_gadget_live_gate_{utc_stamp()}")
...
def verify_agents_exception(root: Path, log_path: Path) -> None:
    ...
    if missing:
        raise SystemExit(f"AGENTS.md missing M34 S1 runtime-gadget authorization markers: {missing}")
...
def verify_m34_manifest(path: Path, log_path: Path) -> None:
    ...
    append_log(log_path, f"m34_s1_manifest_hashes={json.dumps(hashes, sort_keys=True)}")
    ...
    append_log(log_path, f"m34_s1_manifest_runtime_steps={json.dumps(runtime_steps, sort_keys=True)}")
    append_log(log_path, f"m34_s1_manifest_closure={json.dumps(closure, sort_keys=True)}")
    append_log(log_path, f"m34_s1_manifest_ramdisk={json.dumps(ramdisk, sort_keys=True)}")
    append_log(log_path, f"m34_s1_manifest_init_file={init_info.get('file', '')}")
    ...
    if stage.get("stage_number") != 1:
        raise SystemExit(f"M34 S1 stage number mismatch: {stage.get('stage_number')!r}")
    expected_steps = {
    ...
    if runtime_steps != expected_steps:
        raise SystemExit(f"M34 S1 runtime steps mismatch: {runtime_steps!r}")
    required_hashes = {
    ...
    for key, expected in required_hashes.items():
        if hashes.get(key) != expected:
            raise SystemExit(f"M34 S1 manifest hash {key} mismatch: {hashes.get(key)!r} != {expected!r}")
    if tar_members_seen != [EXPECTED_MEMBER]:
        raise SystemExit(f"M34 S1 manifest tar members mismatch: {tar_members_seen!r}")
    ...
    if closure.get("modules") != EXPECTED_MODULES:
        raise SystemExit(f"M34 S1 module closure mismatch: {closure.get('modules')!r}")
    if closure.get("module_count") != len(EXPECTED_MODULES):
        raise SystemExit(f"M34 S1 module closure count mismatch: {closure.get('module_count')!r}")
    if closure.get("module_sha256") != EXPECTED_M34_MODULE_LIST_SHA256:
        raise SystemExit(f"M34 S1 module list SHA mismatch: {closure.get('module_sha256')!r}")
    if any(module in closure.get("modules", []) for module in ("phy-msm-ssusb-qmp.ko", "eud.ko")):
        raise SystemExit("M34 S1 excluded QMP/EUD module leaked into closure")

    if ramdisk.get("replaced_entry") != "init":
        raise SystemExit("M34 S1 manifest did not replace /init")
    if ramdisk.get("added_subset_entry") != EXPECTED_MODULE_ENTRY:
        raise SystemExit(f"M34 S1 module list ramdisk entry mismatch: {ramdisk.get('added_subset_entry')!r}")
    if ramdisk.get("module_files_injected_into_boot_ramdisk") != 0:
        raise SystemExit("M34 S1 must not inject module binaries into boot ramdisk")
    if ramdisk.get("module_list_files_injected_into_boot_ramdisk") != 1:
        raise SystemExit("M34 S1 must inject exactly one module-list text file into boot ramdisk")
    ...
    for required in [
        ...
        if required not in required_strings:
            raise SystemExit(f"M34 S1 required string missing from manifest: {required}")
    for forbidden in [
        ...
        if forbidden in required_strings:
            raise SystemExit(f"M34 S1 forbidden string present in manifest: {forbidden}")

    objdump_path = path.parent / EXPECTED_STAGE / str(init_info.get("objdump_path", ""))
    if not objdump_path.is_file():
        raise SystemExit(f"M34 S1 objdump missing: {objdump_path}")
    ...
    if not any("mov" in line and "#0x111" in line and "// #273" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S1 /init does not load arm64 __NR_finit_module (273)")
    if any("mov" in line and "#0x8e" in line and "// #142" in line for line in objdump.splitlines()):
        raise SystemExit("M34 S1 /init must not load arm64 __NR_reboot (142)")
...
def verify_m34_artifacts(
    *,
    ...
    verify_ap(m34_ap, EXPECTED_M34_AP_SHA256, "m34_s1_candidate", log_path)
...
def observe_park_survival(
    *,
    ...
    next_snapshot = start
    iteration = 0
    append_log(log_path, f"m34_s1_observe_start_utc={utc_now()}")
    append_log(log_path, f"m34_s1_observe_sec={observe_sec}")
    while time.monotonic() < deadline:
        now = time.monotonic()
        elapsed = now - start
        if now >= next_snapshot:
            iteration += 1
            label = f"m34_s1_park_observe_{iteration:03d}"
            ...
            if len(devices) == 1:
                append_log(log_path, f"m34_s1_result=unexpected_odin_before_survival_window elapsed_sec={elapsed:.3f} device={devices[0]}")
                return "unexpected-odin-before-survival-window", devices[0]
            if len(devices) > 1:
                raise SystemExit(f"refusing ambiguous Odin devices during M34 S1 observation: {devices}")
            rows = adb_rows(log_path, f"{label}-adb-extra")
            usable = [row for row in rows if row[1] == "device"]
            if usable:
                append_log(log_path, f"m34_s1_result=unexpected_adb_before_survival_window elapsed_sec={elapsed:.3f} rows={usable}")
                return "unexpected-adb-before-survival-window", None
            next_snapshot = now + snapshot_interval_sec
        time.sleep(0.5)
    append_log(log_path, "m34_s1_survival_window_pass=1")
    append_log(log_path, "m34_s1_result=survived-observation-window-manual-download-required")
...
def rollback_boot_only_from_download(
    *,
    ...
    if rollback_rc != 0 and rollback_target == ROLLBACK_MAGISK:
        append_log(log_path, f"m34_s1_{label}_magisk_rollback_failed_attempting_stock_fallback=1")
    ...
    if rollback_target == ROLLBACK_MAGISK:
        ...
        append_log(log_path, f"m34_s1_{label}_boot_restore_hash_check=skipped rollback_target=stock")
    record_timeline_event(run_dir, "rollback_boot_ready")
    record_timeline_event(run_dir, f"{label}_rollback_boot_ready")
    marker_found = collect_android_pstore(run_dir, log_path, f"post_m34_s1_{label}_rollback", android, marker=EXPECTED_M34_MARKER)
    append_log(log_path, f"m34_s1_{label}_retained_marker_found={int(marker_found)}")
    return (0, android)
...
def rollback_from_download(
    *,
    ...
    devices = odin_devices(odin, log_path, "m34-s1-rollback-only")
    if len(devices) != 1:
        raise SystemExit(f"M34 S1 rollback requires exactly one Odin device, got {devices}")
...
def main(argv: list[str]) -> int:
    ...
    log_path = run_dir / "s22plus_m34_s1_runtime_gadget_live_gate.txt"
    append_log(log_path, f"=== {utc_now()} s22plus M34 S1 runtime gadget live gate ===")
    ...
    if args.offline_check:
        ...
        print(f"offline-check ok: M34 S1 candidate and rollback APs verified; no device action; log={log_path}")
        return 0
    ...
    if args.rollback_from_download:
        ...
        print(f"M34 S1 rollback-from-download completed rc={rc}; log={log_path}")
        return rc
    ...
    if not args.live:
        print(
            "dry-run ok: M34 S1 candidate, rollback APs, AGENTS exception, Android stability, "
    ...
    record_timeline_event(run_dir, "live_session_start")
    print(
        "M34 S1 live gate starting. This candidate should park after stock configfs setup "
    ...
    if odin_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print("download mode did not appear for M34 S1 candidate flash", file=sys.stderr)
        return 2
    ...
    if candidate_rc != 0:
        record_timeline_event(run_dir, "live_session_end")
        print(f"M34 S1 candidate Odin flash failed rc={candidate_rc}; log={log_path}", file=sys.stderr)
    ...
    if not left_download:
        append_log(log_path, "m34_s1_result=no-proof-original-download-never-disconnected")
    ...
    print(f"M34 S1 candidate flashed. Observing for {args.observe_sec}s survival window.", flush=True)
    ...
    if result != "survived-observation-window-manual-download-required":
        if rollback_device is None:
            record_timeline_event(run_dir, "live_session_end")
            print(
                f"M34 S1 stopped before survival proof ({result}). If the device is not in Android, "
    ...
    print(
        "M34 S1 survived the observation window. Enter Download mode manually for rollback now; "
    ...
    if rollback_device is None:
        record_timeline_event(run_dir, "live_session_end")
        print(
            f"M34 S1 survival window passed, but manual Download mode did not appear. "
    ...
    record_timeline_event(run_dir, "live_session_end")
    print(f"M34 S1 live gate completed rc={rc}; result={result}; log={log_path}")
    return rc
