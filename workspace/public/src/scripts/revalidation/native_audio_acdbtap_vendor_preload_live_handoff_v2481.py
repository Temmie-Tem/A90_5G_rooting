#!/usr/bin/env python3
"""V2481 recoverable Android handoff for vendor-path ACDB tap preload.

V2479 proved that `/data/local/tmp` is outside the vendor linker namespace for
`android.hardware.audio.service`. V2481 uses the V2480 temporary Magisk/systemless
capsule to expose the same V2475 32-bit interposer under a vendor-visible path,
then reuses the V2477 ACDB tap capture flow only after maps confirm the preload.
"""
...
RUN_ID = "V2481"
BUILD_TAG = "v2481-audio-acdbtap-vendor-preload-live"
ROOT = v2480.ROOT
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"{RUN_ID.lower()}-acdbtap-vendor-preload-live-{stamp}"


def decision_slug() -> str:
    return "v2481-acdbtap-vendor-preload-live"
...
def module_stage_commands(args: argparse.Namespace) -> list[tuple[str, list[str], bool]]:
    plan = v2480.command_plan(args.module_out_dir)
    steps: list[tuple[str, list[str], bool]] = [("v2481-module-stage-setup", plan["stage_setup"], True)]
    for index, command in enumerate(plan["push_files"]):
        steps.append((f"v2481-module-push-{index}", command, True))
    steps.extend([
        ("v2481-module-install-direct", plan["install_module_direct"], True),
        ("v2481-module-reboot-for-magisk-mount", plan["android_reboot_for_magisk_mount"], False),
    ])
    return steps


def module_verify_command(args: argparse.Namespace) -> list[str]:
    return v2480.command_plan(args.module_out_dir)["verify_vendor_visible_after_reboot"]


def module_cleanup_command(args: argparse.Namespace) -> list[str]:
    return v2480.command_plan(args.module_out_dir)["cleanup_exact_module"]
...
def verified_preload_candidates(record: dict[str, Any]) -> list[str]:
    ...
    for line in stdout.splitlines():
        ...
        if v2476.TAP_SHA256 not in tokens:
            continue
        for candidate in v2480.PRELOAD_CANDIDATES:
...
ls -lZ {shlex.quote(v2476.AUDIO_BINARY)} {' '.join(shlex.quote(path) for path in v2480.PRELOAD_CANDIDATES)} {shlex.quote(v2476.REMOTE_CAPTURE_DIR)} 2>&1 || true
echo A90_ACDBTAP_V2481_CAPTURE_SETUP_OK
""".strip()
...
echo A90_ACDBTAP_V2481_PREFLIGHT_OK preload={shlex.quote(preload_path)}
""".strip()
    return adb_su(args, script)


def manual_start_vendor_preload_command(args: argparse.Namespace, preload_path: str) -> list[str]:
    script = f"""
set -eu
setprop ctl.stop {shlex.quote(v2476.AUDIO_SERVICE)}
sleep 2
OLD_PIDS="$(pidof {shlex.quote(v2476.AUDIO_PROCESS)} 2>/dev/null || true)"
if [ -n "$OLD_PIDS" ]; then
  echo A90_ACDBTAP_OLD_PIDS_STILL_PRESENT "$OLD_PIDS"
fi
(
  export LD_PRELOAD={shlex.quote(preload_path)}
  export A90_ACDBTAP_DIR={shlex.quote(v2476.REMOTE_CAPTURE_DIR)}
  exec {shlex.quote(v2476.AUDIO_BINARY)}
) > {shlex.quote(v2476.REMOTE_STAGE_DIR)}/manual-audio-hal.stdout 2> {shlex.quote(v2476.REMOTE_STAGE_DIR)}/manual-audio-hal.stderr &
echo $! > {shlex.quote(v2476.REMOTE_STAGE_DIR)}/manual-audio-hal.pid
sleep 3
NEW_PIDS="$(pidof {shlex.quote(v2476.AUDIO_PROCESS)} 2>/dev/null || true)"
echo A90_ACDBTAP_MANUAL_HAL_PIDS "$NEW_PIDS"
for pid in $NEW_PIDS; do
  if grep -q libacdbtap /proc/$pid/maps 2>/dev/null; then
    echo A90_ACDBTAP_PRELOAD_CONFIRMED pid=$pid path={shlex.quote(preload_path)}
  fi
done
""".strip()
    return adb_su(args, script)
...
def capture_cleanup_command(args: argparse.Namespace) -> list[str]:
    script = f"""
set -eu
if [ -f {shlex.quote(v2476.REMOTE_STAGE_DIR)}/manual-audio-hal.pid ]; then
  kill "$(cat {shlex.quote(v2476.REMOTE_STAGE_DIR)}/manual-audio-hal.pid)" 2>/dev/null || true
fi
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    module = v2480.materialize_module(args.module_out_dir) if args.materialize_module else {"ok": False, "reason": "not materialized"}
    module_plan = v2480.command_plan(args.module_out_dir)
    module_safety = v2480.command_safety({"command_plan": module_plan, "module": module})
    ...
    commands = {
        ...
        "module_verify_vendor_visible": module_verify_command(args),
        "capture_setup": capture_setup_command(args),
        "preflight_vendor_hal": preflight_vendor_hal_command(args, "<verified-vendor-preload-candidate>"),
        "manual_start_vendor_preload": manual_start_vendor_preload_command(args, "<verified-vendor-preload-candidate>"),
    ...
    payload: dict[str, Any] = {
        ...
        "hard_stops": [
            "abort before playback if no vendor preload candidate has the V2475 SHA",
    ...
    if not module.get("ok"):
        blockers.append(f"V2480 module materialization failed: {module.get('reason')}")
    if not module_safety.get("ok"):
        blockers.append(f"V2480 module command safety failed: {module_safety}")
    ...
    if not safety.get("ok"):
        blockers.append("V2481 command safety failed")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not plan.get("future_live_ready"):
        raise RuntimeError(f"V2481 live inputs are not ready: {plan.get('future_live_blockers')}")
    if not plan.get("command_safety", {}).get("ok"):
        raise RuntimeError(f"V2481 command safety failed: {plan.get('command_safety')}")
    ...
    try:
        ...
        verify_record = route.run_step("v2481-module-verify-vendor-visible", module_verify_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False)
        ...
        if not preload_path:
            result["decision"] = f"{decision_slug()}-vendor-preload-candidate-not-verified-before-rollback"
        ...
        steps.append(route.run_step("acdbtap-v2481-capture-setup", capture_setup_command(args), out_dir, timeout_sec=args.adb_command_timeout))
        steps.append(route.run_step("acdbtap-v2481-preflight-vendor-hal", preflight_vendor_hal_command(args, preload_path), out_dir, timeout_sec=args.adb_command_timeout, check=False))
        manual_start_record = route.run_step("acdbtap-v2481-manual-start-vendor-preload", manual_start_vendor_preload_command(args, preload_path), out_dir, timeout_sec=args.adb_command_timeout, check=False)
        steps.append(manual_start_record)
        ...
        if not v2477.preload_confirmed(manual_start_record):
        ...
        logcat_capture["record"]["name"] = "acdbtap-v2481-live-logcat"
        ...
        steps.append(route.run_step("acdbtap-v2481-capture-cleanup", capture_cleanup_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False))
        capture_cleanup_done = True
        steps.append(route.run_step("v2481-module-cleanup-exact", module_cleanup_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False))
        ...
        if rollback_needed:
            try:
                if not capture_cleanup_done:
                    steps.append(route.run_step("acdbtap-v2481-capture-cleanup-finally", capture_cleanup_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False))
                if module_stage_started and not module_cleanup_done:
                    steps.append(route.run_step("v2481-module-cleanup-exact-finally", module_cleanup_command(args), out_dir, timeout_sec=args.adb_command_timeout, check=False))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--module-out-dir", type=Path, default=v2480.DEFAULT_MODULE_OUT_DIR)
