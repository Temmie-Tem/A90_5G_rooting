#!/usr/bin/env python3
"""V2560 Android handoff for the V2559 ACDB per-device manifest capture.

This wrapper reuses the proven V2490 Android boot/stage/pull/rollback engine.
The new behavior is limited to selecting the V2559 helper/preload artifacts,
forcing the fake audio-cal ioctl policy, and classifying pulled acdbtap/ioctl
events for per-device non-zero calibration records while common topology is
intentionally skipped because its payload is already pinned.
"""
...
import build_android_acdb_per_device_manifest_v2559 as v2559
import native_audio_acdb_ownprocess_get_live_handoff_v2490 as v2490

RUN_ID = "V2560"
BUILD_TAG = "v2560-audio-acdb-per-device-manifest-live"
ROOT = v2559.ROOT
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2560_AUDIO_ACDB_PER_DEVICE_MANIFEST_LIVE_HANDOFF_2026-06-16.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2560-acdb-per-device-manifest-{stamp}"
...
def read_v2559_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
...
def build_v2559_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        build_root=args.v2559_build_root,
        manifest_path=args.v2559_manifest_path,
        clang=v2559.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2559.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    return v2559.manifest(build_args)


def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2559_artifacts:
        build_v2559_artifacts(args)
    manifest = read_v2559_manifest(args.v2559_manifest_path)
...
def summarize_per_device_manifest(artifact_dir: Path) -> dict[str, Any]:
    base_summary = v2490.parse_ownget_artifacts(artifact_dir)
    helper_events_path = artifact_dir / "acdb-per-device-manifest-events.jsonl"
    helper_events = read_jsonl(helper_events_path)
    helper_stages = [event.get("stage") for event in helper_events if event.get("event") == "per_device_helper"]
    ...
    if pass_through_set_events:
        classification = "v2560-boundary-violation-real-audio-set-passthrough"
    elif per_device_success:
        classification = "v2560-per-device-manifest-captured"
    elif topology and not send_audio_cal_v5_reached:
        classification = "v2560-init-common-topology-recaptured-before-per-device"
    elif send_audio_cal_v5_reached:
        classification = "v2560-send-audio-cal-v5-no-per-device-records"
    elif topology_only:
        classification = "v2560-unexpected-topology-only-captured"
    elif partial_success:
        classification = "v2560-partial-nonzero-acdbtap-capture"
    else:
        classification = f"v2560-{base_summary.get('classification', 'no-acdbtap-capture')}"
    return {
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2560-acdb-per-device-manifest-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2559_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            ...
            "per_device_manifest_acceptance": "at least one ordered ret==0 non-zero acdb_ioctl record with out_len not in {4, 4916}; topology is intentionally skipped",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2559 manual-arm helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2560 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2559_artifacts"]
    base_args = to_v2490_args(args, artifacts)
    result = v2490.run_live(base_args)
    pulled_dir = select_pulled_dir_from_result(result)
    per_device_summary = summarize_per_device_manifest(pulled_dir) if pulled_dir else {
        "classification": "v2560-no-pulled-artifacts",
        "full_success": False,
...
def write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "## Scope",
        "",
        "Android own-process ACDB capture handoff using the V2490 checked Android boot/stage/pull/rollback engine and the V2559 per-device helper/preload artifacts. Common topology is intentionally skipped because V2548/V2557 already pinned it.",
        "",
        "## Result",
        "",
        ...
        "## Artifacts",
        "",
        f"- helper_sha256: `{payload.get('v2559_artifacts', {}).get('helper', {}).get('sha256')}`",
        f"- preload_sha256: `{payload.get('v2559_artifacts', {}).get('preload', {}).get('sha256')}`",
        "",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2559-artifacts", action="store_true")
    parser.add_argument("--v2559-build-root", type=Path, default=v2559.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2559-manifest-path", type=Path, default=v2559.DEFAULT_MANIFEST)
