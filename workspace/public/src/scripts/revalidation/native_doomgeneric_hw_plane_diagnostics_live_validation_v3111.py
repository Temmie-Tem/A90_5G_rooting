#!/usr/bin/env python3
"""V3111 rollback-gated live validation for V3110 DOOM hardware plane diagnostics."""
...
RUN_ID = "V3111"
BUILD_TAG = "v3111-doomgeneric-hw-plane-diagnostics-live"
DECISION_PREFIX = "v3111-doomgeneric-hw-plane-diagnostics"
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V3111_DOOMGENERIC_HW_PLANE_DIAGNOSTICS_LIVE_2026-06-23.md"

CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v3110_doomgeneric_hw_plane_diagnostics.img"
CANDIDATE_VERSION = "0.10.110"
CANDIDATE_TAG = "v3110-doomgeneric-hw-plane-diagnostics"
CANDIDATE_SHA256 = "71257ef7fdc9636caa5d40d0c956a59cce36bce7a61ba59beed268bf77d34f57"
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# Native Init V3111 DOOMGENERIC Hardware Plane Diagnostics Live Validation",
        "",
        "## Summary",
        "",
        ...
        *_marker_lines(loop.get("markers", {}) if isinstance(loop.get("markers"), dict) else {}),
        ...
        "- `cpu-fallback-observed` means V3110 safely ran but the large path still used the known stuttering CPU 3:2 scaler; proceed to the pre-scaled-producer fallback.",
        "- `hw_plane.stage=fetch-resources` with `rc=-14` means the legacy `GETRESOURCES` inventory failed before plane enumeration; split pass1/pass2 or cache the CRTC index from KMS init before retrying HW-plane selection.",
        ...
        "## Host Validation",
        "",
        "- `python3 -m py_compile workspace/public/src/scripts/revalidation/native_doomgeneric_hw_plane_diagnostics_live_validation_v3111.py tests/test_native_doomgeneric_hw_plane_diagnostics_live_v3111.py`: PASS",
        "- `PYTHONPATH=tests:workspace/public/src/scripts/revalidation:workspace/public/src/harness python3 -m unittest tests.test_native_doomgeneric_hw_plane_diagnostics_live_v3111`: PASS",
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if not result["pass"]:
            raise RuntimeError(f"V3110 hardware plane scale live validation did not classify cleanly: {classification}")
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"flash exact V3110 image {CANDIDATE_IMAGE}",
            ...
            "parse hw_plane stage/counters/presented/fallback, timing, seq, and pageflip markers",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--live", action="store_true", help="flash V3110, run bounded DOOM loop, then rollback")
