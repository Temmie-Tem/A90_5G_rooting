#!/usr/bin/env python3
"""Build V2577 ARM32 common-topology-entry ACDB capture artifacts.

Host-only unit. The private outputs are:

- a 32-bit Android PIE helper that only calls acdb_loader_init_v3();
- a single 32-bit combined preload exporting acdb_ioctl(), ioctl(),
  a90_arm_capture(), and acdb_loader_send_common_custom_topology().

The key V2577 delta is that acdb_ioctl stays completely silent until the
common-topology hook is entered during init. The hook arms capture immediately
before calling the real acdb_loader_send_common_custom_topology(), so init-time
ACDB noise is skipped but the topology GET inside common-topology is captured.
The preload still fakes audio-cal allocate/dealloc/SET when
A90_ACDB_FAKE_ALLOCATE=1, so this remains measurement-only.
"""
...
RUN_ID = "V2577"
BUILD_TAG = "v2577-acdb-common-topology-entry-capture-host-only"
HELPER_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/a90_acdb_init_drive_exec_linked_v2572.c"
ACDBTAP_SOURCE_REL = preload_base.ACDBTAP_SOURCE_REL
IOCTL_SOURCE_REL = preload_base.IOCTL_SOURCE_REL
COMMON_HOOK_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_common_topology_arm_v2577.c"
...
HELPER_ARTIFACT_NAME = "a90_acdb_common_topology_entry_init_exec_linked_v2577"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_common_topology_entry_capture_v2577.so"
...
def source_state() -> dict[str, Any]:
    ...
    common_source = ROOT / COMMON_HOOK_SOURCE_REL
    ...
    common_text = common_source.read_text(encoding="utf-8", errors="replace") if common_source.exists() else ""
    required = {
        "helper_source_exists": helper_source.exists(),
        "tap_source_exists": tap_source.exists(),
        "ioctl_source_exists": ioctl_source.exists(),
        "common_hook_source_exists": common_source.exists(),
        "helper_calls_init_v3": "acdb_loader_init_v3(A90_ACDB_FILES_PATH, A90_DELTA_DIR, 0U)" in helper_text,
        "helper_init_only_no_arm": "a90_arm_capture" not in helper_text and "acdb_loader_send_common_custom_topology" not in helper_text,
        "common_hook_exports_common_topology": "int32_t acdb_loader_send_common_custom_topology(void)" in common_text,
        "common_hook_arms_before_real": common_text.find("a90_arm_capture()") >= 0 and common_text.find("ret = a90_real_common_topology()") > common_text.find("a90_arm_capture()"),
        "common_hook_calls_real_common_topology": "a90_real_common_topology()" in common_text,
        ...
        "tap_custom_topology_only_macro": "A90_ACDBTAP_CUSTOM_TOPOLOGY_ONLY" in tap_text,
        "tap_custom_topology_size_cmd": "A90_CMD_CUSTOM_TOPO_INFO_SIZE_V3" in tap_text,
    ...
    prohibited = {
        ...
        "persistent_magisk_install": "magisk --install-module" in (helper_text + tap_text + ioctl_text + common_text),
    }
    return {
        "sources": [HELPER_SOURCE_REL, ACDBTAP_SOURCE_REL, IOCTL_SOURCE_REL, COMMON_HOOK_SOURCE_REL],
        "required": required,
...
def vendor_lib_state(readelf: str) -> dict[str, Any]:
    ...
    state["required_for_v2577_ok"] = all(state["required_for_v2577"].values())
    return state
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    helper_obj = obj_dir / "a90_acdb_common_topology_entry_init_exec_linked_v2577.o"
    tap_obj = obj_dir / "libacdbtap_v2475_postinit_manual_arm.o"
    ioctl_obj = obj_dir / "a90_ioctl_trace_preload_v2531.o"
    common_obj = obj_dir / "libacdb_common_topology_arm_v2577.o"
    ...
    common_compile = compile_object(ROOT / COMMON_HOOK_SOURCE_REL, common_obj, clang=clang, env=env, log_dir=log_dir, cflags=PRELOAD_CFLAGS)
    ...
    if tap_compile["ok"] and ioctl_compile["ok"] and common_compile["ok"]:
        preload_link_cmd = [str(lld), *PRELOAD_LDFLAGS, "-o", str(preload_out), str(tap_obj), str(ioctl_obj), str(common_obj)]
    ...
    return {
        ...
        "compile": {"helper": helper_compile, "acdbtap": tap_compile, "ioctl_trace": ioctl_compile, "common_topology_hook": common_compile},
...
def manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            ...
            "unarmed_policy": "all acdb_ioctl calls before common-topology hook a90_arm_capture() pass through with no dump/hash/file I/O",
            "arm_point": "common-topology entry-arm: common-topology hook calls a90_arm_capture() at acdb_loader_send_common_custom_topology() entry before calling the real function",
            "armed_policy": "capture every out_len>0 ACDB call after arm; exit immediately after first ret==0 non-all-zero 4916-byte buffer",
    ...
    payload["ok"] = bool(
        ...
        and payload["vendor_lib_state"].get("required_for_v2577_ok")
