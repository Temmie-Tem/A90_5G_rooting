#!/usr/bin/env python3
"""V2982 live handoff for native-bounded touch sampling via readinput.

This runner flashes the V2981 readinput-timeout candidate image, revalidates the
selected event from the full ``inputscan`` output, runs one native-bounded
``readinput <eventX> <count> <timeout_ms>`` window, and always rolls back to the
v2321 clean USB identity checkpoint. The command only reads evdev events; it
does not inject input or alter keymaps.
"""
...
RUN_ID = "V2982"
BUILD_TAG = "v2982-readinput-timeout-live"
DECISION_PREFIX = "v2982-readinput"
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2982_READINPUT_TIMEOUT_LIVE_2026-06-20.md"

CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2981_readinput_timeout.img"
CANDIDATE_VERSION = "0.10.61"
CANDIDATE_TAG = "v2981-readinput-timeout"
CANDIDATE_SHA256 = "c5ca7f973823f1f4ca5fc63a9c4d6c19582f4af632531f2954459e2f8a827d98"
...
def preflight_state(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "hard_boundary": [
            ...
            "native readinput timeout returns ETIMEDOUT if the sample window times out",
...
def run_timeout_readinput(host: str,
                              port: int,
                              event: str,
                              count: int,
                              *,
    ...
    command = ["readinput", event, str(count), str(timeout_ms)]
    line = a90ctl.encode_cmdv1_line(command)
    data = bytearray()
    started = time.monotonic()
    command_timeout = (timeout_ms / 1000.0) + command_timeout_margin + connect_timeout
    with SerialBridgeLock(timeout_sec=command_timeout, purpose="v2982-readinput"):
...
def render_report(result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- Event under test: `{result.get('event')}` count=`{result.get('count')}` timeout_ms=`{result.get('timeout_ms')}`",
        ...
        f"- Hide before inputscan ok: `{int(bool(result.get('candidate_hide_before_inputscan_ok')))}`",
        f"- Full `inputscan` rc: `{result.get('inputscan_rc')}` selected_found=`{int(bool(result.get('inputscan_event_found')))}` touch_class=`{int(bool(result.get('inputscan_event_is_touch')))}`",
        f"- Hide before readinput ok: `{int(bool(result.get('candidate_hide_before_readinput_ok')))}`",
        f"- `readinput` rc: `{result.get('readinput_rc')}` timeout_ms=`{sample.get('timeout_ms')}`",
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        hide_before_inputscan = base.run_serial_step(out_dir, steps, "candidate-hide-before-inputscan", ["hide"], timeout=60.0, retry_unsafe=True)
        result["candidate_hide_before_inputscan_ok"] = hide_before_inputscan.get("rc") == 0
        inputscan_step = base.run_serial_step(out_dir, steps, "candidate-inputscan-full-before-readinput", ["inputscan"], timeout=120.0, retry_unsafe=True)
        inputscan = inputscan_live.parse_inputscan(stdout_of(inputscan_step))
        selected = next((item for item in inputscan.get("events", []) if item.get("event") == args.event), {})
        selected_classes = {part.strip() for part in str(selected.get("class", "")).split(",") if part.strip()}
        result.update({
            "inputscan_rc": inputscan_step.get("rc"),
            "inputscan": inputscan,
            "inputscan_selected_event": selected,
            "inputscan_event_found": bool(selected),
            "inputscan_event_is_touch": "touch" in selected_classes,
        })
        hide_before_readinput = base.run_serial_step(out_dir, steps, "candidate-hide-before-readinput", ["hide"], timeout=60.0, retry_unsafe=True)
        result["candidate_hide_before_readinput_ok"] = hide_before_readinput.get("rc") == 0
        sample = run_timeout_readinput(
            args.host,
            args.port,
            args.event,
            args.count,
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "commands": [
            ...
            f"readinput {args.event} {args.count} {args.timeout_ms} with native timeout",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--live", action="store_true", help="flash V2981, sample bounded readinput, then rollback to V2321")
