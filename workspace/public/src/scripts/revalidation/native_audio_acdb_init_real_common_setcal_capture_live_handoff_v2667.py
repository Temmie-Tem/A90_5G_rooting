#!/usr/bin/env python3
"""V2667 Android handoff wrapper for V2666 init-real-common ACDB SET capture.

Host-only by default. Live mode reuses the V2490 checked Android
boot/stage/pull/rollback engine, but selects the V2666 helper/preload artifacts
and classifies the captured ordered ``AUDIO_SET_CALIBRATION`` records emitted to
``setcal-events.jsonl``. The live action remains measurement-only: the V2666
...
The own-process send path is the V2666 init-real-common preload:

    acdb_loader_init_v3("/vendor/etc/audconf/OPEN", delta, meta_head)
      -> init-time common hook
      -> patch initialized flag
      -> real acdb_loader_send_common_custom_topology()
      -> exit_group(0)

The V2666 preload intercepts every ``AUDIO_SET_CALIBRATION``, dumps
...
import build_android_acdb_common_init_real_setcal_capture_v2666 as v2666
...
ROOT = v2666.ROOT
RUN_ID = "V2667"
BUILD_TAG = "v2667-audio-acdb-init-real-common-setcal-capture-live-runner"
DEFAULT_OUT_BASE = ROOT / "workspace/private/runs/audio"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2667_AUDIO_ACDB_INIT_REAL_COMMON_SETCAL_CAPTURE_LIVE_HANDOFF_2026-06-18.md"
...
def default_live_out_dir() -> Path:
    ...
    return DEFAULT_OUT_BASE / f"v2667-acdb-init-real-common-setcal-capture-{stamp}"
...
def build_v2666_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    build_args = argparse.Namespace(
        build=True,
        write_report=False,
        build_root=args.v2666_build_root,
        manifest=args.v2666_manifest_path,
        report=v2666.DEFAULT_REPORT,
        clang=v2666.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/clang",
        lld=v2666.v2659.v2630.v2613.v2611.v2608.v2572.TOOLCHAIN_ROOT / "bin/ld.lld",
        readelf=args.readelf,
        file=args.file,
    )
    payload = v2666.make_payload(build_args)
    args.v2666_manifest_path.parent.mkdir(parents=True, exist_ok=True)
    args.v2666_manifest_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    return payload


def read_v2666_manifest(path: Path) -> dict[str, Any]:
    if not path.exists():
    ...
    setcal_contract_ok = bool(
        ...
        and required.get("phase_hook_init_real_event_path_v2666")
        and required.get("phase_hook_init_common_enter")
        and required.get("phase_hook_calls_real_common_during_init")
        and required.get("phase_hook_exits_after_init_real")
        and required.get("phase_hook_fails_closed_on_patch_failure")
...
def selected_artifacts(args: argparse.Namespace) -> dict[str, Any]:
    if args.build_v2666_artifacts:
        build_v2666_artifacts(args)
    manifest = read_v2666_manifest(args.v2666_manifest_path)
...
def summarize_no_pulled_artifacts(result: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    classification = "v2667-no-pulled-artifacts"
    failure_phase = None
    ...
    if native_to_recovery_timeout:
        classification = "v2667-preflash-native-bridge-unavailable"
...
def summarize_setcal_capture(artifact_dir: Path) -> dict[str, Any]:
    ...
    phase_path = artifact_dir / "acdb-v2666-init-real-common-events.jsonl"
    setcal_events = [
    ...
    phase_stages = [
        ...
        if event.get("event") == "v2666_init_real_common"
    ]
    phase_common_return_codes = [
        int32_or_none(event.get("code"))
        for event in phase_events
        if event.get("event") == "v2666_init_real_common"
        and event.get("stage") == "init_real_common_return"
    ]
    phase_common_return_codes = [code for code in phase_common_return_codes if code is not None]
    init_short_success = (
        "init_common_enter" in phase_stages
        and "init_patch_initialized_flag_return" in phase_stages
    )
    reentry_neutralized = "common_reentry_neutralized" in phase_stages
    init_real_common_called = (
        "init_before_real_common" in phase_stages
        and "init_real_common_return" in phase_stages
    )
    ...
    if real_set_events:
        classification = "v2667-boundary-violation-real-audio-set-passthrough"
    elif reentry_neutralized and not rows:
        classification = "v2667-phase-common-reentry-neutralized-no-setcal"
    elif init_real_common_called and not rows:
        classification = "v2667-init-real-common-returned-before-setcal-no-setcal"
    elif init_short_success and not rows:
        classification = "v2667-init-hook-entered-no-setcal"
    elif not rows:
        classification = f"v2667-{base_summary.get('classification', 'no-setcal-records')}"
    elif custom_complete and custom_payloads_dumped:
        classification = "v2667-custom-topology-setcal-captured"
    elif custom_rows:
        classification = "v2667-custom-topology-setcal-partial"
    elif payload_rows or header_only_rows:
        classification = "v2667-setcal-records-no-custom-topology"
    else:
        classification = "v2667-setcal-records-incomplete"

    return {
        "classification": classification,
        "success": classification == "v2667-custom-topology-setcal-captured",
        "partial_success": classification in {
            "v2667-custom-topology-setcal-partial",
            "v2667-setcal-records-no-custom-topology",
            "v2667-phase-common-reentry-neutralized-no-setcal",
            "v2667-init-real-common-returned-before-setcal-no-setcal",
            "v2667-init-hook-entered-no-setcal",
        },
        "operator_valuable": bool(rows or init_short_success or init_real_common_called or reentry_neutralized),
        "counts_toward_fails_twice": (
            False
            if init_short_success or init_real_common_called or reentry_neutralized
        ...
        "init_real_common_called": init_real_common_called,
...
def dry_run_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        "run_id": RUN_ID,
        "build_tag": BUILD_TAG,
        "decision": "v2667-acdb-init-real-common-setcal-capture-live-runner-dry-run",
        "host_only": True,
        "device_action": "none",
        "operator_spec": "docs/OPERATOR_ACDB_IOCTL_INTERPOSE_CAPTURE_SPEC_2026-06-15.md",
        "v2666_artifacts": artifacts,
        "v2490_engine": {
            "run_id": "V2490",
        ...
        "capture_contract": {
            "send_path": "init_v3 -> init-time hook -> patch initialized -> real send_common_custom_topology() -> exit_group(0)",
    ...
    if not artifacts.get("ok"):
        payload["live_blockers"].append("V2666 SET-cal helper/preload artifacts are not ready")
...
def run_live(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not dry.get("ok"):
        raise RuntimeError(f"V2667 live inputs are not ready: {dry.get('live_blockers')}")
    artifacts = dry["v2666_artifacts"]
    ...
    out_dir_raw = result.get("out_dir")
    if out_dir_raw:
        write_json(ROOT / str(out_dir_raw) / "v2667-result.json", wrapper)
    return wrapper


def write_report(path: Path, payload: dict[str, Any]) -> None:
    summary = payload.get("setcal_summary", {})
    artifacts = payload.get("v2666_artifacts", {})
    helper = artifacts.get("helper", {})
    preload = artifacts.get("preload", {})
    lines = [
        ...
        f"- init_real_common_called: `{summary.get('init_real_common_called')}`",
    ...
    if summary.get("failure_phase") or summary.get("failure_evidence"):
        lines.extend([
            ...
            "  V2666 SET-calibration capture path.",
        ])
    if summary.get("classification") == "v2667-init-hook-entered-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2666 init hook was entered:",
            "  `init_common_enter` and `init_patch_initialized_flag_return` were captured;",
            "- no `init_before_real_common` / `init_real_common_return` or SET rows were emitted;",
    ...
    if summary.get("classification") == "v2667-phase-common-reentry-neutralized-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
    ...
    if summary.get("classification") == "v2667-init-real-common-returned-before-setcal-no-setcal":
        lines.extend([
            "",
            "## Failure Analysis",
            "",
            "- live rollback passed and the V2666 preload reached the real common-topology",
            "  function inside the init-time hook (`init_before_real_common` then",
            "  `init_real_common_return`);",
            f"- phase_common_return_codes: `{summary.get('phase_common_return_codes')}`;",
            ...
            "- this is useful frontier evidence: V2666 passed the init-short phase and reached",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--build-v2666-artifacts", action="store_true")
    parser.add_argument("--v2666-build-root", type=Path, default=v2666.DEFAULT_BUILD_ROOT)
    parser.add_argument("--v2666-manifest-path", type=Path, default=v2666.DEFAULT_MANIFEST)
