#!/usr/bin/env python3
"""V2796 live handoff for native-init audio ION devnode.

This is the audio core closure gate: flash the V2796 native-init image, stage the
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2796-audio-ion-devnode/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2796_audio_ion_devnode.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2796_AUDIO_ION_DEVNODE_LIVE_2026-06-19.md"

CYCLE = "V2796"
PROFILE = "internal-speaker-safe"
CANDIDATE_VERSION = "0.9.309"
CANDIDATE_TAG = "v2796-audio-ion-devnode"
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2796-audio-ion-devnode-{now_slug()}"
    ...
    result: dict[str, Any] = {
        "decision": "v2796-audio-ion-devnode-live-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if play.get("rc") != 0 or "audio.play.worker.started=1" not in play_text:
            result["decision"] = "v2796-audio-ion-devnode-start-failed-before-rollback"
            result["play_summary"] = classify_play_output(play_text)
            raise RuntimeError("native audio ION devnode did not start")
        ...
        log_step = run_serial_step(
            out_dir,
            steps,
            "candidate-audio-ion-devnode-log",
        ...
        if not result["play_output_pass"]:
            result["decision"] = "v2796-audio-ion-devnode-live-worker-failed-before-rollback"
            raise RuntimeError("native audio ION devnode did not emit all required pass markers")
        ...
        result["decision"] = "v2796-audio-ion-devnode-native-command-pass-before-rollback"
    except Exception as exc:
        result.setdefault("decision", "v2796-audio-ion-devnode-live-blocked")
        if result["decision"] == "v2796-audio-ion-devnode-live-started":
            result["decision"] = "v2796-audio-ion-devnode-live-blocked"
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    deploy_plan = read_json(DEPLOY_PLAN)
    return {
        "decision": "v2796-audio-ion-devnode-live-dry-run",
