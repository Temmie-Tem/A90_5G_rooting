#!/usr/bin/env python3
"""V2891 live handoff for SD-cached V2889 video fixtures.

This runner flashes the V2889 candidate, reuses or populates a SHA-addressed SD-card video fixture cache, runs the bounded `video stream --manifest ... --video-only --present pageflip` command, records DRM page-flip interval telemetry, then rolls back to v2321 and verifies selftest fail=0.
"""
...
ROOT = repo_root()
RUN_ID = "V2891"
BUILD_TAG = "v2891-video-fixture-cache-live"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2889_video_cadence.img"
CANDIDATE_VERSION = "0.10.30"
CANDIDATE_TAG = "v2889-video-cadence"
CANDIDATE_SHA256 = "6e946878aaf749de333ac27ab229e199a64ae4d07096079c1743f92a05a43a4f"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2891_VIDEO_FIXTURE_CACHE_LIVE_2026-06-19.md"
PREPARE_STREAM = ROOT / "workspace/public/src/scripts/revalidation/prepare_video_stream_v2874.py"
REMOTE_DIR = "/mnt/sdext/a90/runtime/video/v2891"
REMOTE_CACHE_ROOT = "/mnt/sdext/a90/runtime/video/cache"
LEGACY_REMOTE_DIRS = ("/mnt/sdext/a90/runtime/video/v2890",)
...
def preflight_state(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "cadence_target": "30fps full-resolution page-flip stream",
...
def dry_run_payload(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    return {
        "decision": "v2891-video-fixture-cache-live-dry-run" if preflight_ok(state) else "v2891-video-fixture-cache-live-blocked",
...
def run_live(args: argparse.Namespace, out_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "decision": "v2891-video-fixture-cache-live-started",
        "pass": False,
        "preflight": state,
        "steps": steps,
    ...
    try:
        ...
        if not (result["candidate_version_ok"] and result["candidate_selftest_fail0"] and result["candidate_video_status_ok"]):
            result["decision"] = "v2891-video-fixture-cache-candidate-health-failed-before-stream"
        ...
        if stream.get("rc") != 0 or not result["stream_summary"].get("pass"):
            result["decision"] = "v2891-video-fixture-cache-stream-failed-before-rollback"
        ...
        if not result["candidate_selftest_after_stream_fail0"]:
            result["decision"] = "v2891-video-fixture-cache-post-stream-selftest-failed"
            raise RuntimeError("candidate post-stream selftest did not report fail=0")
        result["decision"] = "v2891-video-fixture-cache-live-pass-before-rollback"
        result["pass"] = True
    except Exception as exc:
        result.setdefault("decision", "v2891-video-fixture-cache-live-blocked")
        if result["decision"] == "v2891-video-fixture-cache-live-started":
            result["decision"] = "v2891-video-fixture-cache-live-blocked"
...
def main() -> int:
    args = parse_args()
    out_dir = ROOT / f"workspace/private/runs/video/v2891-video-fixture-cache-live-{now_slug()}"
    ...
    if not preflight_ok(state):
        payload = {
            "decision": "v2891-video-fixture-cache-live-preflight-failed-no-flash",
            "pass": False,
            "preflight": state,
        }
