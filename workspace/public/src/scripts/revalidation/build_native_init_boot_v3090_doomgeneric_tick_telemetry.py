#!/usr/bin/env python3
"""Build V3090 native-init DOOM tick telemetry candidate."""
...
CYCLE = "V3090"
INIT_VERSION = "0.10.101"
INIT_BUILD = "v3090-doomgeneric-tick-telemetry"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3090_DOOMGENERIC_TICK_TELEMETRY_SOURCE_BUILD_2026-06-23.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images", "boot_linux_v3090_doomgeneric_tick_telemetry.img", legacy_fallback=False
)
INIT_BINARY = OUT_DIR / "init_v3090_doomgeneric_tick_telemetry"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3090_doomgeneric_tick_telemetry.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v513_doomgeneric_tick_telemetry"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3090"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3090.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3090.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3090"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3090-tick-telemetry"
...
FRAME_PATH = "/tmp/a90-doomgeneric-v3090-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3090-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3090-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3090-input.sock"
INPUT_UDP_PORT = v3086.INPUT_UDP_PORT
DEVICE_NCM_HOST = v3086.DEVICE_NCM_HOST
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3090-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3090-tick-telemetry.txt"
...
TICK_TELEMETRY_MARKER = "a90.doomgeneric.v3090.tick_telemetry=fake-time-vs-game-tic-summary"

REQUIRED_STRINGS = (
    b"A90 Linux init 0.10.101 (v3090-doomgeneric-tick-telemetry)",
    b"v3090-doomgeneric-tick-telemetry",
    b"doomgeneric-private-link-v3090-tick-telemetry",
    b"/bin/a90_doomgeneric_private_engine_v3090",
...
def _replace_required(source: str, old: str, new: str) -> str:
    if old not in source:
        raise RuntimeError(f"missing V3090 adapter source anchor: {old[:96]!r}")
    return source.replace(old, new, 1)


def v3090_adapter_source() -> str:
    ...
    source = _replace_required(
        ...
        'const char a90_doomgeneric_v3081_frame_ipc_policy[] =\n'
        '    "a90.doomgeneric.v3081.frame_ipc=shared-mmap-seq";\n'
        'const char a90_doomgeneric_v3090_tick_telemetry_policy[] =\n'
    ...
    source = _replace_required(
        ...
        "        marker_checksum(a90_doomgeneric_v3079_pace_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3081_frame_ipc_policy) == 0U ||\n"
        "        marker_checksum(a90_doomgeneric_v3090_tick_telemetry_policy) == 0U) {\n",
    )
    source = _replace_required(
        source,
...
def apply_v3090_globals() -> None:
    ...
    v3086.v3084.v3083.v3081.v3081_adapter_source = v3090_adapter_source
    v3086.apply_v3086_globals()
    V3059.v3059_adapter_source = v3090_adapter_source
...
def render_report(
    ...
    return "\n".join([
        ...
        *marker_lines,
        ...
        f"- Scope: flash exact V3090 boot image via `native_init_flash.py`, health-check, run bounded DOOM loop, then read `{TICK_TELEMETRY_PATH}` to compare fake ticks, game tics, presented frames, and pageflip timing.",
        "",
        "## Metadata",
        "",
...
def main() -> int:
    ...
    doom.update({
        ...
        "cadence_experiment": "tick-telemetry-before-time-model-change",
    ...
    manifest.update({
        ...
        "candidate_type": "doomgeneric-tick-telemetry-candidate",
        "adoption_state": "pending-tick-telemetry-live-validation",
    })
    ...
    (OUT_DIR / "doomgeneric-tick-telemetry-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "doomgeneric-tick-telemetry-candidate",
