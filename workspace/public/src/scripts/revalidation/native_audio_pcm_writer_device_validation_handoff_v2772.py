#!/usr/bin/env python3
"""V2772 live validation for the native-init audio PCM writer.

This is the follow-up to V2771: flash the image that replaced the source-level
`execute-not-implemented-native-pcm` refusal with a bounded native ALSA PCM writer,
prove how far `audio play --execute` gets on hardware, and roll back to V2321.
"""
...
BUILD_MANIFEST = ROOT / "workspace/private/builds/native-init/v2771-audio-pcm-writer/manifest.json"
CANDIDATE_IMAGE = ROOT / "workspace/private/inputs/boot_images/boot_linux_v2771_audio_pcm_writer.img"
...
REPORT_PATH = ROOT / "docs/reports/NATIVE_INIT_V2772_AUDIO_PCM_WRITER_DEVICE_VALIDATION_2026-06-19.md"

CANDIDATE_VERSION = "0.9.294"
CANDIDATE_TAG = "v2771-audio-pcm-writer"
...
def render_report(result: dict[str, Any]) -> str:
    ...
    summary_lines = [
        ...
        "execute_supported={supported} open_attempt={open_attempt} hw_params={hw} "
        "prepare={prepare} write_attempt={write} done={done}".format(
            name=item["name"],
            ok=item["ok"],
            rc=item["rc"],
...
def live_run(args: argparse.Namespace, state: dict[str, Any]) -> dict[str, Any]:
    ...
    out_dir = ROOT / f"workspace/private/runs/audio/v2772-audio-pcm-writer-device-validation-{now_slug()}"
    out_dir.mkdir(parents=True, exist_ok=False)
    write_json(out_dir / "preflight.json", state)
    steps: list[dict[str, Any]] = []
    result: dict[str, Any] = {
        "decision": "v2772-audio-pcm-writer-device-validation-started",
        "out_dir": rel(out_dir),
    ...
    try:
        ...
        if result["old_execute_refusal_observed"]:
            result["decision"] = "v2772-audio-pcm-writer-source-refusal-regression"
        elif result["pcm_done_observed"]:
            result["decision"] = "v2772-audio-pcm-writer-device-pass"
        elif result["alsa_open_attempt_observed"]:
            result["decision"] = "v2772-audio-pcm-writer-reached-alsa-open"
        elif result["execute_supported_observed"]:
            result["decision"] = "v2772-audio-pcm-writer-supported-but-no-open"
        else:
            result["decision"] = "v2772-audio-pcm-writer-device-validation-unexpected-play-execute-result"
    finally:
...
def dry_run(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "decision": "v2772-audio-pcm-writer-device-validation-dry-run",
