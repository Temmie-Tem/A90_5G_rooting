#!/usr/bin/env python3
"""Build V2703 ACDB lower large-buffer custom-topology GET artifacts.

V2702 showed that cal_type 10 / command 0x11394 returns -12 from libaudcal's
insufficient output-buffer branch, not from a missing-table branch.  This
host-only build unit keeps the V2674/V2692 lower hidden-node route, but replaces
the 4 KiB loader-provided GET destination with an own-process 64 KiB buffer and
teaches the acdbtap wrapper to dump lower ADM/ASM/AFE custom-topology indirect
outputs after ret==0.
...
ROOT = v2692.ROOT
RUN_ID = "V2703"
BUILD_TAG = "v2703-acdb-large-buffer-topology-get-build-only"
DEFAULT_BUILD_ROOT = ROOT / "workspace/private/builds/audio" / BUILD_TAG
DEFAULT_MANIFEST = DEFAULT_BUILD_ROOT / "manifest.json"
DEFAULT_REPORT = ROOT / "docs/reports/NATIVE_INIT_V2703_AUDIO_ACDB_LARGE_BUFFER_TOPOLOGY_GET_BUILD_2026-06-18.md"

HELPER_SOURCE_REL = v2692.HELPER_SOURCE_REL
PREINIT_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdb_lower_large_get_inhook_v2703.c"
ACDBTAP_SOURCE_REL = "workspace/public/src/android/acdb_payload_capture/libacdbtap_lower_large_get_v2703.c"
HELPER_ARTIFACT_NAME = "a90_acdb_large_buffer_topology_get_exec_linked_v2703"
PRELOAD_ARTIFACT_NAME = "liba90_acdb_large_buffer_topology_get_combined_preload_v2703.so"
...
def source_state() -> dict[str, Any]:
    ...
    return {
        ...
        "v2703_delta": {
            ...
            "boundary": "do not call fake or real AUDIO_SET_CALIBRATION in the V2703 preinit path",
        },
    }
...
def build(build_root: Path, *, clang: Path, lld: Path, readelf: str, file_cmd: str) -> dict[str, Any]:
    ...
    if isinstance(preload, dict):
        ...
        if isinstance(checks, dict):
            ...
            checks["soname_v2703"] = f"Library soname: [{PRELOAD_ARTIFACT_NAME}]" in preload_dynamic
...
def make_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "capture_contract": {
            ...
            "success_discriminator": "future live run must capture ret==0 non-all-zero indirect lower custom topology buffers for cal_type 10 and 14; raw bytes private only",
