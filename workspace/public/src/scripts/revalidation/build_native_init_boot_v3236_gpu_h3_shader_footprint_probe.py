#!/usr/bin/env python3
"""Build V3236 GPU H3 Shader Footprint probe."""
...
CYCLE = "V3236"
INIT_VERSION = "0.11.45"
INIT_BUILD = "v3236-gpu-h3-shader-footprint-probe"
...
REPORT_PATH = (
    ...
    / "docs"
    / "reports"
    / "NATIVE_INIT_V3236_GPU_H3_SHADER_FOOTPRINT_SOURCE_BUILD_2026-06-25.md"
)
BOOT_IMAGE = workspace_private_input_path(
    "boot_images",
    "boot_linux_v3236_gpu_h3_shader_footprint_probe.img",
    legacy_fallback=False,
)
BASE_BOOT = previous.BOOT_IMAGE
INIT_BINARY = OUT_DIR / "init_v3236_gpu_h3_shader_footprint_probe"
RAMDISK_CPIO = OUT_DIR / "ramdisk_v3236_gpu_h3_shader_footprint_probe.cpio"
HELPER_BINARY = OUT_DIR / "a90_android_execns_probe_v578_gpu_h3_shader_footprint_probe"

ENGINE_BINARY = OUT_DIR / "a90_doomgeneric_private_engine_v3236"
ENGINE_ADAPTER_SOURCE = OUT_DIR / "a90_doomgeneric_native_bridge_v3236.c"
ENGINE_ADAPTER_OBJECT = OBJ_DIR / "a90_doomgeneric_native_bridge_v3236.o"
ENGINE_RAMDISK_PATH = "bin/a90_doomgeneric_private_engine_v3236"
ENGINE_REMOTE_PATH = "/" + ENGINE_RAMDISK_PATH
ENGINE_NAME = "doomgeneric-private-link-v3236-gpu-h3-shader-footprint-probe"

FRAME_PATH = "/tmp/a90-doomgeneric-v3236-raw-fallback-frame.xbgr8888"
SHARED_FRAME_PATH = "/tmp/a90-doomgeneric-v3236-shared-frame.bin"
INPUT_STATE_PATH = "/tmp/a90-doomgeneric-v3236-input.state"
INPUT_SOCKET_PATH = "/tmp/a90-doomgeneric-v3236-input.sock"
PACE_SOCKET_PATH = "/tmp/a90-doomgeneric-v3236-pace.sock"
TICK_TELEMETRY_PATH = "/tmp/a90-doomgeneric-v3236-tick-telemetry.txt"
AUDIO_PCM_STREAM_PATH = "/cache/a90-runtime/a90-doomgeneric-v3236-sfx.pcmstream"

FRAME_SCALE = "1:1-demo-hud-large-groups-gpu-h3-shader-footprint-probe"
FRAME_IPC = "shared-mmap-direct-blit-demo-hud-large-groups-gpu-h3-shader-footprint-probe"

INPUT_THREAD_MARKER = base.INPUT_THREAD_MARKER.replace("v3210", "v3236")
TIME_MODEL_MARKER = base.TIME_MODEL_MARKER.replace("v3210", "v3236")
DEMO_HUD_MARKER = base.DEMO_HUD_MARKER.replace("v3210", "v3236")
PACED_TIME_MARKER = base.PACED_TIME_MARKER.replace("v3210", "v3236")
TICK_TELEMETRY_MARKER = base.TICK_TELEMETRY_MARKER.replace("v3210", "v3236")
SCALE_MARKER = base.SCALE_MARKER.replace("v3210", "v3236")
PHASE_TELEMETRY_MARKER = base.PHASE_TELEMETRY_MARKER.replace("v3210", "v3236")
GAMETIC_FRAME_TELEMETRY_MARKER = base.GAMETIC_FRAME_TELEMETRY_MARKER.replace("v3210", "v3236")
SFX_STREAM_MARKER = "a90.doomgeneric.v3236.audio=real-sfx-pcm-stream-gpu-h3-shader-footprint-probe"
SOUND_MODE = "native-doom-sfx-gpu-h3-shader-footprint-probe-v3236"

SFX_BACKEND_SOURCE = OUT_DIR / "a90_doomgeneric_native_sfx_v3236.c"
SDL_MIXER_STUB = OUT_DIR / "SDL_mixer.h"


def _rewrite_v3236_marker(item: bytes) -> bytes:
    replacements = {
        ...
        b"a90-doomgeneric-v3232": b"a90-doomgeneric-v3236",
        b"a90.doomgeneric.v3232": b"a90.doomgeneric.v3236",
        b"v3232": b"v3236",
        b"V3232": b"V3236",
        b"first-triangle-h3-static-context-mov-f32-shader":
            b"first-triangle-h3-shader-footprint-r1-footprint2-mov-f32-shader",
...
GPU_H3_SHADER_FOOTPRINT_MARKERS = (
    b"gpu.h3.draw.scope=first-triangle-h3-shader-footprint-r1-footprint2-mov-f32-shader",
...
REQUIRED_STRINGS = tuple(_rewrite_v3236_marker(item) for item in previous.REQUIRED_STRINGS) + (
...
def _minimal_gpu_h3_manifest() -> dict[str, Any]:
    return {
        ...
        "scope": "first-triangle-h3-shader-footprint-r1-footprint2-mov-f32-shader",
        ...
        "next_live_validation": [
            "flash-v3236-through-native-init-flash",
            "post-flash-health-check",
            "gpu-g0-fwclass-prepare",
            "gpu-h3-shader-footprint-r1-footprint2-mov-f32-shader-timeout-guard",
...
def v3236_adapter_source() -> str:
    return (
        base.ORIG_V3208_ADAPTER_SOURCE()
        .replace("gpu-h1-shader-state-probe", "gpu-h3-shader-footprint-probe")
        .replace("real-sfx-pcm-stream-gpu-h1-shader-state-probe",
                 "real-sfx-pcm-stream-gpu-h3-shader-footprint-probe")
        .replace("v3208", "v3236")
        .replace("V3208", "V3236")
    )


def _overlay_preserved_v3236_ramdisk() -> dict[str, Any]:
    if not BASE_BOOT.exists():
        raise FileNotFoundError(f"missing V3232 base boot: {BASE_BOOT}")
    if not INIT_BINARY.exists():
        raise FileNotFoundError(f"missing V3236 init binary: {INIT_BINARY}")
    if not HELPER_BINARY.exists():
        raise FileNotFoundError(f"missing V3236 helper binary: {HELPER_BINARY}")
    if not ENGINE_BINARY.exists():
        raise FileNotFoundError(f"missing V3236 DOOM engine binary: {ENGINE_BINARY}")

    removed_stale_entries: list[str] = []
    with tempfile.TemporaryDirectory(prefix="a90-v3236-overlay-") as temp_name:
    ...
    if image_size > BOOT_PARTITION_MAX_BYTES:
        raise RuntimeError(
            f"V3236 boot image too large for boot partition: "
    ...
    if missing_entries:
        raise RuntimeError(f"missing V3236 overlay ramdisk entries: {missing_entries}")

    return {
        "mode": "preserve-v3232-ramdisk-overlay-v3236-init-helper-engine",
...
def _write_candidate_manifest(manifest: dict[str, Any]) -> None:
    (OUT_DIR / "gpu-h3-shader-footprint-probe-candidate.json").write_text(json.dumps({
        "candidate_tag": INIT_BUILD,
        "candidate_type": "gpu-h3-shader-footprint-probe-candidate",
        ...
        "adoption_state": "pending-gpu-h3-shader-footprint-live-validation",
...
def _finalize_manifest_after_overlay(
    ...
    *,
    ...
    overlay["mode"] = "preserve-v3232-ramdisk-overlay-v3236-init-helper-engine"
    manifest_path = OUT_DIR / "manifest.json"
    ...
    manifest["candidate_type"] = "gpu-h3-shader-footprint-probe-candidate"
    manifest["adoption_state"] = "pending-gpu-h3-shader-footprint-live-validation"
...
def _apply_v3236_overrides() -> None:
    replacements = {
        "CYCLE": CYCLE,
        "INIT_VERSION": INIT_VERSION,
        "INIT_BUILD": INIT_BUILD,
        ...
        "SFX_BACKEND_SOURCE_TEXT": (
            ...
            .replace("gpu-h2-3d-state-probe", "gpu-h3-shader-footprint-probe")
        ...
        "v3210_adapter_source": v3236_adapter_source,
        "_overlay_preserved_v3208_ramdisk": _overlay_preserved_v3236_ramdisk,
