...
/*
 * §0.2 write-probe rungs E1/E2/E3a/E3b (self-dd fast-flash tool design §11.2).
 *
 * THIS FILE CARRIES A WRITE PRIMITIVE (pwrite to the boot block). It is a read-then-write-IDENTITY
 * probe confined to parsed tail slack of the boot partition: it resolves boot from sysfs
 * PARTNAME=boot, picks one confirmed-zero E1 target, four confirmed-zero E2 targets, sixteen
 * confirmed-zero E3a targets spread across the observed all-zero sector population, or one E3b
 * contiguous 1 MiB non-zero slack block (past the boot-image content, 1 MiB before the partition end
 * to avoid any AVB footer), writes the exact bytes it just read back to the same offset, fsyncs,
 * verifies via an O_DIRECT
