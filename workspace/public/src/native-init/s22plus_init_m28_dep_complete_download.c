...
/*
 * Samsung S22+ native-init M28 dependency-complete HS-only discriminator.
 *
 * This is an observation-channel candidate, not an ACM bring-up candidate.
 * It fixes the M25/M26/M27 dependency-incomplete closure by loading a
 * modules.dep-respecting module list with hard suppliers included, then
 * requests download mode if it reaches the checkpoint.
 */

#include <stdint.h>
#include <stddef.h>

#ifndef M28_MODULE_LIMIT
#define M28_MODULE_LIMIT 0
#endif

#ifndef M28_VARIANT_LABEL
#define M28_VARIANT_LABEL "S00"
#endif
...
static const char k_marker[] =
    "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD version=0.1 pid1=direct "
    "runtime=freestanding raw_syscalls=1 "
    "variant_label=" M28_VARIANT_LABEL " module_limit=" STR(M28_MODULE_LIMIT) " "
    "modules_dep_complete=/s22plus_m28_dep_complete.modules module_count=" STR(M28_MODULE_LIMIT) " "
    "maximum_speed_dtbo=high-speed qmp_excluded=1 "
    "module_source=stock_vendor_boot_ramdisk module_list=dep_complete_hs_only "
    "module_injection=list_only observation=dep-complete-download "
    "no_android_handoff=1 no_configfs=1 no_acm=1 reboot_request=download\n";

static const char k_download_arg[] = "download";
...
static long sys_reboot(long magic1, long magic2, long cmd, const char *arg) {
    return syscall4(NR_REBOOT, magic1, magic2, cmd, (long)(uintptr_t)arg);
}
...
static void setup_minimal_fs(void) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=mounts variant_label=");
    sb_puts(&sb, M28_VARIANT_LABEL);
    sb_puts(&sb, " module_limit=");
    sb_put_u64(&sb, M28_MODULE_LIMIT);
    sb_puts(&sb, " proc_rc=");
...
static void emit_module(size_t index, const char *name, long open_rc, long finit_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=module variant_label=");
    sb_puts(&sb, M28_VARIANT_LABEL);
    sb_puts(&sb, " module_limit=");
    sb_put_u64(&sb, M28_MODULE_LIMIT);
    sb_puts(&sb, " index=");
    ...
    sb_puts(&sb, " ok=");
    sb_put_u64(&sb, (finit_rc == 0 || finit_rc == -EEXIST) ? 1U : 0U);
    sb_putc(&sb, '\n');
    emit_buf(&sb);
}

static void load_one_module(size_t index, const char *name) {
    char path[256];
    if (!build_module_path(path, sizeof(path), name)) {
        emit_module(index, name, -1, -1);
        return;
    }
    long fd = sys_openat(AT_FDCWD, path, O_RDONLY | O_CLOEXEC, 0);
    if (fd < 0) {
        emit_module(index, name, fd, -1);
        return;
    }
    long rc = sys_finit_module((int)fd, "", 0);
    (void)sys_close((int)fd);
    emit_module(index, name, fd, rc);
}

static void load_modules(void) {
    const size_t module_limit = (size_t)M28_MODULE_LIMIT;
    if (module_limit == 0U) {
        emit("S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=modules_load_skipped variant_label="
             M28_VARIANT_LABEL " module_limit=0\n");
        return;
    }

    static char text[MODULES_LOAD_BUF];
    long fd = sys_openat(AT_FDCWD, "/s22plus_m28_dep_complete.modules", O_RDONLY | O_CLOEXEC, 0);
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=modules_load open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    }
    long n = sys_read((int)fd, text, MODULES_LOAD_BUF - 1);
    (void)sys_close((int)fd);
    if (n < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=modules_load read_rc=");
        sb_put_i64(&sb, n);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    ...
    size_t index = 0;
    size_t pos = 0;
    while (pos < (size_t)n && index < module_limit) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=modules_load_done variant_label=");
    sb_puts(&sb, M28_VARIANT_LABEL);
    sb_puts(&sb, " module_limit=");
    sb_put_u64(&sb, M28_MODULE_LIMIT);
...
__attribute__((noreturn)) static void reboot_download_then_park(void) {
    emit("S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=checkpoint variant_label="
         M28_VARIANT_LABEL " module_limit=" STR(M28_MODULE_LIMIT) " action=reboot_download\n");
    (void)sys_sleep_ms(50);
    long rc = sys_reboot(
        LINUX_REBOOT_MAGIC1,
        LINUX_REBOOT_MAGIC2,
        ...
        k_download_arg);
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M28_DEP_COMPLETE_DOWNLOAD phase=reboot_returned variant_label=");
    sb_puts(&sb, M28_VARIANT_LABEL);
    sb_puts(&sb, " module_limit=");
    sb_put_u64(&sb, M28_MODULE_LIMIT);
    sb_puts(&sb, " rc=");
