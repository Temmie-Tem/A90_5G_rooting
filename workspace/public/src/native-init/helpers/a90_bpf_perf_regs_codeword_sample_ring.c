/*
 * a90_bpf_perf_regs_codeword_sample_ring — V2216 perf regs codeword sampler.
 *
 * Scope:
...
struct sample_value {
    ...
    uint64_t ctx_sp;
    uint64_t ctx_pc;
    uint32_t ctx_pc_insn;
    uint32_t ctx_lr_prev_insn;
    uint32_t ctx_lr_insn;
    uint32_t reserved;
...
static int emit_read_u32_to_sample(struct emitter *e, int value_off, uint8_t base_reg, int candidate_off) {
    int jerr = emit_probe_read_stack(e, -28, 4, base_reg, candidate_off);
    emit(e, BPF_LDX | BPF_MEM | BPF_W, BPF_REG_1, BPF_REG_10, -28, 0);
    emit(e, BPF_STX | BPF_MEM | BPF_W, BPF_REG_9, BPF_REG_1, (int16_t)value_off, 0);
...
static void emit_clear_sample(struct emitter *e) {
    ...
    emit(e, BPF_ALU64 | BPF_MOV | BPF_K, BPF_REG_1, 0, 0, 0);
    emit(e, BPF_STX | BPF_MEM | BPF_W, BPF_REG_9, BPF_REG_1, (int16_t)offsetof(struct sample_value, ctx_pc_insn), 0);
    emit(e, BPF_STX | BPF_MEM | BPF_W, BPF_REG_9, BPF_REG_1, (int16_t)offsetof(struct sample_value, ctx_lr_prev_insn), 0);
    emit(e, BPF_STX | BPF_MEM | BPF_W, BPF_REG_9, BPF_REG_1, (int16_t)offsetof(struct sample_value, ctx_lr_insn), 0);
...
static int build_prog(struct emitter *e, int stats_map_fd, int sample_map_fd) {
    ...
    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_6, BPF_REG_9,
         (int16_t)offsetof(struct sample_value, ctx_pc), 0);
    int pc_null = emit(e, BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_6, 0, 0, 0);
    emit_read_u32_to_sample(e, (int)offsetof(struct sample_value, ctx_pc_insn), BPF_REG_6, 0);
    patch_jump(e, pc_null);

    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_6, BPF_REG_9,
         (int16_t)offsetof(struct sample_value, ctx_lr), 0);
    int lr_null = emit(e, BPF_JMP | BPF_JEQ | BPF_K, BPF_REG_6, 0, 0, 0);
    emit_read_u32_to_sample(e, (int)offsetof(struct sample_value, ctx_lr_prev_insn), BPF_REG_6, -4);
    emit_read_u32_to_sample(e, (int)offsetof(struct sample_value, ctx_lr_insn), BPF_REG_6, 0);
    patch_jump(e, lr_null);
...
static void print_sample(unsigned int idx, const struct sample_value *value) {
    ...
    comm[16] = '\0';
    printf("sample idx=%u seq=%llu tgid=%u pid=%u ctx_x29=0x%016llx ctx_lr=0x%016llx ctx_sp=0x%016llx ctx_pc=0x%016llx ctx_pc_insn=0x%08x ctx_lr_prev_insn=0x%08x ctx_lr_insn=0x%08x fp_slot_next=0x%016llx fp_slot_raw_lr=0x%016llx fp2_slot_next=0x%016llx fp2_slot_raw_lr=0x%016llx comm=%s\n",
           idx,
