/*
 * a90_bpf_timer_object_context — bounded timer_start object probe.
 *
 * Scope:
 *   - tracepoint: timer:timer_start only;
 *   - read tracepoint ctx fields: timer, function, expires, now, flags;
 *   - bpf_probe_read the pointed struct timer_list fields;
 ...
 *   - --busy-observe avoids self-generating schedule_timeout timer_start events;
...
#define A90_VERSION "a90_bpf_timer_object_context v2201"
...
/* include/linux/timer.h on this 4.14 build. */
...
struct summary_value {
    ...
    uint64_t obj_data;
    uint64_t obj_flags;
    uint64_t obj_read_error_count;
    uint64_t obj_function_match_count;
    uint64_t obj_expires_match_count;
    char comm[16];
};
...
static void emit_read_timer_u64(struct emitter *e, int value_off, int timer_off) {
    int jerr = emit_probe_read_stack(e, -48, 8, BPF_REG_6, timer_off);
    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_7, BPF_REG_10, -48, 0);
    emit(e, BPF_STX | BPF_MEM | BPF_DW, BPF_REG_8, BPF_REG_7, value_off, 0);
    int jdone = emit(e, BPF_JMP | BPF_JA, 0, 0, 0, 0);
    patch_jump(e, jerr);
    emit_inc_u64(e, (int)offsetof(struct summary_value, obj_read_error_count));
    patch_jump(e, jdone);
}

static void emit_read_timer_u32_as_u64(struct emitter *e, int value_off, int timer_off) {
    int jerr = emit_probe_read_stack(e, -48, 4, BPF_REG_6, timer_off);
    emit(e, BPF_LDX | BPF_MEM | BPF_W, BPF_REG_7, BPF_REG_10, -48, 0);
    emit(e, BPF_STX | BPF_MEM | BPF_DW, BPF_REG_8, BPF_REG_7, value_off, 0);
    int jdone = emit(e, BPF_JMP | BPF_JA, 0, 0, 0, 0);
    patch_jump(e, jerr);
    emit_inc_u64(e, (int)offsetof(struct summary_value, obj_read_error_count));
...
static int build_timer_start_prog(struct emitter *e,
    ...
    /* R6 = ctx->timer. Object reads are bpf_probe_read only. */
    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_6, BPF_REG_9, CTX_TIMER, 0);
    emit_read_timer_u64(e, (int)offsetof(struct summary_value, obj_entry_next), TIMER_ENTRY_NEXT);
    emit_read_timer_u64(e, (int)offsetof(struct summary_value, obj_entry_pprev), TIMER_ENTRY_PPREV);
    emit_read_timer_u64(e, (int)offsetof(struct summary_value, obj_expires), TIMER_EXPIRES);
    emit_read_timer_u64(e, (int)offsetof(struct summary_value, obj_function), TIMER_FUNCTION);
    emit_read_timer_u64(e, (int)offsetof(struct summary_value, obj_data), TIMER_DATA);
    emit_read_timer_u32_as_u64(e, (int)offsetof(struct summary_value, obj_flags), TIMER_FLAGS);

    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_7, BPF_REG_8,
         (int16_t)offsetof(struct summary_value, obj_function), 0);
    ...
    emit_inc_u64(e, (int)offsetof(struct summary_value, obj_function_match_count));
    patch_jump(e, fn_mismatch);

    emit(e, BPF_LDX | BPF_MEM | BPF_DW, BPF_REG_7, BPF_REG_8,
         (int16_t)offsetof(struct summary_value, obj_expires), 0);
    ...
    emit_inc_u64(e, (int)offsetof(struct summary_value, obj_expires_match_count));
...
static void print_summary(const struct summary_value *value) {
    ...
    printf("summary count=%llu last_function=0x%016llx last_timer=0x%016llx last_expires=%llu last_now=%llu last_timeout=%lld last_flags=0x%llx last_pid=%u last_tgid=%u last_stackid=%lld comm=%s timeout_min=%llu timeout_max=%llu timeout_sum=%llu timeout_eq1=%llu timeout_le4=%llu timeout_eq18000=%llu timeout_ge1000=%llu timeout_zero=%llu obj_entry_next=0x%016llx obj_entry_pprev=0x%016llx obj_expires=%llu obj_function=0x%016llx obj_data=0x%016llx obj_flags=0x%llx obj_read_errors=%llu obj_function_match=%llu obj_expires_match=%llu\n",
           ...
           (unsigned long long)value->obj_entry_next,
           (unsigned long long)value->obj_entry_pprev,
           (unsigned long long)value->obj_expires,
           (unsigned long long)value->obj_function,
           (unsigned long long)value->obj_data,
           (unsigned long long)value->obj_flags,
           (unsigned long long)value->obj_read_error_count,
           (unsigned long long)value->obj_function_match_count,
           (unsigned long long)value->obj_expires_match_count);
}
...
static void usage(const char *argv0) {
    fprintf(stderr,
        "%s\n"
        "usage: %s [--filter-function HEX] [--duration SEC] [--allow-attach] [--busy-observe] [--verbose]\n"
...
int main(int argc, char **argv) {
    ...
    printf("%s\n", A90_VERSION);
    printf("allow_attach=%d duration_sec=%d filter_function=0x%016llx busy_observe=%d verbose=%d\n",
           allow_attach, duration_sec, (unsigned long long)filter_function, busy_observe, verbose);
    ...
    printf("result=v2201-timer-object-context-complete\n");
