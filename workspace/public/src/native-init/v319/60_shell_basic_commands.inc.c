...
static void cmd_help(void) {
    ...
    a90_console_printf("video [status|frame|demo badapple-scale|anim|blitbench|stream --manifest PATH --video-only|cache [status|verify|play] SHA256 [--trust-cache]|cache preset badapple-scale [status|verify|play]]\r\n");
    ...
    a90_console_printf("reaper [status|run|verbose]\r\n");
    ...
    a90_console_printf("video [status|frame|demo|anim|blitbench|flipprobe|stream|cache]\r\n");
    ...
    a90_console_printf("screenapp [network|wifi-status|wifi-profiles|wifi-scan|wifi-ping|audio-status|audio-profile|audio-stages|audio-map|audio-chime|about-version|about-changelog]\r\n");
...
static void cmd_status(void) {
    ...
    char kernelinv_summary[256];
    char sensormap_summary[256];
    ...
    a90_watchdoginv_summary(watchdoginv_summary, sizeof(watchdoginv_summary));
    a90_tracefs_summary(tracefs_summary, sizeof(tracefs_summary));
...
static int cmd_reaper(char **argv, int argc) {
    const char *subcommand = argc > 1 ? argv[1] : "status";
    struct a90_reaper_status status;
    char summary[128];
    char status_text[48];
    bool verbose = false;
    int rc = 0;
    ...
    if (argc > 2) {
        a90_console_printf("usage: reaper [status|run|verbose]\r\n");
        return -EINVAL;
    }

    if (strcmp(subcommand, "run") == 0) {
        rc = a90_reaper_reap_orphans("manual");
        verbose = true;
    } else if (strcmp(subcommand, "verbose") == 0) {
        rc = a90_reaper_reap_orphans("manual-verbose");
        verbose = true;
    } else if (strcmp(subcommand, "status") != 0) {
        a90_console_printf("usage: reaper [status|run|verbose]\r\n");
        return -EINVAL;
    ...
    a90_reaper_summary(summary, sizeof(summary));
    a90_console_printf("reaper: %s\r\n", summary);

    if (verbose) {
        a90_reaper_get_status(&status);
        a90_console_printf("reaper: total=%lu last_poll=%lu last_pid=%ld last_status=0x%x last_text=%s last_ms=%ld\r\n",
                status.total_reaped,
                status.last_poll_reaped,
                (long)status.last_pid,
                status.last_status,
                a90_reaper_status_text(status.last_status, status_text, sizeof(status_text)),
                status.last_reap_ms);
    }
