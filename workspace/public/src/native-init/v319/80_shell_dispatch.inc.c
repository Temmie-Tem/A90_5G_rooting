...
static int handle_boot_write_e1(char **argv, int argc) {
...
static int handle_boot_write_e3a(char **argv, int argc) {
    return a90_boot_write_e3a_cmd(argv, argc);
}

static int handle_boot_write_e3b(char **argv, int argc) {
    return a90_boot_write_e3b_cmd(argv, argc);
}

static int handle_boot_write_e4(char **argv, int argc) {
    return a90_boot_write_e4_cmd(argv, argc);
}

static int handle_boot_write_e5(char **argv, int argc) {
    return a90_boot_write_e5_cmd(argv, argc);
}

static int handle_boot_flash_plan(char **argv, int argc) {
    return a90_boot_flash_plan_cmd(argv, argc);
}

static int handle_boot_flash_f1(char **argv, int argc) {
    return a90_boot_flash_f1_cmd(argv, argc);
}

static int handle_boot_flash_f2(char **argv, int argc) {
...
#define GPU_Z3_PRIMARY_MAX_TARGET_BYTES (32ULL * 1024ULL * 1024ULL)
...
#define GPU_M2_GRAPH_SOURCE_WIDTH 480U
#define GPU_M2_GRAPH_SOURCE_HEIGHT 360U
#define GPU_M2_GRAPH_SOURCE_STRIDE ((GPU_M2_GRAPH_SOURCE_WIDTH + 7U) / 8U)
#define GPU_M2_GRAPH_SOURCE_BYTES \
    ((uint64_t)GPU_M2_GRAPH_SOURCE_STRIDE * GPU_M2_GRAPH_SOURCE_HEIGHT)
#define GPU_M2_GRAPH_DEFAULT_FRAMES 12U
#define GPU_M2_GRAPH_MAX_FRAMES 60U
#define GPU_M2_GRAPH_DEFAULT_INTERVAL_MS 200U
...
#define GPU_KGSL_USER_MEM_TYPE_DMABUF 0x00000003U
...
struct gpu_kgsl_gpuobj_import_dma_buf {
    int fd;
};
...
#define GPU_IOCTL_KGSL_GPUOBJ_IMPORT \
    _IOWR(GPU_KGSL_IOC_TYPE, 0x48, struct gpu_kgsl_gpuobj_import)
...
struct gpu_m2_monitor_graph_summary {
    int result_rc;
    int gpu_create_rc;
    int kms_begin_rc;
    int present_rc;
    ...
    uint64_t elapsed_ns;
    uint64_t fps_milli;
    uint64_t sample_avg_us;
    uint64_t sample_max_us;
    uint64_t graph_avg_us;
    uint64_t graph_max_us;
    uint64_t texture_avg_us;
    uint64_t texture_max_us;
    uint64_t gpu_wait_avg_us;
    uint64_t gpu_wait_max_us;
...
struct gpu_z2_imported_target_summary {
    int result_rc;
    int drm_open_rc;
    ...
    uint32_t kms_base_fb_id;
    int kms_crtc_index;
    uint32_t kms_screen_width;
    uint32_t kms_screen_height;
    uint32_t kms_primary_setcrtc_attempted;
...
struct gpu_z2_linear_import_metrics {
    int import_rc;
    int info_rc;
...
static int gpu_2d_present_alloc_map_bo(int fd,
                                       struct gpu_d3_bo *bo,
                                       uint64_t size,
                                       uint64_t min_size) {
    uint64_t mmap_offset;
...
static int gpu_2d_present_import_dmabuf_bo(int fd,
                                           struct gpu_d3_bo *bo,
                                           int dmabuf_fd,
                                           void *map,
                                           uint64_t size,
                                           uint64_t min_size,
                                           struct gpu_z2_linear_import_metrics *metrics) {
    struct gpu_kgsl_gpuobj_import_dma_buf import_dmabuf;
    struct gpu_kgsl_gpuobj_import import_arg;

    if (metrics != NULL) {
        memset(metrics, 0, sizeof(*metrics));
        metrics->import_rc = -ENOSYS;
        metrics->info_rc = -ENOSYS;
    }
    if (fd < 0 || bo == NULL || dmabuf_fd < 0 ||
        map == NULL || map == MAP_FAILED || size == 0U || min_size == 0U) {
        if (metrics != NULL) {
            metrics->import_rc = -EINVAL;
        }
        return -EINVAL;
    }
    gpu_d3_bo_init(bo);
    memset(&import_dmabuf, 0, sizeof(import_dmabuf));
    import_dmabuf.fd = dmabuf_fd;
    ...
    import_arg.type = GPU_KGSL_USER_MEM_TYPE_DMABUF;
    errno = 0;
    if (ioctl(fd, GPU_IOCTL_KGSL_GPUOBJ_IMPORT, &import_arg) < 0) {
        int rc = negative_errno_or(EIO);

        if (metrics != NULL) {
            metrics->import_rc = rc;
        }
        return rc;
    }
    bo->alloc.id = import_arg.id;
    bo->alloc.size = size;
    bo->alloc.mmapsize = size;
    bo->alloc.va_len = size;
    bo->alloc.flags = import_arg.flags;
    if (metrics != NULL) {
        metrics->import_rc = 0;
        metrics->kgsl_id = import_arg.id;
    }
    ...
    if (bo->info.gpuaddr == 0U || bo->info.size < min_size) {
        if (metrics != NULL) {
            metrics->info_rc = -EINVAL;
        }
        return -EINVAL;
    }
    bo->map = map;
    if (metrics != NULL) {
        metrics->info_rc = 0;
        metrics->kgsl_id = bo->info.id;
        metrics->gpuaddr = bo->info.gpuaddr;
        metrics->size = bo->info.size;
        metrics->flags = bo->info.flags;
        metrics->va_len = bo->info.va_len;
        metrics->va_addr = bo->info.va_addr;
    }
    return 0;
}

static void gpu_2d_present_unmap_free_bo(int fd, struct gpu_d3_bo *bo) {
    if (bo == NULL) {
        return;
    }
...
static void gpu_d3_destroy_session(struct gpu_d3_session *session) {
    ...
    if (session->fd >= 0) {
        gpu_2d_present_unmap_free_bo(session->fd, &session->texture);
        gpu_2d_present_unmap_free_bo(session->fd, &session->texmem);
        gpu_2d_present_unmap_free_bo(session->fd, &session->sampler);
        gpu_2d_present_unmap_free_bo(session->fd, &session->vertex);
        gpu_2d_present_unmap_free_bo(session->fd, &session->fs);
        gpu_2d_present_unmap_free_bo(session->fd, &session->vs);
        gpu_2d_present_unmap_free_bo(session->fd, &session->event);
        gpu_2d_present_unmap_free_bo(session->fd, &session->linear);
        gpu_2d_present_unmap_free_bo(session->fd, &session->color_flag);
        gpu_2d_present_unmap_free_bo(session->fd, &session->color);
        gpu_2d_present_unmap_free_bo(session->fd, &session->cmd);
...
static int gpu_2d_present_create_session_internal(
    struct gpu_d3_session *session,
    const struct video_stream_manifest *manifest,
    struct gpu_d3_video_summary *summary,
    int imported_linear_dmabuf_fd,
    void *imported_linear_map,
    struct gpu_z2_linear_import_metrics *import_metrics,
    uint32_t target_width_override,
    uint32_t target_height_override,
    uint32_t target_stride_override,
    uint64_t target_bytes_override) {
    static const uint32_t vs_shader[GPU_H3_VS_SHADER_DWORDS] = {
        GPU_H3_IR3_MOV_F32F32_R2X_R1X_LO, GPU_H3_IR3_MOV_F32F32_R2X_R1X_HI,
        GPU_H3_IR3_MOV_F32F32_R2Y_R1Y_LO, GPU_H3_IR3_MOV_F32F32_R2Y_R1Y_HI,
        GPU_H3_IR3_MOV_F32F32_R2Z_R1Z_LO, GPU_H3_IR3_MOV_F32F32_R2Z_R1Z_HI,
    ...
    target_override = target_width_override != 0U ||
                      target_height_override != 0U ||
                      target_stride_override != 0U ||
                      target_bytes_override != 0ULL;
    if (target_override) {
        if (target_width_override == 0U ||
            target_height_override == 0U ||
            target_stride_override < target_width_override * GPU_D3_VIDEO_TARGET_BPP ||
            target_bytes_override < (uint64_t)target_stride_override *
                                    target_height_override ||
            target_bytes_override > GPU_Z3_PRIMARY_MAX_TARGET_BYTES) {
            return -EINVAL;
        }
        session->target_width = target_width_override;
        session->target_height = target_height_override;
        session->target_stride = target_stride_override;
        session->target_bytes = target_bytes_override;
    } else {
    ...
    rc = gpu_2d_present_alloc_map_bo(session->fd,
                                     &session->cmd,
                                     GPU_H3_CMD_ALLOC_SIZE,
                                     (uint64_t)GPU_G4_CMD_MAX_DWORDS * 4ULL);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->color,
                                     session->target_bytes, session->target_bytes);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->color_flag,
                                     GPU_D3_VIDEO_COLOR_FLAG_ALLOC_SIZE,
                                     GPU_D3_VIDEO_COLOR_FLAG_ALLOC_SIZE);
    if (rc < 0) return rc;
    if (imported_linear_dmabuf_fd >= 0) {
        rc = gpu_2d_present_import_dmabuf_bo(session->fd,
                                             &session->linear,
                                             imported_linear_dmabuf_fd,
                                             imported_linear_map,
                                             session->target_bytes,
                                             session->target_bytes,
                                             import_metrics);
    } else {
        rc = gpu_2d_present_alloc_map_bo(session->fd, &session->linear,
                                         session->target_bytes, session->target_bytes);
    }
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->event,
                                     GPU_H3_EVENT_ALLOC_SIZE, GPU_H3_EVENT_ALLOC_SIZE);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->vs,
                                     GPU_H1_SHADER_ALLOC_SIZE, sizeof(vs_shader));
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->fs,
                                     GPU_H1_SHADER_ALLOC_SIZE, sizeof(fs_shader));
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->vertex,
                                     GPU_H3_VERTEX_ALLOC_SIZE, GPU_D1_VERTEX_BYTES);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->sampler,
                                     GPU_D1_DESCRIPTOR_ALLOC_SIZE,
                                     GPU_D1_SAMPLER_DESC_DWORDS * 4ULL);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->texmem,
                                     GPU_D1_DESCRIPTOR_ALLOC_SIZE,
                                     GPU_D1_TEXMEMOBJ_DESC_DWORDS * 4ULL);
    if (rc < 0) return rc;
    rc = gpu_2d_present_alloc_map_bo(session->fd, &session->texture,
...
static int gpu_2d_present_create_session(struct gpu_d3_session *session,
                                         const struct video_stream_manifest *manifest,
                                         struct gpu_d3_video_summary *summary) {
    return gpu_2d_present_create_session_internal(session,
                                                  manifest,
                                                  summary,
                                                  -1,
                                                  MAP_FAILED,
...
static int gpu_d3_create_session(struct gpu_d3_session *session,
                                 const struct video_stream_manifest *manifest,
                                 struct gpu_d3_video_summary *summary) __attribute__((unused));
static int gpu_d3_create_session(struct gpu_d3_session *session,
                                 const struct video_stream_manifest *manifest,
                                 struct gpu_d3_video_summary *summary) {
    return gpu_2d_present_create_session(session, manifest, summary);
}
...
static int gpu_2d_present_sync_to_gpu(struct gpu_d3_session *session) {
    struct gpu_kgsl_gpuobj_sync_obj sync_objs[11];
    struct gpu_kgsl_gpuobj_sync sync_arg;
...
static int gpu_2d_present_sync_linear_from_gpu(struct gpu_d3_session *session) {
    struct gpu_kgsl_gpuobj_sync_obj sync_obj;
    struct gpu_kgsl_gpuobj_sync sync_arg;
...
static int gpu_2d_present_submit_wait(struct gpu_d3_session *session,
                                      struct gpu_d3_video_frame_stats *stats) {
    struct gpu_kgsl_command_object cmd_obj;
    struct gpu_kgsl_command_object mem_objs[11];
    struct gpu_kgsl_gpu_command command_arg;
...
static int gpu_2d_present_sample_linear(struct gpu_d3_session *session,
                                        struct gpu_d3_video_frame_stats *stats) {
    ...
    if (session == NULL || stats == NULL ||
        ...
        session->target_stride < session->target_width * sizeof(uint32_t)) {
        return -EINVAL;
    }
    words = (const uint32_t *)session->linear.map;
    row_words = session->target_stride / sizeof(uint32_t);
    center = (session->target_height / 2U) * row_words +
    ...
    for (y = 0; y < session->target_height; ++y) {
        uint32_t x;
        const uint32_t *row = words + ((size_t)y * row_words);

        for (x = 0; x < session->target_width; ++x) {
            uint32_t rgb = row[x] & 0x00ffffffU;

            if (row[x] != GPU_H5_LINEAR_CLEAR_PATTERN) {
...
static int gpu_2d_present_copy_linear_to_kms(struct gpu_d3_session *session,
                                             struct a90_fb *fb,
                                             uint32_t dst_x,
                                             uint32_t dst_y,
                                             struct gpu_d3_video_frame_stats *stats) {
    ...
    rc = gpu_2d_present_sample_linear(session, stats);
    if (rc < 0) {
        return rc;
    }
    __sync_synchronize();
...
static void gpu_d3_validate_linear_semantics(const struct gpu_d3_session *session,
    ...
    for (index = 0; index < GPU_D1_CHECKER_SAMPLE_COUNT; ++index) {
        ...
        value = words[((size_t)target_y * row_words) + target_x] & 0x00ffffffU;
...
static int gpu_2d_present_render_frame_to_linear(
    struct gpu_d3_session *session,
    const struct video_stream_manifest *manifest,
    const uint8_t *frame,
    struct gpu_d3_video_frame_stats *stats) {
    ...
    uint64_t before_sample_ns;
    uint64_t after_sample_ns;
    int rc;
    ...
    rc = gpu_2d_present_sync_to_gpu(session);
    stats->sync_rc = rc;
    if (rc < 0) {
        stats->rc = rc;
        return rc;
    ...
    rc = gpu_2d_present_submit_wait(session, stats);
    ...
    rc = gpu_2d_present_sync_linear_from_gpu(session);
    ...
    before_sample_ns = video_monotonic_ns();
    rc = gpu_2d_present_sample_linear(session, stats);
    after_sample_ns = video_monotonic_ns();
    stats->kms_copy_us = (after_sample_ns - before_sample_ns) / 1000ULL;
    stats->copy_rc = 0;
    if (rc == 0) {
...
static int gpu_2d_present_render_frame_to_kms(struct gpu_d3_session *session,
                                              const struct video_stream_manifest *manifest,
                                              const uint8_t *frame,
                                              struct a90_fb *fb,
                                              uint32_t dst_x,
    ...
    rc = gpu_2d_present_copy_linear_to_kms(session, fb, dst_x, dst_y, stats);
...
static int gpu_d3_render_frame_to_kms(struct gpu_d3_session *session,
                                      ...
                                      uint32_t dst_x,
                                      uint32_t dst_y,
                                      struct gpu_d3_video_frame_stats *stats) __attribute__((unused));
static int gpu_d3_render_frame_to_kms(struct gpu_d3_session *session,
                                      const struct video_stream_manifest *manifest,
                                      const uint8_t *frame,
                                      struct a90_fb *fb,
                                      ...
                                      uint32_t dst_y,
                                      struct gpu_d3_video_frame_stats *stats) {
    return gpu_2d_present_render_frame_to_kms(session, manifest, frame, fb, dst_x, dst_y, stats);
}
...
static int gpu_d3_video_texture_present_child(int write_fd,
    ...
    rc = gpu_2d_present_create_session(&session, &manifest, &summary);
    if (rc < 0) {
    ...
    for (display_index = 0; display_index < limit_frames; ++display_index) {
        ...
        dst_y = 48U;
        rc = gpu_2d_present_render_frame_to_kms(&session, &manifest, frame, fb, dst_x, dst_y, &stats);
        if (rc < 0) {
            summary.failed_frame = frame_index;
            goto out;
        }
...
static int gpu_d3_video_texture_present_probe(int timeout_ms,
    ...
    a90_console_printf("gpu.d3.video.extraction_layer=" GPU_M3_EXTRACT_LAYER "\r\n");
...
static void gpu_m2_fill_graph_manifest(struct video_stream_manifest *manifest,
                                       uint32_t frames,
    ...
    memset(manifest, 0, sizeof(*manifest));
    snprintf(manifest->format, sizeof(manifest->format), "mono1-monitor-graph");
    manifest->width = GPU_M2_GRAPH_SOURCE_WIDTH;
    manifest->height = GPU_M2_GRAPH_SOURCE_HEIGHT;
    manifest->stride = GPU_M2_GRAPH_SOURCE_STRIDE;
    manifest->frame_bytes = (uint32_t)GPU_M2_GRAPH_SOURCE_BYTES;
    manifest->visible_row_bytes = GPU_M2_GRAPH_SOURCE_STRIDE;
    manifest->fps_num = 1000U;
    manifest->fps_den = interval_ms > 0U ? interval_ms : 1U;
    manifest->frame_count = frames;
    manifest->pixel_format = VIDEO_STREAM_PIXEL_FORMAT_MONO1;
    manifest->stream_version = VIDEO_STREAM_VERSION_A90VSTR1;
}

static bool gpu_m2_monitor_graph_summary_passed(const struct gpu_m2_monitor_graph_summary *summary) {
    return summary != NULL &&
           summary->result_rc == 0 &&
           summary->presented_frames == summary->requested_frames &&
           summary->presented_frames > 0U &&
           summary->graph_points > 0U &&
           summary->graph_pixels_set > 0U &&
           summary->changed_total > 0ULL &&
           summary->semantic_sample_count == GPU_D1_CHECKER_SAMPLE_COUNT &&
           summary->semantic_sample_match_count == GPU_D1_CHECKER_SAMPLE_COUNT &&
           summary->semantic_sample_match_count ==
           ...
           summary->semantic_sample_mismatch_count == 0U &&
           summary->semantic_output_other_count == 0U &&
           summary->present_rc == 0;
}

static int gpu_m2_monitor_live_graph_child(int write_fd,
                                           uint32_t requested_frames,
                                           uint32_t interval_ms,
                                           int hold_ms) {
    ...
    uint64_t started_ns;
    uint64_t finished_ns;
    uint64_t sample_sum_us = 0;
    uint64_t graph_sum_us = 0;
    uint64_t texture_sum_us = 0;
    uint64_t gpu_wait_sum_us = 0;
    uint64_t readback_sum_us = 0;
    uint64_t copy_sum_us = 0;
    ...
    memset(&summary, 0, sizeof(summary));
    memset(&d3_summary, 0, sizeof(d3_summary));
    gpu_d3_session_init(&session);
    a90_monitor_graph_reset(&series);
    gpu_m2_fill_graph_manifest(&manifest, requested_frames, interval_ms);
    summary.result_rc = -EIO;
    summary.gpu_create_rc = -1;
    summary.kms_begin_rc = -1;
    summary.present_rc = -1;
    ...
    frame = (uint8_t *)malloc((size_t)GPU_M2_GRAPH_SOURCE_BYTES);
    if (frame == NULL) {
        rc = -ENOMEM;
        goto out;
    }
    rc = gpu_2d_present_create_session(&session, &manifest, &d3_summary);
    if (rc < 0) {
        summary.gpu_create_rc = rc;
        goto out;
    }
    summary.gpu_create_rc = d3_summary.gpu_create_rc;
    summary.pm4_dwords = d3_summary.pm4_dwords;
    summary.target_width = d3_summary.target_width;
    summary.target_height = d3_summary.target_height;
    summary.target_stride = d3_summary.target_stride;
    summary.target_bytes = d3_summary.target_bytes;
    if (interval_ms > 0U) {
        (void)a90_monitor_graph_sample(&series);
        usleep((useconds_t)interval_ms * 1000U);
    }

    stop_auto_hud(false);
    started_ns = video_monotonic_ns();
    for (display_index = 0; display_index < requested_frames; ++display_index) {
        struct gpu_d3_video_frame_stats stats;
        uint64_t frame_start_ns = video_monotonic_ns();
        uint64_t before_sample_ns;
        uint64_t after_sample_ns;
        uint64_t before_graph_ns;
        uint64_t after_graph_ns;
        uint64_t before_present_ns;
        uint64_t after_present_ns;
        uint64_t deadline_ns = started_ns + ((uint64_t)display_index * interval_ns);
        char line[128];
        ...
        before_sample_ns = video_monotonic_ns();
        rc = a90_monitor_graph_sample(&series);
        after_sample_ns = video_monotonic_ns();
        gpu_d3_add_us((after_sample_ns - before_sample_ns) / 1000ULL,
                      &sample_sum_us,
                      &summary.sample_max_us);
        if (rc < 0) {
            summary.failed_frame = display_index;
            goto out;
        }

        before_graph_ns = video_monotonic_ns();
        summary.graph_pixels_set =
            a90_monitor_graph_render_mono1(&series,
                                           frame,
                                           manifest.width,
                                           manifest.height,
                                           manifest.stride);
        after_graph_ns = video_monotonic_ns();
        gpu_d3_add_us((after_graph_ns - before_graph_ns) / 1000ULL,
                      &graph_sum_us,
                      &summary.graph_max_us);
        summary.graph_points = series.count;
        summary.cpu_count = series.cpu_count;
        summary.cluster_count = series.cluster_count;
        snprintf(summary.gpu_model, sizeof(summary.gpu_model), "%s",
                 series.gpu_model[0] != '\0' ? series.gpu_model : "?");
        summary.last_cpu_pct = series.last_cpu_pct;
        summary.last_gpu_pct = series.last_gpu_pct;
        summary.last_mem_pct = series.last_mem_pct;
        summary.last_temp_pct = series.last_temp_pct;
        ...
        a90_draw_rect(fb, 0, 0, fb->width, 108U, 0x05070c);
        a90_draw_text(fb, 48U, 20U, "M2 LIVE MONITOR GRAPHS", 0x66ddff, 3U);
        snprintf(line,
                 sizeof(line),
                 "CPU %ld%%  GPU %ld%%  MEM %ld%%  TEMP %ldC",
                 series.last_cpu_pct,
                 series.last_gpu_pct,
                 series.last_mem_pct,
                 series.last_temp_pct);
        a90_draw_text(fb, 48U, 60U, line, 0xffffff, 2U);
        a90_draw_text(fb, 48U, dst_y + 26U, "CPU", 0x66ddff, 2U);
        a90_draw_text(fb, 48U, dst_y + 206U, "GPU", 0x66ddff, 2U);
        a90_draw_text(fb, 48U, dst_y + 386U, "MEM", 0x66ddff, 2U);
        a90_draw_text(fb, 48U, dst_y + 566U, "TMP", 0x66ddff, 2U);
        snprintf(line,
                 sizeof(line),
                 "POINTS %u/%u  FRAME %u/%u  GPU TEXTURED 2D",
                 series.count,
                 A90_MONITOR_GRAPH_MAX_POINTS,
                 display_index + 1U,
                 requested_frames);
        a90_draw_text(fb, 48U, dst_y + session.target_height + 28U,
                      line, 0xbbbbbb, 2U);
        a90_draw_text(fb,
                      48U,
                      dst_y + session.target_height + 58U,
                      "SHARED KGSL 2D PRESENT",
                      0xffcc33,
                      2U);
        ...
        before_present_ns = video_monotonic_ns();
        summary.present_rc = a90_kms_present("gpu-m2-monitor-graph", false);
        after_present_ns = video_monotonic_ns();
        gpu_d3_add_us((after_present_ns - before_present_ns) / 1000ULL,
                      &present_sum_us,
                      &summary.present_max_us);
    ...
    if (summary.presented_frames > 0U) {
        summary.fps_milli =
            ((uint64_t)summary.presented_frames * 1000000000000ULL) /
            summary.elapsed_ns;
        summary.sample_avg_us = sample_sum_us / summary.presented_frames;
        summary.graph_avg_us = graph_sum_us / summary.presented_frames;
        summary.texture_avg_us = texture_sum_us / summary.presented_frames;
        summary.gpu_wait_avg_us = gpu_wait_sum_us / summary.presented_frames;
        summary.readback_avg_us = readback_sum_us / summary.presented_frames;
        summary.copy_avg_us = copy_sum_us / summary.presented_frames;
...
static int gpu_m2_monitor_live_graph_probe(int timeout_ms,
                                           bool materialize_devnode,
                                           uint32_t requested_frames,
                                           uint32_t interval_ms,
                                           int hold_ms) {
    ...
    memset(&summary, 0, sizeof(summary));
    if (timeout_ms <= 0) {
        timeout_ms = GPU_M2_GRAPH_MAX_TIMEOUT_MS;
    }
    if (timeout_ms > GPU_M2_GRAPH_MAX_TIMEOUT_MS) {
        a90_console_printf("gpu.m2.graph.error=timeout-too-large max_ms=%d\r\n",
                           GPU_M2_GRAPH_MAX_TIMEOUT_MS);
        return -EINVAL;
    }
    if (requested_frames == 0U) {
        requested_frames = GPU_M2_GRAPH_DEFAULT_FRAMES;
    }
    if (requested_frames > GPU_M2_GRAPH_MAX_FRAMES) {
        a90_console_printf("gpu.m2.graph.error=frames-too-large max=%u\r\n",
                           GPU_M2_GRAPH_MAX_FRAMES);
        return -EINVAL;
    }
    if (interval_ms == 0U) {
        interval_ms = GPU_M2_GRAPH_DEFAULT_INTERVAL_MS;
    }
    if (interval_ms > GPU_M2_GRAPH_MAX_INTERVAL_MS) {
        a90_console_printf("gpu.m2.graph.error=interval-too-large max_ms=%u\r\n",
                           GPU_M2_GRAPH_MAX_INTERVAL_MS);
        return -EINVAL;
    }
    if (hold_ms < 0 || hold_ms > GPU_H5_VISUAL_HOLD_MAX_MS) {
        a90_console_printf("gpu.m2.graph.error=bad-hold max_ms=%d\r\n",
                           GPU_H5_VISUAL_HOLD_MAX_MS);
        return -EINVAL;
    }
    ...
    a90_console_printf("gpu.m2.graph.command=gpu m2-monitor-live-graph-probe --frames %u --interval-ms %u --timeout-ms %d --hold-ms %d --materialize-devnode\r\n",
                       requested_frames, interval_ms, timeout_ms, hold_ms);
    a90_console_printf("gpu.m2.graph.kgsl_path=%s\r\n", GPU_G0_DEVNODE);
    a90_console_printf("gpu.m2.graph.drm_path=/dev/dri/card0\r\n");
    a90_console_printf("gpu.m2.graph.texture_source=live-monitor-mono1-graph-cpu-gpu-mem-temp\r\n");
    a90_console_printf("gpu.m2.graph.blit_mode=kgsl-textured-quad-scale-to-960x720-linear-readback-kms-copy\r\n");
    a90_console_printf("gpu.m2.graph.extraction_layer=" GPU_M3_EXTRACT_LAYER "\r\n");
    a90_console_printf("gpu.m2.graph.pipeline=shared-kgsl-2d-present-v1\r\n");
    a90_console_printf("gpu.m2.graph.frames_requested=%u\r\n", requested_frames);
    ...
    a90_console_printf("gpu.m2.graph.source=%ux%u mono1 stride=%u\r\n",
                       ...
                       GPU_M2_GRAPH_SOURCE_STRIDE);
    a90_console_printf("gpu.m2.graph.target=%ux%u stride=%u\r\n",
                       GPU_D3_VIDEO_TARGET_WIDTH,
                       GPU_D3_VIDEO_TARGET_HEIGHT,
                       GPU_D3_VIDEO_TARGET_STRIDE);
    a90_console_printf("gpu.m2.graph.shader_sha256=%s\r\n", GPU_D1_TEXTURED_FS_SHA256);
    a90_console_printf("gpu.m2.graph.power_write_attempted=0\r\n");
    a90_console_printf("gpu.m2.graph.proprietary_blob_attempted=0\r\n");
    a90_console_printf("gpu.m2.graph.kgsl_submit_attempted=1\r\n");
    a90_console_printf("gpu.m2.graph.kms_present_attempted=1\r\n");
    if (materialize_devnode) {
        int mat_rc = gpu_g0_materialize_devnode();

        a90_console_printf("gpu.m2.graph.materialize_requested=1\r\n");
        a90_console_printf("gpu.m2.graph.materialize_rc=%d\r\n", mat_rc);
        if (mat_rc < 0) {
            return mat_rc;
        }
    } else {
        a90_console_printf("gpu.m2.graph.materialize_requested=0\r\n");
    }
    if (pipe(pipefd) < 0) {
        int saved_errno = errno;
        a90_console_printf("gpu.m2.graph.pipe_rc=-1 errno=%d\r\n", saved_errno);
        return -saved_errno;
    }
    pid = fork();
    if (pid < 0) {
        ...
        close(pipefd[0]);
        close(pipefd[1]);
        a90_console_printf("gpu.m2.graph.fork_rc=-1 errno=%d\r\n", saved_errno);
        return -saved_errno;
    }
    if (pid == 0) {
        int child_rc;
        ...
        close(pipefd[0]);
        child_rc = gpu_m2_monitor_live_graph_child(pipefd[1],
                                                   requested_frames,
                                                   interval_ms,
                                                   hold_ms);
        _exit(child_rc == 0 ? 0 : 1);
    ...
    a90_console_printf("gpu.m2.graph.result=%s\r\n",
                       got_result && gpu_m2_monitor_graph_summary_passed(&summary) ?
                           "monitor-live-graph-pass" :
                       (timed_out ? "timeout" : "monitor-live-graph-failed"));
    a90_console_printf("gpu.m2.graph.timed_out=%d\r\n", timed_out ? 1 : 0);
    a90_console_printf("gpu.m2.graph.child_killed=%d\r\n", child_killed ? 1 : 0);
    a90_console_printf("gpu.m2.graph.child_reaped=%d\r\n", child_reaped ? 1 : 0);
    a90_console_printf("gpu.m2.graph.child_status=0x%x\r\n", child_status);
    if (got_result) {
        a90_console_printf("gpu.m2.graph.result_rc=%d\r\n", summary.result_rc);
        a90_console_printf("gpu.m2.graph.gpu_create_rc=%d\r\n", summary.gpu_create_rc);
        a90_console_printf("gpu.m2.graph.kms_begin_rc=%d\r\n", summary.kms_begin_rc);
        a90_console_printf("gpu.m2.graph.present_rc=%d\r\n", summary.present_rc);
        a90_console_printf("gpu.m2.graph.presented=%u\r\n", summary.presented_frames);
        a90_console_printf("gpu.m2.graph.failed_frame=%u\r\n", summary.failed_frame);
        a90_console_printf("gpu.m2.graph.graph_points=%u\r\n", summary.graph_points);
        a90_console_printf("gpu.m2.graph.graph_pixels_set=%u\r\n", summary.graph_pixels_set);
        a90_console_printf("gpu.m2.graph.cpu.count=%u\r\n", summary.cpu_count);
        a90_console_printf("gpu.m2.graph.cluster.count=%u\r\n", summary.cluster_count);
        a90_console_printf("gpu.m2.graph.gpu.model=%s\r\n", summary.gpu_model);
        a90_console_printf("gpu.m2.graph.last.cpu_pct=%ld\r\n", summary.last_cpu_pct);
        a90_console_printf("gpu.m2.graph.last.gpu_pct=%ld\r\n", summary.last_gpu_pct);
        a90_console_printf("gpu.m2.graph.last.mem_pct=%ld\r\n", summary.last_mem_pct);
        a90_console_printf("gpu.m2.graph.last.temp_pct=%ld\r\n", summary.last_temp_pct);
        a90_console_printf("gpu.m2.graph.source_size=%ux%u\r\n",
                           summary.source_width, summary.source_height);
        a90_console_printf("gpu.m2.graph.source_stride=%u\r\n", summary.source_stride);
        a90_console_printf("gpu.m2.graph.source_frame_bytes=%u\r\n",
                           summary.source_frame_bytes);
        a90_console_printf("gpu.m2.graph.target_size=%ux%u\r\n",
                           summary.target_width, summary.target_height);
        a90_console_printf("gpu.m2.graph.target_stride=%u\r\n", summary.target_stride);
        a90_console_printf("gpu.m2.graph.target_bytes=%u\r\n", summary.target_bytes);
        a90_console_printf("gpu.m2.graph.pm4_dwords=%u\r\n", summary.pm4_dwords);
        a90_console_printf("gpu.m2.graph.elapsed_ns=%llu\r\n",
                           (unsigned long long)summary.elapsed_ns);
        a90_console_printf("gpu.m2.graph.fps_milli=%llu\r\n",
                           (unsigned long long)summary.fps_milli);
        a90_console_printf("gpu.m2.graph.timing.unit=us\r\n");
        a90_console_printf("gpu.m2.graph.timing.sample.avg_us=%llu\r\n",
                           (unsigned long long)summary.sample_avg_us);
        a90_console_printf("gpu.m2.graph.timing.sample.max_us=%llu\r\n",
                           (unsigned long long)summary.sample_max_us);
        a90_console_printf("gpu.m2.graph.timing.graph.avg_us=%llu\r\n",
                           (unsigned long long)summary.graph_avg_us);
        a90_console_printf("gpu.m2.graph.timing.graph.max_us=%llu\r\n",
                           (unsigned long long)summary.graph_max_us);
        a90_console_printf("gpu.m2.graph.timing.texture.avg_us=%llu\r\n",
                           (unsigned long long)summary.texture_avg_us);
        a90_console_printf("gpu.m2.graph.timing.texture.max_us=%llu\r\n",
                           (unsigned long long)summary.texture_max_us);
        a90_console_printf("gpu.m2.graph.timing.gpu_wait.avg_us=%llu\r\n",
                           (unsigned long long)summary.gpu_wait_avg_us);
        a90_console_printf("gpu.m2.graph.timing.gpu_wait.max_us=%llu\r\n",
                           (unsigned long long)summary.gpu_wait_max_us);
        a90_console_printf("gpu.m2.graph.timing.readback.avg_us=%llu\r\n",
                           (unsigned long long)summary.readback_avg_us);
        a90_console_printf("gpu.m2.graph.timing.readback.max_us=%llu\r\n",
                           (unsigned long long)summary.readback_max_us);
        a90_console_printf("gpu.m2.graph.timing.copy.avg_us=%llu\r\n",
                           (unsigned long long)summary.copy_avg_us);
        a90_console_printf("gpu.m2.graph.timing.copy.max_us=%llu\r\n",
                           (unsigned long long)summary.copy_max_us);
        a90_console_printf("gpu.m2.graph.timing.present.avg_us=%llu\r\n",
                           (unsigned long long)summary.present_avg_us);
        a90_console_printf("gpu.m2.graph.timing.present.max_us=%llu\r\n",
                           (unsigned long long)summary.present_max_us);
        a90_console_printf("gpu.m2.graph.timing.total.avg_us=%llu\r\n",
                           (unsigned long long)summary.total_avg_us);
        a90_console_printf("gpu.m2.graph.timing.total.max_us=%llu\r\n",
                           (unsigned long long)summary.total_max_us);
        a90_console_printf("gpu.m2.graph.changed_total=%llu\r\n",
                           (unsigned long long)summary.changed_total);
        a90_console_printf("gpu.m2.graph.last_first_word=0x%x\r\n",
                           summary.last_first_word);
        a90_console_printf("gpu.m2.graph.last_center_word=0x%x\r\n",
                           summary.last_center_word);
        a90_console_printf("gpu.m2.graph.semantic.sample_count=%u\r\n",
                           summary.semantic_sample_count);
        a90_console_printf("gpu.m2.graph.semantic.match_count=%u\r\n",
                           summary.semantic_sample_match_count);
        a90_console_printf("gpu.m2.graph.semantic.exact_match_count=%u\r\n",
                           summary.semantic_exact_match_count);
        a90_console_printf("gpu.m2.graph.semantic.edge_tolerant_match_count=%u\r\n",
                           summary.semantic_edge_tolerant_match_count);
        a90_console_printf("gpu.m2.graph.semantic.mismatch_count=%u\r\n",
                           summary.semantic_sample_mismatch_count);
        a90_console_printf("gpu.m2.graph.semantic.output_other_count=%u\r\n",
                           summary.semantic_output_other_count);
        a90_console_printf("gpu.m2.graph.close_rc=%d errno=%d\r\n",
                           summary.close_rc, summary.close_errno);
    }
    if (timed_out) {
        return -ETIMEDOUT;
    }
    return got_result && gpu_m2_monitor_graph_summary_passed(&summary) ?
        0 : -EIO;
}

static int gpu_m3_monitor_extraction_probe(int timeout_ms,
                                           bool materialize_devnode,
                                           uint32_t requested_frames,
                                           uint32_t interval_ms,
                                           int hold_ms) {
    ...
    a90_console_printf("gpu.m3.extract.version=1\r\n");
    a90_console_printf("gpu.m3.extract.scope=" GPU_M3_EXTRACT_SCOPE "\r\n");
    a90_console_printf("gpu.m3.extract.layer=" GPU_M3_EXTRACT_LAYER "\r\n");
    a90_console_printf("gpu.m3.extract.command=gpu m3-monitor-extraction-probe --frames %u --interval-ms %u --timeout-ms %d --hold-ms %d --materialize-devnode\r\n",
                       requested_frames, interval_ms, timeout_ms, hold_ms);
    a90_console_printf("gpu.m3.extract.shared_core=bo-map,sync-to-gpu,submit-wait,linear-readback,kms-copy\r\n");
    a90_console_printf("gpu.m3.extract.live_consumer=m2-monitor-live-graph-probe\r\n");
    a90_console_printf("gpu.m3.extract.regression_consumers=d3-video-texture-present-probe,m2-monitor-live-graph-probe\r\n");
    a90_console_printf("gpu.m3.extract.kgsl_submit_attempted=1\r\n");
    a90_console_printf("gpu.m3.extract.kms_present_attempted=1\r\n");
    a90_console_printf("gpu.m3.extract.power_write_attempted=0\r\n");
    a90_console_printf("gpu.m3.extract.proprietary_blob_attempted=0\r\n");
    rc = gpu_m2_monitor_live_graph_probe(timeout_ms,
                                         materialize_devnode,
                                         requested_frames,
                                         interval_ms,
                                         hold_ms);
    a90_console_printf("gpu.m3.extract.delegate_rc=%d\r\n", rc);
    a90_console_printf("gpu.m3.extract.result=%s\r\n",
                       rc == 0 ? "shared-2d-present-monitor-pass" :
                                 "shared-2d-present-monitor-failed");
    return rc;
}

static int gpu_z2_ioctl_retry(int fd, unsigned long request, void *arg) {
    int rc;
...
static int gpu_z2_materialize_drm_card0(void) {
    char dev_text[64];
    unsigned int major_num = 0;
    unsigned int minor_num = 0;

    if (read_trimmed_text_file("/sys/class/drm/card0/dev",
                               dev_text,
                               sizeof(dev_text)) < 0) {
        return negative_errno_or(ENOENT);
    }
    ...
    errno = 0;
    if (ensure_char_node("/dev/dri/card0", major_num, minor_num) < 0) {
        return negative_errno_or(EIO);
    }
    return 0;
}

static int gpu_z2_msm_gem_info_u64(int fd,
                                   uint32_t handle,
                                   uint32_t info,
                                   uint64_t *value) {
    struct drm_msm_gem_info arg;

    if (fd < 0 || handle == 0U || value == NULL) {
        return -EINVAL;
    }
    ...
    arg.info = info;
    errno = 0;
    if (gpu_z2_ioctl_retry(fd, DRM_IOCTL_MSM_GEM_INFO, &arg) < 0) {
        return negative_errno_or(EIO);
    }
    *value = arg.value;
    return 0;
...
static int gpu_z2_close_drm_gem_handle(int fd, uint32_t handle) {
    ...
    errno = 0;
    if (gpu_z2_ioctl_retry(fd, DRM_IOCTL_GEM_CLOSE, &close_arg) < 0) {
        return negative_errno_or(EIO);
    }
    return 0;
}
...
static bool gpu_z2_imported_target_summary_passed(
    ...
    return summary != NULL &&
           ...
           summary->drm_msm_gem_info_offset_rc == 0 &&
           ...
           summary->target_bytes != 0U &&
           summary->kgsl_size >= summary->target_bytes &&
...
static bool gpu_z3_kms_dumb_imported_target_summary_passed(
    ...
    return summary != NULL &&
           ...
           summary->target_bytes != 0U &&
           summary->drm_dumb_pitch == summary->target_stride &&
           summary->drm_dumb_size >= summary->target_bytes &&
           summary->drm_prime_export_rc == 0 &&
           summary->drm_addfb2_rc == 0 &&
           summary->kgsl_import_rc == 0 &&
           summary->kgsl_info_rc == 0 &&
...
static bool gpu_z3_imported_scanout_primary_summary_passed(
    const struct gpu_z2_imported_target_summary *summary) {
    return gpu_z3_kms_dumb_imported_target_summary_passed(summary) &&
           summary->kms_begin_rc == 0 &&
           summary->kms_base_present_rc == 0 &&
           summary->kms_restore_rc == 0 &&
           summary->kms_info_initialized == 1 &&
           summary->kms_crtc_id != 0U &&
           summary->kms_crtc_index >= 0 &&
           summary->kms_screen_width == summary->target_width &&
           summary->kms_screen_height == summary->target_height &&
           summary->kms_base_fb_id != 0U &&
           summary->kms_primary_setcrtc_attempted == 1U &&
           summary->kms_present_rc == 0 &&
           summary->hold_ms > 0U &&
           summary->kms_disable_plane_rc == 0;
}
...
static int gpu_z2_imported_scanout_target_child(int write_fd,
    ...
    memset(&summary, 0, sizeof(summary));
    memset(&d3_summary, 0, sizeof(d3_summary));
    memset(&import_metrics, 0, sizeof(import_metrics));
    memset(&manifest, 0, sizeof(manifest));
    gpu_d3_session_init(&session);
    a90_monitor_graph_reset(&series);
    gpu_m2_fill_graph_manifest(&manifest, 1U, 1U);
    summary.result_rc = -EIO;
    ...
    summary.kms_restore_rc = present_external && primary_setcrtc ? -ENOSYS : 0;
    ...
    summary.kms_primary_setcrtc_attempted =
        present_external && primary_setcrtc ? 1U : 0U;
    summary.source_width = manifest.width;
    summary.source_height = manifest.height;
    summary.source_stride = manifest.stride;
    summary.source_frame_bytes = manifest.frame_bytes;
    ...
    if (present_external) {
        ...
        summary.kms_base_fb_id = kms_info.fb_id;
        ...
        summary.drm_open_rc = 0;
        if (primary_setcrtc) {
            summary.target_width = kms_info.width;
            summary.target_height = kms_info.height;
            summary.target_stride = kms_info.width * GPU_D3_VIDEO_TARGET_BPP;
            summary.target_bytes = (uint32_t)(
                (uint64_t)summary.target_stride * summary.target_height);
        }
    } else {
        errno = 0;
    ...
    if (use_kms_dumb_target) {
        ...
        dumb.width = primary_setcrtc ? summary.kms_screen_width :
                                       GPU_D3_VIDEO_TARGET_WIDTH;
        dumb.height = primary_setcrtc ? summary.kms_screen_height :
                                        GPU_D3_VIDEO_TARGET_HEIGHT;
        dumb.bpp = 32U;
        errno = 0;
        ...
        if (primary_setcrtc) {
            if (dumb.width == 0U || dumb.height == 0U ||
                dumb.pitch < dumb.width * GPU_D3_VIDEO_TARGET_BPP ||
                dumb.size < (uint64_t)dumb.pitch * dumb.height ||
                dumb.size > GPU_Z3_PRIMARY_MAX_TARGET_BYTES ||
                dumb.size > UINT32_MAX) {
                rc = -EINVAL;
                goto out;
            }
            summary.target_width = dumb.width;
            summary.target_height = dumb.height;
            summary.target_stride = dumb.pitch;
            summary.target_bytes = (uint32_t)dumb.size;
        } else {
    ...
    errno = 0;
    if (gpu_z2_ioctl_retry(drm_fd, DRM_IOCTL_PRIME_HANDLE_TO_FD, &prime_arg) < 0) {
        rc = negative_errno_or(EIO);
        summary.drm_prime_export_rc = rc;
        goto out;
    }
    ...
    addfb2.width = summary.target_width;
    addfb2.height = summary.target_height;
    addfb2.pixel_format = DRM_FORMAT_XBGR8888;
    addfb2.handles[0] = drm_handle;
    addfb2.pitches[0] = summary.target_stride;
    errno = 0;
    if (gpu_z2_ioctl_retry(drm_fd, DRM_IOCTL_MODE_ADDFB2, &addfb2) < 0) {
        rc = negative_errno_or(EIO);
        summary.drm_addfb2_rc = rc;
        goto out;
    }
    fb_id = addfb2.fb_id;
    summary.drm_fb_id = fb_id;
    summary.drm_addfb2_rc = 0;
    ...
    rc = gpu_2d_present_create_session_internal(&session,
                                                &manifest,
                                                &d3_summary,
                                                prime_fd,
                                                map,
                                                &import_metrics,
                                                primary_setcrtc ?
                                                    summary.target_width : 0U,
                                                primary_setcrtc ?
                                                    summary.target_height : 0U,
                                                primary_setcrtc ?
                                                    summary.target_stride : 0U,
                                                primary_setcrtc ?
                                                    summary.target_bytes : 0ULL);
    if (session.linear.map == map) {
        session_owns_map = true;
        map = MAP_FAILED;
    }
    summary.gpu_create_rc = rc;
    summary.kgsl_import_rc = import_metrics.import_rc;
    summary.kgsl_info_rc = import_metrics.info_rc;
    summary.kgsl_id = import_metrics.kgsl_id;
    summary.kgsl_gpuaddr = import_metrics.gpuaddr;
    summary.kgsl_size = import_metrics.size;
    summary.kgsl_flags = import_metrics.flags;
    summary.kgsl_va_len = import_metrics.va_len;
    summary.pm4_dwords = d3_summary.pm4_dwords;
    if (rc < 0) {
        goto out;
    }

    {
        ...
        started_ns = video_monotonic_ns();
        rc = gpu_2d_present_render_frame_to_linear(&session,
                                                   &manifest,
                                                   frame,
                                                   &stats);
        finished_ns = video_monotonic_ns();
        summary.elapsed_ns = finished_ns > started_ns ? finished_ns - started_ns : 1ULL;
        summary.render_rc = rc;
        summary.texture_write_us = stats.texture_write_us;
        summary.gpu_wait_us = stats.gpu_wait_us;
        summary.readback_sync_us = stats.readback_sync_us;
        summary.sample_us = stats.kms_copy_us;
        summary.changed_count = stats.changed_count;
        summary.last_first_word = stats.first_word;
        summary.last_center_word = stats.center_word;
        summary.semantic_sample_count = stats.semantic_sample_count;
        summary.semantic_sample_match_count = stats.semantic_sample_match_count;
    ...
    if (present_external && primary_setcrtc) {
        started_ns = video_monotonic_ns();
        summary.kms_present_rc =
            a90_kms_present_external_fb(fb_id,
                                        "gpu-z3-imported-scanout-primary",
                                        false);
        finished_ns = video_monotonic_ns();
        summary.present_us =
            finished_ns > started_ns ? (finished_ns - started_ns) / 1000ULL : 1ULL;
        if (summary.kms_present_rc < 0) {
            rc = negative_errno_or(EIO);
            summary.kms_present_rc = rc;
            goto out;
        }
        ...
        if (summary.kms_base_fb_id != 0U) {
            summary.kms_restore_rc =
                a90_kms_present_external_fb(summary.kms_base_fb_id,
                                            "gpu-z3-imported-scanout-primary-restore",
                                            false);
            if (summary.kms_restore_rc < 0) {
                rc = negative_errno_or(EIO);
                summary.kms_restore_rc = rc;
                goto out;
            }
        }
    } else if (present_external) {
    ...
    summary.result_rc =
        present_external && primary_setcrtc ?
            (gpu_z3_imported_scanout_primary_summary_passed(&summary) ? 0 : -EIO) :
        present_external ?
...
out:
    if (present_external && primary_setcrtc && drm_fd >= 0 &&
        summary.kms_base_fb_id != 0U &&
        summary.kms_restore_rc == -ENOSYS) {
        summary.kms_restore_rc =
            a90_kms_present_external_fb(summary.kms_base_fb_id,
                                        "gpu-z3-imported-scanout-primary-restore",
                                        false);
        if (summary.kms_restore_rc < 0) {
            summary.kms_restore_rc = negative_errno_or(EIO);
        }
    }
    if (present_external && !primary_setcrtc && drm_fd >= 0) {
    ...
    if (fb_id != 0U && drm_fd >= 0) {
        uint32_t rmfb_id = fb_id;

        errno = 0;
        if (gpu_z2_ioctl_retry(drm_fd, DRM_IOCTL_MODE_RMFB, &rmfb_id) < 0) {
            summary.rmfb_rc = negative_errno_or(EIO);
        }
    }
    if (map != MAP_FAILED) {
    ...
    if (prime_fd >= 0) {
        errno = 0;
        if (close(prime_fd) < 0) {
            summary.close_prime_rc = negative_errno_or(EIO);
        }
        prime_fd = -1;
    }
    ...
    if (summary.result_rc != 0) {
        if (rc < 0) {
            summary.result_rc = rc;
        } else if (summary.render_rc < 0) {
            summary.result_rc = summary.render_rc;
        } else {
            summary.result_rc = -EIO;
        }
    }
...
static int gpu_z2_imported_scanout_target_probe(int timeout_ms,
                                                bool materialize_devnode) {
    int pipefd[2];
    pid_t pid;
    long deadline_ms;
    ...
    if (timeout_ms > GPU_M2_GRAPH_MAX_TIMEOUT_MS) {
        a90_console_printf("gpu.z2.import.error=timeout-too-large max_ms=%d\r\n",
                           GPU_M2_GRAPH_MAX_TIMEOUT_MS);
        return -EINVAL;
    }

    a90_console_printf("gpu.z2.import.version=1\r\n");
    a90_console_printf("gpu.z2.import.scope=gpu-z2-imported-scanout-render-target\r\n");
    a90_console_printf("gpu.z2.import.command=gpu z2-imported-scanout-target-probe --timeout-ms %d --materialize-devnode\r\n",
                       timeout_ms);
    a90_console_printf("gpu.z2.import.kgsl_path=%s\r\n", GPU_G0_DEVNODE);
    a90_console_printf("gpu.z2.import.drm_path=/dev/dri/card0\r\n");
    a90_console_printf("gpu.z2.import.buffer=drm-msm-scanout-gem-prime-fd-kgsl-dmabuf-import\r\n");
    a90_console_printf("gpu.z2.import.target=%ux%u stride=%u bytes=%llu format=XBGR8888\r\n",
                       GPU_D3_VIDEO_TARGET_WIDTH,
                       GPU_D3_VIDEO_TARGET_HEIGHT,
                       GPU_D3_VIDEO_TARGET_STRIDE,
                       (unsigned long long)GPU_D3_VIDEO_TARGET_BYTES);
    a90_console_printf("gpu.z2.import.blit_mode=kgsl-textured-quad-scale-to-imported-scanout-linear\r\n");
    a90_console_printf("gpu.z2.import.kgsl_submit_attempted=1\r\n");
    a90_console_printf("gpu.z2.import.kms_adfb2_attempted=1\r\n");
    a90_console_printf("gpu.z2.import.kms_copy_attempted=0\r\n");
    a90_console_printf("gpu.z2.import.kms_present_attempted=0\r\n");
    a90_console_printf("gpu.z2.import.power_write_attempted=0\r\n");
    a90_console_printf("gpu.z2.import.proprietary_blob_attempted=0\r\n");
    if (materialize_devnode) {
        int kgsl_rc = gpu_g0_materialize_devnode();
        int drm_rc = gpu_z2_materialize_drm_card0();

        a90_console_printf("gpu.z2.import.materialize_requested=1\r\n");
        a90_console_printf("gpu.z2.import.materialize_kgsl_rc=%d\r\n", kgsl_rc);
        a90_console_printf("gpu.z2.import.materialize_drm_rc=%d\r\n", drm_rc);
        if (kgsl_rc < 0) {
            return kgsl_rc;
        }
        if (drm_rc < 0) {
        ...
        a90_console_printf("gpu.z2.import.materialize_requested=0\r\n");
    }
    if (pipe(pipefd) < 0) {
        int saved_errno = errno;
        a90_console_printf("gpu.z2.import.pipe_rc=-1 errno=%d\r\n", saved_errno);
        return -saved_errno;
    }
    pid = fork();
    if (pid < 0) {
        ...
        close(pipefd[0]);
        close(pipefd[1]);
        a90_console_printf("gpu.z2.import.fork_rc=-1 errno=%d\r\n", saved_errno);
        return -saved_errno;
    }
    if (pid == 0) {
        int child_rc;
        ...
        close(pipefd[0]);
        child_rc = gpu_z2_imported_scanout_target_child(pipefd[1], false, 0, false);
        _exit(child_rc == 0 ? 0 : 1);
    }
    close(pipefd[1]);
    deadline_ms = monotonic_millis() + timeout_ms;
    a90_console_printf("gpu.z2.import.child_pid=%ld\r\n", (long)pid);
    ...
    a90_console_printf("gpu.z2.import.result=%s\r\n",
                       got_result && gpu_z2_imported_target_summary_passed(&summary) ?
                           "z2-imported-scanout-render-target-pass" :
                       (timed_out ? "timeout" :
                                    "z2-imported-scanout-render-target-failed"));
    a90_console_printf("gpu.z2.import.timed_out=%d\r\n", timed_out ? 1 : 0);
    a90_console_printf("gpu.z2.import.child_killed=%d\r\n", child_killed ? 1 : 0);
    a90_console_printf("gpu.z2.import.child_reaped=%d\r\n", child_reaped ? 1 : 0);
    a90_console_printf("gpu.z2.import.child_status=0x%x\r\n", child_status);
    if (got_result) {
        a90_console_printf("gpu.z2.import.result_rc=%d\r\n", summary.result_rc);
        a90_console_printf("gpu.z2.import.drm.open_rc=%d\r\n", summary.drm_open_rc);
        a90_console_printf("gpu.z2.import.drm.msm_gem_new_rc=%d handle=%u\r\n",
                           summary.drm_msm_gem_new_rc, summary.drm_handle);
        a90_console_printf("gpu.z2.import.drm.offset_rc=%d offset=0x%llx\r\n",
                           summary.drm_msm_gem_info_offset_rc,
                           (unsigned long long)summary.drm_mmap_offset);
        a90_console_printf("gpu.z2.import.drm.prime_export_rc=%d\r\n",
                           summary.drm_prime_export_rc);
        a90_console_printf("gpu.z2.import.drm.addfb2_rc=%d fb_id=%u\r\n",
                           summary.drm_addfb2_rc, summary.drm_fb_id);
        a90_console_printf("gpu.z2.import.kgsl.import_rc=%d id=%u\r\n",
                           summary.kgsl_import_rc, summary.kgsl_id);
        a90_console_printf("gpu.z2.import.kgsl.info_rc=%d gpuaddr=0x%llx size=%llu flags=0x%llx va_len=%llu\r\n",
        ...
        a90_console_printf("gpu.z2.import.gpu_create_rc=%d render_rc=%d\r\n",
                           summary.gpu_create_rc, summary.render_rc);
        a90_console_printf("gpu.z2.import.source_size=%ux%u stride=%u frame_bytes=%u\r\n",
                           summary.source_width,
                           summary.source_height,
                           summary.source_stride,
                           summary.source_frame_bytes);
        a90_console_printf("gpu.z2.import.target_size=%ux%u stride=%u bytes=%u\r\n",
                           summary.target_width,
                           summary.target_height,
                           summary.target_stride,
                           summary.target_bytes);
        a90_console_printf("gpu.z2.import.pm4_dwords=%u\r\n", summary.pm4_dwords);
        a90_console_printf("gpu.z2.import.graph_pixels_set=%u graph_points=%u\r\n",
                           summary.graph_pixels_set, summary.graph_points);
        a90_console_printf("gpu.z2.import.timing.unit=us\r\n");
        a90_console_printf("gpu.z2.import.timing.texture_us=%llu gpu_wait_us=%llu readback_us=%llu sample_us=%llu\r\n",
        ...
        a90_console_printf("gpu.z2.import.elapsed_ns=%llu\r\n",
                           (unsigned long long)summary.elapsed_ns);
        a90_console_printf("gpu.z2.import.changed_count=%llu\r\n",
                           (unsigned long long)summary.changed_count);
        a90_console_printf("gpu.z2.import.first_word=0x%x center_word=0x%x\r\n",
                           summary.last_first_word, summary.last_center_word);
        a90_console_printf("gpu.z2.import.semantic.sample_count=%u\r\n",
                           summary.semantic_sample_count);
        a90_console_printf("gpu.z2.import.semantic.match_count=%u\r\n",
                           summary.semantic_sample_match_count);
        a90_console_printf("gpu.z2.import.semantic.exact_match_count=%u\r\n",
                           summary.semantic_exact_match_count);
        a90_console_printf("gpu.z2.import.semantic.edge_tolerant_match_count=%u\r\n",
                           summary.semantic_edge_tolerant_match_count);
        a90_console_printf("gpu.z2.import.semantic.mismatch_count=%u\r\n",
                           summary.semantic_sample_mismatch_count);
        a90_console_printf("gpu.z2.import.semantic.output_other_count=%u\r\n",
                           summary.semantic_output_other_count);
        a90_console_printf("gpu.z2.import.cleanup.rmfb_rc=%d close_drm_handle_rc=%d close_prime_rc=%d close_drm_fd_rc=%d\r\n",
                           summary.rmfb_rc,
                           summary.close_drm_handle_rc,
...
static int gpu_z3_imported_scanout_probe(int timeout_ms,
                                         int hold_ms,
                                         bool materialize_devnode,
                                         bool primary_setcrtc) {
    int pipefd[2];
    ...
    if (primary_setcrtc) {
        a90_console_printf("gpu.z3.scanout.scope=gpu-z3-imported-scanout-primary-setcrtc\r\n");
    } else {
        a90_console_printf("gpu.z3.scanout.scope=gpu-z3-imported-scanout-plane-present\r\n");
    }
    a90_console_printf("gpu.z3.scanout.command=gpu %s --timeout-ms %d --hold-ms %d --materialize-devnode\r\n",
                       primary_setcrtc ?
                           "z3-imported-scanout-primary-probe" :
                           "z3-imported-scanout-plane-probe",
                       timeout_ms,
                       hold_ms);
    ...
    if (primary_setcrtc) {
        a90_console_printf("gpu.z3.scanout.target=panel-fullscreen-from-kms-mode format=XBGR8888\r\n");
    } else {
    ...
    if (primary_setcrtc) {
        a90_console_printf("gpu.z3.scanout.present_mode=primary-setcrtc-fullscreen\r\n");
        a90_console_printf("gpu.z3.scanout.present_order=primary-setcrtc-then-restore-base-fb\r\n");
    } else {
    ...
    if (primary_setcrtc) {
        a90_console_printf("gpu.z3.scanout.primary_setcrtc_attempted=1\r\n");
    } else {
        a90_console_printf("gpu.z3.scanout.primary_setcrtc_attempted=0\r\n");
    }
    ...
    a90_console_printf("gpu.z3.scanout.result=%s\r\n",
                       got_result &&
                       (primary_setcrtc ?
                            gpu_z3_imported_scanout_primary_summary_passed(&summary) :
                            gpu_z3_imported_scanout_present_summary_passed(&summary)) ?
                           (primary_setcrtc ?
                                "z3-imported-scanout-primary-setcrtc-pass" :
                                "z3-imported-scanout-plane-present-pass") :
                       (timed_out ? "timeout" :
                                    (primary_setcrtc ?
                                         "z3-imported-scanout-primary-setcrtc-failed" :
    ...
    if (got_result) {
        ...
        a90_console_printf("gpu.z3.scanout.gpu_create_rc=%d render_rc=%d\r\n",
                           summary.gpu_create_rc, summary.render_rc);
        a90_console_printf("gpu.z3.scanout.source_size=%ux%u stride=%u frame_bytes=%u\r\n",
                           summary.source_width,
                           summary.source_height,
                           summary.source_stride,
                           summary.source_frame_bytes);
        a90_console_printf("gpu.z3.scanout.target_size=%ux%u stride=%u bytes=%u\r\n",
                           summary.target_width,
                           summary.target_height,
                           summary.target_stride,
                           summary.target_bytes);
        ...
        a90_console_printf("gpu.z3.scanout.kms.base_fb_id=%u primary_setcrtc_attempted=%u restore_rc=%d\r\n",
                           summary.kms_base_fb_id,
                           summary.kms_primary_setcrtc_attempted,
                           summary.kms_restore_rc);
...
static int gpu_z3_imported_scanout_plane_probe(int timeout_ms,
                                               int hold_ms,
                                               bool materialize_devnode) {
    return gpu_z3_imported_scanout_probe(timeout_ms,
                                         hold_ms,
                                         materialize_devnode,
                                         false);
}

static int gpu_z3_imported_scanout_primary_probe(int timeout_ms,
                                                 int hold_ms,
                                                 bool materialize_devnode) {
    return gpu_z3_imported_scanout_probe(timeout_ms,
                                         hold_ms,
...
static int handle_gpu(char **argv, int argc) {
    ...
    if (strcmp(subcommand, "m0-monitor-sampler-probe") == 0 ||
        strcmp(subcommand, "monitor-sampler-probe") == 0) {
        uint32_t samples = A90_MONITOR_M0_DEFAULT_SAMPLES;
        uint32_t interval_ms = A90_MONITOR_M0_DEFAULT_INTERVAL_MS;

        for (index = 2; index < argc; ++index) {
            if (strcmp(argv[index], "--samples") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   A90_MONITOR_M0_MIN_SAMPLES,
                                   A90_MONITOR_M0_MAX_SAMPLES,
                                   &samples)) {
                    a90_console_printf("gpu.m0.monitor.error=bad-samples\r\n");
                    return -EINVAL;
                }
                ++index;
                ...
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   0,
                                   A90_MONITOR_M0_MAX_INTERVAL_MS,
                                   &interval_ms)) {
                    a90_console_printf("gpu.m0.monitor.error=bad-interval\r\n");
                    return -EINVAL;
                }
                ++index;
            } else {
                a90_console_printf("usage: gpu m0-monitor-sampler-probe [--samples N] [--interval-ms N]\r\n");
                return -EINVAL;
            }
        }
        return a90_monitor_m0_sampler_probe(samples, interval_ms);
    }
    if (strcmp(subcommand, "m1-monitor-dashboard-probe") == 0 ||
        strcmp(subcommand, "monitor-dashboard-probe") == 0) {
        uint32_t samples = A90_MONITOR_M0_DEFAULT_SAMPLES;
        uint32_t interval_ms = A90_MONITOR_M0_DEFAULT_INTERVAL_MS;
        uint32_t hold_ms = A90_MONITOR_M1_DEFAULT_HOLD_MS;
            ...
            if (strcmp(argv[index], "--samples") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   A90_MONITOR_M0_MAX_SAMPLES,
                                   &samples)) {
                    a90_console_printf("gpu.m1.monitor.error=bad-samples\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--interval-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   A90_MONITOR_M0_MAX_INTERVAL_MS,
                                   &interval_ms)) {
                    a90_console_printf("gpu.m1.monitor.error=bad-interval\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--hold-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   0,
                                   A90_MONITOR_M1_MAX_HOLD_MS,
                                   &hold_ms)) {
                    a90_console_printf("gpu.m1.monitor.error=bad-hold\r\n");
                    return -EINVAL;
                }
                ++index;
            } else {
                a90_console_printf("usage: gpu m1-monitor-dashboard-probe [--samples N] [--interval-ms N] [--hold-ms N]\r\n");
                return -EINVAL;
            }
        }
        return a90_monitor_m1_dashboard_probe(samples, interval_ms, hold_ms);
    }
    if (strcmp(subcommand, "m2-monitor-live-graph-probe") == 0 ||
        strcmp(subcommand, "monitor-live-graph-probe") == 0) {
        uint32_t requested_frames = GPU_M2_GRAPH_DEFAULT_FRAMES;
        uint32_t interval_ms = GPU_M2_GRAPH_DEFAULT_INTERVAL_MS;
        uint32_t timeout_ms = GPU_M2_GRAPH_MAX_TIMEOUT_MS;
        uint32_t hold_ms = GPU_M2_GRAPH_DEFAULT_HOLD_MS;
        bool materialize_devnode = false;
            ...
            if (strcmp(argv[index], "--frames") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_FRAMES,
                                   &requested_frames)) {
                    a90_console_printf("gpu.m2.graph.error=bad-frames\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--interval-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_INTERVAL_MS,
                                   &interval_ms)) {
                    a90_console_printf("gpu.m2.graph.error=bad-interval\r\n");
                    return -EINVAL;
                }
                ++index;
                ...
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_TIMEOUT_MS,
                                   &timeout_ms)) {
                    a90_console_printf("gpu.m2.graph.error=bad-timeout\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--hold-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_H5_VISUAL_HOLD_MAX_MS,
                                   &hold_ms)) {
                    a90_console_printf("gpu.m2.graph.error=bad-hold\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--materialize-devnode") == 0) {
                ...
                a90_console_printf("usage: gpu m2-monitor-live-graph-probe [--frames N] [--interval-ms N] [--timeout-ms N] [--hold-ms N] [--materialize-devnode]\r\n");
                return -EINVAL;
            }
        }
        return gpu_m2_monitor_live_graph_probe((int)timeout_ms,
                                               materialize_devnode,
                                               requested_frames,
                                               interval_ms,
                                               (int)hold_ms);
    }
    if (strcmp(subcommand, "m3-monitor-extraction-probe") == 0 ||
        strcmp(subcommand, "monitor-extraction-probe") == 0) {
        uint32_t requested_frames = GPU_M2_GRAPH_DEFAULT_FRAMES;
        uint32_t interval_ms = GPU_M2_GRAPH_DEFAULT_INTERVAL_MS;
        uint32_t timeout_ms = GPU_M2_GRAPH_MAX_TIMEOUT_MS;
        uint32_t hold_ms = GPU_M2_GRAPH_DEFAULT_HOLD_MS;
            ...
            if (strcmp(argv[index], "--frames") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_FRAMES,
                                   &requested_frames)) {
                    a90_console_printf("gpu.m3.extract.error=bad-frames\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--interval-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_INTERVAL_MS,
                                   &interval_ms)) {
                    a90_console_printf("gpu.m3.extract.error=bad-interval\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--timeout-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_M2_GRAPH_MAX_TIMEOUT_MS,
                                   &timeout_ms)) {
                    a90_console_printf("gpu.m3.extract.error=bad-timeout\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--hold-ms") == 0) {
                if (index + 1 >= argc ||
                    !parse_u32_arg(argv[index + 1],
                                   ...
                                   GPU_H5_VISUAL_HOLD_MAX_MS,
                                   &hold_ms)) {
                    a90_console_printf("gpu.m3.extract.error=bad-hold\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--materialize-devnode") == 0) {
                ...
                a90_console_printf("usage: gpu m3-monitor-extraction-probe [--frames N] [--interval-ms N] [--timeout-ms N] [--hold-ms N] [--materialize-devnode]\r\n");
                return -EINVAL;
            }
        }
        return gpu_m3_monitor_extraction_probe((int)timeout_ms,
                                               materialize_devnode,
                                               requested_frames,
                                               interval_ms,
                                               (int)hold_ms);
    }
    if (strcmp(subcommand, "z2-imported-scanout-target-probe") == 0 ||
        strcmp(subcommand, "imported-scanout-target-probe") == 0) {
        bool materialize_devnode = false;
        ...
        for (index = 2; index < argc; ++index) {
            if (strcmp(argv[index], "--timeout-ms") == 0) {
                if (index + 1 >= argc || !gpu_g0_parse_int(argv[index + 1], &timeout_ms)) {
                    a90_console_printf("gpu.z2.import.error=bad-timeout\r\n");
                    return -EINVAL;
                }
                ++index;
            } else if (strcmp(argv[index], "--materialize-devnode") == 0) {
                ...
                a90_console_printf("usage: gpu z2-imported-scanout-target-probe [--timeout-ms N] [--materialize-devnode]\r\n");
                return -EINVAL;
            }
        }
    ...
    if (strcmp(subcommand, "z3-imported-scanout-plane-probe") == 0 ||
        ...
        strcmp(subcommand, "z3-imported-scanout-primary-probe") == 0 ||
        strcmp(subcommand, "imported-scanout-primary-probe") == 0) {
        bool materialize_devnode = false;
        int hold_ms = 12000;
        bool primary_setcrtc =
            strcmp(subcommand, "z3-imported-scanout-primary-probe") == 0 ||
        ...
        for (index = 2; index < argc; ++index) {
            if (strcmp(argv[index], "--timeout-ms") == 0) {
                ...
                a90_console_printf("usage: gpu [z3-imported-scanout-plane-probe|z3-imported-scanout-primary-probe] [--timeout-ms N] [--hold-ms N] [--materialize-devnode]\r\n");
                return -EINVAL;
            }
        }
        return primary_setcrtc ?
            gpu_z3_imported_scanout_primary_probe(timeout_ms,
                                                  hold_ms,
                                                  materialize_devnode) :
            gpu_z3_imported_scanout_plane_probe(timeout_ms,
                                                hold_ms,
                                                materialize_devnode);
    }
...
static const struct shell_command command_table[] = {
    ...
    { "boot-write-e3a", handle_boot_write_e3a, "boot-write-e3a <token>", CMD_DANGEROUS, A90_CMD_GROUP_CORE },
    { "boot-write-e3b", handle_boot_write_e3b, "boot-write-e3b <token>", CMD_DANGEROUS, A90_CMD_GROUP_CORE },
    { "boot-write-e4", handle_boot_write_e4, "boot-write-e4 <token>", CMD_DANGEROUS, A90_CMD_GROUP_CORE },
    { "boot-write-e5", handle_boot_write_e5, "boot-write-e5 <token>", CMD_DANGEROUS, A90_CMD_GROUP_CORE },
