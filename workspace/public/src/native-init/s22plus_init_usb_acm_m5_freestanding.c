 ...
 *
 * Freestanding variant: no glibc startup, no libc calls, and only direct arm64
 * Linux syscalls. M4T2/M4T3 proved this raw PID1 runtime shape on the device;
 * using it here keeps M5 focused on the USB/module/configfs path instead of
 * conflating that path with glibc static PID1 startup.
 */
...
static const char k_marker[] =
    "S22_NATIVE_INIT_USB_ACM_M5 version=0.4 pid1=direct "
    "runtime=freestanding raw_syscalls=1 usb_first_modules=26 "
    "gadget=ss_acm.0 tty=/dev/ttyGS0 no_android_handoff=1 "
    "no_auto_reboot=1 udc_bind_retry=1 acm_cmd_download=1\n";
...
static int cstr_contains(const char *haystack, const char *needle) {
    if (needle[0] == '\0') {
        return 1;
    }
...
static void ensure_chr_node(const char *path, unsigned int mode, unsigned int major_num, unsigned int minor_num) {
    ...
    long rc = sys_mknodat(path, S_IFCHR | mode, make_dev(major_num, minor_num));
    (void)rc;
}
...
static void emit_mounts(long proc_rc, long sys_rc, long dev_rc, long run_rc, long configfs_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=mounts proc_rc=");
...
static void setup_minimal_fs(void) {
    ...
    emit_mounts(proc_rc, sys_rc, dev_rc, run_rc, configfs_rc);
}

static int build_module_path(char *out, size_t size, const char *name) {
    const char prefix[] = "/lib/modules/s22plus-m5/";
...
static void emit_module(size_t index, const char *name, long open_rc, long finit_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=module index=");
...
static void load_usb_modules(void) {
    for (size_t idx = 0; idx < ARRAY_SIZE(k_usb_modules); ++idx) {
        char path[256];
        if (!build_module_path(path, sizeof(path), k_usb_modules[idx])) {
            emit_module(idx + 1, k_usb_modules[idx], -1, -1);
            continue;
        }
        long fd = sys_openat(AT_FDCWD, path, O_RDONLY | O_CLOEXEC, 0);
        if (fd < 0) {
            emit_module(idx + 1, k_usb_modules[idx], fd, -1);
            continue;
        }
        long rc = sys_finit_module((int)fd, "", 0);
        (void)sys_close((int)fd);
        emit_module(idx + 1, k_usb_modules[idx], fd, rc);
        (void)sys_sleep_ms(20);
    }
}
...
static long write_attr(const char *path, const char *value) {
    ...
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=configfs_write path=");
        sb_puts(&sb, path);
        sb_puts(&sb, " open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
...
static int choose_udc(char *buf, size_t size) {
    ...
    for (;;) {
        ...
        while (pos < nread) {
            ...
            if (!cstr_eq(name, ".") && !cstr_eq(name, "..")) {
                if (fallback[0] == '\0') {
                    copy_cstr(fallback, sizeof(fallback), name);
                }
                if (!cstr_contains(name, "dummy")) {
...
static void emit_gadget(long symlink_rc, int have_udc, const char *udc, long udc_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=acm_gadget symlink_rc=");
...
static int create_acm_gadget(void) {
    ...
    if (read_trim("/config/usb_gadget/g1/UDC", current_udc, sizeof(current_udc)) == 0 && current_udc[0] != '\0') {
        emit("S22_NATIVE_INIT_USB_ACM_M5 phase=acm_gadget already_bound=1\n");
        return 1;
    }
    ...
    char udc[128] = "";
    int have_udc = choose_udc(udc, sizeof(udc));
    long udc_rc = -1;
    if (have_udc) {
...
static int command_is_download(const char *cmd) {
    ...
    tmp[len] = '\0';
    return cstr_eq(tmp, "download") || cstr_eq(tmp, "reboot-download") || cstr_eq(tmp, "S22M5_DOWNLOAD");
}

static void emit_command(const char *cmd) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=acm_command cmd=");
    sb_puts(&sb, cmd);
    sb_putc(&sb, '\n');
    emit_buf(&sb);
}
...
static void handle_command(const char *cmd, int fd) {
    ...
    if (!command_is_download(cmd)) {
        ...
        (void)write_all(fd, nack, sizeof(nack) - 1);
        return;
    }
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=reboot_download rc=");
    sb_put_i64(&sb, rc);
    sb_puts(&sb, " returned=1\n");
    emit_buf(&sb);
}
...
static void read_serial_commands(int fd, char *cmd_buf, size_t *cmd_len, size_t cmd_cap) {
    ...
    for (;;) {
        ...
        if (n < 0) {
            struct sbuf sb = {.data = {0}, .len = 0};
            sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=acm_read rc=");
            sb_put_i64(&sb, n);
            sb_putc(&sb, '\n');
            emit_buf(&sb);
            return;
        ...
        for (long i = 0; i < n; ++i) {
            ...
            if (*cmd_len + 1 < cmd_cap) {
                ...
                cmd_buf[cmd_cap - 1] = '\0';
                emit("S22_NATIVE_INIT_USB_ACM_M5 phase=acm_command_overflow\n");
                *cmd_len = 0;
            }
        }
    }
...
static void emit_park(unsigned int tick, int bound, int tty_ready, long open_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_USB_ACM_M5 phase=park tick=");
    sb_put_u64(&sb, tick);
    sb_puts(&sb, " bound=");
