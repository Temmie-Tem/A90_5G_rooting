...
/*
 * Samsung S22+ native-init M26 HS-only prefix/download discriminator.
 *
 * This is an observation-channel candidate, not an ACM bring-up candidate.
 * It loads the first N modules from a M25 HS-only USB2 module list, then
...
#ifndef M26_PREFIX_LIMIT
#define M26_PREFIX_LIMIT 0
#endif

#ifndef M26_PREFIX_LABEL
#define M26_PREFIX_LABEL "P00"
#endif
...
#define MODULES_PREFIX_BUF 8192
...
static const char k_marker[] =
    "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD version=0.1 pid1=direct "
    "runtime=freestanding raw_syscalls=1 "
    "prefix_label=" M26_PREFIX_LABEL " prefix_limit=" STR(M26_PREFIX_LIMIT) " "
    "modules_hs_only_usb2=/s22plus_m26_hs_only_usb2.modules module_count=40 maximum_speed_dtbo=high-speed qmp_excluded=1 "
    "module_source=stock_vendor_boot_ramdisk module_list=boot_ramdisk_hs_only_usb2 "
    "module_injection=list_only observation=prefix-download "
...
static void setup_minimal_fs(void) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=mounts prefix_label=");
    sb_puts(&sb, M26_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M26_PREFIX_LIMIT);
    sb_puts(&sb, " proc_rc=");
...
static void emit_module(size_t index, const char *name, long open_rc, long finit_rc) {
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=module prefix_label=");
    sb_puts(&sb, M26_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M26_PREFIX_LIMIT);
    sb_puts(&sb, " index=");
...
static void load_prefix_modules(void) {
    const size_t prefix_limit = (size_t)M26_PREFIX_LIMIT;
    if (prefix_limit == 0U) {
        emit("S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=modules_prefix_skipped prefix_label="
             M26_PREFIX_LABEL " prefix_limit=0\n");
        return;
    }

    static char text[MODULES_PREFIX_BUF];
    long fd = sys_openat(AT_FDCWD, "/s22plus_m26_hs_only_usb2.modules", O_RDONLY | O_CLOEXEC, 0);
    if (fd < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=modules_prefix open_rc=");
        sb_put_i64(&sb, fd);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    }
    long n = sys_read((int)fd, text, MODULES_PREFIX_BUF - 1);
    (void)sys_close((int)fd);
    if (n < 0) {
        struct sbuf sb = {.data = {0}, .len = 0};
        sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=modules_prefix read_rc=");
        sb_put_i64(&sb, n);
        sb_putc(&sb, '\n');
        emit_buf(&sb);
        return;
    ...
    size_t index = 0;
    size_t pos = 0;
    while (pos < (size_t)n && index < prefix_limit) {
    ...
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=modules_prefix_done prefix_label=");
    sb_puts(&sb, M26_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M26_PREFIX_LIMIT);
...
__attribute__((noreturn)) static void reboot_download_then_park(void) {
    emit("S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=checkpoint prefix_label="
         M26_PREFIX_LABEL " prefix_limit=" STR(M26_PREFIX_LIMIT) " action=reboot_download\n");
    ...
    long rc = sys_reboot(
        ...
        k_download_arg);
    struct sbuf sb = {.data = {0}, .len = 0};
    sb_puts(&sb, "S22_NATIVE_INIT_M26_HS_PREFIX_DOWNLOAD phase=reboot_returned prefix_label=");
    sb_puts(&sb, M26_PREFIX_LABEL);
    sb_puts(&sb, " prefix_limit=");
    sb_put_u64(&sb, M26_PREFIX_LIMIT);
    sb_puts(&sb, " rc=");
