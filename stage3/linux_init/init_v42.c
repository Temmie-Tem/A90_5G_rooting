...
static void kms_draw_status_overlay(struct kms_display_state *state,
    ...
    uint32_t hud_raise = glyph_h + (glyph_h / 2);
    ...
    if (y > hud_raise + scale * 2) {
        y -= hud_raise;
    }

    read_status_snapshot(&snapshot);

    snprintf(line1, sizeof(line1), "A90 INIT UP %.12sS", snapshot.uptime);
    ...
    if (refresh_sec > 0) {
        if (sequence > 0) {
            snprintf(line6, sizeof(line6), "REF %uS SEQ %u", refresh_sec, sequence);
        } else {
            snprintf(line6, sizeof(line6), "REF %uS", refresh_sec);
        }
    } else {
        snprintf(line6, sizeof(line6), "MANUAL REF SEQ %u", sequence);
    }
...
static void auto_hud_loop(unsigned int refresh_sec) {
    ...
    while (1) {
        if (kms_begin_frame(0x000000) == 0) {
            kms_draw_status_overlay(&kms_state, refresh_sec, sequence);
...
static void boot_auto_frame(void) {
    if (kms_begin_frame(0x000000) == 0) {
        ...
        if (kms_present_frame("bootframe") == 0) {
            klogf("<6>A90v42: automatic giant TEST probe applied\n");
            native_logf("boot", "display test probe applied");
        }
    } else {
        klogf("<6>A90v42: automatic giant TEST probe skipped (%d)\n", errno);
...
static const struct shell_command command_table[] = {
    ...
    { "menu", handle_blindmenu, "menu", CMD_BLOCKING | CMD_DANGEROUS },
...
int main(void) {
    ...
    if (wait_for_tty_gs0() == 0) {
        mark_step("3_tty_ready_v42\n");
        native_logf("boot", "ttyGS0 ready");
        klogf("<6>A90v42: ttyGS0 ready\n");
        boot_auto_frame();
        ...
        native_logf("boot", "ttyGS0 missing errno=%d error=%s",
                    saved_errno, strerror(saved_errno));
        klogf("<6>A90v42: ttyGS0 missing (%d)\n", saved_errno);
        while (1) {
            sleep(60);
        }
    }
    ...
    if (attach_console() == 0) {
        mark_step("4_console_attached_v42\n");
        ...
        native_logf("boot", "console attach failed errno=%d error=%s",
                    saved_errno, strerror(saved_errno));
        klogf("<6>A90v42: console attach failed (%d)\n", saved_errno);
    }
