/* Included by stage3/linux_init/init_v132.c. Do not compile standalone. */
...
static bool input_monitor_app_feed_event(const struct input_event *event,
                                         int source_index);
static bool auto_menu_handle_volume_step(unsigned int code,
                                         bool *menu_active,
...
static bool auto_menu_handle_volume_step(unsigned int code,
    ...
    if (active_app != SCREEN_APP_NONE) {
        if (a90_menu_app_is_about(active_app)) {
            size_t page_count = a90_app_about_page_count(active_app,
                                                         about_changelog_index);

            if (page_count > 1) {
                if (delta < 0) {
                    *about_page_index = (*about_page_index + page_count - 1) %
                                        page_count;
                } else {
                    *about_page_index = (*about_page_index + 1) % page_count;
                }
                return true;
            }
        }
    ...
    if (!*menu_active) {
        *menu_active = true;
        ...
        a90_menu_state_move(menu_state, delta);
    }
    return true;
}
...
static void auto_hud_loop(unsigned int refresh_sec) {
    ...
    size_t about_changelog_index = 0;
    size_t about_page_index = 0;
    ...
    unsigned int menu_down_mask = 0;
    bool menu_combo_back_sent = false;
    unsigned int menu_hold_code = 0;
    long menu_hold_next_ms = 0;
    bool has_input;
    int timeout_ms;
    ...
    while (1) {
        ...
        if (active_app == SCREEN_APP_LOG) {
            ...
            draw_screen_about_app(active_app, about_changelog_index, about_page_index);
        ...
        {
            int poll_timeout = active_app == SCREEN_APP_NONE ? timeout_ms : 500;

            if (menu_hold_code != 0) {
                long now_ms = monotonic_millis();
                long wait_ms = menu_hold_next_ms - now_ms;
        ...
        for (fi = 0; fi < 2; fi++) {
            ...
            while (read(fds[fi].fd, &ev, sizeof(ev)) == (ssize_t)sizeof(ev)) {
                ...
                if (ev.type == EV_KEY && ev.value != 2) {
                    unsigned int mask = a90_input_button_mask_from_key(ev.code);
                    ...
                    if (mask != 0) {
                        if (ev.value == 1) {
                            ...
                            if ((mask == A90_INPUT_BUTTON_VOLUP ||
                                ...
                                menu_hold_code == 0) {
                                menu_hold_code = ev.code;
                                menu_hold_next_ms = monotonic_millis() +
                                                    AUTO_MENU_HOLD_REPEAT_START_MS;
                            }
                        } else if (ev.value == 0) {
                            menu_down_mask &= ~mask;
                            if (ev.code == menu_hold_code) {
                                menu_hold_code = 0;
                                menu_hold_next_ms = 0;
                            }
                            if ((menu_down_mask & (A90_INPUT_BUTTON_VOLUP |
                        ...
                        if (ev.value == 1 &&
                            !menu_combo_back_sent &&
                            (menu_down_mask & (A90_INPUT_BUTTON_VOLUP |
                                               A90_INPUT_BUTTON_VOLDOWN)) ==
                            (A90_INPUT_BUTTON_VOLUP | A90_INPUT_BUTTON_VOLDOWN)) {
                            menu_combo_back_sent = true;
                            menu_hold_code = 0;
                            menu_hold_next_ms = 0;
                            ...
                            if (!a90_menu_state_back(&menu_state)) {
                                menu_active = false;
                ...
                if (active_app != SCREEN_APP_NONE) {
                    if (auto_menu_handle_volume_step(ev.code,
                                                     &menu_active,
                                                     active_app,
                                                     &menu_state,
                                                     about_changelog_index,
                ...
                if (ev.code == KEY_VOLUMEUP || ev.code == KEY_VOLUMEDOWN) {
                    auto_menu_handle_volume_step(ev.code,
                                                 &menu_active,
                                                 active_app,
                                                 &menu_state,
                                                 about_changelog_index,
                                                 &about_page_index);
                } else if (ev.code == KEY_POWER && ev.value == 1 && menu_active) {
                    ...
                    if (item->action == SCREEN_MENU_CHANGELOG_ENTRY) {
                        about_changelog_index = a90_menu_state_selected_index(&menu_state);
                        about_page_index = 0;
                        active_app = SCREEN_APP_CHANGELOG_DETAIL;
                        menu_active = false;
                        continue;
                    }

                    if (a90_menu_action_opens_app(item->action, &active_app)) {
                        about_changelog_index = 0;
                        about_page_index = 0;
                        menu_active = false;
                        continue;
...
static void boot_auto_frame(void) {
    if (a90_kms_begin_frame(0x000000) == 0) {
        ...
        if (a90_kms_present("bootframe", true) == 0) {
            klogf("<6>A90v132: boot splash applied\n");
        ...
        klogf("<6>A90v132: boot splash skipped (%d)\n", saved_errno);
        if (!boot_splash_recorded) {
            boot_splash_recorded = true;
