...
static bool transfer_path_has_prefix(const char *path, const char *prefix) {
    size_t prefix_len;
...
static bool transfer_path_has_dotdot(const char *path) {
...
static bool transfer_path_allowed(const char *path) {
    if (path == NULL || path[0] != '/' || transfer_path_has_dotdot(path)) {
        return false;
    }
    return transfer_path_has_prefix(path, SD_WORKSPACE_DIR) ||
           transfer_path_has_prefix(path, A90_RUNTIME_CACHE_ROOT) ||
           transfer_path_has_prefix(path, NATIVE_LOG_FALLBACK_DIR);
}

static int write_argv_values(int fd, const char *path, char **argv, int argc, int start_index) {
    int index;

    for (index = start_index; index < argc; ++index) {
        if (index > start_index) {
            if (write_all_checked(fd, " ", 1) < 0) {
                a90_console_printf("write: %s: %s\r\n", path, strerror(errno));
                return negative_errno_or(EIO);
            }
        }
...
static int cmd_writefile(char **argv, int argc) {
    ...
    rc = write_argv_values(fd, path, argv, argc, 2);
    if (rc < 0) {
        close(fd);
        return rc;
    }
...
static int cmd_appendfile(char **argv, int argc) {
    const char *path;
    int fd;
    int rc;

    if (argc < 3) {
        a90_console_printf("usage: appendfile <path> <value...>\r\n");
        return -EINVAL;
    }

    path = argv[1];
    if (!transfer_path_allowed(path)) {
        a90_console_printf("appendfile: %s: path outside allowed runtime workspaces\r\n", path);
        return -EPERM;
    }

    fd = open(path, O_WRONLY | O_CREAT | O_APPEND | O_CLOEXEC | O_NOFOLLOW, 0600);
    if (fd < 0) {
        a90_console_printf("appendfile: %s: %s\r\n", path, strerror(errno));
        return negative_errno_or(ENOENT);
    }
    ...
    if (close(fd) < 0) {
        a90_console_printf("appendfile: %s: close: %s\r\n", path, strerror(errno));
        return negative_errno_or(EIO);
    }
    a90_console_printf("appendfile: ok\r\n");
    return 0;
}
