...
- local artifact retention: keep `v127` latest, `v126` rollback, and `v48` known-good; older ignored `stage3/boot_linux_v*.img`, `stage3/ramdisk_v*`, and compiled `init_v*` outputs are cleanup candidates
...
- menu gate: v127부터 메뉴 표시 중 deny-by-default allowlist로 관찰/메뉴 제어만 허용하고 `run`/`writefile`/`mountfs`/`mknod*` 차단
