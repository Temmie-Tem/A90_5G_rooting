...
- recovery: TWRP 사용 가능
- current device build (flashed): `A90 Linux init 0.9.68 (v724)`
- official version: `0.9.68`
- build tag: `v724`
- creator: `made by temmie0214`
- current source tree: modular native init under `stage3/linux_init/`, rooted at `init_v724.c`
- current boot image: `stage3/boot_linux_v724.img`
- known-good fallback: `stage3/boot_linux_v48.img`
- active research: native Wi-Fi bring-up — cycle V1250 (SDX50M eSoC power-up gate); see `CLAUDE.md` and `docs/plans/NATIVE_INIT_NEXT_WORK_2026-04-25.md`
...
현재 활성 작업은 native Wi-Fi bring-up이다. 상세 사이클 계획과 안전 경계는
`CLAUDE.md`와 `docs/plans/NATIVE_INIT_NEXT_WORK_2026-04-25.md`를 기준으로 한다.

- SDX50M eSoC power-up gate — `pm-service`가 `/dev/subsys_esoc0`를 열어
  `mdm_subsys_powerup`까지 진입하지만 MDM3가 `OFFLINING`에 머무는 원인(PMIC
  전원 레일 / GPIO142 / PCIe RC1) 규명 (V1250 → V1251 debugfs read-surface)
- WLFW service 69 / BDF / FW-ready / `wlan0` 도달 후 link/DHCP gate (별도 승인 필요)
...
전체 문서 목록과 읽는 순서, 사이클별 리포트(v40–V1250+)는 `docs/README.md`를
...
- `docs/operations/VERSIONING_POLICY.md` / `docs/overview/VERSIONING.md` — 버전 규칙
