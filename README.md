...
- current source tree: modular native init under `workspace/public/src/native-init/`, rooted at `init_v724.c`, with current scripts under `workspace/public/src/scripts/revalidation/`
- current boot image: `workspace/private/inputs/boot_images/boot_linux_v726_wifi_lifecycle.img`
- previous verified boot image: `workspace/private/inputs/boot_images/boot_linux_v725_fasttransport.img`
...
- current TODO map: `docs/plans/NATIVE_INIT_CURRENT_TODO_2026-06-08.md`
- working rules: `docs/operations/WORKING_RULES.md` is the top-level rulebook for version axes, workspace paths, and commit boundaries
- boot/transport contract: `docs/operations/NATIVE_INIT_BOOT_TRANSPORT_CONTRACT.md`
- 버전별 검증 이력은 `docs/overview/PROJECT_STATUS.md` 말미의 "README 이관" 섹션을 참고
- control channel: USB CDC ACM serial (`/dev/ttyGS0` ↔ `/dev/ttyACM0`)
- host bridge: `workspace/public/src/scripts/revalidation/a90_bridge.py ensure --device /dev/ttyACM0`
- host bridge hygiene: `workspace/public/src/scripts/revalidation/a90_bridge.py doctor` and `repair-dirs` keep private logs/run state writable after sudo use
...
- verified baseline: `A90 Linux init 0.9.246 (v726-wifi-lifecycle)`,
  `workspace/private/inputs/boot_images/boot_linux_v726_wifi_lifecycle.img`
- versioning cleanup: 다음 승격은 helper 번호와 혼동하지 않도록
  `V2169` run ID / `0.9.247` native init / `v2169-wifi-lifecycle-baseline`
  build tag 형태로 잡는다.
- artifact cleanup: 새 빌드 산출물은 `workspace/private/builds/`와
  `workspace/private/inputs/boot_images/`에 두고, 공개 가능한 redacted summary만
  `docs/artifacts`에 둔다.
- workspace bootstrap: 새 클론은 `docs/operations/WORKSPACE_STRUCTURE_AND_BOOTSTRAP.md`
  기준으로 ignored local inputs, boot image seeds, structured `tmp` layout을 복구한다.
