#!/usr/bin/env python3
"""V1532 targeted Android tracefs queue/execute pairing handoff for initial RC1 trigger.
...
ORIGINAL_V1521_BUILD_PLAN = v1521.build_plan

DEFAULT_OUT_DIR = Path("tmp/wifi/v1532-android-targeted-tracefs-queue-pair-handoff")
...
LATEST_POINTER = Path("tmp/wifi/latest-v1532-android-targeted-tracefs-queue-pair-handoff.txt")

MODULE_NAME = "a90_v1532_tracefs_queue_pair_sampler"
REMOTE_MODULE_DIR = f"/data/adb/modules/{MODULE_NAME}"
REMOTE_EVIDENCE_DIR = "/data/local/tmp/a90-v1532-tracefs-queue-pair-sampler"
...
def configure_v1521_engine() -> None:
    ...
    v1521.build_plan = build_plan_without_sensitive_native_status
...
def module_prop() -> str:
    return "\n".join(
        [
            f"id={MODULE_NAME}",
            "name=A90 V1532 Targeted Tracefs Queue Pair Sampler",
            "version=1",
            "versionCode=1",
...
AVAILABLE="$OUT/tracefs-available-events.txt"
TARGETED="$OUT/tracefs-targeted-excerpt.txt"
write_status() {{
  now="$(cat /proc/uptime 2>/dev/null | awk '{{print $1}}')"
  echo "A90_V1532_STATUS $1 $now" > "$STATUS"
...
EVENT_LIST="sched/sched_process_exec workqueue/workqueue_queue_work workqueue/workqueue_activate_work workqueue/workqueue_execute_start workqueue/workqueue_execute_end msm_pil_event/pil_event msm_pil_event/pil_notif msm_pil_event/pil_func printk/console"
enable_event() {{
  e="$1"
...
write_status start
echo "A90_V1532_TRACEFS_SETUP_BEGIN" > "$EVENTS"
...
cat "$T/available_events" 2>/dev/null | grep -Ei '^(sched:sched_process_exec|workqueue:workqueue_|msm_pil_event:pil_|printk:console)' > "$AVAILABLE.tmp" || true
mv "$AVAILABLE.tmp" "$AVAILABLE" 2>/dev/null || true
dump_formats
...
(
  ...
  echo A90_V1532_TRACEFS_SAMPLER_END
  echo A90_V1521_POSTFS_SAMPLER_END
) > "$OUT/tracefs-status.log" 2>&1
sleep 1
cat "$T/trace" > "$TRACELOG" 2>/dev/null || true
grep -Ei 'icnss_driver_event_work|pm-service|Binder:|__subsystem_get|workqueue_(queue_work|activate_work|execute_start|execute_end)|pil_notif|pil_event|pil_func|console:' "$TRACELOG" > "$TARGETED.tmp" 2>/dev/null || true
mv "$TARGETED.tmp" "$TARGETED" 2>/dev/null || true
...
def build_plan_without_sensitive_native_status(
    args: argparse.Namespace,
    store: EvidenceStore,
    android_image: Any,
    native_image: Any,
) -> list[tuple[str, list[str] | str, int]]:
    plan = ORIGINAL_V1521_BUILD_PLAN(args, store, android_image, native_image)
    filtered: list[tuple[str, list[str] | str, int]] = []
    for name, command, timeout in plan:
        if name == "native-version":
            filtered.append(("native-selftest", ["python3", "scripts/revalidation/a90ctl.py", "selftest"], timeout))
            continue
        if name == "native-status":
            continue
        if name == "restore-native" and isinstance(command, list):
            command = list(command)
            for idx, value in enumerate(command[:-1]):
                if value == "--verify-protocol":
                    command[idx + 1] = "selftest"
            filtered.append((name, command, timeout))
            continue
        filtered.append((name, command, timeout))
    return filtered
...
def evidence_base(store: EvidenceStore) -> Path:
    ...
    candidate = root / "a90-v1532-tracefs-queue-pair-sampler"
...
def parse_workqueue_records(text: str) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
    event_re = re.compile(r"\s([0-9]+\.[0-9]+):\s+(workqueue_[a-z_]+):\s+(.*)$")
    work_re = re.compile(r"work(?:\s+struct)?(?:=|:)?\s*(0x[0-9a-f]+|[0-9a-f]{8,16})", re.I)
    func_re = re.compile(r"function(?:=|:|\s+)([A-Za-z0-9_.$]+)", re.I)
    for line in text.splitlines():
        match = event_re.search(line)
        if not match:
            continue
        payload = match.group(3)
        work_match = work_re.search(payload)
        func_match = func_re.search(payload)
        records.append(
            {
                "time": float(match.group(1)),
                "event": match.group(2),
                "work": work_match.group(1).lower().removeprefix("0x") if work_match else None,
...
def classify_icnss_workqueue_pairing(records: list[dict[str, Any]]) -> dict[str, Any]:
    icnss_execs = [
        record
        for record in records
        if record.get("event") == "workqueue_execute_start"
        and record.get("function") == "icnss_driver_event_work"
    ]
    paired: list[dict[str, Any]] = []
    for record in icnss_execs:
        ...
        prior = [
            ...
            if candidate.get("work") == work
            and candidate.get("time") is not None
            and candidate["time"] <= record["time"]
            and candidate.get("event") in {"workqueue_queue_work", "workqueue_activate_work"}
        ]
        paired.append(
            {
                "work": work,
                "execute_time": record["time"],
                "execute_line": record["line"],
    ...
    return {
        ...
        "queue_count": sum(1 for record in records if record.get("event") == "workqueue_queue_work"),
        "activate_count": sum(1 for record in records if record.get("event") == "workqueue_activate_work"),
        "execute_start_count": sum(1 for record in records if record.get("event") == "workqueue_execute_start"),
        "icnss_execute_count": len(icnss_execs),
        "icnss_paired_count": sum(1 for item in paired if item["prior_count"] > 0),
        "icnss_pairs": paired[:8],
    }
...
def analyze_pulled_evidence(store: EvidenceStore) -> dict[str, Any]:
    ...
    targeted_text = read_file(base / "tracefs-targeted-excerpt.txt")
    ...
    available_events = read_file(base / "tracefs-available-events.txt")
    ...
    workqueue_records = parse_workqueue_records(trace_all)
    workqueue_pairing = classify_icnss_workqueue_pairing(workqueue_records)
    ...
    trace_counts = {
        ...
        "workqueue_queue": count_lines(trace_all, r"workqueue_queue_work"),
        "workqueue_activate": count_lines(trace_all, r"workqueue_activate_work"),
    ...
    if workqueue_pairing["icnss_paired_count"]:
        decision_hint = "targeted-workqueue-pairing-captured"
    elif trace_counts["total_lines"] and (trace_counts["workqueue_queue"] or trace_counts["workqueue_activate"]):
        decision_hint = "workqueue-queue-events-captured-unpaired"
    elif trace_counts["total_lines"] and (trace_counts["pil_notif"] or trace_counts["workqueue_start"] or trace_counts["irq_entry"]):
    ...
    return {
        ...
        "files_present": {
            ...
            "targeted_excerpt": bool(targeted_text),
            "trace_pipe": bool(trace_pipe_text),
            "setup": bool(setup_log),
            "formats": bool(formats),
            "available_events": bool(available_events),
        },
        "status_text": status_text.strip(),
        "setup_excerpt": setup_log.strip().splitlines()[:40],
        "sample_count": count_lines(status_log, r"A90_V1532_SAMPLE"),
        ...
        "tracefs_analysis": {
            ...
            "first_times": {
                ...
                "workqueue_queue": first_trace_time(trace_all, r"workqueue_queue_work"),
                "workqueue_activate": first_trace_time(trace_all, r"workqueue_activate_work"),
            ...
            "available_workqueue_events": matching_lines(available_events, r"workqueue", 40),
            ...
            "workqueue_excerpt": matching_lines(trace_all, r"workqueue_queue_work|workqueue_activate_work|workqueue_execute_start|workqueue_execute_end|icnss_driver_event_work", 40),
            "targeted_excerpt": targeted_text.strip().splitlines()[:80],
            "workqueue_pairing": workqueue_pairing,
        },
...
def first_status_uptime(text: str) -> float | None:
    match = re.search(r"A90_V1532_SAMPLE index=\d+ uptime=([0-9.]+)", text)
    return float(match.group(1)) if match else None


def last_status_uptime(text: str) -> float | None:
    matches = list(re.finditer(r"A90_V1532_SAMPLE index=\d+ uptime=([0-9.]+)", text))
    return float(matches[-1].group(1)) if matches else None


DECISION_MAP = {
    "v1521-handoff-plan-ready": "v1532-handoff-plan-ready",
    "v1521-handoff-dryrun-ready": "v1532-handoff-dryrun-ready",
    "v1521-magisk-postfs-pre-lower-window-rollback-pass": "v1532-targeted-tracefs-rollback-pass",
    "v1521-magisk-postfs-partial-pre-lower-window-rollback-pass": "v1532-targeted-tracefs-partial-rollback-pass",
    "v1521-magisk-postfs-android-lower-no-pre-window-rollback-pass": "v1532-targeted-tracefs-android-lower-no-pre-window-rollback-pass",
    "v1521-magisk-postfs-partial-android-lower-no-pre-window-rollback-pass": "v1532-targeted-tracefs-partial-android-lower-no-pre-window-rollback-pass",
    "v1521-magisk-postfs-evidence-captured-rollback-review": "v1532-targeted-tracefs-evidence-rollback-review",
    "v1521-magisk-postfs-partial-evidence-captured-rollback-review": "v1532-targeted-tracefs-partial-evidence-rollback-review",
}


def map_decision(decision: str) -> str:
    return DECISION_MAP.get(decision, decision.replace("v1521", "v1532"))


def reason_for(decision: str, base_decision: str) -> str:
    reasons = {
        "v1532-handoff-plan-ready": "plan-only handoff; no device command executed",
        "v1532-handoff-dryrun-ready": "dry-run handoff completed without device mutation",
        "v1532-targeted-tracefs-rollback-pass": "targeted Android tracefs queue/execute evidence was pulled and native rollback completed",
        "v1532-targeted-tracefs-partial-rollback-pass": "partial targeted Android tracefs queue/execute evidence was pulled and native rollback completed",
        "v1532-targeted-tracefs-evidence-rollback-review": "targeted Android tracefs evidence captured and native rollback completed; review tracefs pairing analysis",
        "v1532-targeted-tracefs-partial-evidence-rollback-review": "partial targeted Android tracefs evidence captured and native rollback completed; review tracefs pairing analysis",
    }
        ...
        [
            ...
            markdown_table(
                ...
                [
                    ...
                    ["workqueue_pairing", json.dumps(pairing, sort_keys=True)],
            ...
            markdown_table(
                ...
                [
                    ...
                    ["available_workqueue_events", json.dumps(tracefs.get("available_workqueue_events"), sort_keys=True)],
                    ...
                    ["targeted_excerpt", json.dumps(tracefs.get("targeted_excerpt"), sort_keys=True)],
...
def main() -> int:
    ...
    if leaks:
        manifest["decision"] = "v1532-forbidden-output-hit"
        manifest["pass"] = False
