#!/usr/bin/env python3
"""Run v236 Android linker bounded crash-context capture.
...
DEFAULT_OUT_DIR = Path("tmp/wifi/v236-linker-crash-capture")
...
def helper_command(args: argparse.Namespace, linker_profile: str, profile: str, env_mode: str) -> list[str]:
    linker_path = LINKER_PROFILES[linker_profile]
    command = [
        "run",
        args.helper,
        "--system-root",
...
def classify_single(linker_profile: str, profile: str, env_mode: str, parsed: dict[str, Any]) -> tuple[bool, str, str]:
    ...
    if has_debug_text:
        return True, "android-linker-debug-output-ready", f"linker={linker_profile} profile={profile} env={env_mode} produced linker output"
    if child_signal != 0:
        return True, "android-linker-profile-crashed", f"linker={linker_profile} profile={profile} env={env_mode} signal={child_signal}"
    if child_exit == 0:
        return True, "android-linker-list-baseline-pass", f"linker={linker_profile} profile={profile} env={env_mode} exited 0"
    return True, "android-linker-profile-nonzero", f"linker={linker_profile} profile={profile} env={env_mode} exit={child_exit}"


def run_matrix(store: EvidenceStore, args: argparse.Namespace) -> list[MatrixResult]:
    results: list[MatrixResult] = []
    for env_mode in args.env_modes:
        for linker_profile in args.linker_profiles:
            for profile in args.target_profiles:
                command = helper_command(args, linker_profile, profile, env_mode)
                name = f"helper-{env_mode}-{linker_profile}-{profile}"
                ...
                ok, decision, reason = classify_single(linker_profile, profile, env_mode, parsed)
                fields = parsed.get("fields", {})
                stdout_text = str(parsed.get("stdout", ""))
                stderr_text = str(parsed.get("stderr", ""))
                results.append(
...
def decide(results: list[MatrixResult], null_device_mode: str) -> tuple[str, str, bool]:
    ...
    if all(item.child_signal == 11 for item in results) and all(item.crash_captured for item in results):
        return "android-linker-crash-context-captured", "all selected linker runs crashed with SIGSEGV and ptrace-lite crash context was captured", True
    if any(item.child_signal == 11 for item in results):
        return "android-linker-crash-capture-partial", "SIGSEGV reproduced but at least one ptrace-lite capture marker is missing", False
    if any(item.child_signal == 0 and item.child_exit_code == 0 for item in results):
        return "android-linker-list-baseline-pass", "at least one selected linker run exited 0", True
    return "android-linker-crash-capture-manual-review-required", "matrix completed with mixed non-pass results", True


def build_summary(manifest: dict[str, Any]) -> str:
    lines = [
        "# v236 Linker Crash Capture Probe",
        "",
    ...
    lines.extend(
        [
            ...
            "- compare only `/system/bin/linker64` and `/apex/com.android.runtime/bin/linker64` allowlisted linker paths",
            "",
        ]
    )
    return "\n".join(lines)
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.subcommand == "plan":
        ...
        decision, reason, pass_ok = "android-linker-crash-capture-plan-ready", "plan generated; no helper matrix executed", True
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--linker-profiles",
        default="system-linker,apex-linker",
        help="comma-separated allowlisted linker invocation paths",
    )
    parser.add_argument(
        "--target-profiles",
    ...
    args.linker_profiles = validate_profiles(split_csv(args.linker_profiles), tuple(LINKER_PROFILES), "--linker-profiles")
