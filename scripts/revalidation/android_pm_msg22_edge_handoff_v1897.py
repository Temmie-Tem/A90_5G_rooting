#!/usr/bin/env python3
"""V1897 autonomous Android-good pm-service msg22 edge capture.
...
filtered logcat/dmesg/TFTP evidence and, only when logcat has no visible
pending-client/msg22 edge, arms bounded tracefs uprobes on pm-service's QMI
message dispatch range.  It does not start Wi-Fi HAL, scan/connect, use
credentials, run DHCP/routes, ping, touch eSoC/PCIe/GDSC/PMIC/GPIO/regulator
state, fake ONLINE, send BOOT_DONE, rescan PCI, or bind/unbind platforms.
"""
...
DEFAULT_V1894_REPORT = Path(
    "docs/reports/NATIVE_INIT_V1897_V1894_ANDROID_PENDING_CLIENT_MSG22_PARSER_2026-06-03.md"
)
DEFAULT_V1888_REPORT = Path(
    "docs/reports/NATIVE_INIT_V1897_V1888_PM_MSGID_CAPTURE_DIFF_CLASSIFIER_2026-06-03.md"
)
LATEST_POINTER = Path("tmp/wifi/latest-v1897-android-normal-pm-msg22-edge-handoff.txt")

MODULE_NAME = "a90_v1897_pm_edge"
REMOTE_MODULE_DIR = f"/data/adb/modules/{MODULE_NAME}"
REMOTE_EVIDENCE_DIR = "/data/local/tmp/a90-v1897-pm-edge"
REMOTE_STAGE_PREFIX = "/data/local/tmp/a90_v1897_pm_edge"
STRACE_SOURCE = Path("external_tools/userland/bin/strace-aarch64-static")
TRACEFS_GROUP = "a90pm1897"

REDACT_CMD = "sed -E 's/temmie[[:alnum:]_@.-]*/[REDACTED]/g'"
...
def module_prop() -> str:
    return "\n".join(
        [
            f"id={MODULE_NAME}",
            "name=A90 V1897 pm-service QMI edge observer",
            "version=1",
            "versionCode=1",
            "author=A90 native-init project",
            "description=Temporary Android-good internal-modem pm-service QMI-edge observer. Remove after capture.",
            "",
        ]
    )
...
def sepolicy_rule() -> str:
    return """# Temporary V1897 diagnostic policy for read-only observation.
...
def post_fs_data_script(samples: int, delay_us: int) -> str:
    filter_expr = (
        ...
        "service-notifier|wlanmdsp|wlan[_/-]?pd|wlan/fw|wlfw_service_request|"
...
write_status() {{
  ...
  echo "A90_V1897_STATUS $1 $now" > "$STATUS"
...
dump_firmware_snapshot() {{
  {{
    echo "A90_V1897_FIRMWARE_SNAPSHOT_BEGIN"
    ...
    echo "A90_V1897_FIRMWARE_SNAPSHOT_END"
  }} > "$FW.tmp"
...
snapshot_proc() {{
  ...
  {{
    echo "A90_V1897_PROC label=$label pid=$pid uptime=$uptime"
...
register_uprobe_event() {{
  ...
  echo "-:$GROUP/$label" >> "$TRACE_ROOT/uprobe_events" 2>/dev/null || true
  if [ -n "$fetch" ]; then
    line="p:$GROUP/$label $PM_SERVICE:0x$offset $fetch"
  else
    line="p:$GROUP/$label $PM_SERVICE:0x$offset"
  fi
  if echo "$line" >> "$TRACE_ROOT/uprobe_events" 2>/dev/null; then
    echo "event.$label.register=ok line=$line" >> "$UPROBE_SUMMARY"
  else
    echo "event.$label.register=failed line=$line" >> "$UPROBE_SUMMARY"
    return 1
  fi
  if echo 1 > "$TRACE_ROOT/events/$GROUP/$label/enable" 2>/dev/null; then
    echo "event.$label.enable=ok" >> "$UPROBE_SUMMARY"
  else
    echo "event.$label.enable=failed" >> "$UPROBE_SUMMARY"
    return 1
  fi
  return 0
}}
...
arm_uprobe_if_needed() {{
  ...
  if cat "$LOGCAT" "$DMESG" "$OUT/request-lines.txt" 2>/dev/null | grep -Eiv 'a90_v1897_pm_edge|Magisk|SRC pm_edge_observer' | grep -Eiq 'QMI service peripheral restart|peripheral restart request|msg[_ =-]?(id)?[_ =-]?(0x22|22)|pm_msg22'; then
...
dump_uprobe_trace() {{
  if [ "$UPROBE_ARMED" = "1" ] && [ -r "$TRACE_ROOT/trace" ]; then
    grep -E "$GROUP|pm_msg22" "$TRACE_ROOT/trace" > "$UPROBE.tmp" 2>/dev/null || true
    mv "$UPROBE.tmp" "$UPROBE" 2>/dev/null || true
    echo "hit_count=$(grep -Ec 'pm_msg22' "$UPROBE" 2>/dev/null || echo 0)" >> "$UPROBE_SUMMARY"
    echo "msg22_hit_count=$(grep -Ec 'msg_id=0x22|msg_id=34|pm_msg22' "$UPROBE" 2>/dev/null || echo 0)" >> "$UPROBE_SUMMARY"
  else
    : > "$UPROBE"
  fi
}}
...
summarize_requests() {{
  cat "$OUT"/*.strace.txt "$DMESG" "$LOGCAT" "$UPROBE" 2>/dev/null | grep -Eiv 'a90_v1897_pm_(edge|msg22)|SRC pm_edge_observer|trace_uprobe: Event .*pm_msg22.*doesn.?t exist|event\\.pm_msg22|result=.*msg22|armed=|hit_count=|msg22_hit_count=' | grep -Ei "$FILTER|pm_msg22" | tail -n 1500 > "$OUT/request-lines.txt" || true
  {{
    ...
    grep -Eiv 'a90_v1897_pm_(edge|msg22)|SRC pm_edge_observer|trace_uprobe: Event .*pm_msg22.*doesn.?t exist|event\\.pm_msg22|result=.*msg22|armed=|hit_count=|msg22_hit_count=' "$OUT/request-lines.txt" 2>/dev/null | grep -Eiq 'QMI service peripheral restart|msg[_ =-]?(id)?[_ =-]?(0x22|22)|pm_msg22' && echo 1 || echo 0
    ...
    grep -Ec '.' "$UPROBE" 2>/dev/null || echo 0
    printf 'wlan0_seen='
...
(
  ...
  : > "$UPROBE"
  : > "$UPROBE_SUMMARY"
  echo "A90_V1897_POSTFS_BEGIN" >> "$LOG"
  id >> "$LOG" 2>&1 || true
  ...
  while [ "$i" -lt "$SAMPLES" ]; do
    uptime="$(cat /proc/uptime 2>/dev/null | awk '{{print $1}}')"
    echo "A90_V1897_SAMPLE_BEGIN index=$i uptime=$uptime" >> "$LOG"
    ...
    echo "SRC pm_edge_observer" >> "$LOG"
    cat "$REQ" >> "$LOG" 2>/dev/null || true
    echo "A90_V1521_SAMPLE_END index=$i uptime=$uptime" >> "$LOG"
    echo "A90_V1897_SAMPLE_END index=$i uptime=$uptime" >> "$LOG"
    i=$((i + 1))
...
def prepare_module(store: EvidenceStore, args: argparse.Namespace, execute: bool) -> v1521.StepResult:
    ...
    if not execute:
        return v1521.write_step(
            store,
            "prepare-v1897-magisk-module",
            "host:prepare temporary pm-msg22 Magisk module",
...
def install_module_android_steps(args: argparse.Namespace, store: EvidenceStore) -> list[tuple[str, list[str], int]]:
    ...
    return [
        ("push-v1897-module-prop-android", [*v1521.adb_base(args), "push", str(stage / "module.prop"), remote_prop], args.timeout),
        ("push-v1897-post-fs-data-android", [*v1521.adb_base(args), "push", str(stage / "post-fs-data.sh"), remote_postfs], args.timeout),
        ("push-v1897-sepolicy-android", [*v1521.adb_base(args), "push", str(stage / "sepolicy.rule"), remote_policy], args.timeout),
        ("push-v1897-strace-android", [*v1521.adb_base(args), "push", str(stage / "a90_strace"), remote_strace], args.timeout * 2),
        ("install-v1897-module-android-su", [*v1521.adb_base(args), "shell", "su", "-c", shlex.quote(install_shell)], args.timeout),
    ]
...
def build_plan_v1897(args: argparse.Namespace,
                     store: EvidenceStore,
                     android_image: Any,
...
def configure_v1521_engine() -> None:
    ...
    v1521.build_plan = build_plan_v1897
...
def analyze_pulled_evidence(store: EvidenceStore) -> dict[str, Any]:
    ...
    combined = "\n".join([tftp_trace, rmt_trace, cnss_trace, dmesg, logcat, uprobe_trace, request_lines])
    ...
    return {
        ...
        "sample_count": samples.count("A90_V1897_SAMPLE_BEGIN"),
...
def classify_result(base_decision: str,
    ...
    if not selftest_ok:
        return (
            "v1897-rollback-selftest-failed",
            False,
    ...
    if not base_pass:
        return (
            f"v1897-base-handoff-failed-{base_decision}",
            False,
    ...
    if not files.get("request_summary"):
        return (
            "v1897-capture-insufficient-rollback-pass",
            False,
    ...
    if contaminated:
        return (
            "v1897-android-capture-rejected-degraded-or-pcie-mhi",
            False,
    ...
    if not stateup:
        return (
            "v1897-android-normal-stateup-incomplete-rollback-pass",
            False,
    ...
    if not parser_ok:
        return (
            "v1897-parser-chain-failed-rollback-pass",
            False,
    ...
    if pm_msg22_count > 0 and uprobe_lines > 0:
        return (
            "v1897-android-uprobe-msg22-stateup-native-absent-rollback-pass",
            True,
    ...
    if pm_msg22_count > 0:
        return (
            "v1897-android-logcat-msg22-stateup-native-absent-rollback-pass",
            True,
    ...
    if pending_count > 0:
        return (
            "v1897-android-pending-client-without-msg22-stateup-rollback-pass",
            True,
    ...
    return (
        "v1897-android-stateup-msg22-uprobe-observability-gap-rollback-pass",
        True,
        "normal Android state-up was captured and uprobe fallback ran or was checked, but msg22/pending-client remained unobserved",
        "android-stateup-msg22-uprobe-observability-gap",
    )
...
def main() -> int:
    ...
    if execute:
        ...
        decision = "v1897-android-normal-pm-msg22-edge-plan-ready" if args.command == "plan" else "v1897-android-normal-pm-msg22-edge-dryrun-ready"
        pass_ok = bool(base_pass)
        reason = "plan/dry-run completed without Android-good live capture"
        label = "android-normal-pm-msg22-edge-handoff-ready"
