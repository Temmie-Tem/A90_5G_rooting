#!/usr/bin/env python3
"""V1473 local-only sanity verifier for the V1472 effective-level Wi-Fi test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1473-wifi-test-boot-effective-level-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1473_WIFI_TEST_BOOT_EFFECTIVE_LEVEL_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1472-wifi-test-boot-effective-level-source-build-pass"
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "supervisor_timeout_sec": 60,
        "watch_sec": 45,
        "mount_debugfs": True,
        "pid1_rc1_watcher": True,
        "rc1_watcher_timeout_sec": 65,
        "rc1_watcher_delay_ms": 0,
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1472-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1472-rc1-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1473-wifi-test-boot-effective-level-artifact-sanity-pass",
            True,
            "V1472 effective-level test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1473-wifi-test-boot-effective-level-artifact-sanity-blocked",
        False,
        "V1472 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1472 manifest: `{manifest['_path']}`",
        f"- V1472 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- effective-level contract: `{checks['wifi_test_contract']['ok']}`",
