#!/usr/bin/env python3
"""V1622 local-only sanity verifier for the V1621 pm-service property-root boot."""
...
base.DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1621-pm-service-property-root-test-boot" / "manifest.json"
base.DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1622-pm-service-property-root-artifact-sanity"
base.DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1622_PM_SERVICE_PROPERTY_ROOT_ARTIFACT_SANITY_2026-06-02.md"
)
base.EXPECTED_DECISION = "v1621-pm-service-property-root-test-boot-source-build-pass"
base.EXPECTED_HELPER = "a90_android_execns_probe_v302"
base.EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.110 (v1621-pm-service-property-root)",
    "a90_android_execns_probe v302",
    "A90v1621",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "helper_result": "/cache/native-init-wifi-test-boot-v1621-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if pass_ok:
        return (
            ...
            True,
            "V1621 pm-service property-root test boot artifact passed local sanity",
        )
    return (
        "v1622-pm-service-property-root-artifact-sanity-blocked",
        False,
        f"V1621 artifact sanity failed; base decision was {decision}",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1621 manifest: `{manifest['_path']}`",
        f"- V1621 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- boot property-root markers: `{checks['boot_markers']['ok']}`",
        f"- helper property-root markers: `{checks['helper_markers']['ok']}`",
