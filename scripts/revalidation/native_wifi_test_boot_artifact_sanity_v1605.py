#!/usr/bin/env python3
"""V1605 local-only sanity verifier for the V1604 per_mgr startup-trace test boot."""
...
base.DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1604-per-mgr-startup-trace-test-boot" / "manifest.json"
base.DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1605-per-mgr-startup-trace-artifact-sanity"
base.DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1605_PER_MGR_STARTUP_TRACE_ARTIFACT_SANITY_2026-06-02.md"
)
base.EXPECTED_DECISION = "v1604-per-mgr-startup-trace-test-boot-source-build-pass"
base.EXPECTED_HELPER = "a90_android_execns_probe_v298"
base.EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.106 (v1604-per-mgr-startup-trace)",
    "a90_android_execns_probe v298",
    "A90v1604",
...
def contract_check(wifi_test: dict[str, Any], safety: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "watch_sec": 120,
        "helper_mode": "android-service-window-pm-proxy-contract-pm-first-late-per-proxy-pph-gate-per-mgr-startup-trace-lower-marker",
        "helper_runtime_mode": "wifi-companion-android-wifi-service-window-subsys-trigger-capture",
        "helper_result": "/cache/native-init-wifi-test-boot-v1604-helper.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if pass_ok:
        return (
            "v1605-per-mgr-startup-trace-artifact-sanity-pass",
            True,
            "V1604 per_mgr startup-trace test boot artifact passed local sanity",
        )
    return (
        "v1605-per-mgr-startup-trace-artifact-sanity-blocked",
        False,
        f"V1604 artifact sanity failed; base decision was {decision}",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1604 manifest: `{manifest['_path']}`",
        f"- V1604 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- boot startup-trace markers: `{checks['boot_markers']['ok']}`",
        f"- helper startup-trace markers: `{checks['helper_markers']['ok']}`",
