#!/usr/bin/env python3
"""V1665 rollbackable live handoff for the V1664 pcie1 clock vote proof."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1665-pcie1-clock-vote-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1665_PCIE1_CLOCK_VOTE_HANDOFF_2026-06-02.md"
)
DEFAULT_V1664_MANIFEST = (
    REPO_ROOT
    / "tmp"
    / "wifi"
    / "v1664-pcie1-clock-vote-test-boot"
    / "manifest.json"
)
DEFAULT_TEST_IMAGE = (
    REPO_ROOT
    ...
    / "wifi"
    / "v1664-pcie1-clock-vote-test-boot"
    / "boot_linux_v1664_pcie1_clock_vote.img"
)
TEST_EXPECT_VERSION = "A90 Linux init 0.9.116 (v1664-pcie1-clock-vote)"
DEFAULT_TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v1664.log"
DEFAULT_TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v1664.summary"
DEFAULT_TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1664-helper.result"
DEFAULT_TEST_WATCHER_PATH = "/cache/native-init-wifi-test-boot-v1664-natural-watcher.result"
DEFAULT_TEST_WINDOW_PATH = "/cache/native-init-wifi-test-boot-v1664-clock-vote-window.result"
DEFAULT_DMESG_PATTERN = (
    "A90v1664|pcie1 clock vote|subsystem_get|mdm_subsys_powerup|"
...
def classify_clock_vote(out_dir: Path, handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    pcie1_gdsc_max_use = max_regulator_use(window, "pcie_1_gdsc")
    ...
    return {
        ...
        "clock_vote_excerpt": matching_lines(window, r"pcie1_clock_vote\.|A90_V1664_CLOCK_VOTE_SNAPSHOT", 40),
        "power_excerpt": matching_lines(window, r"pcie_1_gdsc|CLOCK .*gcc_pcie|mdm2ap_timing\.", 36),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1664-manifest", type=Path, default=DEFAULT_V1664_MANIFEST)
...
def main() -> int:
    ...
    if not handoff:
        print(json.dumps({
            "decision": "v1665-pcie1-clock-vote-handoff-manifest-missing",
            "pass": False,
            "base_rc": base_rc,
        }, indent=2))
        return 1
