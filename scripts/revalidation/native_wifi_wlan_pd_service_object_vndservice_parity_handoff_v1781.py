#!/usr/bin/env python3
"""V1781 one-run WLAN-PD service-object VND-service parity handoff."""
...
CYCLE = "V1781"
V1780_OUT = REPO_ROOT / "tmp" / "wifi" / "v1780-service-object-vndservice-parity-test-boot"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v1780/dev/__properties__"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1781-service-object-vndservice-parity-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1781_SERVICE_OBJECT_VNDSERVICE_PARITY_HANDOFF_2026-06-03.md"
)


def configure_base() -> None:
    base.prev.CYCLE = CYCLE
    base.prev.V1735_OUT = V1780_OUT
    ...
    base.prev.base.V1726_OUT = V1780_OUT
    ...
    base.prev.base.prev.V1687_OUT = V1780_OUT
    base.prev.base.prev.DEFAULT_SOURCE_MANIFEST = V1780_OUT / "manifest.json"
    base.prev.base.prev.DEFAULT_TEST_IMAGE = V1780_OUT / "boot_linux_v1780_service_object_vndservice_parity.img"
    base.prev.base.prev.LOCAL_PROPERTY_ROOT = V1780_OUT / "property-runtime" / "layout" / "dev" / "__properties__"
    ...
    base.prev.base.prev.TEST_EXPECT_VERSION = "A90 Linux init 0.9.143 (v1780-service-object-vndservice-parity)"
    base.prev.base.prev.TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v1780.log"
    base.prev.base.prev.TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v1780.summary"
    base.prev.base.prev.TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1780-helper.result"
    base.prev.base.prev.DMESG_PATTERN = (
        "A90v1780|wlan_pd_service_object_visible_trigger|wlan_pd_service_object_visible|"
...
def classify_gate(args: Any,
    ...
    if not version_ok:
        return f"{args.cycle.lower()}-test-boot-version-missing", False, "expected V1780 test boot version was not collected", details
    if not rollback_ok:
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        f"# Native Init {cycle} Service-object VND-service Parity Handoff",
        "",
        "## Summary",
        "",
        f"- Cycle: `{cycle}`",
        ...
        "",
        f"- Policy-load precondition required: `{gate.get('policy_precondition_required')}`",
        f"- Policy-load precondition requested: `{gate.get('policy_precondition_requested')}`",
        f"- Policy-load result: `{gate.get('policy_load_result')}`",
        f"- Policy-load pass: `{gate.get('policy_load_pass')}`",
        ...
        f"- `pm-service` state: `{gate.get('per_mgr_state')}`",
        f"- `pm-service` zombie: `{gate.get('per_mgr_zombie')}`",
