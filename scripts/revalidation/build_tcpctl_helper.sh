...
SRC="${ROOT_DIR}/stage3/linux_init/a90_tcpctl.c"
BIN_DIR="${ROOT_DIR}/external_tools/userland/bin"
OUT_BIN="${BIN_DIR}/a90_tcpctl-aarch64-static"
...
ln -sfn "$(basename "${OUT_BIN}")" "${BIN_DIR}/a90_tcpctl"
...
aarch64-linux-gnu-readelf -d "${OUT_BIN}" >/tmp/a90_tcpctl_dynamic_check.txt 2>&1 || true
cat /tmp/a90_tcpctl_dynamic_check.txt
if ! grep -q "There is no dynamic section" /tmp/a90_tcpctl_dynamic_check.txt; then
