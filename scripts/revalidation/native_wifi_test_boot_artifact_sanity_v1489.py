#!/usr/bin/env python3
"""V1489 local-only sanity verifier for the V1488 timeout-safe auto-readiness Wi-Fi test boot."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1488-wifi-auto-readiness-timeout-safe-test-boot" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1489-wifi-auto-readiness-timeout-safe-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1489_WIFI_AUTO_READINESS_TIMEOUT_SAFE_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1488-wifi-auto-readiness-timeout-safe-test-boot-source-build-pass"
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1489-wifi-auto-readiness-timeout-safe-artifact-sanity-pass",
            True,
            "V1488 timeout-safe auto-readiness test boot artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1489-wifi-auto-readiness-timeout-safe-artifact-sanity-blocked",
        False,
        "V1488 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1488 manifest: `{manifest['_path']}`",
        f"- V1488 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
