#!/usr/bin/env python3
"""V1459 local-only sanity verifier for the V1458 Wi-Fi test boot artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1459-wifi-test-boot-exact-provider-thread-state-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1459_WIFI_TEST_BOOT_EXACT_PROVIDER_THREAD_STATE_ARTIFACT_SANITY_2026-06-01.md"
)
EXPECTED_DECISION = "v1458-wifi-test-boot-exact-provider-thread-state-source-build-pass"
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1458-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1458-rc1-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1459-wifi-test-boot-exact-provider-thread-state-artifact-sanity-pass",
            True,
            "V1458 exact-line provider-trigger thread-state test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1459-wifi-test-boot-exact-provider-thread-state-artifact-sanity-blocked",
        False,
        "V1458 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- V1458 manifest: `{manifest['_path']}`",
            f"- V1458 boot image: `{manifest['boot_image']}`",
            "",
            "## Checks",
            "",
            ...
            f"- exact provider thread-state contract: `{checks['wifi_test_contract']['ok']}`",
...
def main() -> int:
    ...
    result = {
        ...
        "next_gate": "V1460 rollbackable live handoff for only the V1458 image",
    }
