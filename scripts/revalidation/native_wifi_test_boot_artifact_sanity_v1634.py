#!/usr/bin/env python3
"""V1634 local-only sanity verifier for the V1633 natural-path MDM2AP IRQ artifact."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1634-natural-path-mdm2ap-irq-summary-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1634_NATURAL_PATH_MDM2AP_IRQ_SUMMARY_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1633-natural-path-mdm2ap-irq-summary-source-build-pass"
EXPECTED_INIT_MARKERS = (
    "A90 Linux init 0.9.113 (v1633-natural-mdm2ap-irq-summary)",
    "A90v1633",
    "wifi test boot armed",
    "/cache/native-init-wifi-test-boot-v1633.log",
    "/cache/native-init-wifi-test-boot-v1633.summary",
    "/cache/native-init-wifi-test-boot-v1633.pid",
    "/cache/native-init-wifi-test-boot-v1633-supervisor.pid",
    "/cache/native-init-wifi-test-boot-v1633-natural-watcher.result",
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1633-natural-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1633-natural-window.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1634-natural-path-mdm2ap-irq-summary-artifact-sanity-pass",
            True,
            "V1633 natural-path IRQ summary artifact passed local sanity; one rollbackable live handoff may proceed",
        )
    return (
        "v1634-natural-path-mdm2ap-irq-summary-artifact-sanity-blocked",
        False,
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        "# Native Init V1634 Natural-path MDM2AP IRQ Summary Artifact Sanity",
        "",
        "## Summary",
        "",
        "- Cycle: `V1634`",
        ...
        f"- V1633 manifest: `{manifest['_path']}`",
        f"- V1633 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        "## Next",
        "",
        "V1635 may perform one rollbackable live handoff using only the V1633 image, then roll back to `stage3/boot_linux_v724.img` and verify selftest `fail=0`.",
        "",
    ])
