#!/usr/bin/env python3
"""V1671 rollbackable live retry for the V1670 pcie1 clock vote readiness repair."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1671-pcie1-clock-vote-readiness-retry-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1671_PCIE1_CLOCK_VOTE_READINESS_RETRY_HANDOFF_2026-06-02.md"
)
DEFAULT_V1670_MANIFEST = (
    REPO_ROOT
    / "tmp"
    / "wifi"
    / "v1670-pcie1-clock-vote-readiness-repair-test-boot"
    / "manifest.json"
)
DEFAULT_TEST_IMAGE = (
    REPO_ROOT
    ...
    / "wifi"
    / "v1670-pcie1-clock-vote-readiness-repair-test-boot"
    / "boot_linux_v1670_pcie1_clock_vote_readiness.img"
)
TEST_EXPECT_VERSION = "A90 Linux init 0.9.119 (v1670-pcie1-clock-vote-readiness)"
DEFAULT_TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v1670.log"
DEFAULT_TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v1670.summary"
DEFAULT_TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1670-helper.result"
DEFAULT_TEST_WATCHER_PATH = "/cache/native-init-wifi-test-boot-v1670-natural-watcher.result"
DEFAULT_TEST_WINDOW_PATH = "/cache/native-init-wifi-test-boot-v1670-clock-vote-window.result"
DEFAULT_TEST_CLOCK_VOTE_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1670-pcie1-clock-vote-readiness.result"
DEFAULT_DMESG_PATTERN = (
    "A90v1670|pcie1 clock vote|subsystem_get|mdm_subsys_powerup|"
...
def classify_clock_vote(out_dir: Path, handoff: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "clock_vote_excerpt": matching_lines(vote_source, r"pcie1_clock_vote\.|A90_V1664_CLOCK_VOTE_SNAPSHOT", 40),
...
def render_report(result: dict[str, Any]) -> str:
    ...
    lines = [
        "# Native Init V1671 pcie1 Clock Vote Handoff",
        "",
        "## Summary",
        "",
        "- Cycle: `V1671`",
        ...
        *[f"- `{line}`" for line in vote["power_excerpt"]],
        ...
        "## Safety Scope",
        "",
        "This run used only the V1670 bounded clock-debug vote surface inside the",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1670-manifest", type=Path, default=DEFAULT_V1670_MANIFEST)
...
def main() -> int:
    ...
    if not handoff:
        print(json.dumps({
            "decision": "v1671-pcie1-clock-vote-readiness-retry-handoff-manifest-missing",
            "pass": False,
            "base_rc": base_rc,
        }, indent=2))
        return 1
