...
def flash_boot_image(args: argparse.Namespace, serial: str) -> None:
...
def reboot_twrp_to_system(args: argparse.Namespace, serial: str) -> None:
    log("requesting system boot through TWRP")
    result = run_command(adb_base(serial) + ["shell", "twrp reboot system"], check=False)
    if result.returncode != 0:
        log("twrp reboot system failed; falling back to adb reboot")
        run_command(adb_base(serial) + ["reboot"])
