#!/usr/bin/env python3
"""Collect read-only watchdog feasibility evidence."""
...
DEFAULT_EXPECT_VERSION = "A90 Linux init 0.9.58 (v158)"
...
def default_output_path() -> Path:
    ...
    return REPO_ROOT / "tmp" / "watchdog" / f"a90-watchdog-v158-{stamp}.txt"
...
def main() -> int:
    ...
    lines = [
        "A90 HOST WATCHDOG FEASIBILITY",
        f"generated={dt.datetime.now(dt.timezone.utc).isoformat()}",
        "policy=read-only; stat-only /dev/watchdog* and no watchdog open",
