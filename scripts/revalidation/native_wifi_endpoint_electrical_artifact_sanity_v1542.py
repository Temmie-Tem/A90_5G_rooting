#!/usr/bin/env python3
"""V1542 local-only sanity verifier for the V1541 endpoint electrical test boot."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1542-endpoint-electrical-artifact-sanity"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1542_ENDPOINT_ELECTRICAL_ARTIFACT_SANITY_2026-06-02.md"
)
EXPECTED_DECISION = "v1541-endpoint-electrical-observer-test-boot-source-build-pass"
...
def contract_check(wifi_test: dict[str, Any]) -> dict[str, Any]:
    expected = {
        ...
        "rc1_watcher_result": "/cache/native-init-wifi-test-boot-v1541-rc1-watcher.result",
        "rc1_window_sampler": True,
        "rc1_window_result": "/cache/native-init-wifi-test-boot-v1541-endpoint-electrical.result",
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1542-endpoint-electrical-artifact-sanity-pass",
            True,
            "V1541 endpoint electrical observer test boot artifact passed local sanity; a rollbackable live handoff may be planned separately",
        )
    return (
        "v1542-endpoint-electrical-artifact-sanity-blocked",
        False,
        "V1541 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1541 manifest: `{manifest['_path']}`",
        f"- V1541 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
        ...
        f"- absent unsafe/old markers: `{checks['boot_absent_markers']['absent_ok']}`",
        f"- endpoint electrical contract: `{checks['wifi_test_contract']['ok']}`",
