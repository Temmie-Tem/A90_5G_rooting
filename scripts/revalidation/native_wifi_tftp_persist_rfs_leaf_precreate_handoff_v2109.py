#!/usr/bin/env python3
"""V2109 rollbackable handoff for TFTP persist-RFS leaf precreate parity."""
...
OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v2109-tftp-persist-rfs-leaf-precreate-handoff"
HANDOFF_DIR = OUT_DIR / "v2108-handoff"
HANDOFF_REPORT = OUT_DIR / "v2108-handoff-report.md"
REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V2109_TFTP_PERSIST_RFS_LEAF_PRECREATE_HANDOFF_2026-06-05.md"
)
V2108_OUT = REPO_ROOT / "tmp" / "wifi" / "v2108-tftp-persist-rfs-leaf-precreate-test-boot"
V2108_INIT = V2108_OUT / "init_v2108_tftp_persist_rfs_leaf_precreate"
V2108_BOOT = V2108_OUT / "boot_linux_v2108_tftp_persist_rfs_leaf_precreate.img"
REMOTE_PROPERTY_ROOT = "/mnt/sdext/a90/private-property-v317/v2108/dev/__properties__"
TEST_EXPECT_VERSION = "A90 Linux init 0.9.230 (v2108-tftp-persist-rfs-leaf-precreate)"
TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v2108.log"
TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v2108.summary"
TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v2108-helper.result"
EXPECTED_HELPER_VERSION = "a90_android_execns_probe v414"
TTY_PATH = Path("/dev/ttyACM0")
BRIDGE_HOST = "127.0.0.1"
BRIDGE_PORT = 54321
BRIDGE_CAPTURE = OUT_DIR / "host" / "v2109-autostart-bridge.log"
BRIDGE_STDOUT = OUT_DIR / "host" / "v2109-autostart-bridge.stdout.txt"
BRIDGE_STDERR = OUT_DIR / "host" / "v2109-autostart-bridge.stderr.txt"
BRIDGE_PID = OUT_DIR / "host" / "v2109-autostart-bridge.pid"
...
def transport_preflight() -> dict[str, Any]:
    ...
    if ok:
        ...
        reason = "no bridge is listening; start the patched serial bridge before running V2109"
    else:
...
def write_transport_blocked(preflight: dict[str, Any]) -> None:
    ...
    decision = f"v2109-{preflight['label']}-rollback-blocked"
    manifest = {
    ...
    report = "\n".join([
        "# Native Init V2109 Transport Preflight",
        "",
        "## Summary",
        "",
        f"- Cycle: `{CYCLE}`",
        ...
        "## Interpretation",
        "",
        "- V2109 did not enter the flash/test-boot path because command transport was not healthy enough to execute a rollbackable producer-window run.",
        "- This is not WLAN evidence and must not be classified as a producer-side result.",
        "",
        "## Validation",
        "",
        "- `python3 -m py_compile scripts/revalidation/native_wifi_tftp_persist_rfs_leaf_precreate_handoff_v2109.py`",
        "- `git diff --check`",
        "",
        "## Safety",
        "",
        ...
        "## Next",
        "",
        "- Grant `/dev/ttyACM0` access or passwordless sudo, then rerun V2109; the runner will autostart the patched V2110 bridge and proceed only after clean `version` and `selftest` framing.",
        "",
    ])
...
def artifact_hook_check() -> dict[str, Any]:
    ...
    boot_required = (
        ...
        "wifi_companion_start.tftp_persist_rfs_leaf_precreate.owner=vendor_rfs:vendor_rfs",
    ...
    checks: dict[str, Any] = {}
    for path, required in ((V2108_INIT, init_required), (V2108_BOOT, boot_required)):
        key = rel(path)
        forbidden_tokens = init_forbidden if path == V2108_INIT else boot_forbidden
...
def classify(handoff: dict[str, Any],
    ...
    leaves_visible = all(
        isinstance(ns.get("paths"), dict)
        and isinstance(ns["paths"].get(name), dict)
        and intish(ns["paths"][name].get("exists")) == 1
        and intish(ns["paths"][name].get("is_dir")) == 1
        and str(ns["paths"][name].get("mode")) == "0770"
        and intish(ns["paths"][name].get("uid")) == 2903
        and intish(ns["paths"][name].get("gid")) == 2903
        for name in ("persist_rfs_mdm_mpss", "persist_rfs_apq_gnss")
    )
    ...
    if not hook_ok:
        label = "persist-rfs-leaf-precreate-artifact-hook-regression"
        passed = False
        reason = "V2108 artifact does not contain the bounded persist-RFS leaf-precreate contract"
    elif not marker_enabled:
        label = "persist-rfs-leaf-precreate-test-boot-not-executed"
        passed = False
        reason = "V2108 helper runtime markers are absent; the handoff failed before a valid producer-window test boot"
    elif intish(ns.get("audit_ok")) != 1:
        label = "persist-rfs-leaf-precreate-audit-missing"
        passed = False
        reason = "stock tftp_server process namespace audit did not complete"
    elif not parents_ok:
        label = "persist-rfs-leaf-precreate-not-applied"
        passed = False
        reason = "tftp_server process root did not show `/mnt*` parent dirs as root:system 0750"
    elif not leaves_visible:
        label = "persist-rfs-leaf-precreate-leaves-not-visible"
        passed = False
        reason = "tftp_server process root did not show mdm/mpss and apq/gnss persist-RFS leaves as vendor_rfs:vendor_rfs 0770"
    elif wlan0:
        label = "persist-rfs-leaf-precreate-wlan0-progress"
        passed = True
        reason = "native reached wlan0; stop before scan/connect and run the dedicated connectivity gate"
    elif fw_ready:
        label = "persist-rfs-leaf-precreate-fw-ready-progress"
        passed = True
        reason = "native reached FW_READY; chase wlan0 next"
    elif wlanmdsp_seen:
        label = "persist-rfs-leaf-precreate-wlanmdsp-progress"
        passed = True
        reason = "persist-RFS leaf precreate parity allowed a wlanmdsp tftp request"
    elif ota_seen:
        label = "persist-rfs-leaf-precreate-ota-progress-no-wlanmdsp"
        passed = True
        reason = "persist-RFS leaf precreate parity reached ota_firewall but not wlanmdsp"
    elif persist_auto_dir > 0 or persist_mkdir > 0:
        label = "persist-rfs-leaf-precreate-mkdir-persists"
        passed = True
        reason = "persist-RFS leaf precreate parity applied, but stock tftp_server still reports persist-RFS startup mkdir failures"
        ...
        label = "persist-rfs-leaf-precreate-clears-mkdir-no-early-branch"
        passed = True
        reason = "persist-RFS startup mkdir failures cleared, but Android-order tftp bootstrap still did not appear"

    return {
        **base,
        "decision": f"v2109-{label}-rollback-{'pass' if passed else 'blocked'}",
        "label": label,
        "pass": passed,
        "reason": reason,
        "hook_ok": hook_ok,
        ...
        "persist_rfs_leaves_visible_for_tftp": leaves_visible,
...
def configure_prev2081() -> None:
    ...
    prev2103.prev2101.prev2098.prev2096.prev2083.prev2081.V2080_OUT = V2108_OUT
    prev2103.prev2101.prev2098.prev2096.prev2083.prev2081.V2080_INIT = V2108_INIT
    prev2103.prev2101.prev2098.prev2096.prev2083.prev2081.V2080_BOOT = V2108_BOOT
