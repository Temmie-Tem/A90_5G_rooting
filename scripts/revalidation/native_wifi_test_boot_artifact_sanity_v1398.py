#!/usr/bin/env python3
"""V1398 local-only sanity verifier for the V1397 Wi-Fi test boot artifact."""
...
DEFAULT_MANIFEST = REPO_ROOT / "tmp" / "wifi" / "v1397-wifi-test-boot" / "manifest.json"
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1398-wifi-test-boot-artifact-sanity"
DEFAULT_REPORT_PATH = REPO_ROOT / "docs" / "reports" / "NATIVE_INIT_V1398_WIFI_TEST_BOOT_ARTIFACT_SANITY_2026-06-01.md"
EXPECTED_DECISION = "v1397-wifi-test-boot-logging-source-build-pass"
...
EXPECTED_BOOT_MARKERS = (
    "A90 Linux init 0.9.70 (v1397-wifitest)",
...
def header_parity_check(base_boot: Path, test_boot: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="a90-v1398-base-") as base_name, \
            tempfile.TemporaryDirectory(prefix="a90-v1398-test-") as test_name:
...
def decide(checks: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    if all(bool(item) for item in required):
        return (
            "v1398-wifi-test-boot-artifact-sanity-pass",
            True,
            "V1397 logging test boot artifact passed local sanity; a bounded live handoff may be planned separately",
        )
    return (
        "v1398-wifi-test-boot-artifact-sanity-blocked",
        False,
        "V1397 artifact sanity failed; fix local artifact before any flash handoff",
    )
...
def render_report(manifest: dict[str, Any], result: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"- V1397 manifest: `{manifest['_path']}`",
        f"- V1397 boot image: `{manifest['boot_image']}`",
        "",
        "## Checks",
        "",
