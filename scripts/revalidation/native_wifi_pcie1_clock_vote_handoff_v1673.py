#!/usr/bin/env python3
"""V1673 rollbackable live retry for the V1672 pcie1 clock vote direct attempt repair."""
...
DEFAULT_OUT_DIR = REPO_ROOT / "tmp" / "wifi" / "v1673-pcie1-clock-vote-direct-retry-handoff"
DEFAULT_REPORT_PATH = (
    REPO_ROOT
    / "docs"
    / "reports"
    / "NATIVE_INIT_V1673_PCIE1_CLOCK_VOTE_DIRECT_RETRY_HANDOFF_2026-06-02.md"
)
DEFAULT_V1672_MANIFEST = (
    REPO_ROOT
    / "tmp"
    / "wifi"
    / "v1672-pcie1-clock-vote-direct-attempt-test-boot"
    / "manifest.json"
)
DEFAULT_TEST_IMAGE = (
    REPO_ROOT
    ...
    / "wifi"
    / "v1672-pcie1-clock-vote-direct-attempt-test-boot"
    / "boot_linux_v1672_pcie1_clock_vote_direct.img"
)
TEST_EXPECT_VERSION = "A90 Linux init 0.9.120 (v1672-pcie1-clock-vote-direct)"
DEFAULT_TEST_LOG_PATH = "/cache/native-init-wifi-test-boot-v1672.log"
DEFAULT_TEST_SUMMARY_PATH = "/cache/native-init-wifi-test-boot-v1672.summary"
DEFAULT_TEST_HELPER_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1672-helper.result"
DEFAULT_TEST_WATCHER_PATH = "/cache/native-init-wifi-test-boot-v1672-natural-watcher.result"
DEFAULT_TEST_WINDOW_PATH = "/cache/native-init-wifi-test-boot-v1672-clock-vote-window.result"
DEFAULT_TEST_CLOCK_VOTE_RESULT_PATH = "/cache/native-init-wifi-test-boot-v1672-pcie1-clock-vote-direct.result"
DEFAULT_DMESG_PATTERN = (
    "A90v1672|pcie1 clock vote|subsystem_get|mdm_subsys_powerup|"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--v1672-manifest", type=Path, default=DEFAULT_V1672_MANIFEST)
...
def main() -> int:
    ...
    if not handoff:
        print(json.dumps({
            "decision": "v1673-pcie1-clock-vote-direct-retry-handoff-manifest-missing",
            "pass": False,
            "base_rc": base_rc,
        }, indent=2))
        return 1
