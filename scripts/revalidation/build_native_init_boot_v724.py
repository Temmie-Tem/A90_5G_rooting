#!/usr/bin/env python3
"""Build the V724 QRTR/service-locator boot proof native init image.

This builder recompiles the V724 PID1 binary, repacks a ramdisk with the same
trusted helper layout as V319, and reuses the verified V319 boot image header
arguments while replacing only the ramdisk. V724 is disabled by default and
only runs the QRTR/service-locator lower companion proof when its one-shot flag
exists.
"""
...
DEFAULT_INIT_BINARY = LINUX_INIT / "init_v724"
DEFAULT_RAMDISK_DIR = REPO_ROOT / "stage3" / "ramdisk_v724"
DEFAULT_RAMDISK_CPIO = REPO_ROOT / "stage3" / "ramdisk_v724.cpio"
DEFAULT_BOOT_IMAGE = REPO_ROOT / "stage3" / "boot_linux_v724.img"
...
EXPECTED_MARKERS = (
    ...
    "A90v724: qrtr servloc boot proof armed",
    "A90v724: qrtr servloc boot helper spawned",
...
def build_boot_image(args: argparse.Namespace) -> None:
    with tempfile.TemporaryDirectory(prefix="a90-v724-unpack-") as temp_name:
